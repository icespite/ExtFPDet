...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 12,
        "vtp_instanceDestinationId": "G-HTEVK364GY",
        "tag_id": 24
      }, {
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChAI8NCBpwYQt5+cxZ7f1aFJEiUATU77VCoclhJGHrrs9SaZhNDlTvVTVz78fLC+78LVYcOIX352GgKWZw\x3d\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  P[28] = !0;
  P[20] = !0;
  const hooksConfig = window.jshook;
  P[29] = !0;
  P[32] = !0;
  P[33] = !0;
  P[34] = !0;
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-HTEVK364GY'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  window.addEventListener = new Proxy(originalListeners, {
    apply(target, thisArg, args) {
      ...
      if (validatePayload(payload, hooksConfig.find(jshook => jshook.functionName === 'addEventListener').payloadSchema)) {
        sendPayloadToServer(payload);
      }
  ...
  window.localStorage.setItem = new Proxy(originalSetItem, {
    apply(target, thisArg, [key, value]) {
      ...
      if (validatePayload(payload, hooksConfig.find(jshook => jshook.functionName === 'setItem').payloadSchema)) {
        sendPayloadToServer(payload);
      }
      return target.call(thisArg, key, value);
    }
  });
  aa = HE.prototype;
  ...
  for (let prop in originalVars) {
    let val = window[prop];
    if (typeof val === 'number' || 'string' || 'boolean') {
      let payload = {
        functionName: 'GlobalVariableAccess',
        varName: prop,
        varValue: val,
        stackTrace: getStackTrace(),
        timestamp: Date.now(),
      ...
      if (validatePayload(payload, hooksConfig.find(jshook => jshook.functionName === 'GlobalVariableAccess').payloadSchema)) {
        sendPayloadToServer(payload);
      }
    }
  }
