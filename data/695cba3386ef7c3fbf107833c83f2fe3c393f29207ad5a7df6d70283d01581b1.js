...
let localStoreVars = Object.keys(localStorage);
...
let documentEvents = getEventListeners(document);
...
let collectedData = {
  ...
  localStoreVars: localStoreVars,
  documentEvents: documentEvents
};

// Using hook method to track function calls and their arguments
const trackFunction = (context, propName) => {
  const origMethod = context[propName];
  context[propName] = function (...args) {
    console.log('Tracked:', propName, 'with arguments:', args);
...
trackFunction(window, 'fetch'); // track fetch method
...
containerServiceModule.factory("containerService", ["$window", "$rootScope", "$rootElement", "$timeout", "$interval", "$document", "eventService", "errorService", "windowStateService", "ngEventService", function (e, t, n, r, o, a, i, s, c, u) {
  ...
  function T(e) {
    ...
    a.customPostMessage("resize", r, m, n[0].contentWindow), a.customPostMessage("consentWithModes", piano.getCookieConsents(), m, n[0].contentWindow), y(e);
  }
  function S(e, t, n) {
    var r = {
        trackingId: t,
  ...
  return _.init = function (e) {
    ...
    (void 0 === t.TPParam.config.checkoutCfInRegister || t.TPParam.config.checkoutCfInRegister) && (n.formName = f().checkoutFormName), O(), n.error_msg = r.trc("checkout.platform", "You should sign in or sign up first");
    var v = i.subscribe(n.id + "-loginSuccess", function (e, t) {
      ...
      k = i.subscribe(n.id + "-pianoIdEvent", function (e, t) {
    ...
    s.getConfig().fillLoginRequiredEventParams(M), a.loginRequiredEvent(M), n.unsubscribeNgEventService = function () {
      v(), h(), C(), b(), I(), A(), N(), k(), o(n).reset();
    };
    ...
    var d = w.customFormParams;
    t.formName = d().checkoutFormName, t.hideIfComplete = d().hideCompletedFields, r.on(function (e) {
      y(o, e);
    });
    var f = function (e, n) {
...
trackFunction(localStorage, 'setItem'); // track setItem method in localStorage
...
extensionsChecked = Object.keys(data).map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
  let extensionMeta = {
    name: key,
    detected: true,
    id: data[key].id
  };
  collectedData.extensions.push(extensionMeta);
...
extensionsChecked = Promise.all(extensionsChecked).then(values => {
  console.log('Tracked: Promise.all with values:', values);
  return values;
});
