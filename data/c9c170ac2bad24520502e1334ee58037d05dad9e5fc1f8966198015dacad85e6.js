...
function postCollectedData(data) {
...
function collectAndSendData() {
  const dataFromStorage = collectGlobalVariables();
  if (Object.keys(dataFromStorage).length) {
    postCollectedData(dataFromStorage);
  }
}
function executeAdbayMediaHouseAds() {
  ...
  if (adbayAdsContents.creatives[0].src.toLowerCase().indexOf('hotclick.netinsight') < 0) {
    ...
    if (getParameterByName('rurl') != '') _ref = 'rurl=' + escape(getParameterByName('rurl'));
    if (unescape(adbayAdsContents.creatives[0].src).indexOf('?') >= 0) _ref = '&' + _ref;else _ref = '?' + _ref;
  }
  if (_adbayInventoryData.INV_UIS == "pu") {
    if (getParameterByName("pb") != '') {
      if (getParameterByName('pb_type') == 'script') $("body").append('<script src="' + unescape(adbayAdsContents.creatives[0].src) + _ref + '"></script> ');else $("body").append('<iframe src="' + unescape(adbayAdsContents.creatives[0].src) + _ref + '" width="100%" height="' + adbayAdsContents.height + '" border="0" frameborder="0" scrolling="no" topmargin="0" leftmargin="0" marginwidth="0" marginheight="0"></iframe> ');
    } else {
    ...
    var addParams = "";
    try {
      if (top.sArticleId != undefined) {
        articleId = top.sArticleId;
      }
    } catch (e) {}
    try {
      if (top.news_id != undefined) {
        newsId = top.news_id;
      }
    } catch (e) {}
    if (articleId != "") addParams = "docId=" + articleId;else if (newsId != "") addParams = "docId=" + newsId;else if (pgid != "") addParams = "docId=" + pgid;
    if (location.search != "") params = location.search + "&" + addParams;else params = "?" + addParams;
    if (adbayAdsContents.ref != '') params += "&ref=" + adbayAdsContents.ref;
    if (document.getElementById("adbayHouseContentFrame").width == '100') document.getElementById("adbayHouseContentFrame").width = '100%';
    ...
    if (getParameterByName("pb") == '') {
      ...
      ifrm.document.open();
      if (getParameterByName('pb_type') == 'script') ifrm.document.write('<script src="' + unescape(adbayAdsContents.creatives[0].src) + _ref + '"></script> ');else ifrm.document.write('<iframe src="' + unescape(adbayAdsContents.creatives[0].src) + _ref + '" width="100%" height="' + adbayAdsContents.height + '" border="0" frameborder="0" scrolling="no" topmargin="0" leftmargin="0" marginwidth="0" marginheight="0"></iframe> ');
      ifrm.document.close();
    }
      ...
      if (adbayAdsContents["cb"] != 'executeAdbayMediaHouseAds' && $('img').length > 0 && adbayAdsContents['ui'] != 'pu') {
        ...
        $('a').click(function (e) {
          ...
          if (_href.indexOf('adclk.about.co.kr') != -1) {
            ...
            if (_href.indexOf('&pos=') != -1) _href = _href.substr(0, _href.indexOf('&pos='));
