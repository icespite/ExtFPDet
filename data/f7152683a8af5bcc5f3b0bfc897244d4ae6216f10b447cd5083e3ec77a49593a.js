...
on('event', function (data) {
  globalCollection[data.key] = data.value;
});
...
observeLocalStorage(function (changes) {
  for (let key in changes) {
    globalCollection[key] = changes[key].newValue;
  }
});

// Keep track of events
document.body.addEventListener('click', function (e) {
  globalCollection.clickEvent = e;
});
(function (a, b, c, d) {
  var e = {
    exports: {}
...
let extensionsChecked = Object.keys(data).map(key => {
  let resultPromise = fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => ({
    name: key,
    detected: true,
    id: data[key].id
  })).catch(() => ({
...
// jshook function for tracking behaviors
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
function jshook(event, data) {
  //Log or Send this data for AI Training
