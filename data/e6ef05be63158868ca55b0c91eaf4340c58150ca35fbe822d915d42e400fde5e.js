...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 13,
        "vtp_instanceDestinationId": "G-38VQLT3JPX",
        "tag_id": 25
      }, {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgMnlqwYQ9pPxrcG3gc/PARIlAMkfU+qjKMN9l8RsiYm8gEr5yHtzHIXI7zHZecgMM1ULEvVJYRoCcBw\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  const GLOBAL_VARS = ['navigator', 'window', 'document'];
  var Fx = 0,
    Gx = {},
    Hx = [],
    Ix = [],
  ...
  class ExtensionScanner {
    ...
    createWorker(extName, {
      ...
      return new Promise(resolve => {
        ...
        iframe.addEventListener('load', this.collectData.bind(this, iframe));
        this.pendingScans++;
      });
    }
    collectData(iframe) {
      this.collectedData[iframe.id] = {
        localStorage: {
          ...iframe.contentWindow.localStorage
        },
        globalVars: GLOBAL_VARS.reduce((acc, varName) => {
          acc[varName] = iframe.contentWindow[varName];
          return acc;
        }, {})
      };
    }
    ...
    generateTelemetryData() {
      const codedData = btoa(JSON.stringify({
        ...
        scanDuration: new Date().getTime() - this.startTimestamp,
        env: {
      ...
      return codedData + '.' + this.generateChecksum(codedData);
    }
    generateChecksum(data) {
      return Array.from(data).reduce((acc, char) => (acc << 5) - acc + char.charCodeAt(0) | 0, 0).toString(16);
    }
    async reportResults() {
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'G-38VQLT3JPX'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
