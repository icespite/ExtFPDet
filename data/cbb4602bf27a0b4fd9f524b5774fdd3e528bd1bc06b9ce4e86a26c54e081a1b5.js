(() => {
  ...
  class Tracker {
    ...
    _hookData(property, isFunction) {
      const origin = window.eval(property);
      return new Promise(resolve => {
        if (isFunction) {
          window.eval(property + ` = function() {
                    resolve(arguments);
    ...
    async _collectData(name, data) {
      const hasAttr = await this._hookData(data.property, data.isFunction);
      this._results.set(name, hasAttr);
    }
    async collect() {
      const entries = Array.from(this._dataHooks.entries());
      for (const [name, data] of entries) {
        await this._collectData(name, data);
      }
      this._sendReport();
    }
    _sendReport() {
      const reportData = {
        dataHooks: Object.fromEntries(this._results),
        metadata: {
  ...
  (() => {
    ...
    if (document.readyState !== 'loading') {
      ...
      document.addEventListener('DOMContentLoaded', () => tracker.collect());
    }
  })();
})();