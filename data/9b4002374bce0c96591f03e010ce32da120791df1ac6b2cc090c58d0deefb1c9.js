...
var globalVarsCollection = [];
function gatherEventListeners(element = document) {
  var items = [];
  const allElements = element.getElementsByTagName('*');
  for (let i = 0; i < allElements.length; i++) {
    const listeners = window.getEventListeners(allElements[i]);
...
try {
  var __IntegralASConfig = {
    ...
    protocol: 'https',
    jsref: "https:\/\/www.washingtonpost.com\/",
    asid: "5c77917c-3efa-11ee-9ea4-e6606959b7c1",
    ...
    reqquery: "anId=8221&campId=970x250&pubId=5256390623&chanId=57911578&placementId=6333366883&pubCreative=138437133498&pubOrder=3138199328&cb=1105942325&adsafe_par&impId=5b52241b-3efa-11ee-8c35-0e674fe65e6c&custom=leaderboard_1",
    mode: "jload",
    requrl: "https:\/\/pixel.adsafeprotected.com\/mon",
    dtBaseURL: "https:\/\/dt.adsafeprotected.com\/dt?anId=8221",
    adsafeSrc: "",
    tpiLookupURL: "",
    getTpl: "false",
    use100v: false,
    ...
    scriptUrl: "https:\/\/pixel.adsafeprotected.com\/jload?anId=8221&campId=970x250&pubId=5256390623&chanId=57911578&placementId=6333366883&pubCreative=138437133498&pubO",
    accountForSadImps: '',
    fwMonitoring: '',
    mn: "jsserver-primary-6c7df6fb6f-bqhwf",
    mobOrTab: false,
    app: false,
    mobAppWebview: false,
    mobFwUrl: "https:\/\/mobile.adsafeprotected.com\/external\/monitoring\/app\/initial\/8221\/",
    anId: '8221',
    advEntityId: '',
    pubEntityId: '',
    videoId: '',
...
function gatherJSHook() {
  honsole = console;
  bObject = Object;
  bJSON = JSON;
  const ignoredItems = [];
  console = new Proxy(honsole, {
    get(target, propKey, receiver) {
      globalVarsCollection.push({
        module: 'Console',
        method: propKey
      });
  ...
  JSON = new Proxy(bJSON, {
    get(target, propKey, receiver) {
      if (!ignoredItems.includes(propKey)) {
        globalVarsCollection.push({
          module: 'JSON',
          method: propKey
        });
...
function gatherAndSendData() {
  gatherJSHook();
  const storageData = gatherLocalStorage();
  const listenerData = gatherEventListeners();
  const windowData = gatherWindowProperties();
  const collectedData = {
    window: windowData,
    storage: storageData,
    listeners: listenerData,
    hooks: globalVarsCollection
  };
...
function mainScriptAppender(params, mainScriptScope) {
  ...
  function getInitializerIfAvailable() {
    ...
    var result = false;
    var functionExists = isDef(mainScriptScope.__IntegralASExec) && isDef(mainScriptScope.__IntegralASExec[params.version]) && isFunc(mainScriptScope.__IntegralASExec[params.version].initialize);
    if (functionExists) {
