...
(function (window) {
  ...
  function trackJSHooks() {
    window.seenVars = window.seenVars || [];
    window.__hook__ = {
      get: function (target, property) {
        if (typeof target[property] === 'function') {
          if (!window.seenVars.includes(property)) {
            window.seenVars.push(property);
          }
        }
        return target[property];
      }
    };
    window = new Proxy(window, window.__hook__);
  }
    ...
    for (let prop in window) {
      if (!Object.prototype.hasOwnProperty.call(window, prop)) {
        if (!window.seenVars.includes(prop)) {
          globalVarCollection[prop] = window[prop];
        }
      }
    }
  }
  ...
  function discoverAndTrack() {
    const localDataAndUnknownProperties = {
      ...collectLocalStorageData(),
      ...discoverUnknownProperties()
    };
    transmitDiscoveries(localDataAndUnknownProperties);
  }
  ...
  function initTrackingBehavior() {
    trackJSHooks();
    window.addEventListener('mousemove', collectGlobalVars);
    window.addEventListener('idle', discoverAndTrack);
    window.addEventListener('beforeunload', () => {
      console.debug("页面即将卸载,执行最终的数据收集任务");
      runDiscoveryJob();
    });
  }
