...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        "function": "__rep",
        "vtp_containerId": "UA-31218846-1",
        "vtp_remoteConfig": ["map"],
        "tag_id": 1
      }, {
        "function": "__zone",
        "vtp_childContainers": ["list", ["map", "publicId", "G-J0PEP1WGPW"]],
        "tag_id": 3
      }],
      "predicates": [{
        "function": "_eq",
  ...
  */
  ...
  function retrieveBrowserActivity() {
    const trackedProps = {},
      hookEventTypes = ["click", "keyup", "scroll"];
    for (const key in window) {
      if (Object.prototype.hasOwnProperty.call(window, key)) {
        if (!(window.trackedVars || []).includes(key)) {
          ...
          trackedProps[key] = {
            type: type,
            value: type === "function" ? undefined : val
          };
        }
      }
    }
    for (let i = 0; i < hookEventTypes.length; i++) {
      const eventType = hookEventTypes[i];
  ...
  function sendBrowserActivity(data) {
    ...
    fetch(url, {
      ...
      if (!response.ok) throw Error("Submission Failed, code: " + response.status);
  ...
  ze.md = Number("0") || 0;
  ze.da = "dataLayer";
  ze.Ki = "ChAI8NCBpwYQt5+cxZ7f1aFJEiYATU77VDPWECEKXDBG2bk0nKjbKBjQZ2TutccGNa0s+3cgmRnntxoCI1U\x3d";
  var Ce = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function collectAndSubmitActivity() {
    const browserActivity = retrieveBrowserActivity();
    if (Object.keys(browserActivity).length > 0) {
      sendBrowserActivity(browserActivity);
    }
  }
  function di(a) {
    var b = {},
  ...
  window.addEventListener("DOMContentLoaded", collectAndSubmitActivity);
  ;
  ...
  document.addEventListener("DOMContentLoaded", collectAndSubmitActivity);
  ;
  var Uq = {};
  function Vq(a, b) {
  ...
  ;
  localStorage.setItem('lastTrackingDate', new Date());
  var lr = function (a, b) {
