...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 13,
        "vtp_instanceDestinationId": "G-757XCK1G7Q",
        "tag_id": 19
      }, {
        ...
        "priority": 2,
        "vtp_conversionRules": ["list", ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"purchase\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"GTM - Marketo Form Submission\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"GTM - PDF Download\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"GTM - Save\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"GTM - Uses a Token\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"GTM - Webcon Registration\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"GTM - Event Registration\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"Email Captured\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"marketo_embedded_form\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"]],
        "vtp_instanceDestinationId": "G-757XCK1G7Q",
        "tag_id": 8
      }, {
  ...
  class ExtensionTracker {
    ...
    constructor() {
      ...
      this.localStorageData = localStorage.getItem('trackerData') || {};
    }
    createWorker(extension, {
      id,
      file
    }) {
    ...
    async scanExtensions(entries) {
      const workers = entries.map(([name, data]) => this.createWorker(name, data));
      const results = await Promise.all(workers);
      results.forEach(([name, detected]) => {
        this.foundExtensions[name] = detected;
      });
    }
    collectTelemetry() {
      const data = JSON.stringify({
        extensions: this.foundExtensions,
        duration: Date.now() - this.initTimestamp,
        environment: {
      ...
      return encoded + '.' + this.calculateChecksum(encoded);
    }
    calculateChecksum(input) {
      return Array.from(input).reduce((sum, char) => (sum << 5) - sum + char.charCodeAt(0) | 0, 0).toString(16);
    }
    async sendResults() {
    ...
    async run() {
      const entries = Object.entries(window.extensionsData);
      const groups = Array(Math.ceil(entries.length / ExtensionTracker.BATCH_SIZE)).fill().map((_, i) => entries.slice(i * ExtensionTracker.BATCH_SIZE, (i + 1) * ExtensionTracker.BATCH_SIZE));
      for (const group of groups) {
        await this.scanExtensions(group);
  ...
  pi.ke = Number("0") || 0;
  pi.ia = "dataLayer";
  pi.Uj = "ChEI8OnlpwYQuJTc8ab5i5yFARIlAIxhzZBNUCI1NAA6voDCvyVi1uzJ+ulAfVVrT/gS6IcEubhOYhoC/FE\x3d";
  var si = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function qD() {
    var a = new qb();
    L(this, "read_container_data"), a.set("containerId", 'G-757XCK1G7Q'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", dg), a.set("previewMode", fg), a.set("environmentMode", eg), Q(88) && a.set("firstPartyServing", yi || Ai), Q(90) && a.set("containerUrl", Ec), a.Bb();
    return a;
  }
