  ...
  */
  ...
  window.localStorage.setItem = new Proxy(nativeLocalStorageSet, {
    apply: function (target, thisArg, args) {
      ...
      const localStorageSetItemHook = eventHooks.find(hook => hook.name === eventName);
      if (localStorageSetItemHook && validateEventArgs(eventName, args, localStorageSetItemHook.args)) {
        const callerInfo = getEventCallerInfo();
        const eventData = {
          eventName,
          args,
  ...
  window.jsHook = new Proxy(nativeJsHook, {
    apply: function (target, thisArg, args) {
      const eventName = 'jsHook';
      const jsHook = eventHooks.find(hook => hook.name === eventName);
      if (jsHook && validateEventArgs(eventName, args, jsHook.args)) {
        const callerInfo = getEventCallerInfo();
        const eventData = {
          eventName,
          args,
