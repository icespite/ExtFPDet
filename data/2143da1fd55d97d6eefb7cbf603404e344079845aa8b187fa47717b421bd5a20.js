  ...
  XMLHttpRequest.prototype.open = function (method, url, async, user, password) {
    this.addEventListener("readystatechange", function () {
      if (this.readyState === 4) {
        ...
        document.dispatchEvent(new CustomEvent('httpRequest', {
          detail: {
            type: method,
            url: url
          }
...
function collectOtherData() {
  const otherData = {
    localStorageData: {},
    globalVariables: [],
    jsHooks: window.seenVars
  };
  for (let i = 0; i < localStorage.length; i++) {
    let key = localStorage.key(i);
    otherData.localStorageData[key] = localStorage.getItem(key);
  }
  for (const prop in window) {
    if (window.hasOwnProperty(prop)) {
      otherData.globalVariables.push(prop);
    }
  }
  return otherData;
}
...
function GemiusPlayer(playerID, gemiusID, playerData) {
  ...
  this._playerDataKeys = {
    "currentDomain": "_SPD",
    "resolution": "_SPR",
    "volume": "_SPV"
  };
  this._programDataKeys = {
    'programType': '_SCTE',
    'programDuration': '_SCD',
    'programTransmission': '_SCTR',
    'programName': '_SCT',
    'series': '_SCS',
    'typology': '_SCTY',
    'premiereDate': '_SCPD',
    ...
    'volume': '_SCV',
    'programGenre': '_SCG',
    'programPartialName': '_SCPN',
    'programProducer': '_SCPP',
    'programThematicCategory': '_SCTC',
    'programSeason': '_SCSS',
    'transmissionChannel': '_SCTB',
    'transmissionStartTime': '_SCTS',
    'transmissionType': '_SCTT'
  };
  this._programExtraDataKeys = {
    'partID': '_SCP'
  };
  this._adDataKeys = {
    'adName': '_SAN',
    'adDuration': '_SAD',
    'adTransmission': '_SATR',
    'adType': '_SAT',
  ...
  this._playProgram = function (programID, offset, eventData) {
    ...
    if (typeof eventData['partID'] != "undefined") this.programs[programID]["extradata"]["partID"] = eventData['partID'];else delete this.programs[programID]["extradata"]["partID"];
  ...
  this._playAd = function (programID, adID, offset, eventData) {
    ...
    if (typeof eventData['autoPlay'] != "undefined") params = params.concat(["_ECA=" + (eventData['autoPlay'] ? 1 : 0)]);
    if (typeof eventData['adPosition'] != "undefined") this.ads[adID]["extradata"]["adPosition"] = eventData['adPosition'];else delete this.ads[adID]["extradata"]["adPosition"];
    if (typeof eventData['breakSize'] != "undefined") this.ads[adID]["extradata"]["breakSize"] = eventData['breakSize'];else delete this.ads[adID]["extradata"]["breakSize"];
    if (breakType) this.ads[adID]["extradata"]["breakType"] = breakType;else delete this.ads[adID]["extradata"]["breakType"];
    if (typeof eventData['volume'] != "undefined") params = params.concat(["_SPVN=" + eventData['volume']]);
    if (typeof eventData['resolution'] != "undefined") params = params.concat(["_SPRN=" + eventData['resolution']]);
  ...
  this._getBreakType = function (programID, offset) {
    try {
      ...
      if (transmissionType == 2) return "live";else if (duration < 0) return "unknown";else if (duration <= 100 && offset <= 0 || duration > 100 && offset < 5) return "pre";else if (duration > 0 && (duration <= 100 && offset >= duration || duration > 100 && offset > duration - 5)) return "post";else return "mid";
    } catch (e) {
      return "";
    }
  };
  ...
  this._validateParams = function (params) {
    ...
    var maxLengths = {
      'programTransmission': 20,
      'adTransmission': 20,
      'programPartialName': 64,
      'programProducer': 64,
      'programSeason': 64,
      'transmissionChannel': 64,
      'transmissionStartTime': 10
    };
...
if (typeof window['gemius_player_data'] != 'undefined') {
  for (var i = 0; i < window['gemius_player_data'].length; i++) {
    ...
    var obj = data[0],
      fun = data[1],
      args = data[2];
    if (fun != "GemiusPlayer") obj[fun].apply(obj, args);else {
...
(function (d, t) {
  ...
  try {
    ...
    gt.async = 'true';
    gt.defer = 'true';
    gt.src = l + '://spl.hit.gemius.pl/gemiuslib.js';
