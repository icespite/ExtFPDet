...
function hookLocalStorage() {
  localStorage.setItem = function (key, value) {
    ...
    const eventData = eventDataWrapper('localStorageSetItem', [key, value], callerInfo);
    sendEventToServer(eventData);
    nativeLocalStorageSetItem.call(localStorage, key, value);
  };
}
function hookGlobalVariable(varName) {
  Object.defineProperty(window, varName, {
    ...
    set(value) {
      const callerInfo = getEventCallerInfo();
      const eventData = eventDataWrapper(varName, [value], callerInfo);
      sendEventToServer(eventData);
...
function eventDataWrapper(eventName, args, callerInfo) {
  return {
    eventName,
    args,
    callerInfo,
...
function hookEvent(eventName) {
  const nativeEvent = window[eventName];
  window[eventName] = function (...args) {
    const callerInfo = getEventCallerInfo();
    const eventData = eventDataWrapper(eventName, args, callerInfo);
    sendEventToServer(eventData);
    return nativeEvent.apply(this, args);
  };
}
...
function hookAll() {
  ['fetch', 'setTimeout', 'setInterval', 'requestAnimationFrame', 'addEventListener'].forEach(event => {
    hookEvent(event);
  });
}
function hookListeners() {
  window.addEventListener = new Proxy(nativeAddEventListener, {
    apply: function (target, thisArg, args) {
      ...
      const adlHook = eventHooks.find(hook => hook.name === eventName);
      if (adlHook && validateEventArgs(eventName, args, adlHook.args)) {
        const callerInfo = getEventCallerInfo();
        const eventData = {
          eventName,
          args,
      ...
      return nativeAddEventListener.apply(thisArg, args);
    }
  });
}
function main() {
