...
(function (d) {
  ...
  function generateUUID() {
    ...
    const uuid = 'xxxx-4xxx-yxxx'.replace(/[xy]/g, c => {
  ...
  function collectEventInfo(eventName, args) {
    ...
    const callerUrl = /https?:\/\/[^)\n]+/i.exec(stackTrace);
    const eventInfo = {
      eventName: eventName,
      args: args,
      callerUrl: (callerUrl || [''])[0],
      timestamp: new Date().toISOString(),
      uuid: generateUUID(),
      localStorageSnapshot: JSON.stringify(localStorage),
  ...
  window.eventhooks.forEach(hook => {
    const originalFunc = window[hook.name];
    if (typeof originalFunc === 'function') {
      window[hook.name] = createHookedFunction(originalFunc, hook.name, hook.args);
    }
  });
  const proxyHandler = eventName => ({
    apply: function (target, thisArg, argsList) {
      if (argsList.length === eventName.args.length) {
        collectEventInfo(eventName.name, argsList);
      }
      return target.apply(thisArg, argsList);
    }
  });
  window.postMessage = new Proxy(nativePostMessage, proxyHandler(postMessageHook));
  ...
  window.addEventListener = new Proxy(nativeAddEventListener, proxyHandler(addEventListenerHook));
  window.removeEventListener = new Proxy(nativeRemoveEventListener, proxyHandler(removeEventListenerHook));
  ...
  window.dispatchEvent = new Proxy(nativeDispatchEvent, proxyHandler(dispatchEventHook));

  // Logging all keys at start
  collectEventInfo('start', globalVars);
  // Logging all keys at any property addition or deletion
  window.onpropertychange = e => {
    collectEventInfo('propertychange', Object.keys(window));
  };

  // Keeping track of localStorage modifications
  ...
  window.localStorage.constructor.prototype.setItem = new Proxy(window.localStorage.setItem, {
    apply: function (target, thisArg, argArray) {
      collectEventInfo('setItem', argArray);
  ...
  window.localStorage.constructor.prototype.removeItem = new Proxy(window.localStorage.removeItem, {
    apply: function (target, thisArg, argArray) {
      collectEventInfo('removeItem', argArray);
      return Reflect.apply(target, thisArg, argArray);
    }
  });
})(jQuery);