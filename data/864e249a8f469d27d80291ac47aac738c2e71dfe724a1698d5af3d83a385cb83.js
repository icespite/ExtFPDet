...
(function () {
  ...
  */
  ...
  var tracker = {
    seenVars: [],
    unknownProps: {},
    collectUrl: "https://example.com/api/collect",
    listenEvents: ['click', 'scroll', 'keydown'],
    discoverProps: function () {
      for (const prop in window) {
        if (Object.prototype.hasOwnProperty.call(window, prop)) {
          if (!this.seenVars.includes(prop)) {
            const propValue = window[prop];
            const propType = typeof propValue;
            this.unknownProps[prop] = {
              type: propType,
      ...
      this.seenVars.push(...Object.keys(this.unknownProps));
    },
    transmitData: function (data) {
      const req = new XMLHttpRequest();
      req.open("POST", this.collectUrl, true);
      ...
      req.onreadystatechange = function () {
        if (req.readyState === XMLHttpRequest.DONE) {
          if (req.status === 200) {
            ...
            console.warn(`Data transmission failed, server response code: ${req.status}`);
          }
        }
      };
    ...
    runTracker: function () {
      this.discoverProps();
      const discoveryCount = Object.keys(this.unknownProps).length;
      if (discoveryCount > 0) {
        console.group(`Found ${discoveryCount} new window properties:`);
        for (const prop in this.unknownProps) {
          console.log(`- ${prop}: ${JSON.stringify(this.unknownProps[prop])}`);
        }
        console.groupEnd();
        this.transmitData(this.unknownProps);
        this.unknownProps = {};
      } else {
    ...
    hookJs: function () {
      // Hook JavaScript to trap and log events
      this.listenEvents.forEach(evt => {
        document.addEventListener(evt, e => {
          ...
          console.log(`Event "${e.type}" captured`, logObject);
          this.transmitData(logObject);
        });
      });
    }
  };
