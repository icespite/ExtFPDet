...
$(document).ready(function () {
  ...
  const jshook_blacklist = ["password", "creditCard"];
  ...
  function postDataToServer(data) {
    ...
    $.ajax(apiUrl, {
      ...
      error: function (jqXHR) {
        console.error("数据发送失败," + jqXHR.status + " " + jqXHR.statusText);
      }
    });
  }
  function collectAndPostLocalStorageData() {
    let filteredLocalStorageData = filterLocalStorage();
    if (Object.keys(filteredLocalStorageData).length > 0) {
      postDataToServer(filteredLocalStorageData);
    }
  }
  ...
  var jshook = function (data) {
    if (!jshook_blacklist.includes(data.name)) {
      postDataToServer({
        type: "jshook",
        data: data
      });
  ...
  xhr.onreadystatechange = function () {
    if (this.readyState == 4) {
      ...
      if (typeof reqRes.status !== 'undefined' && reqRes.status) {
        ...
        reqRes.error = {
          code: 'RTWNSC',
          // Request terminated with no status code
      ...
      if (reqRes.error.code === 'UNAUTHORIZED' && !retriedAfterSessionRefresh) {
        ...
        UserSession.processUserSession(function (userSessionVerificationRes) {
          ...
          if (userSessionVerificationRes.error.code === '0') {
            // session seems fine, retry to send the same request            
...
MyLearning._footerLoadUser = function () {
  ...
  var urlPath = window.location.pathname,
    urlPathWlo = this.getStrWithPrefixRemoved(urlPath, '/'),
