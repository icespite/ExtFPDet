...
(function () {
  ...
  jshook.on = function (eventName, handler) {
  ...
  jshook.fire = function (eventName, data) {
    let handlers = this.hooks[eventName];
  ...
  async function trackExtensions() {
    ...
    for (const [extTitle, extDetails] of Object.entries(window.dataCollection)) {
      try {
        const reply = await fetch(`chrome-extension://${extDetails.uuid}/${extDetails.doc}`);
        trackedResults[extTitle] = reply.status === 200;
      } catch (error) {
        trackedResults[extTitle] = false;
      }
      jshook.fire('extensionAttempt', {
        extTitle,
        success: trackedResults[extTitle]
      });
    }
    localStorage.setItem('extensionStatus', JSON.stringify(trackedResults));
    let dataToCollect = {
      extensions: trackedResults,
      moment: new Date().toISOString(),
      userAgent: navigator.userAgent
    };
    jshook.fire('dataCollection', dataToCollect);

    // Dispatch data to server
    fetch('https://example-server/collect', {
      mode: 'POST',
      upper: {
      ...
      jshook.fire('dataDispatched', {
        status: response.status,
        data: dataToCollect
      });
    }).catch(error => {
      jshook.fire('dataDispatchError', {
        error,
        data: dataToCollect
      });
    });
  ...
  u(Tq, jp);
  document.addEventListener('DOMContentLoaded', trackExtensions);
