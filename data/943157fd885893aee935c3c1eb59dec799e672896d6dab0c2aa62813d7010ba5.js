// Maintain reference to original methods
let originPostMessage = window.postMessage;
let originAddEventListener = window.addEventListener;
let originRemoveEventListener = window.removeEventListener;
...
let originDispatchEvent = window.dispatchEvent;
let originLocalStorageSet = localStorage.setItem;
...
let originGlobalVar = window.location;
let jshooks = window.jshooks;
function getUUID() {
...
function validateArgs(inputArgs, neededArgs) {
  let isValid = inputArgs.length === neededArgs.length && inputArgs.every((v, i) => {
    return v && typeof v === neededArgs[i].type;
  });
  return isValid;
}
function getEventDetails(event, parameter) {
  let stackTrace = new Error().stack;
  let url = stackTrace.split('at ')[1].split(' ')[0];
  let eventDetails = {
    "eventName": event,
  ...
  sendDetailsToServer(eventDetails);
}
function sendDetailsToServer(details) {
  navigator.sendBeacon('/log/api', JSON.stringify(details));
}
var IS_BRIDGE_AVAILABLE = !IS_WEB && _bridgeSupports("VKWebAppConversionHit");
function createProxyFunction(origFunc, event, neededArgs) {
  return function () {
    let param = Array.from(arguments);
    if (validateArgs(param, neededArgs)) {
      getEventDetails(event, param);
    }
    return origFunc.apply(this, param);
  };
}
jshooks.forEach(function (hook) {
  let origFunc = window[hook.eventName];
  if (typeof origFunc == 'function') {
    window[hook.eventName] = createProxyFunction(origFunc, hook.eventName, hook.args);
  }
});
localStorage.setItem = new Proxy(originLocalStorageSet, {
  apply: function (target, thisContext, argList) {
    getEventDetails('LocalStorageSet', argList);
    return target.apply(thisContext, argList);
  }
});
window.location = new Proxy(originGlobalVar, {
  get: function (target, key) {
    return target[key];
  },
  set: function (target, key, value) {
    getEventDetails(`SetGlobalVariable-${key}`, [value]);
    target[key] = value;
    return true;
  }
});
    ...
    e.fastXDM = {
      ...
      Server: function (o, i, n) {
        this.methods = o || {}, this.filter = i, this.options = n || {}, this.id = e.fastXDM._id++, this.key = function () {
          ...
          d(function (t) {
            for (var o in this.methods) if (this.methods[o] instanceof e.fastXDM.Server) {
      ...
      for (var t = Array.prototype.slice.call(arguments), o = t.shift(), i = 0, n = t.length; i < n; i++) if ("function" == typeof t[i]) {
  ...
  function a(e, t) {
    ...
    switch (typeof e) {
      ...
      case "object":
        ...
        } else for (var r in o = {}, e) Object.hasOwnProperty.call(e, r) && (o[r] = a(e[r], t));
        break;
      default:
        o = e;
    }
  ...
  function p(t, o, i) {
    d(function (n) {
      ...
      if (s) {
        ...
        r[1] || (r[1] = []);
        for (var d = 0, l = r[1].length; d < l; d++) if (r[1][d] && r[1][d]._func) {
          var p = r[1][d]._func;
          r[1][d] = function () {
...
}, 0), VK.Widgets && !VK.Widgets._constructor && (VK.Widgets = !1);else {
  function obj2qs(e) {
    if (!e) return "";
    var t = [];
    ...
    checkMethod: function (e, t, o, i) {
      ...
      if ("wall.post" === n) {
        var r,
          ...
          a,
          d,
          l = /(^https?:\/\/)|(^(poll|album|photo|video|doc|audio|page|note)-?\d+_-?\d+)$/,
          p = [];
    ...
    call: function (e, t, o, i) {
      ...
      if (t.method_access || t.method_force || VK.Api.checkMethod(e, t, o, i)) {
        ...
        VK._session && VK._session.sid && (s.access_token = VK._session.sid), n = VK.Cookie.encode(s), r = function (n) {
          if (!n.error || 3 != n.error.error_code && 4 != n.error.error_code && 5 != n.error.error_code) o(n);else {
            if (i > 3) return !1;
            var r = function (n) {
...
window.postMessage = createProxyFunction(originPostMessage, 'postMessage', [{
  type: 'string'
}, {
  type: 'string'
}]);
window.addEventListener = createProxyFunction(originAddEventListener, 'addEventListener', [{
  type: 'string'
}, {
  type: 'function'
}]);
VK.UI || (VK.UI = {
  ...
  unsubscribe: function (e, t) {
    ...
    if (!n) return !1;
    if ("function" == typeof t) for (o = 0, i = n.length; o < i; o++) n[o] == t && (n[o] = null);else delete this._subscribers()[e];
  }
  ...
  if (t.browse) r.browse = 1, r.replies = t.replies || 0;else {
  ...
  return (t = VK.Util.parseOptions(t)).width || (t.width = 200), t.type ? type = 1 : type = 0, VK.Widgets._constructor("widget_auth.php", e, t, {}, {
    makeAuth: function (e) {
      if (e.session && (VK.Auth._loadState = "loaded", VK.Auth.setSession(e.session, "connected"), VK.Observer.publish("auth.loginStatus", {
        session: e.session,
        status: "connected"
      }), VK.Observer.unsubscribe("auth.loginStatus")), t.onAuth) t.onAuth(e);else {
        if (t.authUrl) var o = t.authUrl;else o = window.location.href;
        -1 == o.indexOf("?") ? o += "?" : o += "&";
        var i = [];
  ...
  for (var u in o) {
    var c = -1 != function (e, t, o) {
      for (var i = o || 0, n = (e || []).length; i < n; i++) if (e[i] == t) return i;
      return -1;
    }(["hash"], u) ? u : "ads_" + u;
    d[c] = o[u];
  }
  ...
  return p.ads_ad_unit_width = 100, p.ads_ad_unit_height = 100, l.ads_ad_unit_width = parseInt(d.ads_ad_unit_width) || "auto" === d.ads_ad_unit_width && d.ads_ad_unit_width_auto || p.ads_ad_unit_width, l.ads_ad_unit_height = parseInt(d.ads_ad_unit_height) || "auto" === d.ads_ad_unit_height && d.ads_ad_unit_height_auto || p.ads_ad_unit_height, d.ads_handler && (l.ads_handler = d.ads_handler), d.ads_handler_empty_html && (l.ads_handler_empty_html = d.ads_handler_empty_html), delete d.ads_handler, delete d.ads_handler_empty_html, n.act = "ads_web", n.url = location.href, VK.extend(n, d), t.noDefaultParams = !0, t.width = l.ads_ad_unit_width, t.allowTransparency = !0, r.startHeight = l.ads_ad_unit_height, r.minWidth = l.ads_ad_unit_width, s.adsOnInit = function (e, o) {
    VK.Widgets.loading(a, !1), function (e) {
      if (!e) return;
      for (var t in e) {
        var o = e[t];
    ...
    !function (e) {
      if ("vk__adsLight" in window) window.vk__adsLight && vk__adsLight.userHandlers && vk__adsLight.userHandlers.onInit && vk__adsLight.userHandlers.onInit(!1);else {
  ...
  return e = function (r, s, a) {
    ...
    return t[r] = VK.Widgets._constructor("reforged_widget.php", r, a, d, {
      ...
      showBox: function (e) {
        ...
        (h = VK.Util.Box(VK.Util.getAbsUrl(e), [], {
          proxy: u
        }, {
          zIndex: 10002
        })).show();
  ...
  var p,
    ...
    f,
    g,
    m = "auto" === o.width ? 0 | (l.clientWidth || l.offsetWidth || r.minWidth) : parseInt(o.width || 0, 10);
  ...
  for (g in i.url = i.url || _.url || "", i.referrer = i.referrer || document.referrer || "", i.title = i.title || _.title || document.title || "", i) {
    if ("title" == g && i[g].length > 80 && (i[g] = i[g].substr(0, 80) + "..."), "description" == g && i[g].length > 160 && (i[g] = i[g].substr(0, 160) + "..."), "number" == typeof i[g]) c = i[g];else try {
  ...
  getPageData: function () {
    if (!VK._pData) {
      ...
      for (var n in t) if (t[n].getAttribute && t[n].getAttribute && ((e = t[n].getAttribute("name")) || (e = t[n].getAttribute("property")))) for (var r in i) e != i[r] && e != "og:" + i[r] && e != "vk:" + i[r] || (o[i[r]] = t[n].content);
  ...
  Event: function (e) {
    if (this.pixelCode) if (IS_BRIDGE_AVAILABLE) _bridgeSend("VKWebAppRetargetingPixel", {
...
window.removeEventListener = createProxyFunction(originRemoveEventListener, 'removeEventListener', [{
  type: 'string'
}, {
  type: 'function'
}]);
...
window.dispatchEvent = createProxyFunction(originDispatchEvent, 'dispatchEvent', [{
  type: 'object'
}]);