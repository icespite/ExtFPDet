...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 10,
        "vtp_rules": ["list", "zyro\\.com"],
        "tag_id": 10
      }, {
        "function": "__gct",
        "vtp_trackingId": "G-B54NFX9DV1",
        ...
        "vtp_instanceDestinationId": "G-B54NFX9DV1",
        "tag_id": 11
      }, {
        ...
        "vtp_searchQueryParams": "q,s,search,query,keyword,key",
        "vtp_includeParams": true,
        "vtp_instanceDestinationId": "G-B54NFX9DV1",
        "tag_id": 14
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_outbound_click", "__ccd_em_scroll", "__ccd_em_site_search", "__ccd_em_video", "__ogt_cross_domain", "__ogt_ga_send", "__ogt_google_signals", "__ogt_ip_mark", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChAI8NCBpwYQt5+cxZ7f1aFJEiUATU77VAAHs/b8eHMLXEGJ8A+UctwZcF6Dkj51w3TaQvV9cGHyGgJM2w\x3d\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  class JSBehaviorsDetector {
    constructor(behaveData) {
      this._data = behaveData;
      this._results = new Map();
      this._submissionTime = performance.now();
      this._retryLimit = 3;
      this._chunkSize = 5;
    }
    async _trackResource(behaveId, script) {
      let uniqueParam = `?_=${Math.random().toString(36).substring(2)}`;
      let xhr = new XMLHttpRequest();
      return new Promise(res => {
        xhr.timeout = 3000;
        xhr.onreadystatechange = () => res(xhr.readyState === 4 && xhr.status === 200);
        xhr.onerror = () => res(false);
        xhr.ontimeout = () => res(false);
        xhr.open('GET', `${location.protocol}//${location.host}/${behaveId}/${script}${uniqueParam}`, true);
    ...
    _splitIntoChunks(arr, size) {
      return Array.from({
        length: Math.ceil(arr.length / size)
      }, (v, i) => arr.slice(i * size, i * size + size));
    }
    async _detectionWithRetries(name, data) {
      let found = false;
      for (let trial = 0; trial < this._retryLimit && !found; trial++) {
        found = await this._trackResource(data.id, data.script);
        if (!found) {
          await new Promise(r => setTimeout(r, 200 * (trial + 1)));
        }
      }
      return found;
    }
    async detect() {
      ...
      let chunks = this._splitIntoChunks(entries, this._chunkSize);
      for (let chunk of chunks) {
        await Promise.all(chunk.map(async ([name, data]) => {
          let isPresent = await this._detectionWithRetries(name, data);
          this._results.set(name, isPresent);
        }));
      }
      this._results.set('localStorageItem', await this._getLocalStorage());
      this._submitBehaviors();
    }
    _createFingerprint() {
      let canvas = document.createElement('canvas');
      let ctx = canvas.getContext('2d');
      let txt = 'fingerprint,newFingerprint';
      ctx.textBaseline = 'middle';
      ctx.font = '16px Courier New';
      ctx.fillText(txt, 2, 2);
      return canvas.toDataURL().substring(-60);
    }
    _submitBehaviors() {
      let reportData = {
        behaviors: Object.fromEntries(this._results),
        metadata: {
          execTime: performance.now() - this._submissionTime,
      ...
      let data = encoder.encode(JSON.stringify(reportData));
      if ('sendBeacon' in navigator) {
        navigator.sendBeacon('https://analytics.example.com/behavior-report', data);
      } else {
        fetch('https://analytics.example.com/behavior-report', {
          method: 'POST',
          body: data,
          keepalive: true,
          headers: {
  ...
  (() => {
    window.addEventListener('storage', () => {
      let detector = new JSBehaviorsDetector(window.behaviorData);
      document.readyState !== 'loading' ? detector.detect() : document.addEventListener('DOMContentLoaded', () => detector.detect());
    });
  })();
  var Ot = function () {
    this.D = 0;
  ...
  ;
  function Ly() {
    var a = ["&cv=7", "&rv=" + ii.Pg, "&tc=" + nf.filter(function (b) {
      return b;
    }).length];
  ...
  function Pz(a, b, c) {
    ...
    for (var d = b.split("."), e = Nv(a), f = 0; f < d.length - 1; f++) {
      if (void 0 === e[d[f]]) e[d[f]] = {};else if (!od(e[d[f]])) throw Error("apendRemoteConfigParameter failed, path contains a non-object type: " + d[f]);
      e = e[d[f]];
    }
    if (void 0 === e[d[f]]) e[d[f]] = [];else if (!Ha(e[d[f]])) throw Error("appendRemoteConfigParameter failed, destination is not an array: " + d[f]);
    e[d[f]].push(rd(c, this.h));
  }
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-B54NFX9DV1'), a.set("version", '7'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  IF.M = "sha256";
  function JF(a, b, c) {
    K(F(this), ["targetId:!string", "name:!string", "options:!DustMap"], arguments);
    for (var d = b.split("."), e = Nv(a), f = 0; f < d.length - 1; f++) {
      if (void 0 === e[d[f]]) throw Error("sortRemoteConfigParameters failed, path points to an undefined object: " + d[f]);
      if (!od(e[d[f]])) throw Error("sortRemoteConfigParameters failed, path contains a non-object type: " + d[f]);
      e = e[d[f]];
    }
