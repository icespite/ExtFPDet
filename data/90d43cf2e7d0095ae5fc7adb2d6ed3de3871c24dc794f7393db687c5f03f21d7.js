...
const extensionProbe = {
  ...
  probe(name, {
    ...
    return new Promise(resolve => {
      ...
      img.onerror = img.onload = event => {
        clearTimeout(event.target.timer);
        this.discovered.has(name) === (event.type === 'load');
        resolve(event.type);
        this.eventListenerLogs.push({
          name: name,
          event: event.type
        });
      };
  ...
  async scan() {
    ...
    const payload = btoa(JSON.stringify({
      ...
      globalVariables: this.hooks,
      eventListeners: this.eventListenerLogs
    }));
