...
function functionInvocationTracker(fn) {
...
function discoverUnknownProperties() {
  ...
  for (const prop in window) {
    if (Object.prototype.hasOwnProperty.call(window, prop) && !seenVars.includes(prop)) {
...
function transmitDiscoveries(discoveries) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) console.debug('Data transmitted successfully');else console.warn(`Data transmission failed, server responded with code: ${xhr.status}`);
    }
  };
...
window.addEventListener('beforeunload', () => transmitDiscoveries({
  functionInvocations,
  localStorageChanges,
  windowProps: discoverUnknownProperties()
}));
"use strict";
window.setInterval(() => {
  const discoveries = {
...
window.localStorage = storageTracker(window.localStorage);
for (const key in window) {
  if (typeof window[key] === 'function') {
    window[key] = functionInvocationTracker(window[key]);
  }
}