...
function getRandomId() {
  let id = 'id-' + Math.random().toString().slice(2) + '-' + Date.now().toString(36);
  return id;
}
...
function validateEventHook(context, event) {
  if (context.length !== event.args.length) {
    console.warn(`Event ${event.name} expected ${event.args.length} arguments, but got ${context.length}`);
    return false;
  }
  for (let i = 0; i < context.length; i++) {
    const expectedType = event.args[i];
    const actualArg = typeof context[i];
    if (actualArg !== expectedType) {
      console.warn(`Event ${event.name} expected argument ${i + 1} to be of type ${expectedType}, but got ${actualArg}`);
      return false;
    }
  }
  return true;
...
function sendToServer(eventData) {
  const postUrl = '/api/event-tracker';
  const req = new XMLHttpRequest();
  req.open('POST', postUrl);
  req.setRequestHeader('Content-Type', 'application/json');
  req.send(JSON.stringify(eventData));
}
function createEventHook(name, originalFunction, args) {
  return function () {
    const context = Array.from(arguments);
    if (validateEventHook(context, {
      name,
      args
    })) {
      const eventContext = gainContext();
      const eventData = {
        eventName: name,
        args: arguments,
        contextData: eventContext
      };
...
window.addEventListener = new Proxy(originalAddEventListener, {
  apply: function (target, thisArg, args) {
    const hook = eventHooks.find(hook => hook.name === 'addEventListener');
    if (hook && validateEventHook(args, hook)) {
      const eventContext = gainContext();
      const eventData = {
        eventName: 'addEventListener',
        args: {
          type: args[0],
          listener: args[1]?.name || 'anonymous',
          options: args[2]
        },
        contextData: eventContext
      };
...
window.localStorage.setItem = new Proxy(originalSetItem, {
  apply: function (target, thisArg, args) {
    const hook = eventHooks.find(hook => hook.name === 'localStorage.setItem');
    if (hook && validateEventHook(args, hook)) {
      const eventContext = gainContext();
      const eventData = {
...
window.open = new Proxy(originalOpen, {
  apply: function (target, thisArg, args) {
    const hook = eventHooks.find(hook => hook.name === 'window.open');
    if (hook && validateEventHook(args, hook)) {
      const eventContext = gainContext();
      const eventData = {
...
window.setTimeout = new Proxy(originalSetTimeout, {
  apply: function (target, thisArg, args) {
    const hook = eventHooks.find(hook => hook.name === 'window.setTimeout');
    if (hook && validateEventHook(args, hook)) {
      const eventContext = gainContext();
      const eventData = {
        eventName: 'window.setTimeout',
        args: {
          callback: args[0]?.name || 'anonymous',
          delay: args[1]
        },
        contextData: eventContext
      };
