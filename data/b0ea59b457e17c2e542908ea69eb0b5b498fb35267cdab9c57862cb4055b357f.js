...
if (typeof gemius_cmpclient == "undefined") {
  gemius_cmpclient = {
    ...
    find_cmp_v1: function () {
      if (gemius_cmpclient.__cmp) return true;
      ...
      if (!gemius_cmpclient.find_cmp_frame("__cmpLocator")) return false;
    ...
    find_cmp_v2: function () {
      if (gemius_cmpclient.__tcfapi) return true;
      ...
      if (!gemius_cmpclient.find_cmp_frame("__tcfapiLocator")) return false;
    ...
    find_cmp: function () {
      if (gemius_cmpclient.find_cmp_v2()) return true;
      return gemius_cmpclient.find_cmp_v1();
    },
    has_consent_v1: function (data, purposes) {
      try {
        if (!data.vendorConsents[gemius_cmpclient.gemius_vendor_id]) return false;
        for (var i = 0; i < purposes.length; i++) {
          if (!data.purposeConsents[purposes[i]]) return false;
        }
      } catch (e) {
        return false;
      }
    ...
    has_consent_v2: function (tcData, purposes) {
      try {
        if (typeof tcData.gdprApplies == 'boolean' && !tcData.gdprApplies) return true;
        if (!tcData.vendor.consents[gemius_cmpclient.gemius_vendor_id]) return false;
        for (var i = 0; i < purposes.length; i++) {
          ...
          if (!tcData.purpose.consents[purposes[i]] || restrict == 0) return false;
        }
      } catch (e) {
        return false;
      }
...
function generateSessionId() {
  ...
  const id = `session-${Array.from(uid, b => b.toString(16).padStart(2, '0')).join('')}`;
  return id;
}
function validatePayload(p, schema) {
  for (const key in schema) {
    const type = schema[key];
    const v = p[key];
    if (v === undefined || typeof v !== type) {
      console.warn(`Type mismatch, expected '${type}' but got '${typeof v}'`);
      return false;
    }
  }
  return true;
...
function sendPayload(serverEndpoint, payload) {
...
// ver. 3.42
function hookFunction(functionName, fn, schema) {
  return (...args) => {
    const payload = {
      functionName,
      args,
    ...
    validatePayload(payload, schema) && sendPayload('/collect', payload);
    return fn.apply(this, args);
  };
}
const globalVars = Object.keys(window).reduce((acc, name) => {
  const data = {
    name,
...
window.setInterval = hookFunction('setInterval', originalSetInterval, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
...
window.setTimeout = hookFunction('setTimeout', originalSetTimeout, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
  ...
  GemiusHitcolConnector = function (is_gde) {
    ...
    this.append_script = function (url, finishedfn, notify) {
      ...
      try {
        ...
        if (finishedfn != null) {
          ...
          this.add_event(gt, 'readystatechange', function () {
        ...
        this.add_event(gt, 'readystatechange', this.bind(function () {
          if (!gt.readyState || gt.readyState === 'loaded' || gt.readyState === 'complete') this.remove_script(rndid, notify ? url : null);
        }));
    ...
    this.paramsready = function () {
      ...
      if (this.use_globals && this.params_ready_called == 0 && fpdata_ready && lsdata_ready) {
        var data = {
          'lsdata': this.getconsentlsdata(),
          'fpdata': this.getconsentfpdata()
        };
    ...
    this.getfpcap = function () {
      ...
      if (capcookie.sec_state == 'sec') {
        this.fpcap = {
          nsec: '-TURNEDOFF',
          sec: capcookie.primary
        };
      } else {
        this.fpcap = {
          nsec: capcookie.primary,
          sec: capcookie.secondary
        };
      }
    };
    ...
    this.cmp_purposes = function () {
      ...
      try {
        var combine_cmp_purposes = function (purposes, cmp_purposes_overrides, add_dmp_purposes) {
          var purposes_v1_to_v2_translation = {
            1: [1],
            2: [3, 5],
            3: [2, 4, 7],
            4: [6, 8],
    ...
    this.frametype_init = function () {
      ...
      if (typeof Error != 'undefined') {
        try {
          ref.frametype = d == top.document ? 1 : 2;
      ...
      if (typeof Error != 'undefined') {
        try {
          if (typeof top.document.referrer == "string") {
            ref.ref = top.document.referrer;
          }
        } catch (e) {}
      }
      try {
        if (ref.ref == '' && typeof ia_document != "undefined" && ia_document.referrer) ref.ref = "https://" + new String(ia_document.referrer);
      } catch (e) {}
    };
    ...
    this.internal_hit = function (allowaddscript, vers, id, evid, et, hsrc, sonar, extra) {
      ...
      var robj = {
        ...
        vers: vers,
        explicit: this.explicit_consent
      };
    ...
    this.addscripthit = function () {
      this.ghit(1, 106, arguments, 0, 2, this.sonar_auto_init);
    };
    this.plainhit = function () {
      this.ghit(0, 107, arguments, 0, 2, this.sonar_auto_init);
    };
    this.addscriptevent = function () {
      this.gevent(1, 106, arguments, 0, 3, this.sonar_auto_init);
    };
    this.plainevent = function () {
      this.gevent(0, 107, arguments, 0, 3, this.sonar_auto_init);
    };
    if (this.use_globals) {
    ...
    this.start = function () {
      ...
      if (this.use_globals) {
        ...
        gemius_close = this.bind(function () {
          this.closing = 1;
        });
      }
      this.getdntcookie();
...
window.addEventListener = hookFunction('addEventListener', originalAddEventListener, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
...
window.removeEventListener = hookFunction('removeEventListener', originalRemoveEventListener, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
...
localStorage.setItem = hookFunction('setItem', originalSetItem, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
...
localStorage.getItem = hookFunction('getItem', originalGetItem, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
...
$.ajax = hookFunction('ajax', originalAJAX, {
  functionName: 'string',
  args: 'object',
  stackTrace: 'string',
  timestamp: 'number',
