...
const globalVarCollector = function () {
  let allVars = Object.getOwnPropertyNames(window);
  return allVars;
};
/**
...
*/
fbq.version = "2.9.124";
const collectTrackedGlobalVar = function () {
  return globalVarCollector().filter(variable => !['localStorage', 'sessionStorage'].includes(variable)).map(variable => ({
    name: variable,
...
function collectEventData(eventName, args) {
  const globalVars = collectTrackedGlobalVar();
...
function createEventProxy(originalFunc, eventName, expectedArgs) {
  return new Proxy(originalFunc, {
    apply: function (target, thisArg, argumentsList) {
      if (validateArgs(argumentsList, expectedArgs)) {
...
window.localStorage.setItem = createEventProxy(nativeLocalStorageSetItem, 'setItem', [{
  type: 'string',
  required: true
}, {
  type: 'string',
...
window.addEventListener = createEventProxy(window.addEventListener, 'addEventListener', [{
  type: 'string',
  required: true
}, {
  type: 'function',
...
window.removeEventListener = createEventProxy(window.removeEventListener, 'removeEventListener', [{
  type: 'string',
  required: true
}, {
  type: 'function',
...
window.dispatchEvent = createEventProxy(window.dispatchEvent, 'dispatchEvent', [{
  type: 'object',
  required: true
}]);
