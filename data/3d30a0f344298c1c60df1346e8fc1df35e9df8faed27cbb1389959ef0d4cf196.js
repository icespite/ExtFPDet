(function (opts_) {
  ...
  */
  ...
  function jsHook(preFunc, postFunc) {
  ...
  function detectEventListeners() {
    for (const type of ['click', 'input']) {
      document.body.addEventListener(type, event => {
        globalVar[`last_${type}_event`] = event;
      }, true);
    }
  }
  ...
  async function detectExtensions() {
    ...
    for (const [extName, extData] of Object.entries(window.data)) {
      try {
        ...
        IFrames.forEach(iFrame => {
          try {
            results[`IFrame_${iFrame.id}`] = iFrame.contentWindow.eval(`(!!window.${extName})`) ? true : false;
          } catch (e) {}
        });
      } catch (e) {
    ...
    fetch = jsHook(args => globalVar.lastFetchArgs = args, result => globalVar.lastFetchResult = result);
  }
  ...
  "cx": "015440420724022033189:hmifmuod28s",
  "language": "en",
  "theme": "V2_DEFAULT",
  "uiOptions": {
    "resultsUrl": "",
