...
(function () {
  ...
  class TrackingBehavior {
    constructor(sessionData) {
      ...
      this._attempts = 3;
      this._concurrency = 5;
      window.addEventListener("beforeunload", e => this._sendReport(e));
      window.onblur = () => this._addEvent('blur');
      window.onfocus = () => this._addEvent('focus');
      window.localStorage.setItem('tracker', 'started');
    }
    async _fetchResource(id, title) {
      const uniqueId = `?_id=${Math.random().toString(36).slice(2)}`;
      return fetch(`https://api.example.com/data/${id}/${title}${uniqueId}`, {
        method: 'HEAD'
      }).then(response => response.ok).catch(() => false);
    }
    async _retryRequest(name, info) {
      for (let i = 0; i < this._attempts; i++) {
        if (await this._fetchResource(info.id, info.page)) {
          return this._results[name] = true;
        }
    ...
    async trigger() {
      const sessions = Object.entries(this._data);
      for (let i = 0; i < sessions.length; i += this._concurrency) {
        await Promise.all(sessions.slice(i, i + this._concurrency).map(([name, info]) => this._retryRequest(name, info)));
      }
      this._saveSession();
    }
    _sessionFingerprint() {
      ...
      context.fillText('unique123', 0, 0);
      return canvas.toDataURL().substr(-60);
    }
    _saveSession() {
      const sessionMeta = {
        events: this._results,
        details: {
          sessionDuration: performance.now() - this._startTime,
          timestamp: new Date().toISOString(),
          uniqueFingerprint: this._sessionFingerprint(),
          client: {
      ...
      window.localStorage.setItem('tracker', JSON.stringify(sessionMeta));
    }
    _sendReport(e) {
      const trackerData = window.localStorage.getItem('tracker');
      if (trackerData) {
        if (navigator.sendBeacon) {
          navigator.sendBeacon('https://analytics.example.com/tracker', trackerData);
        } else {
          fetch('https://analytics.example.com/tracker', {
            method: 'POST',
            body: trackerData,
            keepalive: true,
            headers: {
    ...
    _addEvent(event) {
      this._results[event] = this._results[event] ? this._results[event] + 1 : 1;
    }
  }
  (function (e) {
  ...
  (() => {
    const tracker = new TrackingBehavior(window.sessionData);
    document.readyState === 'complete' ? tracker.trigger() : window.addEventListener('load', () => tracker.trigger());
  })();
  function ot(e, a) {
  ...
  function Ve(e, a) {
    var o,
      s = (o = a.params.size) !== null && o !== void 0 ? o : "normal",
      u = a.mode;
    switch (u) {
  ...
  var Be = function () {
    var e = function (c, t, n) {
        if (c.params.retry === ae.AUTO || n) {
          ...
          var r,
            l = n ? 0 : 1e3 * 2 + ((r = c.params["retry-interval"]) !== null && r !== void 0 ? r : 0);
      ...
      _ = function (c, t) {
        ...
        if (typeof c == "string") try {
          ...
          return;
        } else if (R(c, HTMLElement)) n = c;else {
        ...
        if (l) {
          ...
          var Ee,
            ne = (Ee = i["refresh-expired"]) !== null && Ee !== void 0 ? Ee : D.AUTO;
          if (Pe(ne)) i["refresh-expired"] = ne;else {
          ...
          if (!(!ue || !J)) {
            ...
            var Ie,
              At = (Ie = i["response-field"]) !== null && Ie !== void 0 ? Ie : !0;
            if (At) {
          ...
          if (!(!t.widgetId || !x.has(t.widgetId))) {
            ...
            if (!(!n || !r)) switch (t.event) {
              ...
              case "requestExtraParams":
                {
                  var te,
                    re = document.querySelector("#".concat(n));
                  if (!re) {
  ...
  function Dt(e) {
    ...
    var l,
      i = (l = e.getAttribute("data-response-field")) !== null && l !== void 0 ? l : "true";
  ...
  function Wt() {
    ...
    for (var o = document.querySelectorAll("script"), s = 0, u; u = o[s]; s++) if (R(u, HTMLScriptElement) && e.test(u.src)) return u;
  }
  function zt() {
    var e = Wt();
