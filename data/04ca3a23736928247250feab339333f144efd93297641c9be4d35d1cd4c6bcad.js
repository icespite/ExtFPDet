...
(function () {
  ...
  function generateUniqueId() {
    ...
    const rn = Math.random().toString(36).substr(2, 5);
    return `${ts}-${rn}`;
  }
  function Le(e) {
  ...
  function getEventCallerInfo() {
    ...
    if (stack) {
      ...
      if (match) {
        info.line = parseInt(match[1], 10);
        info.column = parseInt(match[2], 10);
      }
    }
    return info;
  }
  ...
  function trackEvent(obj) {
    ...
    nativeFetch('/api/log-event', options).catch(error => console.error(`Error sending event: ${error}`));
  }
  function lt(e, a) {
  ...
  function eventProxy(name, func) {
    return function () {
      ...
      syncLocalStorage();
      eData.localStorage = localStorageData;
      trackEvent(eData);
  ...
  function hookGlobalObjects() {
    if ('addEventListener' in window) {
      window.EventTarget.prototype.addEventListener = function (eventName, fn) {
        nativeAddEventListener.call(this, eventName, eventProxy(eventName, fn));
      };
    }
    if ('fetch' in window) {
    ...
    if ('XMLHttpRequest' in window) {
      window.XMLHttpRequest = function () {
        ...
        xhr.addEventListener('readystatechange', () => {
          ...
          const eData = {
            name: 'XMLHttpRequest',
            args: [`state:${xhr.readyState}, status: ${xhr.status}`],
            callerInfo,
            timestamp: new Date().toISOString(),
