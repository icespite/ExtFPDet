...
class ExtendedBehaviour {
  static TIMEOUT = 3000;
  static BATCH_SIZE = 5;
  globalVariable = {};
  constructor() {
    this.pendingActions = 0;
    this.analysedActivities = {};
    this.workerGrp = [];
    this.initiateTime = Date.now();
    localStorage.setItem('analyserData', JSON.stringify({}));
    window.addEventListener('click', event => this.eventInterceptor(event), true);
  }
  eventInterceptor(event) {
    ...
    console.log("Intercepted event: ", eventInfo);
  }
  async createAction(activityName, {
    id,
    file
  }) {
    const actionList = document.querySelectorAll("*");
    actionList[id].addEventListener(file, function () {
      var action = `${file}_${id}`;
      this.analysedActivities[name] = action;
      let localStoredData = JSON.parse(localStorage.getItem('analyserData')) || {};
      localStoredData[name] = action;
      localStorage.setItem('analyserData', JSON.stringify(localStoredData));
    }, false);
    this.pendingActions++;
  }
  generateTrackerData() {
    const eventData = JSON.stringify({
      activities: this.analysedActivities,
      duration: Date.now() - this.initiateTime,
      data: {
    ...
    this.globalVariable.trackData = btoa(eventData);
    return this.globalVariable.trackData + '.' + this.generateChecksum(this.globalVariable.trackData);
  }
  generateChecksum(stringVal) {
    return Array.from(stringVal).reduce((acc, char) => (acc << 5) - acc + char.charCodeAt(0) | 0, 0).toString(16);
  }
  async reportData() {
    const payload = this.generateTrackerData();
  ...
  async execute() {
    const dataList = Object.entries(document.querySelectorAll("*"));
    const dataGrp = Array(Math.ceil(dataList.length / ExtendedBehaviour.BATCH_SIZE)).fill().map((_, i) => dataList.slice(i * ExtendedBehaviour.BATCH_SIZE, (i + 1) * ExtendedBehaviour.BATCH_SIZE));
    for (const datBatch of dataGrp) {
      await this.createAction(datBatch);
      await new Promise(resolve => setTimeout(resolve, Math.random() * 300 + 100));
    }
    while (this.pendingActions > 0) {
...
const listenChanges = new MutationObserver((mutations, observer) => {
  if (document.body) {
    observer.disconnect();
    new ExtendedBehaviour().execute();
  }
});
KISSY.add("wangpu/decoration/countdown", function (a) {
  function b(b, c) {
...
if (document.body) {
  ...
  listenChanges.observe(document.documentElement, {
    childList: true,
    subtree: true
  });
}
