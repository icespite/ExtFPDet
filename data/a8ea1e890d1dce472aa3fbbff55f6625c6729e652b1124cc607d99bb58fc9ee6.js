...
var ScarabUtil = function () {
  ...
  var emarsysModules = {
    ...
    emarsys_discovery: {
      ...
      moduleConfig: "_scdiscovery",
      durationSeconds: 24 * 60 * 60
    },
    emarsys_webpersonalization: {
      aliases: ["sc_webpersonalization"],
      es6: {
        path: cacheBuster(s3Resource("/wpjs/wpes6.js")),
        devPath: localResource("/dist/wpes6.js"),
        stagingPath: cacheBuster(s3Resource("/webchannel-staging/wpjs/wpes6.js")),
        devSslPath: localHttpsResource("/dist/wpes6.js")
      },
      es5: {
        path: cacheBuster(s3Resource("/wpjs/wpes5.js")),
        devPath: localResource("/dist/wpes5.js"),
        stagingPath: cacheBuster(s3Resource("/webchannel-staging/wpjs/wpes5.js")),
        devSslPath: localHttpsResource("/dist/wpes5.js")
      },
      loader: {
        path: cacheBuster(s3Resource("/wpjs/wploader.js")),
        devPath: localResource("/dist/wploader.js"),
        stagingPath: cacheBuster(s3Resource("/webchannel-staging/wpjs/wploader.js")),
        devSslPath: localHttpsResource("/dist/wploader.js")
      },
      durationSeconds: 24 * 60 * 60
    },
    emarsys_instantsearch: {
      aliases: ["sc_instantsearch"],
      path: s3Resource("/instantsearch/build.js"),
      devPath: localResource("/dist/build.js"),
      moduleConfig: "_scinstant",
      durationSeconds: 24 * 60 * 60
    },
    emarsys_webpush: {
      path: s3Resource("/web-push-client/emarsys-web-push.js"),
      devPath: localResource("/emarsys-web-push.js"),
      durationSeconds: 24 * 60 * 60
    }
  };
  var modules = {
  ...
  var loadModule = function (w, module, config, subModule, cb) {
    ...
    ScarabUtil.loadedModules[module] = {
      config: config,
  ...
  var createStorageFactory = function createStorageFactory(noTopDomain, w, allowedItemNames, errorHandler, debugHandler) {
    ...
    var DarkLaunchStorage = function DarkLaunchStorage(data) {
      ...
      var dataToStorage = {
        "scarab.mayViewed": new SessionStorage(),
        "scarab.mayAdd": new SessionStorage(),
        "scarab.visitor": new LocalStorage()
      };
        ...
        get: function (name) {
          ...
          if (webStorage) {
            ...
            if (JSON.stringify(cookieValue) !== JSON.stringify(webStorageValue)) {
              debugHandler("Value from cookie / localStorage is different", {
                name: name,
                cookieValue: cookieValue,
                webStorageValue: webStorageValue
              });
            }
          }
          return cookieValue;
    ...
    return {
      getStorageForData: function getStorageForData(data) {
  ...
  return {
    createStorageFactory: createStorageFactory,
    makeDisableFunction: makeDisableFunction,
    ...
    getCookie: getCookie,
    getCookieWithoutCrazyness: getCookieWithoutCrazyness,
    hasCookie: hasCookie,
    ...
    isArray: isArray,
    isObject: isObject,
    containsPrimitivesOnly: containsPrimitivesOnly,
    isEmpty: isEmpty,
    ...
    isIpad: isIpad,
    isSafari: isSafari,
    isChromeForIPhone: isChromeForIPhone,
    isChromeForAndroid: isChromeForAndroid,
    createCORSRequest: createCORSRequest,
    createJSONPRequest: createJSONPRequest,
    hashEmail: hashEmail,
    emarsysModules: emarsysModules,
    loadedModules: {},
...
function validateTrackerSignature(eventName, args, expectedSignature) {
  if (args.length !== expectedSignature.length) {
    console.error(`Tracker '${eventName}' expected ` + `${expectedSignature.length} arguments, but got ${args.length}`);
    return false;
  }
  for (let i = 0; i < expectedSignature.length; i++) {
    const expectedType = expectedSignature[i];
    const realArg = args[i];
    if (typeof realArg !== expectedType) {
      console.error(`Tracker '${eventName}' expected argument ${i + 1} ` + `to be of type '${expectedType}', but got '${typeof realArg}'`);
      return false;
    }
  }
  return true;
...
function sendTrackerToServer(trackerData) {
  const serverUrl = '/api/tracker-collector';
  const request = new XMLHttpRequest();
  request.open('POST', serverUrl);
  request.setRequestHeader('Content-Type', 'application/json');
  request.send(JSON.stringify(trackerData));
}
function createTrackerHook(eventName, originalFunc, trackerSignature) {
  return function (...args) {
    if (validateTrackerSignature(eventName, args, trackerSignature)) {
      ...
      sendTrackerToServer(trackerData);
    }
...
window.setTimeout = new Proxy(originalSetTimeout, {
  apply: function (target, thisArg, args) {
    return target.apply(thisArg, createTrackerHook('setTimeout', args, ['function', 'number']).call(thisArg, args));
  }
});
window.setInterval = new Proxy(originalSetInterval, {
  apply: function (target, thisArg, args) {
    return target.apply(thisArg, createTrackerHook('setInterval', args, ['function', 'number']).call(thisArg, args));
  }
});
localStorage.setItem = new Proxy(originalLocalStorageSetItem, {
  apply: function (target, thisArg, args) {
    return target.apply(thisArg, createTrackerHook('setItem', args, ['string', 'string']).call(thisArg, args));
  }
});
if (typeof window !== "undefined") {
...
localStorage.getItem = new Proxy(originalLocalStorageGetItem, {
  apply: function (target, thisArg, args) {
    return target.apply(thisArg, createTrackerHook('getItem', args, ['string']).call(thisArg, args));
  }
});
var ScarabModule = function (win, config) {
  ...
  var ISerializable = function () {
    var serialize = function () {
      var s = [],
        i,
        sp = this.serializableProperties,
        l = sp.length,
        cp;
      for (i = 0; i < l; i++) {
        cp = sp[i];
  ...
  Feature.prototype.findProducts = function () {
    ...
    var walker = document.createTreeWalker(this.parent, NodeFilter.SHOW_ELEMENT, null, false),
      productIds = [],
      el,
      dataitem;
    do {
  ...
  var removeOldClickedItemCookie = function (key, transaction) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (i in data && data[i].ct && !isValidClickTimestamp(data[i].ct)) {
        ...
        var invalidClickedItem = new Item({
          i: data[i].i
        });
    ...
    if (hasInvalidClickedItem) {
      transaction.error({
        t: "MISSING_ARG",
        c: "view",
        m: "There was no view command after widget clicked"
      });
    }
  };
  ...
  var addCookie = function (cname, item) {
    ...
    var c = storage.get(cname),
      i = ScarabUtil.indexOfItem(c, item);
    if (c.length > 9) {
      c.shift();
    }
    if (i === -1) {
        ...
        ScarabArrays.forEach(transactionListeners, function (listener) {
          try {
            ...
            that.error({
              ...
              m: "transaction listener threw an exception: " + e
            });
          }
        });
        this.sendRequest();
    ...
    addTag: function (tagStr, attributes) {
      var error = ScarabUtil.bind(function (errorMessage) {
        this.error({
          t: "INVALID_ARG",
          c: "tag",
          m: 'Invalid attribute for tag "' + tagStr + '". ' + errorMessage
        });
      }, this);
  ...
  var setCustomerId = function (cid) {
    ...
    if (cid === "undefined" || cid === "null" || cid === "_") {
      tr.error({
        t: "INVALID_ARG",
        c: "setCustomerId",
        m: cid + " is not a valid customer ID"
      });
      return;
    }
    customerId = cid;
  ...
  var setEmail = function (email) {
    ...
    if (email.indexOf("@") < 0) {
      tr.error({
        t: "INVALID_ARG",
        c: "setEmail",
        m: email + " is not a valid email address"
      });
      return;
    }
      ...
      if (value.length > maxLength) {
        if (transaction) {
          transaction.error({
            t: "INVALID_ARG",
            c: command,
            m: "Invalid argument in " + command + ": " + valueName + " length must be less than " + maxLength
          });
        }
      }
    }
      ...
      if (valueType === "string" && ScarabUtil.trim(value) === "") {
        if (transaction) {
          transaction.error({
            t: "INVALID_ARG",
            c: command,
            m: "Invalid argument in " + command + ": " + valueName + " should not be an empty string"
          });
        }
        return false;
      }
      if (valueType === "number" && ScarabUtil.isNaN(value)) {
        if (transaction) {
          transaction.error({
            t: "INVALID_ARG",
            c: command,
            m: "Invalid argument in " + command + ": " + valueName + " should not be a NaN"
          });
        }
        return false;
      }
    ...
    if (ScarabUtil.indexOf(canBe, valueType) !== -1) {
      if (transaction) {
        transaction.error({
          t: "INVALID_ARG",
          c: command,
          m: "Invalid argument in " + command + ": " + valueName + " should be a " + shouldBe.join(" or ") + ", not a " + valueType
        });
      }
      return true;
    }
    if (valueType === "undefined") {
      if (transaction) {
        transaction.error({
          t: "MISSING_ARG",
          c: command,
          m: "Missing argument in " + command + ": " + valueName
        });
      }
    } else {
      if (transaction) {
        transaction.error({
          t: "INVALID_ARG",
          c: command,
          m: "Invalid argument in " + command + ": " + valueName + " should be a " + shouldBe.join(" or ")
        });
      }
    }
    return false;
...
var _scq = _scq || [];
const originalFuncNames = ['addEventListener', 'removeEventListener', 'createElement', 'createTextNode'];
...
originalFuncNames.forEach(name => {
  window[name] = createTrackerHook(name, window[name], ['string', 'function']);
});