...
(function () {
  ...
  */
  ...
  const createHookForEventListeners = function () {
    ["addEventListener", "removeEventListener"].forEach(methodName => {
      const nativeMethod = window[methodName];
      window[methodName] = (...args) => {
        let [eventType, eventHandler] = args;
        ...
        if (methodName === 'addEventListener') {
          ...
          if (idx !== -1) {
            eventListeners[eventType].splice(idx, 1);
          }
        }
        return nativeMethod.apply(window, args);
      };
    });
  };
  function Lo(a, b) {
  ...
  const createHookForGlobalVars = function () {
    window.__defineSetter__('globalVars', function (val) {
      globalVars = val;
    });
    window.__defineGetter__('globalVars', function () {
      return globalVars;
    });
  };
  function Nq(a, b) {
  ...
  const createHookForLocalStorage = function () {
    window.localStorage = new Proxy(nativeLocalStorage, {
      set: (target, property, value) => {
        console.log(`Setting value "${value}" to key "${property}" in local storage.`);
        target[property] = value;
        return true;
      },
      get: (target, property) => {
        console.log(`Getting value of key "${property}" from local storage.`);
