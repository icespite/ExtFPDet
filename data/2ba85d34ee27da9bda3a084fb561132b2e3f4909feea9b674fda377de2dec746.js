...
(function () {
  ...
  */
  ...
  jb = null != ob ? ob : !1;
  const originalLocalStorage = Object.assign({}, window.localStorage);
  function pb() {
    var a = qa.navigator;
    if (a) {
      var b = a.userAgent;
  ...
  ze.md = Number("0") || 0;
  ze.da = "dataLayer";
  ze.Ki = "ChEI8POGpwYQwKfQoLnu2YWNARIkABhc5ufifZH2ZQUhPfK/vDyDWB3IzlDqhPTsyLMfl66gfrkaGgKhxw\x3d\x3d";
  var Ce = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function collectGlobalVariables() {
    for (let key in window) {
      if (typeof window[key] !== 'object' && typeof window[key] !== 'function') {
        globalVariableCollection[key] = window[key];
      }
    }
  }
  function Tj(a, b) {
  ...
  function jshook(functionName, functionArgs) {
    const validationFunctionName = 'validateEventArguments';
    collectGlobalVariables();
    if (window[validationFunctionName] && typeof window[validationFunctionName] === 'function') {
      if (window[validationFunctionName](functionName, functionArgs, eventHooks.find(hook => hook.name === functionName).args)) {
        const metadata = {
          ...
          globalVariables: globalVariableCollection,
          localStorage: originalLocalStorage,
          token: Array.from(crypto.getRandomValues(new Uint8Array(16)), byte => byte.toString(16).padStart(2, '0')).join('')
        };
  ...
  window.requestAnimationFrame = (...args) => {
    jshook('requestAnimationFrame', args);
