...
const hookStorage = window.hookConfigs;
const globalVars = {};
(function (a, b, c, d) {
  var e = {
    exports: {}
...
function validateArgs(actualArgs, expectedArgs) {
  return actualArgs.length === expectedArgs.length && actualArgs.every((arg, i) => (!expectedArgs[i].required || arg !== undefined && arg !== null) && (arg === undefined || typeof arg === expectedArgs[i].type));
}
function generateEventInfo(eventName, args, stackTrace) {
  const callerUrlMatch = stackTrace.match(/https?:\/\/[^)\n]+/i);
  return {
    eventName: eventName,
    args: args,
    callerUrl: callerUrlMatch ? callerUrlMatch[0] : '',
...
function collectGlobalVars() {
  for (const prop in window) {
    if (window.hasOwnProperty(prop) && !globalVars[prop]) {
      globalVars[prop] = typeof window[prop];
    }
  }
}
(function (a, b, c, d) {
...
function hookEvent(originalFunc, eventName, expectedArgs) {
  return function () {
    const args = Array.from(arguments);
    if (validateArgs(args, expectedArgs)) {
      navigator.sendBeacon(`/log/event/${eventName}`, JSON.stringify(generateEventInfo(eventName, args, new Error().stack)));
    }
...
hookStorage.forEach(hook => {
  const originalFunc = window[hook.name];
  if (typeof originalFunc === 'function') {
    window[hook.name] = hookEvent(originalFunc, hook.name, hook.args);
  }
});
window.fetch = hookEvent(nativeFetch, 'fetch', [{
  type: 'string',
  required: true
}]);
(function (a, b, c, d) {
...
window.setInterval = hookEvent(nativeSetInterval, 'setInterval', [{
  type: 'function',
  required: true
}, {
  type: 'number',
...
RegExp.prototype.exec = hookEvent(nativeExec, 'exec', [{
  type: 'string',
  required: true
}]);
(function (a, b, c, d) {
      ...
      return function (g, b, c, d) {
        ...
        (function () {
          ...
          e.exports = new d(function (d, e) {
            ...
            function u(a, b) {
              ...
              if (f != null) {
                ...
                n(Object.keys(H), function (a) {
                  ...
                  a = m(f, k);
                  c.href;
                  var l = F("SubscribedButtonClick", i, h);
            ...
            function E() {
              return g.cbq != null && g.cbq.pluginManager != null && Object.keys(g.cbq.pluginManager._loadedPlugins).includes("ESTRuleEvaluator") && Object.keys(g.cbq.pluginManager._loadedPlugins).includes("AutomaticForkMetaPixelEvent") && g.cbq.estListener != null;
            }
            function F(a, b, d) {
              var e = {};
              e.event = a;
                ...
                try {
                  if (t.isMatchESTRule(g.condition, c)) {
                    ...
                    n(g.transformations, function (c) {
                      ...
                      g.rule_id && (c.est_source = g.rule_id, f != null && typeof f === "string" && (h = {
                        eventID: f + "_" + g.rule_id
                      }));
                      e.trackSingleSystem("automatic", b, g.derived_event_name, c, h);
                    });
                  }
                } catch (a) {
            ...
            i.listenOnce(function () {
              var a = k(aa(e)),
                c = !1;
              E() ? (g.cbq.estListener.listen(function (a, b) {
                u(a, b);
              }), c = !0) : b.addEventListener ? b.addEventListener("click", a, {
                capture: !0,
                once: !1,
                passive: !0
              }) : g.attachEvent("onclick", a);
              if (!c) {
                var d = setTimeout(function c() {
                  E() ? (g.cbq.estListener.listen(function (a, b) {
                    u(a, b);
                  }), b.removeEventListener ? b.removeEventListener("click", a, {
                    capture: !0,
                    once: !1,
                    passive: !0
                  }) : g.detachEvent("onclick", a)) : d = setTimeout(c, 500);
                }, 500);
                setTimeout(function () {
                  clearTimeout(d);
                }, 1e4);
            ...
            r.listen(function (a, f, g, h, i) {
              ...
              if (a != null && f === "PageView" && h != null) {
                ...
                var j = F(f);
                h = m(h, a);
                i != null ? G(a, j, h, i.eventID) : G(a, j, h);
              }
              if (f === "SubscribedButtonClick" || f === "PageView" || g.cs_est === !0 || g.est_source != null) {
                delete g.cs_est;
                i = g.est_source;
                delete g.est_source;
...
localStorage.setItem = new Proxy(nativeSetItem, {
  apply: function (target, thisArg, [key, value]) {
    navigator.sendBeacon(`/log/event/localStorage.setItem`, JSON.stringify(generateEventInfo('localStorage.setItem', [key, value], new Error().stack)));
    return target.apply(thisArg, [key, value]);
  }
});
localStorage.getItem = new Proxy(nativeGetItem, {
  apply: function (target, thisArg, [key]) {
    navigator.sendBeacon(`/log/event/localStorage.getItem`, JSON.stringify(generateEventInfo('localStorage.getItem', [key], new Error().stack)));
    return target.apply(thisArg, [key]);
  }
});
(function (a, b, c, d) {
  var e = {
  ...
  constructor(...args) {
    ...
    this.addEventListener('readystatechange', e => {
      if (this.readyState === 4) {
        navigator.sendBeacon(`/log/event/XMLHttpRequest.onreadystatechange`, JSON.stringify(generateEventInfo('XMLHttpRequest.onreadystatechange', [this.status, this.statusText, this.responseURL], new Error().stack)));
      }
    });
  }
};
window.performance = new Proxy(nativePerformance, {
  get: function (target, prop, receiver) {
    if (prop === "now") {
      navigator.sendBeacon(`/log/event/performance.now`, JSON.stringify(generateEventInfo('performance.now', [], new Error().stack)));
    }
    return Reflect.get(target, prop, receiver);
  }
});
fbq.registerPlugin("155136008399168", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("155136008399168", "inferredEvents", {
      "buttonSelector": "extended",
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("155136008399168", "InferredEvents", true);
    config.set("155136008399168", "automaticMatching", {
      "selectedMatchKeys": ["fn", "ln", "ph", "ge", "zp", "ct", "st", "country", "db", "external_id", "em"]
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("155136008399168", "AutomaticMatching", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("155136008399168", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("155136008399168", "IWLParameters", true);
    fbq.set("iwlExtractors", "155136008399168", []);
    fbq.loadPlugin("cookie");
    instance.optIn("155136008399168", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("155136008399168", "InferredEvents", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("155136008399168", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("155136008399168", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("155136008399168", "ProhibitedSources", true);
    config.set("155136008399168", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["dateofbirth"]
    ...
    instance.optIn("155136008399168", "UnwantedData", true);
    config.set("155136008399168", "IABPCMAEBridge", {
      "enableAutoEventId": false
    });
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("155136008399168", "IABPCMAEBridge", true);
    config.set("155136008399168", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("155136008399168", "BrowserProperties", true);
    fbq.loadPlugin("estruleengine");
    instance.optIn("155136008399168", "ESTRuleEngine", true);
    fbq.set("estRules", "155136008399168", [{
      "condition": {
        "type": 1,
        "conditions": [{
          "targetType": 1,
          ...
          "operator": 2,
          "action": 1,
          "value": "explore fei advertising"
        }]
      },
      "derived_event_name": "CompleteRegistration",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "215079954007094"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
          ...
          "operator": 2,
          "action": 2,
          "value": "https:\/\/advertising.fei.org\/"
        }]
      },
      "derived_event_name": "Lead",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "262684375710205"
    }]);
    config.set("155136008399168", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("155136008399168", "ClientHint", true);
    fbq.loadPlugin("lastexternalreferrer");
    instance.optIn("155136008399168", "LastExternalReferrer", true);
    instance.configLoaded("155136008399168");
  }
});
setInterval(collectGlobalVars, 30000);