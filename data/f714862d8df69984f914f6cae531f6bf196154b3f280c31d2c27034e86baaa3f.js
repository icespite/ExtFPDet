class Inspector {
  ...
  inspectLocalStorage() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      localStorageStates[key] = localStorage.getItem(key);
    }
    this.sendInsightsToBackend(localStorageStates);
  }
  inspectGlobalScope() {
    const newFindings = {};
    for (const key in window) {
      if (window.hasOwnProperty(key) && !this.seenVars.includes(key)) {
        ...
        this.seenVars.push(key);
      }
    }
    this.sendInsightsToBackend(newFindings);
  }
  async sendInsightsToBackend(insights) {
    ...
    try {
      ...
      if (response.ok) {
        console.info("Insights successfully sent to the backend.");
      } else {
        console.warn(`Failed to send insights, server responded with status code: ${response.status}`);
      }
    } catch (error) {
      console.error("Failed to send insights due to an error:", error);
    }
  }
}
/*
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    ...
    inspector.inspectGlobalScope();
    inspector.inspectLocalStorage();
  }
});
window.addEventListener("unload", () => {
  inspector.sendInsightsToBackend(inspector.seenVars);
});