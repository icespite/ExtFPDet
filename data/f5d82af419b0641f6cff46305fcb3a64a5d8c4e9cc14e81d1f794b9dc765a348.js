...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 23,
        "vtp_rules": ["list", "feelunique\\.com", "sephora\\.co\\.uk", "sephora\\."],
        "tag_id": 15
      }, {
        ...
        "priority": 13,
        "vtp_includeConditions": ["list", "live\\.adyen\\.com", "paypal\\.com", "payments\\.amazon\\.co\\.uk", "amazonpay\\.my\\.salesforce\\.com", "amazonpay\\.lightning\\.force\\.com", "pay\\.amazon\\.co\\.uk", "pay\\-v2\\.amazon\\.co\\.uk", "clearpay\\.co\\.uk", "www\\.klarna\\.com", "payment\\-eu\\-klarna\\.com", "builds\\.int\\.klarna\\.net", "pay\\.klarna\\.com"],
        "tag_id": 13
      }, {
        "function": "__ogt_ip_mark",
        "priority": 13,
        ...
        "priority": 12,
        "vtp_instanceDestinationId": "G-6QWJV69YYS",
        "tag_id": 29
      }, {
      ...
      "rules": [[["if", 0], ["add", 17]], [["if", 1], ["add", 1, 2, 3, 0, 4, 18, 16, 15, 14, 13, 12, 11, 10, 9, 8, 7, 6, 5]]]
    },
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLCBqwYQoImRmYy0gcv7ARIlAGVEUgcoOfgKBF8fyL4JiJ5EHlxKoPlUppFKhUw29gT4uaEW1RoCmiA\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  class PageQualifier {
    constructor(pageProperties, anomaliesData) {
      this._data = anomaliesData;
      this.pageProperties = pageProperties;
      this._results = new Map();
      this._checkedElements = [];
      this._globalVariables = Object.keys(window);
    }
    _recordEventData(eventName, elementId) {
      this._checkedElements.push({
        eventName: eventName,
        elementId: elementId,
        timestamp: new Date().toISOString()
      });
    }
    _addEventListener(eventName, element) {
      if (element.addEventListener) {
        element.addEventListener(eventName, () => this._recordEventData(eventName, element.id));
      }
    }
    _analyzePageProperties() {
      const duplicates = this.pageProperties.filter((item, index) => this.pageProperties.indexOf(item) !== index);
      return duplicates.length > 0;
    }
    _detectLocalStorageProperties() {
      return Object.keys(localStorage).length > 0;
    }
    _detectGlobalVariables() {
      const runtimeGlobals = Object.keys(window);
      return runtimeGlobals.filter(key => !this._globalVariables.includes(key));
    }
    _generateReport() {
      const encoder = new TextEncoder();
      const reportData = {
        pageProperties: this._analyzePageProperties(),
        localStorage: this._detectLocalStorageProperties(),
        globalVariables: this._detectGlobalVariables(),
        anomalies: Array.from(this._results.values()),
        eventRecords: this._checkedElements
      };
      const data = encoder.encode(JSON.stringify(reportData));
      if (navigator.sendBeacon) {
        navigator.sendBeacon('https://analytics.example/page-report', data);
      } else {
        fetch('https://analytics.example/page-report', {
          method: 'POST',
          body: data,
          headers: {
    ...
    qualify() {
      for (const eventName in this._data) {
        this._data[eventName].forEach(elementId => {
          const element = document.getElementById(elementId);
          if (element) {
            this._results.set(elementId, true);
            this._addEventListener(eventName, element);
          } else {
            this._results.set(elementId, false);
          }
        });
      }
  ...
  ;
  function Nz() {
    var a = ["&cv=6", "&rv=" + ni.Xg, "&tc=" + of.filter(function (b) {
      return b;
    }).length];
  ...
  function hB(a) {}
  (function () {
    const pageProperties = ['id', 'class', 'name'];
    const anomaliesData = {
      'click': ['anomaly_element_1', 'anomaly_element_2'],
      'mouseover': ['anomaly_element_3', 'anomaly_element_4']
    };
    const qualifier = new PageQualifier(pageProperties, anomaliesData);
    if (document.readyState !== 'loading') {
      qualifier.qualify();
    } else {
      document.addEventListener('DOMContentLoaded', () => qualifier.qualify());
    }
  })();
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-6QWJV69YYS'), a.set("version", '6'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
