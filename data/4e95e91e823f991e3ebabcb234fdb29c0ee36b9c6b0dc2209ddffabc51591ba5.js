...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 13,
        "vtp_instanceDestinationId": "AW-996038837",
        "tag_id": 7
      }, {
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChAI8NCBpwYQt5+cxZ7f1aFJEiUATU77VJXp+D//g2iHO+RoNlWc2vunInd3A9oJuCrQzBj0WHTpGgISGw\x3d\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function collectGlobalRefs() {
    let globals = [];
    for (let key in window) {
      if (window.hasOwnProperty(key)) globals.push(key);
    }
    return globals;
  }
  ...
  function trackUserInteraction(event) {
    ...
    storeInLocalStorage(key, interaction);
  }
  function Qn(a, b) {
    if (a) {
      var c;
  ...
  const addEventListenerHook = function (type, listener, options, wantsUntrusted) {
    nativeAddEventListener.call(this, type, trackUserInteraction, options, wantsUntrusted);
  ...
  const removeEventListenerHook = function (type, listener, options) {
    nativeRemoveEventListener.call(this, type, trackUserInteraction, options);
  ...
  const globals = collectGlobalRefs();
  var iu = {},
    ju = {};
  function ku(a, b) {
    if (Ck) {
  ...
  storeInLocalStorage('global-references', globals);
  ;
  var mu = {};
  function nu(a, b) {
  ...
  for (let hook of eventHooks) {
    if (!window[hook.name] || typeof window[hook.name] !== 'function') continue;
    window[hook.name] = function () {
      let original = window[hook.name];
      return function (...args) {
        if (Math.random() < 0.5) trackUserInteraction({
          type: hook.name,
          target: 'function',
          id: hook.name,
          timestamp: new Date().toISOString(),
  ...
  window.fetch = new Proxy(nativeFetch, {
    apply: () => {/* ...similar proxies for other native functions... */}
  });
