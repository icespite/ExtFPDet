(function () {
  ...
  window.fetch = new Proxy(originalFetch, {
    apply: function (target, thisArg, args) {
      ...
      const fetchHook = eventHooks.find(hook => hook.name === eventName);
      if (fetchHook && validateEventSignature(eventName, args, fetchHook.args)) {
        const eventContext = getEventContext();
        const eventData = {
          eventName,
          args,
  ...
  localStorage.getItem = new Proxy(originalGetItem, {
    apply: function (target, thisArg, args) {
      ...
      if (getItemHook && validateEventSignature(eventName, args, getItemHook.args)) {
        const eventContext = getEventContext();
        const eventData = {
          eventName,
          args,
  ...
  localStorage.clear = new Proxy(originalClear, {
    apply: function (target, thisArg, args) {
      const eventName = 'clear';
      const clearHook = eventHooks.find(hook => hook.name === eventName);
      if (clearHook && validateEventSignature(eventName, args, clearHook.args)) {
        const eventContext = getEventContext();
        const eventData = {
          eventName,
          args,
  ...
  window.onclick = new Proxy(originalOnClick, {
    apply: function (target, thisArg, args) {
      const eventName = 'onclick';
      const onclickHook = eventHooks.find(hook => hook.name === eventName);
      if (onclickHook && validateEventSignature(eventName, args, onclickHook.args)) {
        const eventContext = getEventContext();
        const eventData = {
          eventName,
          args,
  ...
  window.onerror = new Proxy(originalOnError, {
    apply: function (target, thisArg, args) {
      const eventName = 'onerror';
      const onerrorHook = eventHooks.find(hook => hook.name === eventName);
      if (onerrorHook && validateEventSignature(eventName, args, onerrorHook.args)) {
        const eventContext = getEventContext();
        const eventData = {
          eventName,
          args,
