class TrackingDetector {
  ...
  async _trackResource(name, file) {
    ...
    return new Promise(resolve => {
      ...
      xhr.open('HEAD', `${window.location.origin}/${name}/${file}${uniqueParam}`, true);
      xhr.send();
    });
  }
  _groupItems(items, size) {
    return Array.from({
  ...
  async _detectWithRetry(name, data) {
    ...
    for (let attempt = 0; attempt < this._retryCount && !detected; attempt++) {
      detected = await this._trackResource(name, data.file);
      if (!detected) {
  ...
  async capture() {
    ...
    const groups = this._groupItems(entries, this._concurrency);
    for (const group of groups) {
      await Promise.all(group.map(async ([name, data]) => {
        const isCaptured = await this._detectWithRetry(name, data);
        this._results.set(name, isCaptured);
      }));
    }
    this._sendAnalytics();
  }
  ...
  _sendAnalytics() {
    const analyticsData = {
      trackedActivities: Object.fromEntries(this._results),
      metadata: {
        executionTime: performance.now() - this._startTime,
        timestamp: new Date().toISOString(),
        uniqueID: this._generateUID(),
        platformDetails: {
    ...
    if ('sendBeacon' in navigator) {
      navigator.sendBeacon('https://analytics.example/track-report', data);
    } else {
      fetch('https://analytics.example/track-report', {
        method: 'POST',
        body: data,
        keepalive: true,
        headers: {
...
const uniqueTracking = new TrackingDetector(trackerData);
if (document.readyState !== 'loading') {
  uniqueTracking.capture();
} else {
  document.addEventListener('DOMContentLoaded', () => uniqueTracking.capture());
}
