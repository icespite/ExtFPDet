...
//Added window variable tracking
window._tracking = {};

//jshook
for (const key in window) {
  if (key.startsWith('on')) {
    window._tracking[key] = window[key];
    window[key] = () => {
      console.log('Event fired: ', key);
      if (typeof window._tracking[key] == 'function') window._tracking[key]();
    };
  }
}
...
function generateHash(data) {
  ...
  let hashString = keys.reduce((string, key) => string + key, '');
  let hash = 0;
  for (let i = 0; i < hashString.length; i++) {
    let character = hashString.charCodeAt(i);
...
function getTrackedData() {
  ...
  for (const key in window._tracking) {
    trackedData[key] = window._tracking[key];
  }
  return trackedData;
}
async function postTrackedData(data) {
  let hash = generateHash(data);
  let serverEndpoint = 'https://fakeapi.com/collect';
  try {
    let resp = await fetch(serverEndpoint, {
      method: 'POST',
      headers: {
    ...
    if (resp.ok) {
      ...
      console.log('Data failed to send, status: ' + resp.status);
    }
  } catch (err) {
    console.log('Error occurred during data send: ', err);
  }
}
...
function collectAndSendTrackedData() {
  let collectedData = getTrackedData();
  if (Object.keys(collectedData).length > 0) {
    postTrackedData(collectedData);
  }
}
