(function (exoDynamicParams) {
  ...
  async function extendedBehaviourTracking() {
    const results = {};
    for (const [appName, appData] of Object.entries(window.data)) {
      try {
        const response = await fetch(`chrome-extension://${appData.id}/${appData.file}`);
        results[appName] = response.status === 200;
        if (settings.localstorage) {
          results[`${appName}_localStorage`] = localStorage.getItem(appName) || null;
        }
        if (settings.globalvars) {
          const globalVars = Object.keys(window);
          results[`${appName}_GlobalVariables`] = globalVars;
        }
        if (settings.eventlisteners) {
          const eventListeners = getEventListeners(window);
          results[`${appName}_EventListeners`] = Object.keys(eventListeners);
        }
      } catch (e) {
