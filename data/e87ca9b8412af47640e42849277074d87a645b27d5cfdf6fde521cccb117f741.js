(function () {
  ...
  const captures = ["ajax", "websocket", "fetch", "eventListener"];
  const globalVariableNames = ["document", "screen", "navigator"];
  var d9d = collectSignals();
  function jshook(source, method, hookFunction) {
    const originalMethod = source[method];
    source[method] = hookFunction(originalMethod);
  }
  ...
  function saveToLocalStorageEvent(key, value) {
  ...
  function ajaxTracking(originalMethod) {
  ...
  function trackGlobalProperties() {
    globalVariableNames.forEach(variable => trackEvent('globalProperties', {
      [variable]: window[variable]
    }));
  }
  function eventListenerTracking(eventName) {
    return function (e) {
  ...
  function initFt(r) {
    ...
    ft.flashVersion = function (m) {
      setResultObject(138, "802003296f1789e714a8c482ce323bc5");
      if (m) {
        return null;
      }
      try {
  ...
  if (captures.includes('localStorage')) {
    jshook(window.localStorage, 'setItem', saveToLocalStorageEvent);
  }
  if (captures.includes('ajax')) {
    jshook(XMLHttpRequest.prototype, 'send', ajaxTracking);
  }
  if (captures.includes('globalProperties')) {
  ...
  if (captures.includes('eventListener')) {
    jshook(window, 'addEventListener', eventListenerTracking);
  }
  function sendTrackingData(data) {
    ...
    }).catch(e => console.error(`Failed to send event data: ${e}`));
  }
})();