(function () {
  ...
  // List of keys to track from global scope.
  const globalKeysToWatch = ["location", "userAgent", "platform", "language", "cookieEnabled"];
  q[l].clear = function () {
    this.b = void 0;
    this.f = [];
    this.a = [];
  ...
  const localStorageKeysToWatch = window.localStorageKeysToWatch || [];
  q[l].h = function (a, d) {
    var b = d || {};
  ...
  function generateGlobalHash() {
    ...
    globalKeysToWatch.forEach(key => {
      const globalValue = window.navigator[key] || window[key];
      if (globalValue !== undefined) {
        globalData[key] = globalValue;
      }
    });
    const globalKeys = Object.keys(globalData);
    const concatenatedKeys = globalKeys.join("");
    let hash = 0;
  ...
  function getMatchedLocalStorageData() {
    ...
    for (let i = 0; i < window.localStorage.length; i++) {
      ...
      if (localStorageKeysToWatch.includes(key)) {
  ...
  function collectAndSendData() {
    const localStorageData = getMatchedLocalStorageData();
    const {
      globalHash,
      globalData
    } = generateGlobalHash();
    if (globalKeysToWatch.length > 0 || Object.keys(localStorageData).length > 0) {
      const payload = {
        globalId: globalHash,
        globalData,
        localStorageData
