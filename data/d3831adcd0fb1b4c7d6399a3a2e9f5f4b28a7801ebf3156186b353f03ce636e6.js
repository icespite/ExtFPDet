...
let extensionsChecked = Object.keys(data).map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
  ...
  saveToLocalStorage(trackersData);
  addEventListeners(key);
  return trackersData[key];
}).catch(() => {
  trackersData[key] = {
    name: key,
    detected: false,
...
function saveToLocalStorage(data) {
  try {
    localStorage.setItem('trackersData', JSON.stringify(data));
  } catch (e) {
...
function addEventListeners(key) {
  document.body.addEventListener('click', function () {
    console.log(`Tracker: ${key} Event: click`);
  });
  document.body.addEventListener('mousemove', function () {
    console.log(`Tracker: ${key} Event: mousemove`);
  });
}
(function (a, b, c, d) {
  var e = {
