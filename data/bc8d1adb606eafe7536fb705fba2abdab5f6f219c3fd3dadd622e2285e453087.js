...
(function (q, g, r) {
  ...
  function validateEventArguments(eventName, args, expectedArgs) {
    if (args.length !== expectedArgs.length) {
      console.warn(`Invalid arguments count for event '${eventName}'. Expected ${expectedArgs.length}, got ${args.length}.`);
      return false;
    }
    return expectedArgs.every((expectedType, i) => expectedType === 'any' || typeof args[i] === expectedType);
  }
  function t(a, d) {
    var c = !1,
      e = g.bind(a, a.push);
  ...
  function createEventWrapper(originalFunc, eventName, eventArgs) {
  ...
  Object.keys(monitoredFunctions).forEach(funcName => {
    const originalFunc = monitoredFunctions[funcName];
    const funcHook = eventHooks.find(hook => hook.name === funcName);
    if (funcHook) {
      window[funcName] = new Proxy(originalFunc, {
        apply: function (target, thisArg, args) {
          if (validateEventArguments(funcName, args, funcHook.args)) {
            const payload = {
              eventName: funcName,
              args,
              metadata: getEventMetadata()
