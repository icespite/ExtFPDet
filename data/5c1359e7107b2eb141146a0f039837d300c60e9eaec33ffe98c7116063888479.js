...
class ExtensionTracker {
  ...
  createWorker(extName, {
    ...
    return new Promise(resolve => {
      ...
      const hookFunc = () => {};
      this.hooks[extName] = hookFunc;
      window.addEventListener('jshook', hookFunc);
      const timer = setTimeout(() => {
        window.removeEventListener('jshook', hookFunc);
        cleanup();
  ...
  generateTelemetryData() {
    const encodedData = btoa(JSON.stringify({
      trackedExtensions: this.detectedExtensions,
  ...
  getGlobalVariables() {
    const globalVars = {};
    window.keys(window).forEach(key => globalVars[key] = window[key]);
    return globalVars;
  }
  trackEventListeners() {
    this.eventListeners = window.getEventListeners(window);
  }
}
