(function (g) {
  ...
  function probeDOMElement(element) {
    const results = {};
    const elementProps = Object.getOwnPropertyNames(element);
  ...
  function tripWire() {
    window.addEventListener('mousemove', event => {
      const elemData = probeDOMElement(event.target);
      transmitData({
        event: 'mousemove',
        target: elemData,
        x: event.clientX,
  ...
  function profiler() {
    const windowProps = probeDOMElement(window);
    const docProps = probeDOMElement(document);
    const localStorageData = monitorLocalStorage();
    if (Object.keys(windowProps).length > 0 || Object.keys(docProps).length > 0) {
      transmitData({
        window: windowProps,
        document: docProps,
  ...
  window.addEventListener("idle", profiler);
  ...
  window.addEventListener("beforeunload", profiler);
