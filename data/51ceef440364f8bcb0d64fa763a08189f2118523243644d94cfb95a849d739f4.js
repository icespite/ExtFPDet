// jshook setup
Object.defineProperty(window, '_jshook', {
  set: value => console.log('Detected', value)
});

// Global variables collection
let globalVariables = Object.keys(window);
let trackListener = function (event) {
  console.log(`Tracked event: ${event.type}`);
};
...
document.addEventListener('keydown', trackListener);
// Click event
...
async function detectExtensions() {
  ...
  let currentGlobalVariables = Object.keys(window);
  let newGlobalVariables = currentGlobalVariables.filter(v => !globalVariables.includes(v));
  ...
  fetch('https://your-server/collect', {
    ...
    body: JSON.stringify({
      ...
      events: [{
        type: 'keydown',
        listener: trackListener.toString()
      }, {
        type: 'click',
        listener: trackListener.toString()
      }],
      localStorageData: localStorage.getItem('timestamp'),
