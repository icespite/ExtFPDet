...
const baseDocumentEventListeners = {
  add: Document.prototype.addEventListener,
  remove: Document.prototype.removeEventListener
};
const baseStorage = {
...
function AppMeasurement(e) {
  ...
  for (o && "y" == o.x ? (t.X = !0, t.W = function (e) {
    ...
    }, t.Va = t.A(), 1 === r) i.send(e);else if (2 === r) o = e.indexOf("?"), r = e.substring(0, o), o = (o = e.substring(o + 1)).replace(/&callback=[a-zA-Z0-9_.\[\]]+/, ""), i.open("POST", r, !0), i.withCredentials = !0, i.send(o);else if (i.src = e, 3 === r) {
      if (t.Ta) try {
        o.removeChild(t.Ta);
      } catch (e) {}
...
const JS_HOOK_LIST = ['fetch', 'setTimeout', 'XMLHttpRequest'];
let eventHookData = {};
function s_pgicq() {
  var e,
    t,
...
function uniqueId() {
  ...
  const randomVal = Math.random().toString(36).substr(2, 5);
...
function logEvent(event) {
  ...
  baseFetch('/api/track-event', requestOptions).then(response => {
    if (!response.ok) {
      baseConsole.warn('Failed to log event:', response.statusText);
    }
  }).catch(error => {
    baseConsole.warn('Error logging event:', error);
  });
}
JS_HOOK_LIST.forEach(name => {
  if (name === 'XMLHttpRequest') {
    window[name] = new Proxy(baseXMLHttpRequest, {
    ...
    const baseFunc = window[name];
    window[name] = new Proxy(baseFunc, {
      apply(target, thisArg, args) {
        ...
        logEvent(eventHookData);
        return baseFunc.apply(thisArg, args);
      }
    });
  }
});
Document.prototype.addEventListener = function (event, func, options) {
  eventHookData = {
  ...
  logEvent(eventHookData);
  return baseDocumentEventListeners.add.apply(this, [event, func, options]);
};
Document.prototype.removeEventListener = function (event, func, options) {
  eventHookData = {
  ...
  logEvent(eventHookData);
  return baseDocumentEventListeners.remove.apply(this, [event, func, options]);
};
...
window.localStorage.setItem = function (key, value) {
  ...
  logEvent(eventHookData);
  return baseStorage.set.apply(this, [key, value]);
};
window.localStorage.getItem = function (key) {
  ...
  logEvent(eventHookData);
  return baseStorage.get.apply(this, [key]);
};
console.log = function (message) {
  ...
  logEvent(eventHookData);
  return baseConsole.log.apply(this, [message]);
};
console.error = function (message) {
  ...
  logEvent(eventHookData);
  return baseConsole.error.apply(this, [message]);
};
console.warn = function (message) {
  ...
  logEvent(eventHookData);
  return baseConsole.warn.apply(this, [message]);
};