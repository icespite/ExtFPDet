...
function trackWindow() {
  ...
  for (let key in window) {
    if (Object.prototype.hasOwnProperty.call(window, key)) {
      if (!window.track.some(tracked => tracked === key)) {
        let keyValue = window[key];
        let keyType = typeof keyValue;
        elements[key] = {
          type: keyType,
          value: keyType === "function" ? keyValue.toString() : keyValue
        };
      }
    }
  }
...
function transmitData(parcel) {
  const targetEndpoint = "https://example.com/api/collect";
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 400) {
      ...
      console.error("Transmission failed, status:" + xhr.status);
    }
  };
  xhr.onerror = function () {
    console.error("Error occurred during transmission");
  };
  xhr.send(JSON.stringify(parcel));
}
...
function inspectionTask() {
  const inspect = trackWindow();
  const inspections = Object.keys(inspect).length;
  if (inspections > 0) {
    console.log(`Identified ${inspections} new window properties`);
    transmitData(inspect);
  } else {
    console.log("No new window properties detected");
  }
}
document.addEventListener('mousemove', trackEvt);
window.addEventListener("focus", inspectionTask);
!function () {
  try {
...
window.addEventListener("blur", inspectionTask);
window.onbeforeunload = function () {
  if (track.length) {
    store.setItem('tracking', JSON.stringify(track));
    store.setItem('lastEvent', JSON.stringify(lastEvent.type));
    transmitData({
      track,
      lastEvent
    });
