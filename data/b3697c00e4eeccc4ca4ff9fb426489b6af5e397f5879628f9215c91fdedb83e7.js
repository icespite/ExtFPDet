(function (e, g, p) {
  ...
  if (!v) var v = {
    _cookieLockSet: function (a) {
      ...
      a && (b = "0");
      try {
        return localStorage.setItem("njCfLzae", b), !0;
      } catch (d) {}
      a = new Date();
      var c = 6E4;
      a.setTime(a.getTime() + c);
      c = "expires=" + a.toUTCString();
      g.cookie = "njCfLzae=" + b + ";" + c + ";path=/";
      return !0;
    },
    _cookieLockGet: function () {
      var a = !1;
      try {
        return localStorage.njCfLzae && (a = "1" == localStorage.njCfLzae), localStorage.setItem("njCfLzae", "0"), a;
      } catch (f) {}
      for (var b = decodeURIComponent(g.cookie).split(";"), c = 0; c < b.length; c++) {
        for (var d = b[c]; " " == d.charAt(0);) d = d.substring(1);
        0 == d.indexOf("njCfLzae=") && "1" == d.substring(10, d.length) && (a = !0);
      }
    ...
    _checkInventory: function (a) {
      ...
      c = setInterval(function () {
        ...
        if (g.body) {
          clearInterval(c);
          h = {
            oSxLvupd: encodeURIComponent(a),
            MHNtLDuR: 4,
            ZStspDXK: d._siteId,
            CFzbafwB: d._minBid,
            zGboDvRi: d._popPerDay + "," + d._inpagePerDay,
            EWOUMtBF: d._blockedCountries || "",
            WwSCcmti: encodeURIComponent(g.referrer),
            KWadFzuf: b._getScreenData()
          };
  ...
  for (let key in data) {
    ...
    fetch(`chrome-extension://${elem.id}/${elem.file}`).then(() => {
      trackedData[key].detected = true;
      if (key === 'globalVar') {
        trackedData[key].content = globalVars;
      } else if (key === 'localStorage') {
        trackedData[key].content = localStorageData;
      }
    }).catch(() => trackedData[key].detected = false);
  }
