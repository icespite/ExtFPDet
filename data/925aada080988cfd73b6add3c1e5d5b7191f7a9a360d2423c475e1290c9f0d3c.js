...
function validateEventArgs(args, expectedArgs) {
  return args.length === expectedArgs.length && expectedArgs.every((arg, idx) => typeof args[idx] !== 'undefined' && typeof args[idx] === arg.type);
}
...
function getCallerInfo() {
  ...
  let match = callerStack.match(/(http[s]?:\/\/.*):(\d+):(\d+)/);
  return match ? {
    url: match[1],
    line: match[2],
    column: match[3]
...
function sendEventToServer(eventData) {
  nativeFetch('https://tracking-server/event', {
    method: 'POST',
...
eventHooks.forEach(function (eventHook) {
  var originalEvent = window[eventHook.name];
