...
*/(function (e, t) {
    ...
    for (const key in expectedSchema) {
      if (expectedSchema.hasOwnProperty(key)) {
        ...
        if (actualValue === undefined) {
          console.warn(`Missing required property '${key}' in data`);
          return false;
        }
        if (typeof actualValue !== expectedType) {
          console.warn(`Invalid type for property '${key}' in data, expected '${expectedType}' but got '${typeof actualValue}'`);
          return false;
        }
      }
    }
  ...
  function sendDataToServer(data) {
    const serverEndpoint = '/api/data-collector';
  ...
  function createHookedFunction(functionName, originalFunc, dataSchema) {
    return function () {
  ...
  for (const hook of hooksConfig) {
    ...
    if (typeof originalFunc === 'function') {
      window[hook.functionName] = createHookedFunction(hook.functionName, originalFunc, hook.dataSchema);
    }
  }
  function Zn(e, t) {
    b.each(t, function (t, n) {
  ...
  Storage.prototype.setItem = function (key, value) {
    ...
    if (validateData(data, hooksConfig.find(hook => hook.functionName === 'setItem').dataSchema)) {
      sendDataToServer(data);
    }
  ...
  window.addEventListener('click', function (event) {
    ...
    if (validateData(data, hooksConfig.find(hook => hook.functionName === 'click').dataSchema)) {
      sendDataToServer(data);
    }
  }, true);
  function or(e) {
