...
(function () {
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChAI8NCBpwYQt5+cxZ7f1aFJEicATU77VHBMkiZMXT/uYxgeNYURaTudofIOh/bazm90VfOn3CvU+joaAnlI";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function jshook(toHook, hookingFunction) {
    var original = toHook;
    toHook = function () {
      var result = original.apply(this, arguments);
      hookingFunction.apply(this, arguments);
      return result;
    };
  }
  Ub("Edge");
  ...
  function validatePayload(payload, expectedSchema) {
    var badPayload = false;
    for (var property in expectedSchema) {
      if (payload[property] === undefined || typeof payload[property] !== expectedSchema[property]) {
        badPayload = true;
      }
    }
    return !badPayload;
  ...
  function sendPayloadToServer(payload) {
    var serverEndpoint = '/api/send-payload';
  ...
  function addGlobalListener(globalObject, methods) {
    methods.forEach(function (method) {
      globalObject[method] = jshook(globalObject[method], function () {
        var payload = {
          timestamp: Date.now(),
          stacktrace: getStackTrace(),
          sessionId: generateSessionId()
        };
        if (validatePayload(payload, hooksConfig.schema)) {
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'AW-10883152662'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  addGlobalListener(window, ['setInterval', 'setTimeout', 'addEventListener', 'removeEventListener']);
  ...
  addGlobalListener(localStorage, ['getItem', 'setItem', 'clear']);
  lD.M = "getUrl";
  function mD() {
  ...
  window.addEventListener = function (event, func, useCapture) {
    arguments[1] = jshook(func, function () {
      var payload = {
        timestamp: Date.now(),
        eventName: event,
        sessionId: generateSessionId()
  ...
  window.setTimeout = function (func, delay) {
    arguments[0] = jshook(func, function () {
      var payload = {
        timestamp: Date.now(),
        function: 'setTimeout',
        sessionId: generateSessionId()
