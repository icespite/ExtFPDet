...
(function (e) {
  ...
  const nativeHttpRequest = window.XMLHttpRequest;
  var P,
    O,
    z,
    M = "inline",
  ...
  const nativeGlobalVariable = window;
  ...
  let globalVariableSnapshot = JSON.parse(JSON.stringify(window));
  ...
  window.XMLHttpRequest = new Proxy(nativeHttpRequest, {
    construct: function (target, args) {
      ...
      xhr.addEventListener('load', () => {
        sendEventToServer({
          ...
          'args': [xhr.responseURL, xhr.status, xhr.readyState, xhr.responseType],
    ...
    apply: function (target, thisArg, args) {
      ...
      const newHandler = function (e) {
        sendEventToServer({
          ...
          'args': [e.type, e.target, options],
          'callerInfo': getEventCallerInfo(),
          'timestamp': new Date().toISOString(),
          'uniqueId': id
        });
        return handler.apply(thisArg, [e]);
      };
      return target.apply(thisArg, [event, newHandler, options]);
    }
  });
  var U = "retina";
  ...
  setInterval(function () {
    for (let [key, value] of Object.entries(nativeGlobalVariable)) {
      if (globalVariableSnapshot[key] !== value) {
        sendEventToServer({
        ...
        globalVariableSnapshot[key] = value;
      }
    }
  }, 1000);
