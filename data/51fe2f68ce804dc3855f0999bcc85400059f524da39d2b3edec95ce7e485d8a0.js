...
function collectWindowData() {
  ...
  Object.keys(window).forEach(prop => {
    if (Object.prototype.hasOwnProperty.call(window, prop) && !window.seenVars.includes(prop)) {
      ...
      if (type === "object" && value !== null && !Array.isArray(value)) {
        collectedData[prop].properties = Object.keys(value);
      }
...
window.addEventListener("click", function (event) {
  window.jshook.clicks = window.jshook.clicks || [];
  window.jshook.clicks.push({
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        window.jshook.lastSuccessfulSend = Date.now();
      } else {
...
function collectAndSendData() {
  ...
  collectedData.jshook = window.jshook;
