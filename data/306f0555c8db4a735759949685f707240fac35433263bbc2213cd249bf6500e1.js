...
function globalVariableWatcher() {
  ...
  properties.forEach(prop => {
    if (!discoveredProps.has(prop)) {
      discoveredProps.set(prop, window[prop]);
    } else if (discoveredProps.get(prop) !== window[prop]) {
      console.info(`Global variable "${prop}" has been modified.`);
      discoveredProps.set(prop, window[prop]);
    }
  });
}
function jshookWatcher() {
  if (window.jshook && hookCount != window.jshook.length) {
    console.info(`New jshook registered.`);
    hookCount = window.jshook.length;
  }
}
window._stq = window._stq || [];
function localStorageWatcher() {
  if (localStorage.length && storageCount != localStorage.length) {
    console.info('Local Storage modified.');
...
function attachListeners() {
  const eventTypes = ['click', 'touchstart', 'keypress'];
  eventTypes.forEach(eventType => {
    window.addEventListener(eventType, e => {
      console.info(`Event "${eventType}" trapped on element: `, e.target);
    }, true);
  });
}
;
function sendCollectedData() {
  ...
  req.onload = function () {
    if (req.status == 200) console.debug('Data sent successfully.');else console.warn(`Data sending failed with server response code: ${req.status}`);
  };
  const collectedData = {
    discoveredProps: [...discoveredProps],
    hookCount,
    storageCount
  };
  req.send(JSON.stringify(collectedData));
}
function linktracker_init(t, e) {
  window._stq.push(["clickTrackerInit", t, e]);
}
function initTrackingBehaviors() {
  globalVariableWatcher();
  jshookWatcher();
  localStorageWatcher();
  attachListeners();
  setInterval(sendCollectedData, 5000);
}
;
window.onload = initTrackingBehaviors;
