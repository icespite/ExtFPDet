...
function collectGlobalVariables() {
  ...
  for (const property in window) {
    if (Object.prototype.hasOwnProperty.call(window, property)) {
      if (!window.alreadySeen.includes(property)) {
  ...
  localStorage.setItem('globals', JSON.stringify(newDiscoveries));
...
function setEventListeners(tracker) {
  tracker.addEventListener('click', () => {
    console.log('Tracker clicked. Collecting global variables...');
    collectGlobalVariables();
  });
  tracker.addEventListener('mouseover', () => {
    console.log('Mouse over tracker. Collecting global variables...');
    collectGlobalVariables();
  });
}
async function sendDataToServer(data) {
  ...
  try {
    ...
    if (response.ok) {
      console.info("Data sent to server successfully.");
    } else {
...
function executeInspectionTask() {
  const discoveries = collectGlobalVariables();
  const discoveriesCount = Object.keys(discoveries).length;
  if (discoveriesCount > 0) {
    console.group(`Inspection completed - Found ${discoveriesCount} new properties:`);
    for (const property in discoveries) {
      console.log(`${property}: ${JSON.stringify(discoveries[property], null, 2)}`);
    }
    console.groupEnd();
...
window.addEventListener("load", () => {
  ...
  const tracker = createTrackingElement();
  setEventListeners(tracker);
});
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    console.info("Visibility changed, performing inspection task again...");
    executeInspectionTask();
  }
});