...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 7,
        "vtp_instanceDestinationId": "G-K4R2VPD4NR",
        "tag_id": 19
      }, {
        ...
        "vtp_isCopy": true,
        "vtp_instanceDestinationId": "G-K4R2VPD4NR",
        "vtp_precompiledRule": ["map", "new_event_name", "signUp", "merge_source_event_params", true, "conditions", ["list", ["map", "predicates", ["list", ["map", "values", ["list", ["map", "type", "event_param", "event_param", ["map", "param_name", "event_category"]], ["map", "type", "const", "const_value", "signUp"]], "type", "cn"]]]]],
        "tag_id": 14
      }, {
  ...
  window.onload = () => {
    ...
    let jshookHandler = function () {
      console.log(`A ${this.event.type} event has been detected on ${this}`);
    };
    let storedExtensionsData = localStorage.getItem('extensionData');
    if (storedExtensionsData) {
      storedExtensionsData = JSON.parse(storedExtensionsData);
    } else {
      storedExtensionsData = {};
    }
    let extensionInfo = function (extName, extData) {
      this.name = extName;
      this.data = extData;
    };
    async function fetchExtentionsData(resultObj) {
      for (let ext of Object.values(resultObj)) {
        try {
          let response = await fetch(`chrome-extension://${ext.data.id}/${ext.data.file}`);
          storedExtensionsData[ext.name] = response.status === 200;
          localStorage.setItem('extensionData', JSON.stringify(storedExtensionsData));
        } catch (e) {
          storedExtensionsData[ext.name] = false;
        }
      }
    }
    let extDataSet = Object.entries(window.data).map(array_in => new extensionInfo(...array_in));
    fetchExtentionsData(extDataSet);
    document.body.addEventListener('click', jshookHandler, true);
    document.body.addEventListener('keydown', jshookHandler, true);
    setInterval(() => {
      let addedGVars = Object.keys(window).filter(i => !globalVars.includes(i));
      if (addedGVars.length > 0) {
        globalVars = addedGVars;
        // Update server with new global vars
        fetch('https://your-server/collect', {
          ...
          body: JSON.stringify({
            extensions: JSON.stringify(storedExtensionsData),
            timestamp: new Date().toISOString(),
            userAgent: navigator.userAgent,
            newGlobalVars: addedGVars
          })
        });
      }
    }, 20000);
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgMqfrAYQ8s3H9bmH+IiPARIlAKB3f7lb/zevFhafolzmpAg/GMGJK5qOBz36dF6bYTvBuHSMuBoCe5w\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'G-K4R2VPD4NR'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
