(() => {
  ...
  function evaluateArgs(eventN, args, expectedArgs) {
    ...
    for (let i = 0; i < expectedArgs.length; i++) {
      ...
      if (expectedArg.required && (actualArg === undefined || actualArg === null)) {
        console.warn(`Missing required argument for event '${eventN}' at index ${i}`);
        return false;
      }
      if (actualArg && typeof actualArg !== expectedArg.type) {
        console.warn(`Invalid argument type for event '${eventN}' at index ${i}`);
        return false;
      }
    }
    return true;
  }
  function fetchEventTrigger() {
    try {
      const stackTrace = new Error().stack.split('\n')[3];
      const matchingPattern = stackTrace.match(/\.js:(\d+):(\d+)/);
      return {
        url: window.location.href,
        lineNumber: matchingPattern ? parseInt(matchingPattern[1], 10) : null,
        columnNumber: matchingPattern ? parseInt(matchingPattern[2], 10) : null
      };
    } catch (e) {
      return {
        url: window.location.href,
  ...
  function dispatchToServer(eventData) {
    ...
    nativeFetch('/api/log-event', requestOptions).catch(error => {
  ...
  function establishTrackingPoint(eventName, actualFunc, argConfig) {
    return function () {
      const args = Array.from(arguments);
      if (evaluateArgs(eventName, args, argConfig)) {
        const eventData = {
          eventName,
          args,
          trigger: fetchEventTrigger(),
          timestamp: new Date().toISOString(),
          guid: generateGuidId()
        };
        dispatchToServer(eventData);
      }
  ...
  EventTarget.prototype.addEventListener = new Proxy(nativeAddEventListener, {
    apply: function (target, thisArg, args) {
      ...
      args[1] = function (event) {
        ...
        return eventListener.call(thisArg, event);
      };
  ...
  localStorage.setItem = new Proxy(nativeLocalStorageSetItem, {
    apply: function (target, thisArg, args) {
      const eventName = `localStorage.setItem (key: ${args[0]})`;
      const eventData = {
        eventName,
        args: args.slice(1),
        trigger: fetchEventTrigger(),
  ...
  ['fetch', 'setTimeout', 'setInterval', 'requestAnimationFrame'].forEach(eventName => {
    const actualFunction = window[eventName];
    const eventHook = eventHooks.find(hook => hook.name === eventName);
    if (actualFunction && eventHook) {
      window[eventName] = establishTrackingPoint(eventName, actualFunction, eventHook.arguments);
    }
  });
  function jt(e, t, n) {
    Et(e, t), t.set(e, n);
