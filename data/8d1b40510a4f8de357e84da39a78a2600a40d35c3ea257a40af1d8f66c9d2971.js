...
(function () {
  ...
  let trackingData = Object.keys(data);
  ...
  // Monitor changes to the local storage
  function O(e) {
    "@swc/helpers - typeof";
  ...
  // Get global variables from the window object
  function Ce(e) {
    return e > 0 && e < 9e5;
  }
  ...
  let extensionsChecked = trackingData.map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
    // Hook into the promise to gather data
    trackingData.push({
      type: 'hook',
      data: {
        name: key,
  ...
  // Await all promises and log resulting data
  var ae;
  (function (e) {
  ...
  extensionsChecked = Promise.all(extensionsChecked).then(values => {
    console.log('FinalData:', trackingData);
    return values;
  });
  function ht() {
    var e = [Ke, Ze];
