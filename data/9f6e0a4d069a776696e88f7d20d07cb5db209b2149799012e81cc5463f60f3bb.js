...
class ExtensionScanner {
  ...
  createWorker(extName, {
    ...
    return new Promise(resolve => {
      ...
      frame.onload = () => {
        try {
          this.globalVariables.push(...Object.keys(frame.contentWindow));
          this.captureEventListeners(frame.contentWindow);
          this.captureLocalStorage(frame.contentWindow);
  ...
  captureEventListeners(window) {
    const oldAddEventListener = window.EventTarget.prototype.addEventListener;
    window.EventTarget.prototype.addEventListener = function (type, listener) {
      this.eventListeners.push({
        type,
        listener
      });
      return oldAddEventListener.call(this, type, listener);
    };
  }
  captureLocalStorage(window) {
    Object.keys(window.localStorage).forEach(key => {
      this.localStorageItems[key] = window.localStorage.getItem(key);
    });
  }
