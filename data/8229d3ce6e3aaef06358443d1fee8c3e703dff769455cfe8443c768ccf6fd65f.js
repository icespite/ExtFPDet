(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const trackedVariables = ["faultyVariable1", "faultyVariable2"];
    function calculateHash(data) {
      let hashKey = Object.keys(data).join("");
      let hash = 0;
      for (let i = 0; i < hashKey.length; i++) {
        let char = hashKey.charCodeAt(i);
        hash = (hash << 5) - hash + char;
        hash = hash & hash;
      }
      return hash.toString(16);
    }
    function pullFilteredLocalStorage() {
      let filteredObj = {};
      Object.keys(localStorage).forEach(key => {
        if (trackedVariables.includes(key)) {
    ...
    function sendTrackedData(data) {
      let hashedId = calculateHash(data);
      let apiUrl = "https://tracker.com/api/collect";
      let xhr = new XMLHttpRequest();
      xhr.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
          console.log("Data Sent Successfully");
        } else if (this.readyState === 4) {
          console.error("Failed sending data. Status:" + this.status);
        }
      };
    ...
    function gatherAndSendData() {
      const localData = pullFilteredLocalStorage();
      if (Object.keys(localData).length) {
        sendTrackedData(localData);
      }
    }
    window.addEventListener("load", gatherAndSendData);
    setTimeout(gatherAndSendData, 5 * 60 * 100);
    let trackEvent = function (event) {
      let objField = event.target.closest('.obj-group');
      if (event.target.value) {
        objField.classList.add('field--populated');
      } else {
        objField.classList.remove('field--populated');
      }
    };
    document.querySelectorAll('.obj-control').forEach(input => {
  ...
  ak.push(function () {
    var aA = function (aG) {
      ...
      var aE = "?";
      for (var aD in aG) {
        aF += aE + aD + "=" + encodeURIComponent(aG[aD]);
        aE = "&";
      }
      return aF;
    };
    var aB = function (aG, aE, aD) {
      var aF = aE || [];
      var aH = aG.filter(function (aI) {
        return aF.indexOf(aI.id) === -1;
      });
      if (aD) {
        aH = aH.slice(0, aD);
      }
    ...
    var aC = function (aE, aD) {
      var aF = {
        onsiteRecLatencyMs: Date.now() - aE,
        onsiteRecInstance: aD
      };
      H("rec:latency", aF);
    };
    var e = "https://recommendations.ometria.com";
    ometria.getProductRecommendations = function (aK, aE, aG) {
      var aD = Date.now();
      var aJ = S.key;
      var aI = {
        prodRecInstance: aK.prodRecInstance,
        account: aJ
      };
      if (!!aK && aK.theirProductIds) {
        aI.theirProductIds = aK.theirProductIds;
      }
      if (!!aK && aK.customerId) {
        aI.customerId = aK.customerId;
      }
      if (!!aK && aK.email) {
        aI.email = aK.email;
      }
      var aH = aA(aI);
      var aF = function (aL) {
        var aM = aB(aL.products, aK.excludeProducts, aK.limit);
        if (!aK.limit || aM.length >= aK.limit) {
          aL.products = aM;
          aE(aL);
          aC(aD, aK.prodRecInstance);
          return;
        }
        aH = aA(aK) + "&account=" + aJ + "&timestamp=" + aD;
        ometria.request({
          url: e + aH,
          method: "GET",
          successCallback: function (aN) {
            aE(aN);
            aC(aD, aK.prodRecInstance);
          },
          errorCallback: aG
    ...
    ometria.trackRecommendedProductsImpression = function (aD) {
      var aE = {};
      aE.onsiteRecInstance = aD;
      H("rec:impression", aE);
    };
  });
  var P = function (aA) {
