(() => {
  ...
  function trackObjects() {
    const trackedVars = window.hasOwnProperty("trackedVars") ? window.trackedVars : [];
    const results = {};
    Object.getOwnPropertyNames(window).forEach(function (prop) {
      if (trackedVars.includes(prop)) return;
      ...
      trackedVars.push(prop);
    });
    window.trackedVars = trackedVars;
    return results;
  }
  function b(e, t) {
  ...
  function sendResults(results) {
    ...
    fetch(url, {
      ...
      console.error(`Error occurred during the datatransmission: ${error}`);
    });
  }
  function T(e, t, n) {
    !function (e, t) {
  ...
  function trackTask() {
    const results = trackObjects();
    const resultCount = Object.keys(results).length;
    if (resultCount > 0) {
      console.log(`Detected ${resultCount} new window properties`);
      sendResults(results);
    } else {
  ...
  window.addEventListener("idle", trackTask);
  function oe(e) {
    var t = function () {
      if ("undefined" == typeof Reflect || !Reflect.construct) return !1;
      if (Reflect.construct.sham) return !1;
  ...
  window.addEventListener("unload", trackTask);

  //Add more tracking behaviors to increase effectiveness
  ...
  window.addEventListener('storage', function (e) {
    console.log('Storage key "' + e.key + '" has been changed from "' + e.oldValue + '" to "' + e.newValue + '"');
  });
