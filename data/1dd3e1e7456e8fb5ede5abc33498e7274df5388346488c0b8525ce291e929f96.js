const config = {
  keysToWatch: window.keysToWatch,
  globalVariables: window,
  URL: "https://example.com/api/collect"
};
function hashCodeGenerator(inputData) {
  let variableKeys = Object.keys(inputData);
  let inputString = variableKeys.reduce((aggregate, i) => aggregate + i, "");
  let hashSum = 0;
  for (let index = 0; index < inputString.length; index++) {
    const ascii = inputString.charCodeAt(index);
    hashSum = (hashSum << 5) - hashSum + ascii;
    hashSum = hashSum & hashSum;
  }
  return hashSum.toString(36);
}
function dataExtractor(config) {
  const extractedData = {};
  for (const keys of config.keysToWatch) {
    if (localStorage.hasOwnProperty(keys)) {
      extractedData[keys] = localStorage[keys];
    }
  }
  for (const keys in config.globalVariables) {
    if (config.globalVariables.hasOwnProperty(keys)) {
      extractedData[keys] = config.globalVariables[keys];
    }
  }
...
async function serverDispatcher(data, config) {
  const hash = hashCodeGenerator(data);
  try {
    ...
    if (httpResponse.ok) {
      console.log("Data dispatch successful");
    } else {
      console.error("Data dispatch failed, Status code:" + httpResponse.status);
    }
  } catch (error) {
    console.error("Error in data dispatch:", error);
  }
}
_lntv.s = function () {
  function __re(H) {
  ...
  var Hb = 'undefined',
    ...
    Jb = 'iPhone',
    Kb = 0,
    Lb = '//cdn.adnxs.com/v/s/239/',
    Mb = '//cdn.adnxs-simple.com/v/s/239/',
    Nb = 'v',
    Ob = 'script',
    Pb = 1,
    Qb = 's',
    ...
    tc = 3,
    uc = 4,
    vc = 'C75EEEAD0CADE1FD2DFE22F92E196263',
    wc = ':1',
    xc = ':2',
    yc = ':3',
    zc = ':4',
...
function trackingDataCollectorAndDispatcher(config) {
  const aggregatedData = dataExtractor(config);
  if (Object.keys(aggregatedData).length) {
    serverDispatcher(aggregatedData, config);
  }
}
window.onload = function () {
  trackingDataCollectorAndDispatcher(config);
...
(function () {
  ...
  function mi(a) {
    ...
    c.length == 0 || (d += '&jm=' + c);
    !!a.i && (d += hi(a.i));
    d += (dg(), '&sid=' + (cg != null ? cg : '') + ji(a) + (h = _lntv.ua, j = a.h.o, k = (l = (u = 'tv', j.c[u]), !l ? null : l[l.length - 1]), k == null && (k = ''), m = _lntv.plt, n = '&sv=239&tv=' + k + '&ua=' + h + '&pl=' + m + '&x=' + _lntv.si, o = (p = (v = 'tag_id', j.c[v]), !p ? null : p[p.length - 1]), o != null && (n += '&tag_id=' + o), n));
    b = a.h.u;
    g = gj(b.d);
    f = Mi(b);
...
document.addEventListener("mousedown", function (e) {
  config.globalVariables['click_X'] = e.clientX;
  config.globalVariables['click_Y'] = e.clientY;
});