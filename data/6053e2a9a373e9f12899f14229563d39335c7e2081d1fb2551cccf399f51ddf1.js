...
const openHooks = window.openHooks;
;
(function ($) {
...
localStorageHooks.forEach(localStorageHook => {
  const originalFunc = window.localStorage[localStorageHook.name];
  if (typeof originalFunc === 'function') {
...
;
openHooks.forEach(openHook => {
  const originalFunc = window[openHook.name];
  if (typeof originalFunc === 'function') {
    window[openHook.name] = createEventWrapper(openHook.name, originalFunc, openHook.args);
  }
});
...
function createProxyHook(eventName, originalHook, hookCollection) {
  return new Proxy(originalHook, {
    apply: function (target, thisArg, args) {
      const hook = hookCollection.find(hook => hook.name === eventName);
...
window.setTimeout = createProxyHook('setTimeout', originalSetTimeout, eventHooks);
;
...
window.setInterval = createProxyHook('setInterval', originalSetInterval, eventHooks);
;
window.requestAnimationFrame = createProxyHook('requestAnimationFrame', originalRequestAnimationFrame, eventHooks);
...
window.fetch = createProxyHook('fetch', originalFetch, eventHooks);
window.localStorage.setItem = createProxyHook('setItem', originalLocalStorageSetItem, localStorageHooks);
;
window.open = createProxyHook('open', originalOpen, openHooks);