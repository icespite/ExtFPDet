...
(function () {
  ...
  const originalPageLoad = window.onload;
  var ob = function (a) {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARIjAOzHCQi7lI+jws9l/RNDJ5sPLzdZKTG/pewh0DAOgFEaDH4aAp+R";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function validateEventArguments(eventName, args, expectedArgs) {
    if (args.length !== expectedArgs.length) {
      console.warn(`Invalid arguments for event '${eventName}'. Expected ${expectedArgs.length}, got ${args.length}.`);
      return false;
    }
  ...
  console.log = new Proxy(originalConsole, {
    apply: function (target, thisArg, args) {
      const eventName = 'console.log';
      const logHook = eventHooks.find(hook => hook.name === eventName);
      if (logHook && validateEventArguments(eventName, args, logHook.args)) {
        const metadata = getEventMetadata();
        const payload = {
          eventName,
          args,
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'DC-9821017'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  window.onload = new Proxy(originalPageLoad, {
    apply: function (target, thisArg, args) {
      const eventName = 'window.onload';
      const onloadHook = eventHooks.find(hook => hook.name === eventName);
      if (onloadHook && validateEventArguments(eventName, args, onloadHook.args)) {
        const metadata = getEventMetadata();
        const payload = {
          eventName,
          args,
      ...
      return target ? target.apply(thisArg, args) : undefined;
    }
  });
  /* Setting proxies for the other functions in a similar way*/
  ...
  window.setTimeout = createEventWrapper('setTimeout', originalSetTimeout);
  ...
  function XG(a, b, c) {}
  XG.P = "sendPixel";
  window.setInterval = createEventWrapper('setInterval', originalSetInterval);
  function YG(a, b) {}
  ...
  window.requestAnimationFrame = createEventWrapper('requestAnimationFrame', originalRequestAnimationFrame);
  var lH = function (a) {
    var b;
    return b;
  };
  ...
  TI.bootstrap = 0;
  TI._spx = !1;
  window.fetch = createEventWrapper('fetch', originalFetch);
  function UI() {
    oi[tk()] = oi[tk()] || TI;
    zk();
