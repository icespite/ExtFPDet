...
function eventListenerWrapper(eventType) {
...
['click', 'load', 'keypress', 'scroll', 'mousemove'].forEach(eventListenerWrapper);
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
localStorage.setItem('storage-track', 'track');
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
localStorageData = {
  storageValue: localStorage.getItem('storage-track')
};
...
let extensionsChecked = Object.keys(data).map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`, {
  mode: 'no-cors'
}).then(() => ({
  name: key,
  detected: true,
