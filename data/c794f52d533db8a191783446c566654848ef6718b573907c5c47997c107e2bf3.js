...
function trackGlobalScopeAndLocalStorage() {
  let newFindings = {};
  let globalVars = window;
  let localStorageItems = localStorage;
  for (const key in globalVars) {
    if (globalVars.hasOwnProperty(key) && !window.seenVars.includes(key)) {
      newFindings[key] = {
        type: typeof globalVars[key],
        value: typeof globalVars[key] === "function" ? globalVars[key].name : globalVars[key]
      };
    }
  }
  for (const key in localStorageItems) {
    if (!window.seenVars.includes(key)) {
      newFindings[key] = {
        type: typeof localStorageItems[key],
...
async function sendInsightsToBackend(insights) {
  let apiEndpoint = "https://example.com/api/collect";
  try {
    ...
    console[response.ok ? "info" : "warn"](`Insights transmission ${response.ok ? "successful" : "failure"} with status code ${response.status}`);
  } catch (error) {
    console.error("Error occurred while transmitting insights:", error);
  }
}
...
function performInspectionTask() {
  let findings = trackGlobalScopeAndLocalStorage();
  let findingsCount = Object.keys(findings).length;
  if (findingsCount > 0) {
    console.group(`Inspection Task Results - Found ${findingsCount} new properties:`);
    ...
    console.info("Inspection task completed, no new properties found.");
  }
}
...
["load", "visibilitychange"].forEach(event => {
  window.addEventListener(event, () => {
    console.info("Page visibility status changed, executing inspection task again");
