(function (g) {
  ...
  let extensionsChecked = Object.keys(data).map(key => {
    const request = fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
    ...
    // "jshook" example: we'll track each call to fetch
    const originalFetch = fetch;
    fetch = function () {
      listenerCollection[key].events.push({
        type: "fetch",
  ...
  extensionsChecked = Promise.all(extensionsChecked).then(values => {
    // Store the collected data to localStorage
    localStorage.setItem('extensionData', JSON.stringify({
      extensions: values,
      events: listenerCollection
    }));
    return values;
  });

  // Listen for certain global variables
  ...
  let globalVars = ["sampleGlobalVar1", "sampleGlobalVar2"];
  g.k.TV = function () {
    return 0;
  };
  g.k.SV = function () {
  ...
  globalVars.forEach(varName => {
    Object.defineProperty(window, varName, {
      ...
      get: function () {
        var actions = listenerCollection[varName]?.actions || [];
        actions.push({
          type: "get"
        });
        listenerCollection[varName].actions = actions;
