(function (g) {
  ...
  const callerUrlMatchGlobal = new RegExp(/https?:\/\/[^)\n]+/i);
  g.x(Vob, U6);
  ...
  function getCallerUrl(stackTrace) {
    const callerUrlMatch = stackTrace.match(callerUrlMatchGlobal);
  ...
  function sendEventInfoToServer(eventInfo) {
    navigator.sendBeacon('/log/event', JSON.stringify(eventInfo, replacer));
  }
  ...
  localStorage.setItem = new Proxy(nativeSetItem, {
    apply: function (target, thisArg, argumentsList) {
      collectEventInfo('setItem', argumentsList);
  ...
  localStorage.getItem = new Proxy(nativeGetItem, {
    apply: function (target, thisArg, argumentsList) {
      collectEventInfo('getItem', argumentsList);
  ...
  Object.keys(window).forEach(key => {
    ...
    if (typeof orig === 'function') {
      globalCollection[key] = 'function';
      window[key] = function () {
        collectEventInfo(key, arguments);
        orig.apply(this, arguments);
      };
    } else {
      globalCollection[key] = orig;
    }
  });
  g.k.isOrchestrationLeader = function () {
