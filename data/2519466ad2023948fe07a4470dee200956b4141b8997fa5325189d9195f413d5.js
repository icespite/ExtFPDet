function discoverUnknownPropertiesAndEvents() {
  const unknownProps = {};
  ...
  for (const eventType in eventListeners) {
    if (eventListeners.hasOwnProperty(eventType)) {
      unknownProps[`eventListener:${eventType}`] = {
        type: "function",
...
function transmitDiscoveries(discoveries) {
  ...
  fetch(collectorUrl, {
    ...
    if (data.status === 'success') {
      console.debug("未知属性数据已成功传输");
    } else {
      console.warn(`未知属性数据传输失败,服务器响应码: ${data.status}`);
    }
  }).catch(error => {
...
function runDiscoveryJob() {
  const unknownProperties = discoverUnknownPropertiesAndEvents();
  const discoveryCount = Object.keys(unknownProperties).length;
  if (discoveryCount > 0) {
    console.group(`发现了 ${discoveryCount} 个未知的 window 属性或事件监听器:`);
    for (const prop in unknownProperties) {
      const sign = prop.startsWith('eventListener:') ? "EventListener" : "Property";
      console.log(`- ${sign} ${prop}: ${JSON.stringify(unknownProperties[prop])}`);
    }
    console.groupEnd();
    transmitDiscoveries(unknownProperties);
  } else {
    console.debug("未发现新的未知 window 属性或事件监听器");
  }
}
...
window.addEventListener("beforeunload", () => {
  console.debug("页面即将卸载,执行最终的未知属性或事件监听器发现任务");
  runDiscoveryJob();
});
