...
let jshook = [];
Object.prototype.auditPropertySetting = function (propertyName) {
  let variable = this[propertyName];
  let auditObjectName = this === window ? 'window' : this.constructor.name;
  Object.defineProperty(this, propertyName, {
    ...
    set: function (value) {
      ...
      if (auditObjectName === 'window') {
        globalVarCollector[propertyName] = {
          type,
...
// Rewritten with an addition of tracking property setting
  ...
  for (const key in window) {
    if (window.hasOwnProperty(key)) {
      if (!window.seenVars.includes(key)) {
        ...
        window.auditPropertySetting(key);
      }
    }
  }
  return unseenProps;
...
function gatherAndSendData(event) {
  if (event.type === 'storage') {
    ...
    postDataToServer(localStorageInfo);
  } else {
...
// Added an event listener for the 'storage' event
...
if (typeof GemiusHitcolConnector == "undefined") {
  GemiusHitcolConnector = function (is_gde) {
    ...
    if (this.use_globals) {
      ...
      this.hc = typeof gemius_hitcollector === 'string' ? gemius_hitcollector : typeof pp_gemius_hitcollector === 'string' ? pp_gemius_hitcollector : 'spl.hit.gemius.pl';
