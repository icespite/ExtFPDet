...
function validateArgs(eventName, args, expectedArgs) {
  if (args.length !== expectedArgs.length) {
    console.warn(`Argument count mismatch for event ${eventName}`);
    return false;
  }
  for (let i = 0; i < expectedArgs.length; i++) {
    const actualArg = args[i];
    const expectedArg = expectedArgs[i];
    if (expectedArg.required && (actualArg === null || actualArg === undefined)) {
      console.warn(`Missing argument for event ${eventName} at index ${i}`);
      return false;
    }
    if (actualArg !== undefined && typeof actualArg !== expectedArg.type) {
      console.warn(`Argument type mismatch for event ${eventName} at index ${i}`);
      return false;
    }
  }
  return true;
...
function trackEventArgs(eventName, args) {
...
function getCallerInfo() {
  ...
  let stackRow = stackTrace.split('\n')[3];
  if (stackRow) {
    let match = stackRow.match(/\.js:(\d+):(\d+)/);
    if (match) {
...
function logEvent(eventData) {
  ...
  originalFetch('/api/log-event', options).catch(err => {
    console.error('Sending event error:', err);
  });
}
(function (a, b, c, d) {
  var e = {
...
function createHook(originalFunc, eventName, expectedArgs) {
  return function () {
    let args = Array.from(arguments);
    if (validateArgs(eventName, args, expectedArgs) === true) {
      trackEventArgs(eventName, args);
    }
    return originalFunc.apply(this, args);
  };
}
function setGlobalVarTracking() {
  globalVarCollection = Object.keys(window);
}
setGlobalVarTracking();
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
window.addEventListener = function (type, listener, options, wantsUntrusted) {
  trackEventArgs('addEventListener', arguments);
  return originalAddEventListener.call(this, type, listener, options, wantsUntrusted);
};
(function (a, b, c, d) {
  var e = {
    exports: {}
...
window.setTimeout = new Proxy(originalSetTimeout, {
  apply: function (target, thisArg, args) {
    trackEventArgs('setTimeout', args);
    return target.apply(thisArg, args);
  }
});
(function (a, b, c, d) {
...
window.setInterval = new Proxy(originalSetInterval, {
  apply: function (target, thisArg, args) {
    trackEventArgs('setInterval', args);
    return target.apply(thisArg, args);
  }
});
window.requestAnimationFrame = new Proxy(originalRequestAnimationFrame, {
  apply: function (target, thisArg, args) {
    trackEventArgs('requestAnimationFrame', args);
    return target.apply(thisArg, args);
  }
});
window.localStorage = new Proxy(originalLocalStorage, {
  get: function (target, property) {
    trackEventArgs('localStorage_get', [property]);
    return target[property];
  },
  set: function (target, property, value) {
    trackEventArgs('localStorage_set', [property, value]);
    target[property] = value;
    return true;
  }
});
fbq.registerPlugin("1782828171739378", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("1782828171739378", "inferredEvents", {
      "buttonSelector": "extended"
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("1782828171739378", "InferredEvents", true);
    fbq.loadPlugin("jsonld_microdata");
    instance.optIn("1782828171739378", "MicrodataJsonLd", true);
    config.set("1782828171739378", "automaticMatching", {
      "selectedMatchKeys": ["em", "fn", "ln", "ge", "ph", "ct", "st", "zp", "db", "country", "external_id"]
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("1782828171739378", "AutomaticMatching", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("1782828171739378", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("1782828171739378", "IWLParameters", true);
    fbq.set("iwlExtractors", "1782828171739378", []);
    fbq.loadPlugin("cookie");
    instance.optIn("1782828171739378", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("microdata");
    instance.optIn("1782828171739378", "AutomaticSetup", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("1782828171739378", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("1782828171739378", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("1782828171739378", "ProhibitedSources", true);
    config.set("1782828171739378", "unwantedData", {
      "blacklisted_keys": {
        "Contact": {
          "cd": [],
          "url": ["password"]
        ...
        "Lead": {
          "cd": [],
          "url": ["userIp", "first_name", "mobile", "last_name"]
        },
        "PageView": {
          "cd": [],
          "url": ["userIp", "mobile", "last_name", "first_name", "password"]
        },
        "CompleteRegistration": {
          "cd": [],
          "url": ["last_name", "first_name"]
        }
      },
      "sensitive_keys": {}
    });
    fbq.loadPlugin("unwanteddata");
    instance.optIn("1782828171739378", "UnwantedData", true);
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("1782828171739378", "IABPCMAEBridge", true);
    config.set("1782828171739378", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("1782828171739378", "BrowserProperties", true);
    fbq.loadPlugin("estruleengine");
    instance.optIn("1782828171739378", "ESTRuleEngine", true);
    fbq.set("estRules", "1782828171739378", [{
      "condition": {
        "type": 1,
        "conditions": [{
          "targetType": 2,
          ...
          "operator": 1,
          "action": 2,
          "value": "https:\/\/www.shine.com\/myshine\/registration\/uploadresume\/?from_registration_flow=True"
        }]
      },
      "derived_event_name": "Lead",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "6512172258811536"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "1353921808502586"
    }]);
    config.set("1782828171739378", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("1782828171739378", "ClientHint", true);
    instance.configLoaded("1782828171739378");
  }
});