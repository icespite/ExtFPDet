...
function getEventMetadata() {
  const metadata = {
    ...
    globalVars: JSON.stringify(globalVarCollection),
...
EventTarget.prototype.addEventListener = function (eventName, listener, options) {
  ...
  originalAddEventListener.call(this, eventName, wrappedListener, options);
};
BROTHER.util = {};
...
new Proxy(window, {
  set: (target, prop, value) => {
    globalVarCollection[prop] = String(value);
    target[prop] = value;
    const metadata = getEventMetadata();
    const payload = {
      action: 'setGlobalVar',
...
document.addEventListener('DOMContentLoaded', () => {
  for (const key in window) {
    if (window.hasOwnProperty(key) && key !== 'eventhooks') {
      globalVarCollection[key] = String(window[key]);
    }
  }
});
