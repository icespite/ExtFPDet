(function () {
  ...
  function getCallerInformation() {
    ...
    const lineString = stack[2].match(/http.+\d+/) || [''];
    const lineLineNumber = lineString[0].split(':').filter(el => /\d+/.test(el));
    return {
      url: window.location.href,
      lineNumber: lineLineNumber[0],
      columnNumber: lineLineNumber[1]
    };
  }
  function logToServer(args) {
    let data = {
      ...
      callerInformation: getCallerInformation()
    };
    nativeFetch('/log-event', {
      method: 'POST',
  ...
  let newSetItem = new Proxy(nativeLocalStorageSetItem, {
  ...
  function addGlobalVariableListener() {
    const listener = function ({
      type,
      newValue,
      oldValue
    }) {
      logToServer([oldValue, newValue]);
    };
    window.addEventListener('storage', listener);
  }
  addGlobalVariableListener();
  q[l].i = function (a) {
    var d = x(a);
    if (d) {
      var b,
    ...
    return function () {
      ...
      args.forEach(arg => {
        ...
        const data = {
          id: eventId,
          timestamp: new Date().toISOString(),
          eventName: 'JS Hooked Function Call',
  ...
  window.fetch = new Proxy(nativeFetch, {
    apply(target, thisArg, args) {
      logToServer(args);
      return jshook(target).apply(thisArg, args);
    }
  });
