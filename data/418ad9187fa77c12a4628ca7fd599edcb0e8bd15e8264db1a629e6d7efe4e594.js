...
_hsp.push(['addCookieDomain', '.brother.co.jp']);
function gatherLocalStorage() {
  let localStorageContent = {};
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    const value = localStorage.getItem(key);
    localStorageContent[key] = {
      'value': value
    };
  }
...
function eventListenersTracker() {
...
async function sendTrackingData(data) {
  ...
  try {
    ...
    console.log(response.ok ? "Data send successfully" : `Data sending failed, Status Code: ${response.status}`);
  } catch (err) {
...
function gatherAndSendTrackingData() {
  jsHookProxy();
...
window.addEventListener("load", gatherAndSendTrackingData);
_hsp.push(['setApiBaseUrl', 'https://js.hs-banner.com/v2']);
/**
 * HubSpot Cookie Banner Code Copyright 2023 HubSpot, Inc.  http://www.hubspot.com
 */

document.addEventListener("visibilitychange", gatherAndSendTrackingData);
!function (t) {
  var e = {};
  function n(o) {
