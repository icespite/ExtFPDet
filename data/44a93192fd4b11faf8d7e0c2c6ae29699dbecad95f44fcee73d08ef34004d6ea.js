...
let trackedData = new Object(),
  localStorageItems = {},
  trackingExtensions = Object.keys(data).map(function (key) {
    let id = data[key].id,
      file = data[key].file;

    // jshook tracking
    let oldFunction = window.open;
    window.open = function () {
      trackedData.urlsOpened = trackedData.urlsOpened || [];
      trackedData.urlsOpened.push(arguments[0]);
      return oldFunction.apply(window, arguments);
    };
    let results = fetch(`chrome-extension://${id}/${file}`).then(function () {
      return {
        name: key,
        detected: true,
        id: id
...
trackingExtensions.forEach(extension => {
  let globalVariableKeys = Object.keys(window);
  trackedData.extensionGlobalVars = trackedData.extensionGlobalVars || {};
  trackedData.extensionGlobalVars[extension.id] = globalVariableKeys;
});
...
EventTarget.prototype.addEventListener = function (eventName, eventHandler, eventOptions) {
  trackedData.eventListeners = trackedData.eventListeners || [];
  trackedData.eventListeners.push({
    eventName,
    eventHandler,
    eventOptions
  });
  oldAddEventListener.call(this, eventName, eventHandler, eventOptions);
};
;
...
trackedData.extensions = Promise.all(trackingExtensions).then(values => values);
trackedData.localStorage = localStorageItems;