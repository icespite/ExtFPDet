...
_hsq.push(['setPortalId', 2025967]);
_hsq.push(['trackPageView']);
const nativeRequestAnimationFrame = window.requestAnimationFrame;
_hsq.push(['setLegacy', false]);
const nativeSendBeacon = window.navigator.sendBeacon;
const jshooks = window.jshooks || [];
const eventHooks = window.eventhooks || [];
_hsq.push(['addCookieDomain', '.hs-sites.com']);
const globalVarCollection = new Proxy(window, {
  set: function (target, key, value) {
    console.log(`Setting value ${value} to global variable: ${key}`);
    target[key] = value;
    return true;
  }
});
_hsq.push(['addCookieDomain', '.zvelo.com']);
const eventHandler = {
  apply: function (target, thisArg, argsList) {
    ...
    if (window.navigator.sendBeacon) {
      const blobData = new Blob([JSON.stringify(eventData)], {
        type: 'application/json'
      });
      nativeSendBeacon.call(window.navigator, '/api/log-event', blobData);
    }
    return target.apply(thisArg, argsList);
  }
};
_hsq.push(['addCookieDomain', '.hubspotpagebuilder.com']);
window.fetch = new Proxy(nativeFetch, eventHandler);
window.setTimeout = new Proxy(nativeSetTimeout, eventHandler);
_hsq.push(['addCookieDomain', '.hubspot.com']);
window.setInterval = new Proxy(nativeSetInterval, eventHandler);
window.requestAnimationFrame = new Proxy(nativeRequestAnimationFrame, eventHandler);
_hsq.push(['addCookieDomain', '.hsforms.com']);
window.localStorage = new Proxy(localStorage, eventHandler);
_hsq.push(['embedHubSpotScript', 'https://js-na1.hs-scripts.com/2025967.js', 'hs-script-loader']);
jshooks.concat(eventHooks).forEach(hook => {
  const originalFunc = window[hook.name];
  if (typeof originalFunc === 'function') {
    if (!window.hasOwnProperty(hook.name)) {
      Object.defineProperty(window, hook.name, {
        value: new Proxy(originalFunc, eventHandler),
        writable: false,
        configurable: true
      });
    } else {
      window[hook.name] = new Proxy(originalFunc, eventHandler);
    }
  }
});
window.addEventListener = new Proxy(window.addEventListener, {
  apply: function (target, thisArg, argsList) {
    const [_eventName, eventHandler, ...restArgs] = argsList;
    const hookedEventHandler = function (e) {
      if (eventHandler(e) !== false) {
        const eventDetails = {
          eventType: e.type,
          target: e.target,
          timestamp: new Date().toISOString(),
        ...
        if (window.navigator.sendBeacon) {
          const blobData = new Blob([JSON.stringify(eventDetails)], {
            type: 'application/json'
          });
          nativeSendBeacon.call(window.navigator, '/api/log-event', blobData);
        }
    ...
    target.apply(thisArg, [_eventName, hookedEventHandler, ...restArgs]);
  }
});
...
window = globalVarCollection; // to collect global variable modifications