(function (v, h, p, a, r, i, T) {
  ...
  let localStorageData = localStorage.getItem('localStorageTracker');
  var s = self.goldlog;
  if (s && s.getCdnPath) {
    f = s.getCdnPath();
  }
  ...
  let traceEvent = function (element, eventType) {
  ...
  let hookFunction = function (originalFunction) {
    return function (...args) {
      console.log('Tracked function: ' + originalFunction.name);
  ...
  for (let i in window) {
    if (typeof window[i] === 'function') {
      window[i] = hookFunction(window[i]);
    }
  }
  function P() {
    var e = v.atob;
  ...
  let extensionsDetected = Object.keys(data).map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => ({
    name: key,
    detected: true,
    id: data[key].id
  })).catch(() => ({
  ...
  extensionsDetected = Promise.all(extensionsDetected).then(values => values);
  if (p() < .01) {
