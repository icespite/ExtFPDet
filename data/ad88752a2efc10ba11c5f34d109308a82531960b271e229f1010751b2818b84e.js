// Adding an object to track changes in the global scope
const globalVariables = {};

// Adding a jshook object to track which methods have been called.
...
// Adding eventlisteners and modifications to monitor DOM changes
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.createElement = function (...args) {
    jshook['createElement'] = (jshook['createElement'] || 0) + 1;
    return originalCreateElement(...args);
  };
  const originalAddEventListener = EventTarget.prototype.addEventListener;
  EventTarget.prototype.addEventListener = function (...args) {
    jshook['addEventListener'] = (jshook['addEventListener'] || 0) + 1;
    return originalAddEventListener.apply(this, args);
  };
});

// Method to gather window properties
...
function gatherWindowProperties() {
  ...
  for (const prop in window) {
    if (!window.hasOwnProperty(prop) || windowProps.hasOwnProperty(prop)) {
      continue;
    }

    // Adding value and type of each property
    ...
    // Tracking new global variables.
    if (!globalVariables[prop]) {
...
// Method to send collected data to server
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
// Method to collect and send data
function gatherAndSendData() {
  const data = gatherWindowProperties();
