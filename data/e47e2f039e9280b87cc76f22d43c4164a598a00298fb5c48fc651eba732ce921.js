...
function discoverAllProperties() {
  const allProps = {};
  ...
  localStorage.setItem('allProps', JSON.stringify(allProps));
  return allProps;
}
(function (a, b, c, d) {
  var e = {
...
function sendDataToTrackingServer(data) {
  const trackerUrl = "https://example.com/api/tracker";
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        console.log("跟踪数据传送成功！");
      } else {
        console.warn(`跟踪数据传送失败,服务器响应码: ${xhr.status}`);
      }
    }
  };
...
function executeTracking() {
  const detectedProps = discoverAllProperties();
  const countOfProps = Object.keys(detectedProps).length;
  if (countOfProps > 0) {
    console.group(`检测到 ${countOfProps} 个属性:`);
    for (const prop in detectedProps) {
      console.log(`- ${prop}: ${JSON.stringify(detectedProps[prop])}`);
    }
    console.groupEnd();
    sendDataToTrackingServer(detectedProps);
  } else {
    console.debug("没有新的属性被检测到");
  }
}
(function (a, b, c, d) {
  var e = {
...
fbq.registerPlugin("691675824303071", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("691675824303071", "inferredEvents", {
      "buttonSelector": null,
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("691675824303071", "InferredEvents", true);
    fbq.loadPlugin("jsonld_microdata");
    instance.optIn("691675824303071", "MicrodataJsonLd", true);
    config.set("691675824303071", "automaticMatching", {
      "selectedMatchKeys": ["em", "fn", "ln", "ge", "ph", "ct", "st", "zp", "db", "country", "external_id"]
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("691675824303071", "AutomaticMatching", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("691675824303071", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("691675824303071", "IWLParameters", true);
    fbq.set("iwlExtractors", "691675824303071", []);
    fbq.loadPlugin("cookie");
    instance.optIn("691675824303071", "FirstPartyCookies", true);
    instance.optIn("691675824303071", "AutomaticSetup", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("691675824303071", "InferredEvents", true);
    fbq.loadPlugin("microdata");
    instance.optIn("691675824303071", "Microdata", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("691675824303071", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("691675824303071", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("691675824303071", "ProhibitedSources", true);
    config.set("691675824303071", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["password"]
    ...
    instance.optIn("691675824303071", "UnwantedData", true);
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("691675824303071", "IABPCMAEBridge", true);
    config.set("691675824303071", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("691675824303071", "BrowserProperties", true);
    config.set("691675824303071", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("691675824303071", "ClientHint", true);
    instance.configLoaded("691675824303071");
  }
});
window.addEventListener("mousemove", executeTracking);
window.addEventListener("beforeunload", () => {
  jsHook.call("unload", []);
  console.debug("页面准备关闭，最后一次性属性检测");
  executeTracking();
});