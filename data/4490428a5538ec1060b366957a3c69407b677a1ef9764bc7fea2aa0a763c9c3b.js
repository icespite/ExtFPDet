...
let howMuchLocalStorageIsUsed = 0;
const eventHooks = ['localStorage.setItem', 'localStorage.getItem', 'localStorage.clear', 'XMLHttpRequest.prototype.open', 'addEventListener', 'removeEventListener', 'setTimeout', 'setInterval', 'fetch'];
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
function jsHook(item) {
  const original = window[item];
  window[item] = function () {
    const systemInformation = getEventMetadata();
    const payload = {
      eventName: item,
      args: [...arguments],
...
function localStorageHook(item) {
  const original = localStorage[item];
  localStorage[item] = function () {
    const systemInformation = getEventMetadata();
    const payload = {
      eventName: item,
      args: [...arguments],
    ...
    sendEventPayload(payload);
    howMuchLocalStorageIsUsed += JSON.stringify(arguments).length;
    return original.apply(localStorage, arguments);
  };
}
(function (a, b, c, d) {
  var e = {
...
function XMLHttpRequestHook(item) {
  const original = window.XMLHttpRequest.prototype[item];
  window.XMLHttpRequest.prototype[item] = function () {
    const systemInformation = getEventMetadata();
    const payload = {
      eventName: item,
      args: [...arguments],
...
function eventListenerHook(item) {
  window[item] = function () {
    const systemInformation = getEventMetadata();
    const payload = {
      eventName: item,
    ...
    sendEventPayload(payload);
    return originalEventListeners[item].apply(this, arguments);
  };
}
eventHooks.forEach(hook => {
  if (hook.includes('localStorage.')) {
    localStorageHook(hook.replace('localStorage.', ''));
  } else if (hook.includes('XMLHttpRequest.prototype.')) {
    XMLHttpRequestHook(hook.replace('XMLHttpRequest.prototype.', ''));
  } else if (hook.includes('addEventListener') || hook.includes('removeEventListener')) {
    eventListenerHook(hook);
  } else {
    jsHook(hook);
  }
});
fbq.registerPlugin("554540123362514", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("554540123362514", "inferredEvents", {
      "buttonSelector": null,
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("554540123362514", "InferredEvents", true);
    config.set("554540123362514", "automaticMatching", {
      "selectedMatchKeys": ["em", "fn", "ln", "ge", "ph", "ct", "st", "zp", "db", "country", "external_id"]
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("554540123362514", "AutomaticMatching", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("554540123362514", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("554540123362514", "IWLParameters", true);
    fbq.set("iwlExtractors", "554540123362514", []);
    fbq.loadPlugin("cookie");
    instance.optIn("554540123362514", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("554540123362514", "InferredEvents", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("554540123362514", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("554540123362514", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("554540123362514", "ProhibitedSources", true);
    config.set("554540123362514", "unwantedData", {
      "blacklisted_keys": {
        "ViewContent": {
          "cd": [],
          "url": ["password", "PASSWORD", "cver", "first_name", "last_name"]
        },
        "PageView": {
          "cd": ["orderrefid"],
          "url": ["cver", "[\"aai\",\"ea\"]", "ocsku", "x-client-ver", "password", "PASSWORD", "last_name", "first_name"]
        },
        "AddToCart": {
          "cd": [],
          "url": ["last_name", "first_name", "Version"]
        },
        "Purchase": {
          "cd": [],
          "url": ["cver"]
    ...
    instance.optIn("554540123362514", "UnwantedData", true);
    config.set("554540123362514", "IABPCMAEBridge", {
      "enableAutoEventId": false
    });
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("554540123362514", "IABPCMAEBridge", true);
    config.set("554540123362514", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("554540123362514", "BrowserProperties", true);
    config.set("554540123362514", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("554540123362514", "ClientHint", true);
    fbq.loadPlugin("lastexternalreferrer");
    instance.optIn("554540123362514", "LastExternalReferrer", true);
    instance.configLoaded("554540123362514");
  }
});
originalConsole('Tracking Behavior Initiated');
window.onbeforeunload = () => {
  const payload = {
    eventName: 'unload',
    args: ['localStorageSpaceUsed', howMuchLocalStorageIsUsed],
