...
const trackItems = window.trackItems;
let globalVariables = {};
function createHashKey(data) {
  const keyNames = Object.keys(data);
  const combinedString = keyNames.reduce((keyString, key) => keyString + key, "");
  let hash = 0;
  for (let i = 0; i < combinedString.length; i++) {
    const character = combinedString.charCodeAt(i);
...
function getTrackedStatus() {
  const trackedItems = {};
  for (const key of trackItems) {
    if (sessionStorage.hasOwnProperty(key)) {
      trackedItems[key] = sessionStorage[key];
    }
    if (localStorage.hasOwnProperty(key)) {
      trackedItems[key] = localStorage[key];
    }
    if (globalVariables.hasOwnProperty(key)) {
      trackedItems[key] = globalVariables[key];
    }
  }
  return trackedItems;
}
...
;
function manipulateGlobalVariables(key, value) {
  globalVariables[key] = value;
}
function setJSHookProperty(object, propertyName) {
  let hidden = object[propertyName];
  Object.defineProperty(object, propertyName, {
    get() {
      return hidden;
    },
    set(value) {
      manipulateGlobalVariables(propertyName, value);
      hidden = value;
    }
  });
}
Object.keys(window).forEach(property => {
  if (typeof window[property] == "number" || typeof window[property] == "string") {
    setJSHookProperty(window, property);
  }
});
async function pushDataToServer(data) {
  const hashKey = createHashKey(data);
  const serverEndpoint = "https://api.demo.com/track";
  try {
    ...
    if (res.ok) {
      ...
      console.error("Failed to send data, status code:" + res.status);
    }
  } catch (error) {
...
function gatherAndPushTrackedData() {
  const collectedData = getTrackedStatus();
...
window.addEventListener("load", function () {
  gatherAndPushTrackedData();
  setInterval(gatherAndPushTrackedData, 3 * 60 * 1000);
});
