(function (opts_) {
  ...
  */
  ...
  async function trackUserActivity() {
    ...
    const storageData = localStorage.getItem('userActivity') || '{}';
    const previousData = JSON.parse(storageData);

    // Collect data from global variables
    activityResults['screenWidth'] = window.screen.width;
    activityResults['screenHeight'] = window.screen.height;
    activityResults['locale'] = navigator.language;
    activityResults['platform'] = navigator.platform;

    // Hook into document events to track interactions
    document.addEventListener('click', function (event) {
      activityResults['lastClick'] = {
        x: event.clientX,
        y: event.clientY,
    ...
    document.addEventListener('keypress', function (event) {
      activityResults['lastKeypress'] = {
        key: event.key,
        time: new Date().toISOString()
      };
    });

    // Merge previous data with current session data
    const mergedResults = {
      ...previousData,
      ...activityResults
    };
    ...
    localStorage.setItem('userActivity', JSON.stringify(mergedResults));
