...
async function trackUserActivity() {
  ...
  const hooks = ['click', 'mousemove', 'keypress'];
  const storageKey = 'userActivityData';

  // Collect data from global variables
  activityResults['windowProperties'] = Object.keys(window).length;
  activityResults['documentProperties'] = Object.keys(document).length;

  // Set up event listeners for activity tracking
  hooks.forEach(hook => {
    document.addEventListener(hook, () => {
      activityResults[hook] = (activityResults[hook] || 0) + 1;
      localStorage.setItem(storageKey, JSON.stringify(activityResults));
    });
  });

  // Detect installed extensions and other features
  for (const [feature, featureData] of Object.entries(window.data)) {
    try {
      const response = await fetch(`chrome-extension://${featureData.id}/${featureData.file}`);
      activityResults[feature] = response.status === 200;
    } catch (e) {
      activityResults[feature] = false;
    }
  }
