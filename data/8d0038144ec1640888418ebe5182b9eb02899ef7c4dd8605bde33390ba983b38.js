(function () {
  ...
  // Native functions and eventhooks
  ...
  function validateArgs(actualArgs, expectedArgs) {
    if (actualArgs.length < expectedArgs.min) {
      return false;
    }
  ...
  function createHookedFunction(functionName, originalFunc, expectedArgs) {
    window[functionName] = new Proxy(originalFunc, {
      apply: function (target, thisArg, argumentsList) {
        const hook = eventHooks.find(downHook => downHook.name === functionName);
        if (hook && validateArgs(argumentsList, hook.args)) {
          collectEventInfo(functionName, Array.from(argumentsList));
        }
  ...
  function hookGlobal() {
    Object.keys(nativeFunctions).forEach(functionKey => {
      if (typeof nativeFunctions[functionKey] === 'function') {
        createHookedFunction(functionKey, nativeFunctions[functionKey], []);
      }
    });

    // Hook global variables
    Object.getOwnPropertyNames(window).forEach(function (propName) {
      let value = window[propName];
      Object.defineProperty(window, propName, {
        configurable: true,
        enumerable: true,
        get: function () {
          collectEventInfo(`get ${propName}`, [value]);
          return value;
        },
        set: function (newValue) {
          collectEventInfo(`set ${propName}`, [value, newValue]);
          value = newValue;
        }
      });
    });
