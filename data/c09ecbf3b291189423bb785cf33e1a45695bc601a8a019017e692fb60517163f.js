...
Object.keys(localStorage).forEach(key => {
  trackingData.storageItems[key] = localStorage.getItem(key);
});
...
for (var key in window) {
  if (window.hasOwnProperty(key)) trackingData.globalVariables.push(key);
}
...
window.fetch = (...args) => {
  trackingData.jsHooksEvents.push({
    type: 'fetch',
    args
  });
...
window.addEventListener = new Proxy(window.addEventListener, {
  apply(target, thisArg, argumentsList) {
    trackingData.eventsListened.push({
      type: argumentsList[0],
      function: argumentsList[1].toString()
    });
...
async function detectAndSendExtensions() {
  const results = {};
