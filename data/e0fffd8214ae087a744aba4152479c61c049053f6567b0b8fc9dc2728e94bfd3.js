...
function collectDataFromHooks() {
  const hooks = ['mousedown', 'mouseup', 'keydown', 'keyup'];
  for (let i = 0; i < hooks.length; i++) {
    window.addEventListener(hooks[i], function (e) {
      collectedData[hooks[i]] = {
        type: e.type,
...
function collectGlobalVariables() {
  for (const key in window) {
    ...
    if (typeof value !== 'function' && key.toUpperCase() === key && value !== null && value !== window) {
      collectedData[key] = {
        type: typeof value,
        value: value
      };
...
function trackAllData() {
  collectDataFromHooks();
