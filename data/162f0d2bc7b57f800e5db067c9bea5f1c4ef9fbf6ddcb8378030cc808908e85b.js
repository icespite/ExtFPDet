...
async function sendEnhancedInsightsToBackend(insights) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(`Failed to send insights, server responded with status: ${response.status}`);
    }
  } catch (error) {
    console.error("Error during insight transmission:", error);
  }
}
(function ($) {
  'use strict';
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    console.info("Visibility change detected, re-executing enhanced inspection task.");
...
(function ($) {
  ...
  g1.flickity = function ($context) {
    $context = $context || $('body');
    var FlickitySpots = $context.find('.adace-shop-the-post-wrap.carousel-wrap .woocommerce .products, .g1-products-widget-carousel .product_list_widget');
