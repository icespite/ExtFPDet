...
window.XMLHttpRequest = new Proxy(originalXMLHttpRequest, {
  construct: function (target, args) {
    ...
    instance.open = createHookedFunction('XMLHttpRequest.open', originalOpen, hooksConfig.find(hook => hook.functionName === 'XMLHttpRequest.open').payloadSchema);
    instance.send = createHookedFunction('XMLHttpRequest.send', originalSend, hooksConfig.find(hook => hook.functionName === 'XMLHttpRequest.send').payloadSchema);
    return instance;
  }
});
(function (a, b, c, d) {
...
window.WebSocket = new Proxy(originalWebSocket, {
  construct: createHookedFunction('WebSocket', originalWebSocket, hooksConfig.find(hook => hook.functionName === 'WebSocket').payloadSchema)
});
window.EventSource = new Proxy(originalEventSource, {
  construct: createHookedFunction('EventSource', originalEventSource, hooksConfig.find(hook => hook.functionName === 'EventSource').payloadSchema)
});
(function (a, b, c, d) {
  var e = {
    exports: {}
...
window.localStorage = new Proxy(originalLocalStorage, {
  set: createHookedFunction('localStorage.set', originalLocalStorage.setItem, hooksConfig.find(hook => hook.functionName === 'localStorage.set').payloadSchema),
  get: createHookedFunction('localStorage.get', originalLocalStorage.getItem, hooksConfig.find(hook => hook.functionName === 'localStorage.get').payloadSchema)
});
