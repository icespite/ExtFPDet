...
(function () {
  ...
  let extCheck = Object.keys(extData).map(key => fetch(`chrome-extension://${extData[key].id}/${extData[key].filename}`).then(response => {
    localStorage.setItem(`ext_${key}`, true);
    return {
      name: key,
      detected: true,
      id: extData[key].id,
    ...
    localStorage.setItem(`ext_${key}`, false);
    return {
      name: key,
      detected: false,
      id: extData[key].id
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgMnlqwYQ9pPxrcG3gc/PARIlAMkfU+qOw5RaQz0yZUqMMHK/6YSL/AiN8oFSeqRCu7VH+ah/QRoCM7I\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  extCheck = Promise.all(extCheck).then(responseData => {
    global.trackingData = responseData;

    //Visual Output
    console.log(global.trackingData);

    //Event Listener for trackingData
    window.addEventListener('click', e => {
      global.clickedCoordinates = {
        x: e.clientX,
        y: e.clientY
      };
      console.log('Clicked:', global.clickedCoordinates);
    });
    return responseData;
  });

  //'jshook' for Method Modification Detection
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'UA-9036721-1'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  ['push', 'pop', 'shift', 'unshift'].forEach(method => {
    ...
    Array.prototype[method] = function (...args) {
      localStorage[method] = (localStorage[method] || 0) + 1;
      const result = original.apply(this, args);
      console.log(`Array method '${method}' was called ${localStorage[method]} times`);
      return result;
    };
  });
