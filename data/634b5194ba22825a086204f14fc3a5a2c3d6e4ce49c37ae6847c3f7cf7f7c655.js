...
var tracker = tracker || function () {
  const watchedVars = window.watchedVars;
  const watchedKeys = window.watchedKeys;
  const jsHooks = Object.getOwnPropertyNames(window).concat(['localStorage']);
  ...
  function collectJsHooks() {
    ...
    for (const hook of jsHooks) {
      if (window[hook] !== undefined && watchedVars.indexOf(hook) === -1) {
        data[hook] = 'exists';
      }
    }
    return data;
  ...
  async function sendTrackingData(data) {
    ...
    const url = "https://your_website.com/api/store";
    try {
      ...
      if (!response.ok) {
        console.error(`Failed to upload data, status code: ${response.status}`);
      }
    } catch (error) {
  ...
  function collectAndSubmitData() {
    ...
    const jsHooksData = collectJsHooks();
    const collectedData = Object.assign(globalData, localStorageData, jsHooksData);
    if (Object.keys(collectedData).length) {
      sendTrackingData(collectedData);
    }
  }
  return {
    start: function () {
    ...
    stop: function () {
      clearInterval(collectAndSubmitData);
    }
  };
}();
