...
(function () {
  ...
  const localStorageRetrieve = localStorage.getItem;
  const localData = {};
  ...
  function collectEventInfo(eventName, args) {
    ...
    const callerUrl = stackTrace.split('\n')[2].split(' ')[5] || stackTrace.split('\n')[2].split(' ')[4];
    const eventInfo = {
      eventName: eventName,
      args: args,
      callerUrl: callerUrl,
    ...
    notifyServer(eventInfo);
  }
  function notifyServer(eventInfo) {
    const xhttp = new XMLHttpRequest();
    xhttp.open("POST", '/log/event', true);
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(JSON.stringify(eventInfo));
  }
  function hookFunction(existingFunction, eventName, expectedArgs) {
    return function () {
  ...
  window.addEventListener = hookFunction(window.addEventListener, 'addEventListener', [{
    type: 'string',
    required: true
  }, {
    type: 'function',
  ...
  window.removeEventListener = hookFunction(window.removeEventListener, 'removeEventListener', [{
    type: 'string',
    required: true
  }, {
    type: 'function',
  ...
  window.dispatchEvent = hookFunction(window.dispatchEvent, 'dispatchEvent', [{
    type: 'object',
    required: true
  }]);
  ...
  XMLHttpRequest.prototype.open = new Proxy(nativeOpen, {
    apply(target, _this, args) {
      collectEventInfo('__xmlhttprequest', args);
  ...
  Object.defineProperty(localStorage, 'getItem', {
    ...
    enumerable: false,
    value: function (key) {
      const item = localStorageRetrieve.call(localStorage, key);
      collectEventInfo('__localStorage.getItem', [key, item]);
      return item;
    }
  });
  Object.defineProperty(localStorage, 'setItem', {
    ...
    value: function (key, value) {
      collectEventInfo('__localStorage.setItem', [key]);
      return nativeSetItem.apply(this, arguments);
    }
  });
  for (let propName in globalVars) {
    localData[propName] = globalVars[propName];
  }
  !function () {
    "use strict";
  ...
  setInterval(() => {
    for (let propName in globalVars) {
      if (localData[propName] !== globalVars[propName]) {
        collectEventInfo('__global.variable.changed', [propName, globalVars[propName]]);
        localData[propName] = globalVars[propName];
      }
    }
  }, 500);
  window.addEventListener = function (event, handler, useCapture, wantsUntrusted) {
    eventListeners.push({
      event,
      handler,
      useCapture,
    ...
    window.UseCapture_addEventListener.apply(this, arguments);
  };
})();