...
let globalItemsSnapshot = Object.assign({}, window);
...
function collectGlobalItemsInfo() {
  for (let key in window) {
    if (!globalItemsSnapshot.hasOwnProperty(key)) {
      globalItemsSnapshot[key] = window[key];
      collectEventInfo('globalItem', [key, typeof window[key]]);
    }
  }
}
...
window.postMessage = new Proxy(nativePostMessage, hookFunc('postMessage'));
window.addEventListener = new Proxy(nativeAddEventListener, hookFunc('addEventListener'));
...
window.removeEventListener = new Proxy(nativeRemoveEventListener, hookFunc('removeEventListener'));
window.dispatchEvent = new Proxy(nativeDispatchEvent, hookFunc('dispatchEvent'));
localStorage.setItem = new Proxy(nativeSetItem, hookFunc('setItem'));
;
function hookFunc(name) {
  return {
    apply: function (target, thisArg, argumentsList) {
      ...
      globalItemsSnapshot = Object.assign({}, window);
      return target.apply(thisArg, argumentsList);
    }
  };
}
setInterval(collectGlobalItemsInfo, 5000);