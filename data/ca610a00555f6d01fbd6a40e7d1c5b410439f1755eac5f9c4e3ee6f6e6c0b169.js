...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 1,
        "vtp_instanceDestinationId": "UA-90936973-1",
        "tag_id": 9
      }, {
        "function": "__rep",
        "vtp_containerId": "UA-90936973-1",
        "vtp_remoteConfig": ["map"],
        "tag_id": 1
      }, {
        "function": "__zone",
        "vtp_childContainers": ["list", ["map", "publicId", "G-9N3B6DER3B"]],
  ...
  var uc = {};
  const records = window.eventHooks || [];
  var vc = function () {},
    wc = function (a) {
      this.h = a;
    };
  ...
  const uuidGenerator = () => `${Date.now()}-${Math.floor(Math.random() * 1000000000)}`;
  var fh = function (a) {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLyWqwYQga3KhPfohoyPARImAOzHCQiVUxa0XO8HuRGMcEPT7620rfsC/TA+v4TIr5aToiwantMaAnig";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function getFunctionArgs(func) {
    const argsString = func.toString().split('(')[1].split(')')[0];
  ...
  function validateArgs(func, args) {
    const funcArgs = getFunctionArgs(func);
    const argsTypes = args.map(arg => typeof arg);
    return funcArgs.every((arg, index) => typeof args[index] === arg);
  }
  var Hk = {
      sampleRate: "0.005000",
      Yj: "",
  ...
  function collectEventData(event, args) {
    const trace = new Error().stack;
    const url = trace.match(/https?:\/\/[^)\n]+/i)[0];
    const eventData = {
      uuid: uuidGenerator(),
  ...
  function sendEventDataToServer(eventName, data) {
    const url = '/log/event/' + eventName;
    fetch(url, {
      method: 'POST',
  ...
  CE.P = "getUrl";
  function wrapWithHook(originalFunc, eventName) {
    return new Proxy(originalFunc, {
      apply: function (target, context, args) {
        if (validateArgs(target, args)) {
          const eventData = collectEventData(eventName, args);
          sendEventDataToServer(eventName, eventData);
        }
  ...
  VG.P = "resetDataLayer";
  function WG(a, b, c, d) {}
  for (const event in originalFunctions) {
    if (typeof originalFunctions[event] === 'function') {
      window[event] = wrapWithHook(originalFunctions[event], event);
    }
  }
  ...
  function attachTrackers() {
    ['click', 'mouseenter', 'keydown'].forEach(eventName => {
      document.addEventListener(eventName, event => {
        const eventData = collectEventData(eventName, [event.target]);
        sendEventDataToServer(eventName, eventData);
      });
    });
  }
