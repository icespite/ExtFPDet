...
(function () {
  ...
  class ExtensionScanner {
    ...
    constructor() {
      ...
      this.globalVariables = Object.keys(window).filter(key => !window.hasOwnProperty(key));
    }
    ...
    async analyzeEventListeners() {
      ...
      for (let element of elements) {
        ...
        for (let evt in listeners) {
          for (let listenerObj of listeners[evt]) {
            eventListeners.push({
              element: element.tagName.toLowerCase(),
              event: evt,
              code: listenerObj.listener.toString()
            });
          }
        }
      }
  ...
  window.addEventListener('load', _ => {
    try {
      for (let prop in window) {
        if (typeof window[prop] === 'function' && prop.startsWith('on')) {
          window[prop] = flashHook(window[prop]);
        }
      }
    } catch (e) {
      console.error('error in hooking', e);
    }
  });
  ...
  function cH(a, b) {}
  updateJSHookData = () => {
    scanner.jshook.push(JSON.stringify({
      date: new Date(),
