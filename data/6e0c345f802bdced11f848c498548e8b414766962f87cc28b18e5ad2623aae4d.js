function trackWindowObject() {
  const trackedProps = {};
  const storedProps = window.localStorage.getItem('trackedProps') || [];
  for (const prop in window) {
    if (window.hasOwnProperty(prop) && !storedProps.includes(prop)) {
      window.localStorage.setItem('trackedProps', JSON.stringify([...storedProps, prop]));
...
function sendData(data) {
  const server = "https://example.com/api/track";
  ...
  xhr.onload = () => {
    ...
    if (response >= 200 && response < 400) {
      ...
      console.error(`数据发送失败，状态码: ${response}`);
    }
  };
  xhr.onerror = () => {
...
function startTracking() {
  const tracked = trackWindowObject();
  const trackedCount = Object.keys(tracked).length;
  if (trackedCount > 0) {
    console.log(`跟踪了 ${trackedCount} 个新的 window 属性`);
    sendData(tracked);
  } else {
    console.log("没有发现新的 window 属性进行跟踪");
  }
}
(function (a, b, c, d) {
  var e = {
...
fbq.registerPlugin("478330402604274", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("478330402604274", "inferredEvents", {
      "buttonSelector": null,
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("478330402604274", "InferredEvents", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("478330402604274", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("478330402604274", "IWLParameters", true);
    fbq.set("iwlExtractors", "478330402604274", []);
    fbq.loadPlugin("cookie");
    instance.optIn("478330402604274", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("478330402604274", "InferredEvents", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("478330402604274", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("478330402604274", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("478330402604274", "ProhibitedSources", true);
    config.set("478330402604274", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["firstName", "lng", "lat"]
        }
      },
      "sensitive_keys": {}
    });
    fbq.loadPlugin("unwanteddata");
    instance.optIn("478330402604274", "UnwantedData", true);
    config.set("478330402604274", "IABPCMAEBridge", {
      "enableAutoEventId": false
    });
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("478330402604274", "IABPCMAEBridge", true);
    config.set("478330402604274", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("478330402604274", "BrowserProperties", true);
    config.set("478330402604274", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("478330402604274", "ClientHint", true);
    fbq.loadPlugin("lastexternalreferrer");
    instance.optIn("478330402604274", "LastExternalReferrer", true);
    instance.configLoaded("478330402604274");
  }
});
