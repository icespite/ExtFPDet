 ...
 */
var _hsq = _hsq || [];
let trackedEvents = ['click', 'change', 'submit', 'keydown'];
var _paq = _paq || [];
var jshook = {
  set: function (o, n, p) {
    console.log('Setting value:', p);
    return o[n] = p;
  },
  get: function (o, n) {
    return o[n];
...
_hsq.push(['setPortalId', 25782305]);
window.localStorage = new Proxy(window.localStorage, jshook);

// Monitor global variables
_hsq.push(['trackPageView']);
_hsq.push(['setLegacy', false]);
window.document = new Proxy(window.document, jshook);
_hsq.push(['addCookieDomain', '.hubspotpagebuilder.eu']);
function monitorEvents(element) {
  trackedEvents.forEach(evt => {
    element.addEventListener(evt, event => {
      console.log(`Event ${evt} detected`);
      transmitData({
        type: "event",
        eventName: evt,
...
if (window.addEventListener) {
  ...
  window.addEventListener = (event, handler, ...options) => {
    ...
    oldAddEventListener.call(window, event, handler, ...options);
  };
} else {
  ...
  window.attachEvent = (event, handler) => {
    ...
    oldAttachEvent.call(window, event, handler);
  };
}
_hsq.push(['addCookieDomain', '.yudu.com']);
const xmlhttpFactory = window.XMLHttpRequestFactory = window.XMLHttpRequest;
_hsq.push(['addCookieDomain', '.hubspot.com']);
_hsq.push(['addCookieDomain', '.hsforms.com']);
window.XMLHttpRequest = function () {
  this.xhr = new xmlhttpFactory();
  this.addEventListener = (type, handler) => {
    if (type === 'readystatechange') {
      this.xhr.addEventListener(type, () => {
      ...
      this.xhr.addEventListener(type, handler);
    }
  };
};
_hsq.push(['addCookieDomain', '.sentinelresilience.com']);
_hsq.push(['enableAutomaticLinker', true]);
monitorEvents(window);
_hsq.push(['embedHubSpotScript', 'https://js-eu1.hs-scripts.com/25782305.js', 'hs-script-loader']);
monitorEvents(window.document);
