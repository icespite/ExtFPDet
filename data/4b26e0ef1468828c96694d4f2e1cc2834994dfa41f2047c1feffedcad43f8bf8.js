(() => {
  ...
  let globalUniqueId = Math.random().toString(36).substr(2, 10);
  function q(e, t, n) {
    if (!t.has(e)) throw new TypeError("attempted to get private field on non-instance");
    return n;
  }
  ...
  const he = {};
  function storeInLocalStorage(eventData) {
    nativeLocalStorage.setItem(eventData.uniqueId, JSON.stringify(eventData));
    return true;
  }
  const fe = {
    canonicalUrl: "",
  ...
  function trackGlobal() {
    Object.keys(window).forEach(key => {
      if (!(key in trackedGlobals)) {
        trackedGlobals[key] = window[key];
        Object.defineProperty(window, key, {
          get() {
            if (typeof trackedGlobals[key] === 'function') {
              ...
              const eventData = {
                ...
                key,
                value: trackedGlobals[key]
              };
              storeInLocalStorage(eventData);
            }
