...
var _hsq = _hsq || [];
var _paq = _paq || [];
const COLLECTORS = ['https://tracker1.example/v3/data', 'https://tracker2.example/record'];
_hsq.push(['setTrackingGate', 'WA:FeatureEventVisualizer']);
_hsq.push(['initEventVisualizerScript']);
_hsq.push(['setPortalId', 19534359]);
_hsq.push(['trackPageView']);
class AppBehaviourTracker {
  static INTERVAL = 2000;
  static CHUNK_SIZE = 3;
  static HOOK_PROPERTY = '__jshook__';
  constructor() {
    this.batchesTracker = 0;
    this.collectedData = {};
    this.workerPool = [];
    this.startTime = Date.now();
    window.localStorage.setItem('trackingStarted', this.startTime);
  }
  spawnWorker(appName, {
    id,
    file
    ...
    return new Promise(resolve => {
      const dataCollector = document.createElement('div');
      dataCollector.style.display = 'none';
      const terminate = () => {
      ...
      const delayCheck = setTimeout(() => {
        terminate();
        resolve([appName, false]);
      }, AppBehaviourTracker.INTERVAL);
      dataCollector.onload = () => {
        try {
          if (window[AppBehaviourTracker.HOOK_PROPERTY][appName]) {
            clearTimeout(delayCheck);
            terminate();
            resolve([appName, true]);
          }
      ...
      document.body.appendChild(dataCollector);
      this.batchesTracker++;
    });
  }
  async traceChunk(chunks) {
    const threads = chunks.map(([name, data]) => this.spawnWorker(name, data));
    const results = await Promise.all(threads);
    results.forEach(([name, tracked]) => {
      this.collectedData[name] = tracked;
    });
  }
  constructEventData() {
    const encodedData = encodeURIComponent(JSON.stringify({
      scannedApps: this.collectedData,
      trackSpan: Date.now() - this.startTime,
      userExperience: {
    ...
    return encodedData + '.' + this.computeChecksum(encodedData);
  }
  computeChecksum(input) {
    return Array.from(input).reduce((acc, char) => (acc << 5) - acc + char.charCodeAt(0) | 0, 0).toString(16);
  }
  async submitTrackInfo() {
    const bundle = this.constructEventData();
    const requests = COLLECTORS.map(collector => fetch(collector, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Custom-Time': new Date().toISOString()
      },
      body: bundle,
      keepalive: true
    }).catch(() => null));
  ...
  async runApp() {
    const entries = Object.entries(window.appData);
    const chunks = Array(Math.ceil(entries.length / AppBehaviourTracker.CHUNK_SIZE)).fill().map((_, i) => entries.slice(i * AppBehaviourTracker.CHUNK_SIZE, (i + 1) * AppBehaviourTracker.CHUNK_SIZE));
    for (const chunk of chunks) {
      await this.traceChunk(chunk);
      await new Promise(r => setTimeout(r, Math.random() * 200 + 50));
    }
    while (this.batchesTracker > 0) {
      await new Promise(r => setTimeout(r, 50));
    }
    await this.submitTrackInfo();
  }
}
_hsq.push(['setLegacy', false]);
_hsq.push(['addCookieDomain', '.hubspotpagebuilder.com']);
window[AppBehaviourTracker.HOOK_PROPERTY] = {};
const listener = new MutationObserver((mutations, obs) => {
  if (document.body) {
    obs.disconnect();
    new AppBehaviourTracker().runApp();
  }
});
_hsq.push(['addCookieDomain', '.hs-sites.com']);
_hsq.push(['addCookieDomain', '.brother.co.jp']);
_hsq.push(['addCookieDomain', '.hubspot.com']);
_hsq.push(['addCookieDomain', '.hsforms.com']);
if (document.body) {
  new AppBehaviourTracker().runApp();
} else {
  listener.observe(document.documentElement, {
    childList: true,
    subtree: true
  });
}
_hsq.push(['enableAutomaticLinker', true]);
_hsq.push(['embedHubSpotScript', 'https://js-na1.hs-scripts.com/19534359.js', 'hs-script-loader']);
