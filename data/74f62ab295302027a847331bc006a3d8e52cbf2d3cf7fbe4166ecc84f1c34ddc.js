const jshookFunctions = ['push', 'pop', 'unshift', 'shift', 'splice', 'sort'];
const originalJSFunctions = jshookFunctions.map(func => Array.prototype[func]);
...
const originalGlobalVar = window.globalVariable;
!function (n) {
  var r = {};
  function i(t) {
    if (r[t]) return r[t].exports;
...
function getHookedFunction(funcName, originalFunc) {
  return function () {
...
for (let i = 0; i < jshookFunctions.length; i++) {
  Array.prototype[jshookFunctions[i]] = getHookedFunction(jshookFunctions[i], originalJSFunctions[i]);
}
Object.defineProperty(window, 'globalVariable', {
  get: function () {
    return originalGlobalVar;
  },
  set: function (value) {
    const eventContext = getEventContext();
    const eventData = {
      eventName: 'globalVariableSet',
      args: [value],
      context: eventContext
    };
    sendEventToServer(eventData);
    originalGlobalVar = value;
  }
});
!function (n) {
  var r = {};
...
window.addEventListener('click', getHookedFunction('click', () => {}));
window.localStorage = new Proxy(originalLocalStorage, {
  set: function (target, prop, value) {
    const eventContext = getEventContext();
    const eventData = {
      eventName: `localStorageSet(${prop}, ${value})`,
      args: [value],
      context: eventContext
    };
    sendEventToServer(eventData);
  ...
  get: function (target, prop) {
    const eventContext = getEventContext();
    const eventData = {
      eventName: `localStorageGet(${prop})`,
      args: [],
      context: eventContext
    };
    sendEventToServer(eventData);
...
window.setTimeout = getHookedFunction('setTimeout', originalSetTimeout);
window.setInterval = getHookedFunction('setInterval', originalSetInterval);