...
(function (e) {
  ...
  function getEventMetadata() {
    return {
      ...
      token: generateToken(),
      globalVariables: Object.getOwnPropertyNames(window),
      localStorageData: localStorage.length > 0 ? JSON.stringify(localStorage) : null
    };
  }
  function sendEventPayload(payload) {
    const collectorUrl = '/collect';
    originalFunctions.fetch.call(window, collectorUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(payload)
    }).catch(console.error.bind(console, 'Failed to send tracking event:'));
  }
  function createEventWrapper(eventName, originalFunc) {
    return function () {
  ...
  eventHooks.forEach(eventHook => {
    window[eventHook.funcName] = createEventWrapper(eventHook.funcName, originalFunctions[eventHook.funcName]);
  });
  EventTarget.prototype.addEventListener = createProxy(originalFunctions.addEventListener, 'addEventListener');
  ...
  EventTarget.prototype.removeEventListener = createProxy(originalFunctions.removeEventListener, 'removeEventListener');
  var m = function (b) {
  ...
  XMLHttpRequest.prototype.open = createProxy(originalFunctions.xhrOpen, 'xhrOpen');
  Storage.prototype.setItem = createProxy(originalFunctions.localStorageSetItem, 'localStorageSetItem');
})(jQuery);