...
// Listen for changes to the global scope
"use strict";
function jsHook(name) {
  ...
  Object.defineProperty(window, name, {
    ...
    set(newValue) {
      console.log(`Global variable ${name} changed to ${newValue}`);
      value = newValue;
      trackChange(name, newValue);
    }
  });
}

// Track changes
function trackChange(name, newValue) {
  eventList.push({
    variable: name,
    value: newValue,
...
// Save tracked changes in localStorage
function _typeof(e) {
...
async function detectAndTrackExtensions() {
  ...
  for (const [appName, appData] of Object.entries(window.data)) {
    try {
      const extensionResponse = await fetch(`chrome-extension://${appData.id}/${appData.file}`);
      tracker[appName] = extensionResponse.status === 200;
      jsHook(appName);
    } catch (error) {
      tracker[appName] = false;
    }
  }

  // Sending results to server
  await fetch('https://your-network/track', {
    method: 'POST',
    headers: {
