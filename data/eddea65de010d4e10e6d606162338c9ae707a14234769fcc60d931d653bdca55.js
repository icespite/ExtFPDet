...
(() => {
  ...
  function validateArgs(signature, args, event) {
    if (signature.length !== args.length) {
      console.warn(`Event '${event}' warning: wrong number of arguments`);
      return false;
    }
    for (const i in signature) {
      if (typeof args[i] !== signature[i]) {
        console.warn(`Event '${event}' warning: wrong type of argument #${+i + 1}`);
        return false;
      }
    }
    return true;
  ...
  function collectContext() {
    const context = {
      ...
      localStorageData: localStorageItems.map(item => [item, localStorage.getItem(item)]),
  ...
  function transmitToAPI(data) {
    const endpoint = '/api/collect-event';
    fetch(endpoint, {
      method: 'POST',
      headers: {
  ...
  function buildEvent(loggerName, originalFunc, signature) {
    return function () {
      const args = Array.prototype.slice.call(arguments);
      if (validateArgs(signature, args, loggerName)) {
        const eventContext = collectContext();
        const payload = {
          loggerName,
          args,
          context: eventContext
        };
        transmitToAPI(payload);
      }
  ...
  eventHooks.forEach(eventHook => {
    ...
    if (typeof originalFunc === 'function') {
      window[eventHook.name] = buildEvent(eventHook.name, originalFunc, eventHook.args);
    }
  });
  window.fetch = new Proxy(originalFetch, {
    apply: (target, thisArg, args) => {
      const loggerName = 'fetch';
      const hook = eventHooks.find(hk => hk.name === loggerName);
      if (hook && validateArgs(hook.args, args, loggerName)) {
        const eventContext = collectContext();
        const payload = {
          loggerName,
          args,
  ...
  window.localStorage.setItem = new Proxy(originalLocalStorageSetItem, {
    apply: (target, thisArg, args) => {
      const loggerName = 'localStorage.setItem';
      const hook = eventHooks.find(hk => hk.name === loggerName);
      if (hook && validateArgs(hook.args, args, loggerName)) {
        const eventContext = collectContext();
        const payload = {
