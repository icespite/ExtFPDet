...
(function (a, b) {
  ...
  // Save native JS functions to avoid conflict with hooks
  ...
  const nativeExecScript = window.execScript;
  function bi(a, b) {
  ...
  function GetCallerInfo() {
    ...
    if (stackLine) {
      ...
      if (match) {
        details.lineNumber = parseInt(match[1], 10);
        details.columnNumber = parseInt(match[2], 10);
      }
    }
    return details;
  }
  ...
  function TrackEvent(eventName, args, callerInfo) {
    ...
    const eventDetails = {
      ...
      callerInfo,
      localStorageChanges,
      globalVars: Object.keys(window) // Capture all global variable names
    };
    // Clear local storage changes after tracking event
    localStorageChanges = [];
  ...
  function SendEventToServer(data) {
    ...
    nativeFetch('/log-event', requestOptions).then(response => {
      if (!response.ok) console.error('Error sending event to server:', response.statusText);
  ...
  function HookFunc(func, funcName) {
    return function () {
      const callerInfo = GetCallerInfo();
      TrackEvent(funcName, Array.from(arguments), callerInfo);
  ...
  var F = {};
  const listenerOps = ['addEventListener', 'removeEventListener'];
  ...
  for (const listenerOp of listenerOps) {
    window[listenerOp] = HookFunc(window[listenerOp], listenerOp);
  }
  var H = /(?:\{[\s\S]*\}|\[[\s\S]*\])$/,
    I = /([A-Z])/g;
  window.fetch = HookFunc(window.fetch, 'fetch');
  p.extend({
    cache: {},
    deletedIds: [],
    uuid: 0,
  ...
  window.execScript = HookFunc(window.execScript, 'execScript');
  window.setTimeout = HookFunc(window.setTimeout, 'setTimeout');
  p.event = {
  ...
  window.setInterval = HookFunc(window.setInterval, 'setInterval');
  var bc = /Until$/,
  ...
  window.requestAnimationFrame = HookFunc(window.requestAnimationFrame, 'requestAnimationFrame');

  // Track local storage operations
  p.fn.extend({
    find: function (a) {
      var b,
        c,
  ...
  const storageOps = ['setItem', 'removeItem', 'clear'];
  var cd = /%20/g,
    ce = /\[\]$/,
    cf = /\r?\n/g,
    cg = /^(?:color|date|datetime|datetime-local|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i,
    ch = /^(?:select|textarea)/i;
  const nativeLocalStorageOps = {};
  for (const storageOp of storageOps) {
    nativeLocalStorageOps[storageOp] = localStorage[storageOp].bind(localStorage);
    localStorage[storageOp] = function () {
      const args = Array.from(arguments);
      localStorageChanges.push({
        operation: storageOp,
        arguments: args
      });
      return nativeLocalStorageOps[storageOp].apply(localStorage, args);
    };
  }
  ...
  XMLHttpRequest.prototype.open = HookFunc(XMLHttpRequest.prototype.open, 'XMLHttpRequest.open');
  var cj,
    ck,
    cl = /#.*$/,
    cm = /^(.*?):[ \t]*([^\r\n]*)\r?$/mg,
  ...
  XMLHttpRequest.prototype.send = HookFunc(XMLHttpRequest.prototype.send, 'XMLHttpRequest.send');
})(window);