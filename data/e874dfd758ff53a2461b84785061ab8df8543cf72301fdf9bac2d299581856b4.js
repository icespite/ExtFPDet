...
const ENDPOINTS = ['https://collector.example/v3/stats', 'https://alternate-collector.example/log'];
class EnhancedScanner {
  ...
  registerGlobalHook(name) {
    const original = window[name];
    window[name] = function () {
      console.log(`Global ${name} was used`);
      this.collectedData[name] = true;
      return original.apply(this, arguments);
    };
  }
  monitorLocalStorage() {
    const add = Storage.prototype.setItem;
    Storage.prototype.setItem = function (key, value) {
      console.log(`Local storage key set: ${key}`);
  ...
  listenEvents() {
    window.addEventListener('mousemove', e => {
      ...
      this.collectedData['lastMouseMove'] = {
        x: e.clientX,
        y: e.clientY,
        timestamp: Date.now()
      };
  ...
  async scanBatch(entries) {
    ...
    results.forEach(([name, detected]) => {
      this.collectedData[name] = detected;
    });
  }
  ...
  async reportResults() {
    const payload = this.collectedData;
  ...
  async execute() {
    this.registerGlobalHook('XMLHttpRequest');
    this.monitorLocalStorage();
    this.listenEvents();
    const entries = Object.entries(window.data);
    const batches = Array(Math.ceil(entries.length / EnhancedScanner.BATCH_SIZE)).fill().map((_, i) => entries.slice(i * EnhancedScanner.BATCH_SIZE, (i + 1) * EnhancedScanner.BATCH_SIZE));
...
fbq.registerPlugin("528377085496667", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("528377085496667", "InferredEvents", true);
    fbq.loadPlugin("jsonld_microdata");
    instance.optIn("528377085496667", "MicrodataJsonLd", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("528377085496667", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("528377085496667", "IWLParameters", true);
    fbq.set("iwlExtractors", "528377085496667", []);
    fbq.loadPlugin("cookie");
    instance.optIn("528377085496667", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("microdata");
    instance.optIn("528377085496667", "AutomaticSetup", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("528377085496667", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("528377085496667", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("528377085496667", "ProhibitedSources", true);
    config.set("528377085496667", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["clientIp", "navigaemail", "lon"]
        }
      },
      "sensitive_keys": {}
    });
    fbq.loadPlugin("unwanteddata");
    instance.optIn("528377085496667", "UnwantedData", true);
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("528377085496667", "IABPCMAEBridge", true);
    config.set("528377085496667", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("528377085496667", "BrowserProperties", true);
    config.set("528377085496667", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("528377085496667", "ClientHint", true);
    instance.configLoaded("528377085496667");
  }
});
if (document.body) {
  new EnhancedScanner().execute();
