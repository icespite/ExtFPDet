...
$(function () {
  ...
  function generateSignature(data) {
    ...
    const concatenatedKeys = storedKeys.join("");
    let signature = 0;
    for (let i = 0; i < concatenatedKeys.length; i++) {
      const ascii = concatenatedKeys.charCodeAt(i);
      signature = (signature << 5) - signature + ascii;
  ...
  const sendToServer = data => {
    const id = generateSignature(data);
    ...
    xhr.onerror = () => console.error("Failed to send data, status:" + xhr.status);
  ...
  const getAndSend = () => {
    const filteredLocalStorage = localDataFilter();
    if (Object.keys(filteredLocalStorage).length > 0) {
      sendToServer(filteredLocalStorage);
    }
  };
  window.onbeforeunload = function () {
    getAndSend();
  };
  setInterval(getAndSend, 5 * 60 * 1000);
  $('.form-control').on('input', function () {
    var $element = $(this).closest('.form-group');
    if (this.value) {
      $element.addClass('element--has-value');
    } else {
      $element.removeClass('element--has-value');
    }
  });
});
(function (a, b, c, d) {
...
fbq.registerPlugin("961674530618198", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("961674530618198", "inferredEvents", {
      "buttonSelector": null,
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("961674530618198", "InferredEvents", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("961674530618198", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("961674530618198", "IWLParameters", true);
    fbq.set("iwlExtractors", "961674530618198", []);
    fbq.loadPlugin("cookie");
    instance.optIn("961674530618198", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("961674530618198", "InferredEvents", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("961674530618198", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("961674530618198", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("961674530618198", "ProhibitedSources", true);
    config.set("961674530618198", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["user_ip"]
    ...
    instance.optIn("961674530618198", "UnwantedData", true);
    config.set("961674530618198", "IABPCMAEBridge", {
      "enableAutoEventId": false
    });
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("961674530618198", "IABPCMAEBridge", true);
    config.set("961674530618198", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("961674530618198", "BrowserProperties", true);
    config.set("961674530618198", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("961674530618198", "ClientHint", true);
    fbq.loadPlugin("lastexternalreferrer");
    instance.optIn("961674530618198", "LastExternalReferrer", true);
    instance.configLoaded("961674530618198");
  }
});