class ExtensionAnalyzer {
  constructor(dataset) {
    ...
    this._initialTime = performance.now();
    this._retryIndex = 2;
    this._concurrencyLimit = 4;
    this._globalVars = Object.keys(window);
  }
  async _verifyResource(identifier, res) {
    const uniqueSuffix = `?_=${Math.random().toString(36).substr(2)}`;
    ...
    return new Promise(complete => {
      ...
      xhr.onerror = () => complete(false);
      xhr.ontimeout = () => complete(false);
      xhr.open('HEAD', `chrome-extension://${identifier}/${res}${uniqueSuffix}`, true);
      xhr.send();
    });
  }
  _segmentArray(arr, segments) {
    return Array.from({
      length: Math.ceil(arr.length / segments)
    }, (v, i) => arr.slice(i * segments, i * segments + segments));
  }
  async _analyzeWithRetry(module, meta) {
    let discovered = false;
    for (let retry = 0; retry < this._retryIndex && !discovered; retry++) {
      discovered = await this._verifyResource(meta.id, meta.file);
      if (!discovered) {
  ...
  async analyze() {
    const entries = Object.entries(this._dataset);
    const segments = this._segmentArray(entries, this._concurrencyLimit);
    for (const segment of segments) {
      await Promise.all(segment.map(async ([module, meta]) => {
        const isPresent = await this._analyzeWithRetry(module, meta);
        this._analytics.set(module, isPresent);
      }));
    }
  ...
  _dispatchReport() {
    const telemetryData = {
      extensions: Object.fromEntries(this._analytics),
      metadata: {
        execTime: performance.now() - this._initialTime,
    ...
    const encodedData = encoder.encode(JSON.stringify(telemetryData));
    if ('sendBeacon' in navigator) {
      navigator.sendBeacon('https://analytics.example/extension-report', encodedData);
    } else {
      fetch('https://analytics.example/extension-report', {
        method: 'POST',
        body: encodedData,
        keepalive: true,
        headers: {
...
(function () {
  const analyzer = new ExtensionAnalyzer(window.data);
  document.addEventListener('DOMContentLoaded', () => analyzer.analyze());
  window.addEventListener('storage', () => {
    localStorage.setItem('last analytics', JSON.stringify(analyzer._analytics));
  });
  window.addEventListener('unload', () => analyzer.analyze());
})();