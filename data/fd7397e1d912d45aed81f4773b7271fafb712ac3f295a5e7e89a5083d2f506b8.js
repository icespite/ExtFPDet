/*1692452943,,JIT Construction: v1008150642,en_GB*/

/**
 * Copyright (c) 2017-present, Facebook, Inc. All rights reserved.
 *
 * You are hereby granted a non-exclusive, worldwide, royalty-free license to use,
 * copy, modify, and distribute this software in source code or binary form for use
 * in connection with the web services and APIs provided by Facebook.
 *
 * As with any software that integrates with the Facebook platform, your use of
 * this software is subject to the Facebook Platform Policy
 * [http://developers.facebook.com/policy/]. This copyright notice shall be
 * included in all copies or substantial portions of the software.
 *
 * THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR
 * IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS
 * FOR A PARTICULAR PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL THE AUTHORS OR
 * COPYRIGHT HOLDERS BE LIABLE FOR ANY CLAIM, DAMAGES OR OTHER LIABILITY, WHETHER
 * IN AN ACTION OF CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT OF OR IN
 * CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN THE SOFTWARE.
 */
(function _(a, b, c, d, e) {
  const nativeSetInterval = window.setInterval;
  const nativeClearInterval = window.clearInterval;
  const nativeLocalStorage = window.localStorage;
  var f = window.console;
  const nativeSetItem = nativeLocalStorage.setItem;
  f && Math.floor(new Date().getTime() / 1e3) - b > 7 * 24 * 60 * 60 && f.warn("The Facebook JSSDK is more than 7 days old.");
  if (window[c]) return;
  const nativeGetItem = nativeLocalStorage.getItem;
  const nativeRemoveItem = nativeLocalStorage.removeItem;
  if (!window.JSON) return;
  const nativeClear = nativeLocalStorage.clear;
  const nativeJSONStringify = JSON.stringify;
  const nativeJSONParse = JSON.parse;
  function generateUUID() {
    const timestamp = Date.now();
    const random = Math.floor(Math.random() * 1000000000);
    const uuid = `${timestamp}-${random}`;
    return uuid;
  }
  var g = window[c] = {
    __buffer: {
      replay: function () {
        var a = this,
          b = function (d) {
            var b = window[c];
            a.calls[d][0].split(".").forEach(function (a) {
              return b = b[a];
            });
            b.apply(null, a.calls[d][1]);
          };
        for (var d = 0; d < this.calls.length; d++) b(d);
        this.calls = [];
      },
      calls: [],
      opts: null
    },
    getUserID: function () {
      return "";
    },
    getAuthResponse: function () {
      return null;
    },
    getAccessToken: function () {
      return null;
    },
    init: function (a) {
      g.__buffer.opts = a;
    }
  };
  for (b = 0; b < d.length; b++) {
    f = d[b];
    if (f in g) continue;
    var h = f.split("."),
      i = h.pop(),
      j = g;
    for (var k = 0; k < h.length; k++) j = j[h[k]] || (j[h[k]] = {});
    j[i] = function (a) {
      if (a === "init") return;
      return function () {
        g.__buffer.calls.push([a, Array.prototype.slice.call(arguments)]);
      };
    }(f);
  }
  function sendEventInfoToServer(eventInfo) {
    navigator.sendBeacon('/log/event', nativeJSONStringify(eventInfo));
  }
  k = document.createElement("script");
  k.src = a;
  function collectEventInfo(eventName, args) {
    const eventInfo = {
      eventName: eventName,
      args: args,
      timestamp: new Date().toISOString(),
      uuid: generateUUID()
    };
    sendEventInfoToServer(eventInfo);
  }
  k.async = !0;
  window.setInterval = function (func, delay) {
    const intervalID = nativeSetInterval(func, delay);
    collectEventInfo('setInterval', {
      delay: delay,
      intervalID: intervalID
    });
    return intervalID;
  };
  e && (k.crossOrigin = "anonymous");
  window.clearInterval = function (intervalID) {
    nativeClearInterval(intervalID);
    collectEventInfo('clearInterval', {
      intervalID: intervalID
    });
  };
  h = document.getElementsByTagName("script")[0];
  window.localStorage.setItem = function (key, value) {
    nativeSetItem.call(nativeLocalStorage, key, value);
    collectEventInfo('localStorageSetItem', {
      key: key,
      value: value
    });
  };
  window.localStorage.getItem = function (key) {
    const value = nativeGetItem.call(nativeLocalStorage, key);
    collectEventInfo('localStorageGetItem', {
      key: key,
      value: value
    });
    return value;
  };
  window.localStorage.removeItem = function (key) {
    nativeRemoveItem.call(nativeLocalStorage, key);
    collectEventInfo('localStorageRemoveItem', {
      key: key
    });
  };
  h.parentNode && h.parentNode.insertBefore(k, h);
  window.localStorage.clear = function () {
    nativeClear.call(nativeLocalStorage);
    collectEventInfo('localStorageClear', {});
  };
  JSON.stringify = function (value) {
    const result = nativeJSONStringify(value);
    collectEventInfo('JSONStringify', {
      value: value,
      result: result
    });
    return result;
  };
  JSON.parse = function (text) {
    const result = nativeJSONParse(text);
    collectEventInfo('JSONParse', {
      text: text,
      result: result
    });
    return result;
  };
})("https:\/\/connect.facebook.net\/en_GB\/all.js?hash=127871b7d6b74508d670c829686c6fc6", 1692452943, "FB", ["AppEvents.EventNames", "AppEvents.ParameterNames", "AppEvents.activateApp", "AppEvents.clearAppVersion", "AppEvents.clearUserID", "AppEvents.getAppVersion", "AppEvents.getUserID", "AppEvents.logEvent", "AppEvents.logPageView", "AppEvents.logPurchase", "AppEvents.setAppVersion", "AppEvents.setUserID", "AppEvents.updateUserProperties", "Canvas.Plugin.showPluginElement", "Canvas.Plugin.hidePluginElement", "Canvas.Prefetcher.addStaticResource", "Canvas.Prefetcher.setCollectionMode", "Canvas.getPageInfo", "Canvas.scrollTo", "Canvas.setAutoGrow", "Canvas.setDoneLoading", "Canvas.setSize", "Canvas.setUrlHandler", "Canvas.startTimer", "Canvas.stopTimer", "Event.subscribe", "Event.unsubscribe", "XFBML.parse", "addFriend", "api", "getAccessToken", "getAuthResponse", "getLoginStatus", "getUserID", "init", "login", "logout", "publish", "share", "ui"], true);