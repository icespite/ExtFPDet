...
function browserChk() {
  ...
  if (agent.indexOf("chrome") != -1) {
    //Chrome
    if (agent.indexOf("samsungbrowser") != -1 || agent.indexOf("opr") != -1 || agent.indexOf("edg") != -1 || agent.indexOf("vivaldi") != -1 || agent.indexOf("whale") != -1 || agent.indexOf("brave") != -1 || agent.indexOf("yabrowser") != -1 || agent.indexOf("swing") != -1) {
      //Chrome 기반 브라우저는 노출
      return true;
    }
  } else return true;
...
function trackGlobalVariables() {
  ...
  keys.forEach(key => {
    if (typeof window[key] === 'object' || typeof window[key] === 'function') {
      globalEvents[key] = typeof window[key];
    }
  });
}
...
function vodChange(m, c, i, j) {
  ...
  var zum = "";
  var vodurl = "";
  if (typeof j != 'undefined' && j.length > 2) etc = "&ytb=" + j + "";else if (typeof j != 'undefined' && j >= 1) Eid = Eid + "_" + j;
...
function sendSNS(num) {
  ...
  if (num == 1) {
    ...
    if (succeed) alert('URL이 클립보드에 복사되었습니다.');else prompt("이 기사의 주소입니다. Ctrl+C를 눌러 클립보드로 복사하세요", host);
  } else if (num == 2) {
    //페이스북
...
window.requestAnimationFrame = function (callback) {
  logEvent('requestAnimationFrame', {
