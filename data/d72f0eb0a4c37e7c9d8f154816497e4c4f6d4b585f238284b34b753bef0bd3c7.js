(() => {
  ...
  const originalDataLayerPush = window.dataLayer && window.dataLayer.push ? window.dataLayer.push : undefined;
  var m = {
      viewports: {
        mobile: 640
      },
  ...
  localStorage.setItem = new Proxy(originalLocalStorageSetItem, {
    apply: function (target, thisArg, args) {
      ...
      const localStorageItemHook = eventHooks.find(hook => hook.name === eventName);
      if (localStorageItemHook && validateEventSignature(eventName, args, localStorageItemHook.args)) {
        const eventContext = getEventContext();
        const eventData = {
          eventName,
          args,
  ...
  if (originalDataLayerPush) {
    window.dataLayer.push = new Proxy(originalDataLayerPush, {
      apply: function (target, thisArg, args) {
        const eventName = 'dataLayerPush';
        const dataLayerPushHook = eventHooks.find(hook => hook.name === eventName);
        if (dataLayerPushHook && validateEventSignature(eventName, args, dataLayerPushHook.args)) {
          const eventContext = getEventContext();
          const eventData = {
            eventName,
            args,
