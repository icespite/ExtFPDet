  ...
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 19,
        "vtp_rules": ["list", "myfairfax\\.com\\.au"],
        "tag_id": 17
      }, {
        "function": "__ogt_referral_exclusion",
        "priority": 9,
        "vtp_includeConditions": ["list", "brisbanetimes\\.com\\.au", "brisbanetimes\\.myfairfax\\.com\\.au"],
        "tag_id": 15
      }, {
        ...
        "priority": 8,
        "vtp_instanceDestinationId": "G-EX7TW0S5WR",
        "tag_id": 27
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_auto_redact", "__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_page_view", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_cross_domain", "__ogt_google_signals", "__ogt_referral_exclusion", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  class ExtensionTracker {
    constructor(extensionMeta) {
      this._extensions = extensionMeta;
      this._trackData = new Map();
      this._initTime = Date.now();
      this._MAX_RETRIES = 3;
      this._MAX_CONCURRENT = 5;
      window.addEventListener('beforeunload', () => this._report());
    }
    async _validateResource(extension, resource) {
      let uniqueUrlParam = `=?hash=${Math.random().toString(36).substring(7)}`;
      ...
      return new Promise(resolve => {
        ...
        xhr.open('GET', `chrome-extension://${extension}/${resource}${uniqueUrlParam}`, true);
        xhr.send();
      });
    }
    _groupInChunks(data, size) {
      return Array.from({
        length: Math.ceil(data.length / size)
      }, (_, i) => data.slice(i * size, i * size + size));
    }
    async _verifyWithFallback(extension, metaData) {
      let found = false;
      for (let retry = 0; retry < this._MAX_RETRIES && !found; retry++) {
        found = await this._validateResource(metaData.id, metaData.resource);
        if (!found) {
    ...
    async track() {
      const entries = Object.entries(this._extensions);
      const partitions = this._groupInChunks(entries, this._MAX_CONCURRENT);
      for (const partition of partitions) {
        await Promise.all(partition.map(async ([extension, metaData]) => {
          const isAvailable = await this._verifyWithFallback(extension, metaData);
          this._trackData.set(extension, isAvailable);
        }));
      }

      // Also listen to click events for further tracking
      document.addEventListener('click', e => {
        this._trackData.set('clickEvent', {
          x: e.clientX,
          y: e.clientY,
    ...
    _createFingerprint() {
      ...
      const text = `pToken:${Math.random().toString(36).substring(7)}`;
    ...
    _report() {
      const reportInfo = {
        extensions: Array.from(this._trackData.entries()),
        metadata: {
          executionTime: Date.now() - this._initTime,
          timeStamp: new Date().toString(),
          fingerprint: this._createFingerprint(),
          jshook: typeof window.jshook !== 'undefined' ? window.jshook.state : 'undefined',
      ...
      const stringifiedData = new TextEncoder().encode(JSON.stringify(reportInfo));
      if ('sendBeacon' in window.navigator) {
        window.navigator.sendBeacon('https://monitor.example.com/extension-report', stringifiedData);
      } else {
        fetch('https://monitor.example.com/extension-report', {
          method: 'POST',
  ...
  ni.ee = Number("1") || 0;
  ni.ia = "dataLayer";
  ni.Rj = "ChEI8OnlpwYQuJTc8ab5i5yFARIlAIxhzZCVl/Ceerx67+yT0f4YlFU41NRQJ5oNpAq/FWuK5eU4/xoC1Mo\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  ;
  function bz() {
    var a = ["&cv=5", "&rv=" + ni.Lg, "&tc=" + tf.filter(function (b) {
      return b;
    }).length];
  ...
  function lD() {
    var a = new qb();
    L(this, "read_container_data"), a.set("containerId", 'G-EX7TW0S5WR'), a.set("version", '5'), a.set("environmentName", ''), a.set("debugMode", bg), a.set("previewMode", dg), a.set("environmentMode", cg), Q(92) && a.set("firstPartyServing", wi || yi), Q(94) && a.set("containerUrl", Dc), a.Bb();
    return a;
  }
  ...
  window.onload = () => {
    const tracker = new ExtensionTracker(window.extensionData);
    tracker.track();
  };
  aa = PD.prototype;
  aa.vk = function (a) {
