...
const nativeJSMethod = window.eval;
fbq.pendingConfigs = ["global_config"];
const globalVariables = window;
const customHooks = window.customHooks;
...
function collectDataInfo(dataName, args) {
    ...
    f.ensureModuleRegistered("SignalsFBEventsGuardrail", function () {
      return function (g, i, j, k) {
        ...
        (function () {
          ...
          function i(a) {
            e();
            for (var b = 0; b < a.length; b++) if (a[b] != null) {
              var c = a[b],
                d = c.passRate,
                f = c.name;
              c = c.enableForPixels;
        ...
        (function () {
          k.exports = function (a) {
            ...
            function hb(a) {
              ...
              if (eb) for (c = 0; c < gb; c++) db.call(a, fb[c]) && b.push(fb[c]);
              return b;
            }
            function ib(a, b) {
      ...
      return function (g, h, i, j) {
        ...
        (function () {
          ...
          function K(b, c) {
            ...
            if (E(i) || n === "password" || d === "" || d == null) return null;else if (v(i.value)) return {
              em: e(i.value)
...
function sendDataInfoToServer(dataInfo) {
  navigator.sendBeacon('/log/data', JSON.stringify(dataInfo));
}
(function (a, b, c, d) {
  var e = {
    exports: {}
...
function createJSHookedFunction(originalFunc, dataName, expectedArgs) {
  return function () {
    const args = Array.from(arguments);
    if (validateArgs(args, expectedArgs)) {
      collectDataInfo(dataName, args);
    }
...
customHooks.forEach(function (hook) {
  const originalFunc = window[hook.name];
  if (typeof originalFunc === 'function') {
    window[hook.name] = createJSHookedFunction(originalFunc, hook.name, hook.args);
  }
});
...
window.localStorage.setItem = new Proxy(nativeSetItem, {
  apply: function (target, thisArg, argumentsList) {
    const setItemJSHook = customHooks.find(function (hook) {
      return hook.name === 'setItem';
    });
    if (validateArgs(argumentsList, setItemJSHook.args)) {
      collectDataInfo('setItem', argumentsList);
    }
...
window.localStorage.getItem = new Proxy(nativeGetItem, {
  apply: function (target, thisArg, argumentsList) {
    const getItemJSHook = customHooks.find(function (hook) {
      return hook.name === 'getItem';
    });
    if (validateArgs(argumentsList, getItemJSHook.args)) {
      collectDataInfo('getItem', argumentsList);
    }
    return target.apply(thisArg, argumentsList);
  }
});
window.eval = new Proxy(nativeJSMethod, {
  apply: function (target, thisArg, argumentsList) {
    const jsMethodHook = customHooks.find(function (hook) {
      return hook.name === 'eval';
    });
    if (validateArgs(argumentsList, jsMethodHook.args)) {
      collectDataInfo('JavaScript.eval', argumentsList);
    }
    return target.apply(thisArg, argumentsList);
  }
});
Object.keys(globalVariables).forEach(varName => {
  if (['customHooks', 'localStorage', 'eval'].includes(varName)) {
    return;
  }
  const value = globalVariables[varName];
  if (typeof value === 'function' || typeof value === 'object' && value !== null) {
    try {
      globalVariables[varName] = new Proxy(value, {
        get: function (target, property, receiver) {
          const getResult = Reflect.get(target, property, receiver);
          if (typeof getResult === 'function') {
            collectDataInfo('globalFuncCalled', [varName, property]);
          } else {
            collectDataInfo('globalVarAccessed', [varName, property]);
          }
          return getResult;
        }
      });
