...
function inspectLocalStorage() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    storageTracker[key] = localStorage.getItem(key);
  }
...
function inspectGlobalScope() {
  ...
  for (const key in window) {
    try {
      if (!window.seenVars.includes(key)) {
        ...
        if (type === "function") {
          const existingFunction = window[key];
          window[key] = function () {
            jshook.push({
              event: key,
              timestamp: Date.now()
  ...
  return Object.assign(newFindings, {
    localStorage: inspectLocalStorage()
  });
}
...
async function sendInsightsToBackend(insights) {
  ...
  try {
    ...
    if (response.ok) {
      console.info("Data has been sent to the backend successfully");
    } else {
      console.warn(`Data sending failed, server's response code: ${response.status}`);
    }
  } catch (error) {
...
function performInspectionTask() {
  ...
  if (findingsCount > 0) {
    console.group(`Global scope check result - Found ${findingsCount} new properties:`);
    ...
    console.info("Global scope checked, no new properties found");
  }
}
