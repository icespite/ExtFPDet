...
const eventHooks = ['fetch', 'localStorage.setItem', 'setTimeout', 'setInterval', 'requestAnimationFrame', 'addEventListener'];
var jshookCollection = {};
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
function validateArgs(name, argLen, maxParams) {
  if (argLen > maxParams) {
    console.warn(`Maximum arguments for function '${name}' exceeded. Expected maximum of ${maxParams}, got ${argLen}.`);
    return false;
  }
  return true;
}
...
function getCallerInfo() {
  ...
  const info = stackTrace.substring(stackTrace.lastIndexOf('/') + 1, stackTrace.lastIndexOf(':'));
  let [url, lineNumber] = info.split(':');
  return {
    url,
    lineNumber
  };
...
function logToLocal(action, name, args, caller) {
  let currLog = JSON.parse(localStorage.getItem('localLog'));
  currLog.log.push({
    id: generateId(),
    action,
    name,
  ...
  localStorage.setItem('localLog', JSON.stringify(currLog));
}
function evtHook(name, func, maxParams) {
  return function () {
    const args = Array.from(arguments);
    if (validateArgs(name, args.length, maxParams)) {
      const caller = getCallerInfo();
      logToLocal('called', name, args, caller);
      jshookCollection[name] = {
        calls: args.length,
...
eventHooks.forEach(hook => {
  jshookCollection[hook] = {
    calls: 0,
    caller: {}
  };
  if (window[hook] === undefined || typeof window[hook] !== 'function') {
    localLog.log.push({
      id: generateId(),
      action: 'overwriting',
      name: hook
  ...
  window[hook] = evtHook(hook, window[hook], hook === 'setTimeout' || hook === 'setInterval' ? 2 : hook === 'addEventListener' ? 3 : 1);
});
window.onbeforeunload = function () {
  const logLength = JSON.parse(localStorage.getItem('localLog')).log.length;
  if (logLength > 0) {
    nativeFetch('/log', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        'jshookCollection': jshookCollection,
        'log': JSON.parse(localStorage.getItem('localLog')).log
      }),
      mode: 'cors',
      cache: 'no-cache'
    });
