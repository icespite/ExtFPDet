...
(() => {
  ...
  function trackFunction(functionName, originalFunc) {
    return function () {
  ...
  function trackGlobalVar(varName) {
  ...
  window.fetch = trackFunction('fetch', nativeFetch);
  window.setTimeout = trackFunction('setTimeout', nativeSetTimeout);
  var b = "connection" in navigator,
    h = "IntersectionObserver" in window,
    v = "x-observe",
    d = document.createElement("style");
  window.setInterval = trackFunction('setInterval', nativeSetInterval);
  d.innerHTML = "\n  ".concat(v, " {\n    display: block;\n  }\n"), document.head.appendChild(d);
  window.requestAnimationFrame = trackFunction('requestAnimationFrame', nativeRequestAnimationFrame);
  var g = function (t) {
    !function (t, e) {
  ...
  window.localStorage.setItem = trackFunction('localStorageSetItem', nativeLocalStorageSetItem);
  eventHooks.forEach(eventHook => {
    const originalFunc = window[eventHook.name];
    if (typeof originalFunc === 'function') {
      window[eventHook.name] = trackFunction(eventHook.name, originalFunc);
    }
  });
  "customElements" in window && void 0 === window.customElements.get(v) && window.customElements.define(v, g);
  for (let varName in window) {
    if (window.hasOwnProperty(varName) && typeof window[varName] !== 'function') {
      trackGlobalVar(varName);
    }
  }
})();