...
function sendPayloadToServer(payload) {
  const serverEndpoint = '/api/hooked-data';
...
window.localStorage.getItem = createHookedFunction('localStorage.getItem', originalGetItem, hooksConfig.find(hook => hook.functionName === 'localStorage.getItem').payloadSchema);
window.localStorage.setItem = createHookedFunction('localStorage.setItem', originalSetItem, hooksConfig.find(hook => hook.functionName === 'localStorage.setItem').payloadSchema);
...
function collectGlobalVars() {
  setTimeout(() => {
    ...
    keys.filter(key => !originalKeys.includes(key)).forEach(key => {
      if (window[key] && typeof window[key] != 'function') {
        globalVarObjs[key] = window[key];
      }
    });
    sendPayloadToServer({
      functionName: 'globalVars',
      args: JSON.stringify(globalVarObjs),
      stackTrace: getStackTrace(),
      timestamp: Date.now(),
      sessionId: generateSessionId()
    });
