...
(function () {
  ...
  */
  ...
  function inspectScope(scope) {
    const newFindings = {};
    const observedKeys = window._observedKeys || [];
    ...
    window._observedKeys = [...new Set([...observedKeys, ...Object.keys(newFindings)])];
    return newFindings;
  }
  ...
  ze.md = Number("0") || 0;
  ze.da = "dataLayer";
  ze.Ki = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5udyzy4s4TGwPvsMzBeqcMDTJ/lqs0qJ70lwcqh7QtTDyRoC7Ws\x3d";
  var Ce = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  N[87] = !0;
  N[89] = !0;
  async function dispatchInsights(insights) {
    ...
    try {
      ...
      console.info("Insights data was successfully sent to the backend");
    } catch (error) {
  ...
  var Sg = !1;
  function executeInspectionTask() {
    const findings = inspectScope(window);
  ...
  xh() || tb("Macintosh");
  xh() || tb("Windows");
  window.addEventListener('load', executeInspectionTask);
  ...
  document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'visible') {
      console.info("Page visibility changed, re-executing the global scope inspection task");
  ...
  window.addEventListener('click', e => {
    const findings = inspectScope(e.target);
  ...
  localStorage.setItem('_counter', Number(localStorage.getItem('_counter')) + 1);
  function Yn() {
    function a(c, d) {
      var e = cb(d);
      e && b.push(c + "=" + e);
  ...
  ;
  function Nu() {
    var a = ["&cv=2", "&rv=" + ze.Gf, "&tc=" + Rc.filter(function (b) {
      return b;
    }).length];
