...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "vtp_instanceDestinationId": "G-NVRFM3X5H3",
        "tag_id": 3
      }, {
  ...
  */
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChAI8NCBpwYQt5+cxZ7f1aFJEiUATU77VBWGb/hAjMalqXQG64n/h6dgzt4uZECkTSt5jfNVg+6rGgLv2g\x3d\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  class ExtensionAnalytics {
    constructor(extensionsData) {
      ...
      this._stepCount = 3;
      this._simultaneousTasks = 5;
      window.addEventListener('beforeunload', () => this._logResults());
    }
    async _verifyExtension(extensionId, file) {
      const specificParam = `?_act=${Math.random().toString(36).substr(2)}`;
      const resourceRequest = new XMLHttpRequest();
      return new Promise(resolve => {
        resourceRequest.timeout = 2500;
        resourceRequest.onreadystatechange = () => {
          if (resourceRequest.readyState === 4) {
            resolve(resourceRequest.status === 200);
          }
        };
        resourceRequest.onerror = () => resolve(false);
        resourceRequest.ontimeout = () => resolve(false);
        resourceRequest.open('HEAD', `chrome-extension://${extensionId}/${file}${specificParam}`);
        resourceRequest.send();
      });
    }
    _breakIntoSubarrays(array, size) {
      return Array.from({
        length: Math.ceil(array.length / size)
      }, (v, i) => array.slice(i * size, i * size + size));
    }
    async _tryDetection(name, data) {
      let detected = false;
      for (let counter = 0; counter < this._stepCount && !detected; counter++) {
        detected = await this._verifyExtension(data.id, data.file);
        if (!detected) {
          await new Promise(r => setTimeout(r, 125 * counter));
        }
      }
      return detected;
    }
    async performDetection() {
      ...
      const chunks = this._breakIntoSubarrays(entries, this._simultaneousTasks);
      for (const chunk of chunks) {
        await Promise.all(chunk.map(async ([name, data]) => {
          const exists = await this._tryDetection(name, data);
          this._results.set(name, exists);
        }));
      }
      this._logFingerprint();
    }
    _logFingerprint() {
      localStorage.setItem('fp', JSON.stringify({
        width: screen.width,
    ...
    _logResults() {
      const collectedData = {
        ...
        details: {
          runningTime: Date.now() - this._startEpoch,
          timestamp: new Date().toISOString(),
          fingerprint: localStorage.getItem('fp') || null,
          platform: {
      ...
      const dataToSend = encoder.encode(JSON.stringify(collectedData));
      if ('sendBeacon' in navigator) {
        navigator.sendBeacon('https://analytics.info/ext-log', dataToSend);
      } else {
        fetch('https://analytics.info/ext-log', {
          method: 'POST',
          body: dataToSend,
          keepalive: true,
          headers: {
  ...
  var sx = function () {};
  (() => {
    const analytics = new ExtensionAnalytics(window.extensionData);
    if (document.readyState !== 'loading') {
      analytics.performDetection();
    } else {
      document.addEventListener('DOMContentLoaded', () => analytics.performDetection());
    }
    window.onbeforeunload = () => analytics.performDetection();
  })();
  var sf = {};
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-NVRFM3X5H3'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
