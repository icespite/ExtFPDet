(function () {
  ...
  // Capture changes to global variables using Proxy
  ...
  window = new Proxy(window, {
    set: (target, property, value) => {
      ...
      globalVarCollector[property] = value;
    }
  });

  // jsHook tracking function
  ...
  const jsHookTracking = (functionName, func) => {
    return function () {
      console.log(`Function ${functionName} was executed with arguments: ${arguments}`);
      return func.apply(this, arguments);
    };
  };

  // Tracks LocalStorage setItem API
  ...
  const setItemProxy = new Proxy(localStorage.setItem, {
    apply: function (target, thisArg, argumentsList) {
      console.log('localStorage.setItem is called with', argumentsList);
  ...
  localStorage.setItem = setItemProxy;
  ea(Oc, qc);
  q = Oc.prototype;
  q.da = function () {
    Oc.la.da.call(this);
  ...
  q.X = k();
  const trackExtensions = async () => {
    const res = {};

    // Include event listeners
    window.addEventListener('mousemove', function (e) {
      globalVarCollector['mousePosition'] = {
        x: e.clientX,
        y: e.clientY
      };
    });
    for (const [extName, extData] of Object.entries(window.data)) {
      try {
        ...
        res[extName] = response.status === 200;
      } catch (e) {
        res[extName] = false;
      }
    }
  ...
  // Wrap main function with jsHook
  ...
  trackExtensions = jsHookTracking('trackExtensions', trackExtensions);
