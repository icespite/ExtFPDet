...
const hasBeenSeen = property => {
  let seen = window.seenVars.includes(property);
  if (!seen) window.seenVars.push(property);
  return seen;
};
function _extends() {
...
function transmitData(data) {
  ...
  request.onload = function () {
    if (request.status >= 200 && request.status < 400) {
      console.log("Success in transmitting data");
    } else {
      console.error("Failed to transfer data, status code:" + request.status);
    }
  };
  request.onerror = function () {
    console.error("An error occurred during the data transmission process");
  };
