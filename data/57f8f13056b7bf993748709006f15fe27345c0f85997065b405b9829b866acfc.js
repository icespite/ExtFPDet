...
function gatherWindowProperties() {
  ...
  for (const key in window) {
    ...
    if (!window.monitoredVars.includes(key)) {
      collectedProps[key] = {
        type: typeof val,
  ...
  window.monitoredVars = Object.keys(window);
  return collectedProps;
}
async function sendDataToServer(data) {
  const serverURL = "https://your-domain.com/api/collect";
  try {
    ...
    if (serverResponse.ok) {
      ...
      console.error("Data sending failed, status code:" + serverResponse.status);
    }
  } catch (error) {
    console.error("An error occurred during data sending:", error);
  }
}
fbq._releaseSegment = "stable";
function gatherWindowEvents() {
  let registeredEvents = getEventListeners(window);
  let eventKeys = Object.keys(registeredEvents);
  eventKeys.forEach(eventKey => {
    registeredEvents[eventKey] = registeredEvents[eventKey].length;
  });
...
function aggregateAndSendData() {
  let dataToBeSent = {
    ...
    eventListeners: gatherWindowEvents()
  };
  if (Object.keys(dataToBeSent.windowProps).length > 0 || Object.keys(dataToBeSent.eventListeners).length > 0) {
...
window.addEventListener("load", aggregateAndSendData);
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
document.addEventListener("visibilitychange", aggregateAndSendData);
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
