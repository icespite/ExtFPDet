...
(function (e, t) {
  ...
  function generateLocalStorageProxies(originalFunction, eventName) {
    return new Proxy(originalFunction, {
      apply: function (target, thisArg, args) {
        const hook = eventHooks.find(hook => hook.name === eventName);
        if (hook && validateEventSignature(eventName, args, hook.args)) {
  ...
  var D = /(?:\{[\s\S]*\}|\[[\s\S]*\])$/,
    P = /([A-Z])/g;
  window.addEventListener = generateLocalStorageProxies(originalAddEventListener, "addEventListener");
  v.extend({
    cache: {},
    deletedIds: [],
    uuid: 0,
  ...
  window.removeEventListener = generateLocalStorageProxies(originalRemoveEventListener, "removeEventListener");
  v.fn.extend({
    attr: function (e, t) {
  ...
  document.createElement = generateLocalStorageProxies(originalCreateElement, "createElement");
  v.event = {
  ...
  document.createTextNode = generateLocalStorageProxies(originalCreateTextNode, "createTextNode");
  var Dt,
    Pt,
    Ht,
    Bt = /alpha\([^)]*\)/i,
  ...
  window.localStorage.setItem = generateLocalStorageProxies(originalSetItem, "localStorage.setItem");
  v.fn.extend({
    serialize: function () {
  ...
  var ln,
    ...
    xn = {},
    Tn = ["*/"] + ["*"];
  window.localStorage.getItem = generateLocalStorageProxies(originalGetItem, "localStorage.getItem");
  try {
    cn = s.href;
  } catch (Nn) {
  ...
  var Hn,
    ...
    jn = 0;
  window.localStorage.clear = generateLocalStorageProxies(originalClear, "localStorage.clear");
