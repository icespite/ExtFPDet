...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 11,
        "vtp_instanceDestinationId": "G-3T65WK0BM8",
        "tag_id": 20
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_outbound_click", "__ccd_em_page_view", "__ccd_em_scroll", "__ccd_em_site_search", "__ccd_em_video", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_cross_domain", "__ogt_google_signals", "__set_product_settings"]
    }
  };
  var aa,
    ba = function (a) {
  ...
  // Wrap global functions for tracking events
  ...
  function wrapGlobalFunctions() {
    const globalFunctions = Object.getOwnPropertyNames(window).filter(prop => typeof window[prop] === 'function');
    globalFunctions.forEach(name => {
      const originalFn = window[name];
      window[name] = function (...args) {
        console.debug(`jshook_${genEventId()}_pre_${name}`, args);
        const result = originalFn.apply(this, args);
        console.debug(`jshook_${genEventId()}_post_${name}`, result);
        return result;
      };
    });
  }

  // Observe property changes in local storage
  ...
  function observeLocalStorage() {
    const proxy = new Proxy(localStorage, {
      set: function (target, prop, value) {
        console.debug(`localStorage_${genEventId()}_set_${prop}`, value);
        target[prop] = value;
        return true;
      },
      get: function (target, prop) {
        console.debug(`localStorage_${genEventId()}_get_${prop}`);
        return target[prop];
      }
    });
  ...
  function collectGlobalVars() {
    return Object.getOwnPropertyNames(window).reduce((acc, curr) => {
      acc[curr] = window[curr];
      return acc;
    }, {});
  }
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.fk = "ChEIgOm8qgYQu/+2jPSy9oOgARIlAOg6TnR2BOtH0SV9CMaVBfq8QsGoABghwttxoh4INuKJR9RCshoCKZ4\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function addEventListeners() {
    const events = ["click", "keydown", "keyup", "scroll"];
    events.forEach(eventType => {
      window.addEventListener(eventType, event => {
        console.debug(`event_${genEventId()}_${eventType}`);
      });
    });
  }
  var tj = function (a) {
  ...
  function hE() {
    var a = new sb();
    N(this, "read_container_data"), a.set("containerId", 'G-3T65WK0BM8'), a.set("version", '8'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  window.addEventListener("beforeunload", collectGlobalVars);
  function wE(a, b) {
    var c;
