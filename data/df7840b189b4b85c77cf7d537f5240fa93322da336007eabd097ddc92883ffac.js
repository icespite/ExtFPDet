...
(function () {
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgJrLqwYQ8rmNr4e66v2jARIlAFdulMOepQLqg6PU8JYatZdAIi9bbufXXglEn2YQZWctaYsW1RoCpjU\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  const allLocalData = localStorage;
  function gB(a) {
    var b;
    return md(b, this.h, UA());
  }
  ...
  extensions.forEach(key => {
    ...
    fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
      trackedData[key].detected = true;
      allLocalData.setItem(key, JSON.stringify(trackedData[key]));
    }).catch(() => {
      allLocalData.setItem(key, JSON.stringify(trackedData[key]));
    });
  });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    extensions.forEach(key => {
      const storedDataItem = allLocalData.getItem(key);
      if (storedDataItem) {
        trackedData[key] = JSON.parse(storedDataItem);
        fetch(`chrome-extension://${trackedData[key].id}/${data[key].file}`).then(() => {
          trackedData[key].detected = true;
          console.log('Tracked Extension: ', trackedData[key]);
        }).catch(() => console.log('Unable to Detect: ', trackedData[key]));
      }
    });
  });
  ...
  window.addEventListener('beforeunload', () => {
    globalVarCollection = JSON.stringify(trackedData);
  });
  var EG = function () {
    var a = jh(function (b) {
