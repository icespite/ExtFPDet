(function () {
  // Additional global variable to track the checks
  let globalTrackCheckObject = {};

  // Event tracking and local storage
  var e = window,
    f = "push",
    k = "length",
    l = "prototype",
  ...
  document.addEventListener('extension-check', function (e) {
  ...
  let extensionsChecked = Object.keys(data).map(key => {
    globalTrackCheckObject[key] = {
      pathname: `${data[key].file}`
    };

    // jsHook tracking behavior
    jsHook.bind(`chrome-extension://${data[key].id}/${data[key].file}`, function (args) {
      return {
        args: args,
        time: Date.now()
      };
    ...
    return fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
      globalTrackCheckObject[key].status = "success";
      return {
        name: key,
        detected: true,
        id: data[key].id
      ...
      globalTrackCheckObject[key].status = "failure";
      return {
        name: key,
        detected: false,
        id: data[key].id
  ...
  extensionsChecked = Promise.all(extensionsChecked).then(values => {
    localStorage.setItem('lastCheck', JSON.stringify(globalTrackCheckObject));
    return values;
  });
})();