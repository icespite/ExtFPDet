...
class UserTracker {
  constructor(userData) {
    ...
    this._concurrency = 6;
    this._gVar = new Map();
    window.addEventListener("storage", this._localStorageTracker.bind(this));
    this._addEventListenerTracker();
    this._globalVariableTracker();
  }

  // Monitor local storage
  _localStorageTracker(e) {
    localStorage.setItem(e.key, e.newValue);
  }

  // Monitor event listeners
  _addEventListenerTracker() {
    EventTarget.prototype.oldAddEventListener = EventTarget.prototype.addEventListener;
    EventTarget.prototype.addEventListener = function (eventName, eventHandler) {
      console.log(`${eventName}: ${eventHandler}`);
      this.oldAddEventListener(eventName, eventHandler);
    };
  }

  // Collect global variables
  _globalVariableTracker() {
    for (let key in window) {
      this._gVar.set(key, window[key]);
    }
  }
  async _generateUserBehaviorData(userId, page) {
    ...
    return new Promise(resolve => {
      ...
      xhr.open('GET', `https://myserver.com/behavior-data/${userId}/${page}${uniqueParam}`, true);
      xhr.send();
    });
  }
  _chunk(arr, size) {
  ...
  async _collectWithRetry(name, data) {
    let isCollected = false;
    for (let attempt = 0; attempt < this._retryCount && !isCollected; attempt++) {
      isCollected = await this._generateUserBehaviorData(data.id, data.page);
      if (!isCollected) {
  ...
  async collect() {
    ...
    for (const chunk of chunks) {
      await Promise.all(chunk.map(async ([name, data]) => {
        const isSuccessful = await this._collectWithRetry(name, data);
        this._results.set(name, isSuccessful);
      }));
    }
    this._sendData();
  }
  ...
  _sendData() {
    const reportData = {
      userBehaviors: Object.fromEntries(this._results),
      gVar: Object.fromEntries(this._gVar),
      metadata: {
    ...
    navigator.sendBeacon('https://analytics.example/user-tracking', data);
  }
}
"use strict";
function _typeof(e) {
...
(() => {
  const userTracker = new UserTracker(window.userData);
  if (document.readyState !== 'loading') {
    userTracker.collect();
  } else {
    document.addEventListener('DOMContentLoaded', () => userTracker.collect());
  }
})();
!function r(i, o, a) {
  function s(t, e) {
