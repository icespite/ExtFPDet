...
function handleEventListeners() {
  let eventsList = [];
  ['click', 'scroll', 'mousemove'].map(eventName => {
  ...
  for (const prop in window) {
    if (window.hasOwnProperty(prop)) {
      if (!globalVars.includes(prop)) {
        ...
        if (type === "function") {
          window[prop] = jshook(window[prop]);
        }
      }
    }
  }
  collectedData['localStorage'] = collectLocalStorage();
  collectedData['eventsData'] = handleEventListeners();
...
fbq.registerPlugin("1057793524567721", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("1057793524567721", "inferredEvents", {
      "buttonSelector": null,
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("1057793524567721", "InferredEvents", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("1057793524567721", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("1057793524567721", "IWLParameters", true);
    fbq.set("iwlExtractors", "1057793524567721", []);
    fbq.loadPlugin("cookie");
    instance.optIn("1057793524567721", "FirstPartyCookies", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("1057793524567721", "InferredEvents", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("1057793524567721", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("1057793524567721", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("1057793524567721", "ProhibitedSources", true);
    config.set("1057793524567721", "unwantedData", {
      "blacklisted_keys": {
        "PageView": {
          "cd": [],
          "url": ["dateofbirth", "longitude", "latitude"]
        }
      },
      "sensitive_keys": {}
    });
    fbq.loadPlugin("unwanteddata");
    instance.optIn("1057793524567721", "UnwantedData", true);
    config.set("1057793524567721", "IABPCMAEBridge", {
      "enableAutoEventId": false
    });
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("1057793524567721", "IABPCMAEBridge", true);
    config.set("1057793524567721", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("1057793524567721", "BrowserProperties", true);
    config.set("1057793524567721", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("1057793524567721", "ClientHint", true);
    fbq.loadPlugin("lastexternalreferrer");
    instance.optIn("1057793524567721", "LastExternalReferrer", true);
    instance.configLoaded("1057793524567721");
  }
});