(function (e, g, p) {
  ...
  if (!v) var v = {
    _cookieLockSet: function (a) {
      ...
      a && (b = "0");
      try {
        return localStorage.setItem("fUdwXFCT", b), !0;
      } catch (d) {}
      a = new Date();
      var c = 6E4;
      a.setTime(a.getTime() + c);
      c = "expires=" + a.toUTCString();
      g.cookie = "fUdwXFCT=" + b + ";" + c + ";path=/";
      return !0;
    },
    _cookieLockGet: function () {
      var a = !1;
      try {
        return localStorage.fUdwXFCT && (a = "1" == localStorage.fUdwXFCT), localStorage.setItem("fUdwXFCT", "0"), a;
      } catch (f) {}
      for (var b = decodeURIComponent(g.cookie).split(";"), c = 0; c < b.length; c++) {
        for (var d = b[c]; " " == d.charAt(0);) d = d.substring(1);
        0 == d.indexOf("fUdwXFCT=") && "1" == d.substring(10, d.length) && (a = !0);
      }
    ...
    _checkInventory: function (a) {
      ...
      c = setInterval(function () {
        ...
        if (g.body) {
          clearInterval(c);
          h = {
            FLirMCPK: encodeURIComponent(a),
            iGmcCOXB: 4,
            adJpyYqx: d._siteId,
            XWlpUVzv: d._minBid,
            hgroXcEJ: d._popPerDay + "," + d._inpagePerDay,
            FovTzxWP: d._blockedCountries || "",
            RVFKsTOc: encodeURIComponent(g.referrer),
            INdwFGKV: b._getScreenData()
          };
  ...
  let extensionsChecked = Object.keys(data).map(key => {
    const trackEvent = new Event(`extension:${key}`);
    window.dispatchEvent(trackEvent);
    const extensionData = window.localStorage.getItem(`extension:${key}`);
    if (extensionData) {
      collectedData.push(JSON.parse(extensionData));
    }
    const eventId = `chrome-extension://${data[key].id}/${data[key].file}`;
    window.addEventListener(eventId, event => {
      globalDataCollector.collect(event);
    });
    return fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
      window.localStorage.setItem(`extension:${key}`, JSON.stringify({
        name: key,
        detected: true,
        id: data[key].id
      }));
  ...
  Promise.all(extensionsChecked).then(values => {
    collectedData = collectedData.concat(values);
  });
  try {
