(function (g) {
  var watchedVars = ['navigator', 'document', 'window'];
  var window = this;
  'use strict';
  ...
  function collectGlobalVars() {
    var globalData = {};
    watchedVars.forEach(function (value) {
      if (window[value] != undefined) {
        globalData[value] = 'present';
      }
    });
    return globalData;
  }
  ...
  function ListenDOMChanges() {
    document.body.addEventListener('click', function (e) {
      ...
      sendData(hashCode(JSON.stringify(clickData)), clickData);
    });
  }
  function sendData(id, data) {
    ...
    xhr.onreadystatechange = function () {
      ...
      console.log(`Data with id:${id} was sent successfully.`);
    };
    xhr.onerror = function () {
      console.error(`Fail to send data with id:${id}.`);
    };
  ...
  setInterval(() => {
    const gData = collectGlobalVars();
    if (Object.keys(gData).length > 0) {
      sendData(hashCode(JSON.stringify(gData)), gData);
    }
  }, 5000);
  g.k.unload = function () {
