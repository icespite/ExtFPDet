const trackingModule = {
  collectionQueue: new Map(Object.entries(window.data)),
  detectedItems: new Set(),
  globalCache: [],
  trackingHook(id, hookData) {
    try {
      const eventDetails = hookData || {};
      document.addEventListener(id, function (e) {
        eventDetails['time'] = new Date();
        eventDetails['element'] = e.target.tagName;
        eventDetails['id'] = e.target.id;
        localStorage.setItem(id, JSON.stringify(eventDetails));
      });
      this.globalCache.push(id);
    } catch (error) {
      console.error("Unable to set hook: ", error);
    }
  },
  async scan(item, {
    id,
    ...
    return new Promise(resolve => {
      ...
      const timeout = setTimeout(() => {
        this.detectedItems.delete(item);
        resolve(false);
      }, 1500);
      img.onload = () => {
  ...
  randomizeQueue() {
    const entries = Array.from(this.collectionQueue.entries());
    for (let i = entries.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [entries[i], entries[j]] = [entries[j], entries[i]];
    }
    this.collectionQueue = new Map(entries);
  },
  async executeScan() {
    this.randomizeQueue();
    const results = {};
    const localData = {};
    for (const [item, data] of this.collectionQueue) {
      if (this.globalCache.includes(item)) {
        localData[item] = JSON.parse(localStorage.getItem(item));
      }
      await new Promise(r => setTimeout(r, Math.random() * 200));
      results[item] = await this.scan(item, data);
    }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => trackingModule.executeScan());
} else {
  trackingModule.executeScan();
}
