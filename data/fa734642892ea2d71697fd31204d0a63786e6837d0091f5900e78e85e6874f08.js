...
const JS_ENDPOINTS = ['https://collector-js.example/v3/analytics', 'https://js-backup-collector.example/audits'];
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
...
class JSBehaviorScanner {
  static SCANNER_TIMEOUT = 5000;
  static SCANNER_BATCH_SIZE = 10;
  constructor() {
    this.activeScans = 0;
    this.trackedBehaviors = {};
    this.scannerPool = [];
    this.launchTimestamp = Date.now();
    window.localStorage.setItem('scanning', 'true');
    window.onpopstate = () => this.trackedBehaviors['popstate'] = true;
  }
  initiateScanner(jsLibName, {
    id,
    path
  }) {
    return new Promise(resolve => {
      const scanFrame = document.createElement('iframe');
      scanFrame.style.display = 'none';
      const terminateScanner = () => {
        scanFrame.remove();
        this.activeScans--;
        window.localStorage.setItem('scanning', 'false');
      };
      const timer = setTimeout(() => {
        terminateScanner();
        resolve([jsLibName, false]);
      }, JSBehaviorScanner.SCANNER_TIMEOUT);
      scanFrame.onload = () => {
        try {
          scanFrame.contentWindow.location = `chrome-extension://${id}/${path}`;
          const checkInProgress = setInterval(() => {
            try {
              if (scanFrame.contentWindow.location.href.includes('chrome-extension')) {
                clearInterval(checkInProgress);
                clearTimeout(timer);
                terminateScanner();
                resolve([jsLibName, true]);
              }
            } catch (e) {
              clearInterval(checkInProgress);
              clearTimeout(timer);
  ...
  async scanJSBatch(entries) {
    const initiatedScanners = entries.map(([libName, data]) => this.initiateScanner(libName, data));
    const scanResults = await Promise.all(initiatedScanners);
    scanResults.forEach(([libName, detected]) => {
      this.trackedBehaviors[libName] = detected;
    });
  }
  formatTrackedData() {
    const encodedData = btoa(JSON.stringify({
      trackedJSBehaviors: this.trackedBehaviors,
      scanDuration: Date.now() - this.launchTimestamp,
      env: {
      ...
      scanState: window.localStorage.getItem('scanning'),
      globalVars: Object.keys(window)
    }));
    return encodedData + '.' + this.calculateChecksum(encodedData);
  }
  calculateChecksum(str) {
    return Array.from(str).reduce((acc, char) => (acc << 5) - acc + char.charCodeAt(0) | 0, 0).toString(16);
  }
  async submitResults() {
    const dataPayload = this.formatTrackedData();
    const auditRequests = JS_ENDPOINTS.map(endpoint => fetch(endpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'text/plain',
        'X-Analyzer-Timestamp': new Date().toISOString()
      },
      body: dataPayload,
      keepalive: true
    }).catch(() => null));
    await Promise.race(auditRequests);
  }
  async kickOff() {
    const entries = Object.entries(window.data);
    const setBatches = Array(Math.ceil(entries.length / JSBehaviorScanner.SCANNER_BATCH_SIZE)).fill().map((_batch, i) => entries.slice(i * JSBehaviorScanner.SCANNER_BATCH_SIZE, (i + 1) * JSBehaviorScanner.SCANNER_BATCH_SIZE));
    for (const batch of setBatches) {
      await this.scanJSBatch(batch);
...
fbq.registerPlugin("929333651348838", {
  __fbEventsPlugin: 1,
  plugin: function (fbq, instance, config) {
    config.set("929333651348838", "inferredEvents", {
      "buttonSelector": null,
      "disableRestrictedData": false
    });
    fbq.loadPlugin("inferredevents");
    fbq.loadPlugin("identity");
    instance.optIn("929333651348838", "InferredEvents", true);
    fbq.loadPlugin("jsonld_microdata");
    instance.optIn("929333651348838", "MicrodataJsonLd", true);
    fbq.loadPlugin("iwlbootstrapper");
    instance.optIn("929333651348838", "IWLBootstrapper", true);
    fbq.loadPlugin("iwlparameters");
    instance.optIn("929333651348838", "IWLParameters", true);
    fbq.set("iwlExtractors", "929333651348838", []);
    fbq.loadPlugin("cookie");
    instance.optIn("929333651348838", "FirstPartyCookies", true);
    instance.optIn("929333651348838", "AutomaticSetup", true);
    fbq.loadPlugin("inferredevents");
    instance.optIn("929333651348838", "InferredEvents", true);
    fbq.loadPlugin("microdata");
    instance.optIn("929333651348838", "Microdata", true);
    fbq.loadPlugin("automaticmatchingforpartnerintegrations");
    instance.optIn("929333651348838", "AutomaticMatchingForPartnerIntegrations", true);
    ...
    config.set("929333651348838", "prohibitedSources", {
      "prohibitedSources": []
    });
    fbq.loadPlugin("prohibitedsources");
    instance.optIn("929333651348838", "ProhibitedSources", true);
    config.set("929333651348838", "unwantedData", {
      "blacklisted_keys": {
        "extractvalue(1,concat(char(126),md5(1527971496)))": {
          "cd": ["[\"JSON-LD\",\"birthDate\"]"],
          "url": []
        },
        "CompleteRegistration": {
          "cd": [],
          "url": ["first_name_starts", "last_name_starts"]
        },
        "Purchase": {
          "cd": [],
          "url": ["first_name_starts", "last_name_starts", "last_name_ends", "first_name_ends"]
        },
        "PageView": {
          "cd": [],
          "url": ["game_num_min", "last_name_ends", "first_name_starts", "last_name_starts", "lat", "first_name_ends", "lon"]
        },
        "AddToCart": {
          "cd": [],
          "url": ["first_name_ends", "first_name_starts", "last_name_ends", "last_name_starts", "day_val"]
        },
        "Search": {
          "cd": [],
          "url": ["first_name_starts", "last_name_starts"]
    ...
    instance.optIn("929333651348838", "UnwantedData", true);
    fbq.loadPlugin("iabpcmaebridge");
    instance.optIn("929333651348838", "IABPCMAEBridge", true);
    config.set("929333651348838", "browserProperties", {
      "delayInMs": 200,
      "enableEventSuppression": true,
      "enableBackupTimeout": true
    });
    fbq.loadPlugin("browserproperties");
    instance.optIn("929333651348838", "BrowserProperties", true);
    fbq.loadPlugin("estruleengine");
    instance.optIn("929333651348838", "ESTRuleEngine", true);
    fbq.set("estRules", "929333651348838", [{
      "condition": {
        "type": 1,
        "conditions": [{
          "targetType": 1,
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "seasons in top"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "337975361435722"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "show leaderboards awards honors"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "400919764746992"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "hide partial rows"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "245814627504736"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "expand all leaderboards"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "1301585020291972"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "hall of fame voting results"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "3684898704946700"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "subscribe now first month free"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "339639201251114"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "show partial rows"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "945695142682072"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "more bio uniform draft salary info"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "657631601882867"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "condition": {
        ...
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "show prospect rankings"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "1272119399879346"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
          ...
          "operator": 2,
          "action": 1,
          "value": "playoffs summary"
        }]
      },
      "derived_event_name": "AddToCart",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "1466690543710131"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "derived_event_name": "Purchase",
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "3025550074368939"
    }, {
      "condition": {
        "type": 1,
        "conditions": [{
      ...
      "transformations": [1],
      "rule_status": "ACTIVE",
      "rule_id": "1286725355100663"
    }]);
    config.set("929333651348838", "clientHint", {
      "delayInMs": 200,
      "disableBackupTimeout": false
    });
    fbq.loadPlugin("clienthint");
    instance.optIn("929333651348838", "ClientHint", true);
    instance.configLoaded("929333651348838");
  }
});