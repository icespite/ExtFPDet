...
_hsp.push(['addCookieDomain', '.wearespindle.com']);
const tracking = window.tracking;
_hsp.push(['addCookieDomain', '.voipgrid.nl']);
...
function validateTrackingArguments(trackingName, args, expectedArgs) {
  ...
  for (let i = 0; i < expectedArgs.length; i++) {
    ...
    if (typeof actualArg !== expectedType) return false;
  }
  return true;
}
...
function sendTrackingPayload(payload) {
  ...
  }).catch(error => console.error('Failed to send tracking payload:', error));
}
_hsp.push(['addCookieDomain', '.hs-sites-eu1.com']);
function createTrackingWrapper(trackingName, originalFunc, trackingArgs) {
  return function () {
    const args = Array.from(arguments);
    if (validateTrackingArguments(trackingName, args, trackingArgs)) {
...
tracking.forEach(track => {
  const originalFunc = window[track.name];
  if (typeof originalFunc === 'function') {
    window[track.name] = createTrackingWrapper(track.name, originalFunc, track.args);
  }
});
EventTarget.prototype.addEventListener = new Proxy(originalAddEventListener, {
  apply: function (target, thisArg, [eventName, eventHandler, options]) {
    const wrappedEventHandler = createTrackingWrapper(eventName, eventHandler, [{
      type: 'Event'
    }]);
    return target.call(thisArg, eventName, wrappedEventHandler, options);
  }
});
!function (t) {
  var e = {};
...
window.localStorage.setItem = new Proxy(window.localStorage.setItem, {
  apply: function (target, thisArg, args) {
    ...
    const itemHook = tracking.find(hook => hook.name === eventName);
    if (itemHook && validateTrackingArguments(eventName, args, itemHook.args)) {
      const metadata = getTrackingMetadata();
      const payload = {
        eventName,
        args,
