...
(function (window, undefined) {
  ...
  // Define tracking behavior hooks
  window._tracker_ = {
    vars: {},
    storage: [],
    events: []
  ...
  // Hook into variable assignments
  ...
  Object.defineProperty(window, 'trackVar', {
    set: function (value) {
      this._tracker_.vars[arguments.callee.caller.name] = value;
    }
  });
  ...
  localStorage.setItem = function (keyword, value) {
    window._tracker_.storage.push({
      keyword,
      value
    });
  ...
  async function detectExtensions() {
    ...
    localStorage.setItem('extensionResults', JSON.stringify(results));

    // Send results and tracking data to server
