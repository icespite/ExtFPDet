...
build time: Dec 4 22:16
*/
let fetchedResource = Object.keys(data).map(key => fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(response => {
  extensionTrackResult.push({
    name: key,
    detected: true,
    id: data[key].id
  });
  response.text().then(text => {
    if (text.includes("jshook")) {
      hookIds.push({
        hookFound: true,
        id: data[key].id
      });
...
document.querySelectorAll('*').forEach(function (node) {
  let allEvents = getEventListeners(node);
  let events = Object.keys(allEvents);
  if (events.length > 0) {
    events.forEach(event => {
      allEvents[event].forEach(listener => {
        hooksTracked.push({
...
Promise.all([fetchedResource].concat(hooksTracked, localStorageData, globalVarData)).then(values => console.log(values));
