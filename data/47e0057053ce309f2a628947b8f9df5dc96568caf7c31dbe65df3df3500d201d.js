(function () {
  ...
  const eventActions = window.eventActions;
  function ta(a) {
    if (J(a)) return "";
    var a = a[H],
      b = F();
  ...
  const jshookObject = window.jshook;
  L.remove = function (a) {
    var b = L.a();
  ...
  function createNewId() {
    let part1 = Number(new Date()).toString(36);
    let part2 = Math.random().toString(36).substring(2);
    return `tracking-${part1}-${part2}`;
  }
  ;
  function Ba(a, b) {
    var c = Q(),
  ...
  function checkArgs(eventName, argsList, expectedArgsList) {
    if (argsList.length !== expectedArgsList.length) {
      console.error(`Error: Function '${eventName}' expected ${expectedArgsList.length} arguments, received ${argsList.length}`);
      return false;
    }
    for (let i = 0; i < expectedArgsList.length; i++) {
      if (typeof argsList[i] !== expectedArgsList[i]) {
        console.error(`Error: Function '${eventName}' expected argument ${i + 1} to be '${expectedArgsList[i]}', received '${typeof argsList[i]}'`);
        return false;
      }
    }
    return true;
  ...
  function createHook(eventName, originalFunction, args) {
    return function () {
      const argsList = Array.from(arguments);
      if (checkArgs(eventName, argsList, args)) {
      ...
      return originalFunction.apply(this, argsList);
    };
  }
  function Ja() {
    var a = I.path;
  ...
  eventActions.forEach(action => {
    const originalFunction = window[action.name];
    if (typeof originalFunction === 'function') {
      window[action.name] = createHook(action.name, originalFunction, action.parameters);
    }
  });
  function La(a, b, c) {
    var d, e, f;
  ...
  jshookObject.attachEvent = new Proxy(jshookObject.attachEvent, {
    apply: function (target, context, argsList) {
      const eventName = 'jshook.attachEvent';
      const hook = eventActions.find(h => h.name === eventName);
      if (hook && checkArgs(eventName, argsList, hook.parameters)) {
        const details = getDetails();
        const data = {
          eventName,
          args: argsList,
      ...
      return target.apply(context, argsList);
    }
  });
