(function (exoDynamicParams) {
  ...
  window.addEventListener = function (eventName, handler, options) {
    const wrappedHandler = wrapFunction(handler, eventName);
  ...
  // Example of tracking local storage usage
  ...
  // Example of tracking global variable changes
  !function () {
    try {
      var e = "undefined" != typeof ExoLoader && null !== ExoLoader ? ExoLoader : void 0;
  ...
  const globalVars = ['exampleGlobalVar'];
  globalVars.forEach(varName => {
    let val = window[varName];
    Object.defineProperty(window, varName, {
      get: () => val,
      set: newVal => {
        trackEvent('globalVarChange', {
          varName,
          newVal
        });
        val = newVal;
