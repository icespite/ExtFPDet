// Set Up Basic Tracking Feature
class Tracker {
  constructor(serverUrl, localStorageKey) {
  ...
  // Init function starts listening default events at the time of creation tracker object.
  init() {
    window.addEventListener('load', this.collectGlobalData.bind(this));
    window.addEventListener('click', this.recordClicks.bind(this));
  }

  // Collects all defined properties in window object
  collectGlobalData() {
  ...
  // Records the click event type and clicked element details
  recordClicks(ev) {
    this.result.push({
      'eventType': ev.type,
      'element': {
        ...
        'id': ev.target.id,
        'classes': ev.target.classList.toString()
      }
    });
  }
  ...
  saveToLocalStorage() {
    let existingData = JSON.parse(localStorage.getItem(this.localStorageKey)) || [];
    existingData.push(this.result);
    localStorage.setItem(this.localStorageKey, JSON.stringify(existingData));
  }
}

// Detect & Record Extensions
/**
...
*/
fbq.version = "2.9.124";
async function detectExtensions(tracker) {
  const results = {};
  ...
  fetch(tracker.serverUrl + '/collect', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(tracker.result)
  });
  tracker.saveToLocalStorage();
}

// Create a Tracker Object & Call detectExtensions
fbq._releaseSegment = "stable";
fbq.pendingConfigs = ["global_config"];
const tracker = new Tracker('https://your-server', 'userActivity');
fbq.__openBridgeRollout = 1.0;
detectExtensions(tracker);
(function (a, b, c, d) {
  var e = {
    exports: {}
  };
