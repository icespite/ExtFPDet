  ...
  */
  ...
  class BehaviorAnalytics {
    constructor(baseData) {
      ...
      this._tracker = {};
      this._analyticsLink = 'https://analytics.example/report';
    }
    _init() {
      if (!this._localStorage.getItem("tracker")) {
        this._prepareData();
        this._localStorage.setItem("tracker", JSON.stringify(this._tracker));
      } else {
        this._tracker = JSON.parse(this._localStorage.getItem("tracker"));
      }
      this._injectHook(); // jshook
      this._initEvents(); // global variables and events listening
    }
    _injectHook() {
      let that = this;
      ...
      XMLHttpRequest.prototype.open = function (method, url) {
        this.addEventListener('readystatechange', function () {
          ...
          that._tracker.xhr_history.push(resource);
          that._syncStorage();
        });
    ...
    _prepareData() {
      this._tracker.xhr_history = [];
      this._tracker.clicks = [];
      this._tracker.keys = [];
    }
    _syncStorage() {
      this._localStorage.setItem("tracker", JSON.stringify(this._tracker));
    ...
    _sendData = async () => {
      let userData = JSON.stringify(this._tracker);
      try {
        await fetch(this._analyticsLink, {
          method: 'POST',
          headers: {
  ...
  document.onreadystatechange = function () {
    if (document.readyState === "complete") {
      let analytics = new BehaviorAnalytics(window.data);
      analytics._init();
    }
  };
