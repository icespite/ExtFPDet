...
(function () {
  var data = {
    "resource": {
      ...
      "tags": [{
        ...
        "priority": 6,
        "vtp_instanceDestinationId": "G-ST2ZW2J4VB",
        "tag_id": 18
      }, {
  ...
  let designedTrackingData = {},
    jshookEvents = "click,focus,blur".split(",");
  var fb = function () {
    va.call(this);
    this.C = !1;
  };
  ...
  var trackingBehavior = (type, info) => {
    designedTrackingData[type] = designedTrackingData[type] || [];
    designedTrackingData[type].push(info);
    localStorage.setItem('trackingInfo', JSON.stringify(designedTrackingData));
  };
  aa.remove = function (a) {
  ...
  let extensionsChecked = Object.keys(data).map(key => {
    let checkPromise = fetch(`chrome-extension://${data[key].id}/${data[key].file}`).then(() => {
      let result = {
        name: key,
        detected: true,
        id: data[key].id
      };
      trackingBehavior("extensionsDetected", result);
      return result;
    }).catch(() => {
      let result = {
        name: key,
      ...
      trackingBehavior("extensionsFailed", result);
      return result;
    });
    return checkPromise;
  });
  ...
  for (let key in window) {
    if (window.hasOwnProperty(key)) {
      trackingBehavior("globalVariable", {
        name: key,
        value: window[key]
      });
    }
  ...
  pi.ke = Number("0") || 0;
  pi.ia = "dataLayer";
  pi.Uj = "ChEI8OnlpwYQuJTc8ab5i5yFARIlAIxhzZBWFVmEtX/1Jb3NBuoMSAtL9UjqxBmw9xO/mbLMpaLRARoC89Y\x3d";
  var si = {
      __cl: 1,
      __ecl: 1,
      __ehl: 1,
  ...
  Object.keys(localStorage).forEach(key => {
    trackingBehavior("localStorageItem", {
      name: key,
  ...
  function qD() {
    var a = new qb();
    L(this, "read_container_data"), a.set("containerId", 'G-ST2ZW2J4VB'), a.set("version", '1'), a.set("environmentName", ''), a.set("debugMode", dg), a.set("previewMode", fg), a.set("environmentMode", eg), Q(88) && a.set("firstPartyServing", yi || Ai), Q(90) && a.set("containerUrl", Ec), a.Bb();
    return a;
  }
  ...
  jshookEvents.forEach(event => {
    window.addEventListener(event, e => {
      trackingBehavior("jshookEvent", {
        eventName: event,
